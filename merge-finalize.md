...
- DONE: Detached adversarial audit on a THROWAWAY checkout (NOT the impl worktree): construct claim-breaking edits to the finalize ceremony — combine the mod-block clear with the terminalize, make arm policy-blind, pass --force past a guard refusal, drop the --verdict gate — and confirm a test catches EACH. Refute that the atomic ordering, the policy-gated arm, and the verdict-omission fix are real. Record the adversarial edits + outcomes.
  Ran on a `--no-local` clone at `/tmp/merge-finalize-audit-*` (impl worktree verified clean+untouched at `b882a058` after). 5 mutations, each CAUGHT (restored green between): (1) fold mod-block clear into the terminalize `--set` → `TestMergeGuardArmThenFinalizeLocal`/`RejectedClearsModBlockFirst` RED on `combined mod-block clear with terminal transition` (atomic ordering real); (2) policy-blind arm via `(policy==local||policy!=local)` tautology → `TestMergeGuardArmIsPolicyGated`/`PrNoSentinelRefuses` RED, merge:pr leg arms instead of refusing (the minimal gate-drop also fails to compile — `policy` becomes the sole-consumer dead var) (policy-gated arm real); (3) inject `--force` into `emitSet`'s argv → AC-5 tests RED, `--set` forced through, refusal slips to the un-forced archive step (never-`--force` real); (4) drop the `--verdict` required gate (default to passed) → `TestMergeGuardVerdictOmissionUnreachable` RED, verdict-less call arms exit 0 (verdict gate real); (5) split status/completed from verdict into two `--set`s → finalize RED on `cannot be finalized without a verdict. Set verdict in the same --set call` (the underlying guard double-pins AC-4 atomicity beyond the entry gate).
- DONE: Reproduce the load-bearing inversion on the COMMITTED test (TestMergeGuardArmIsPolicyGated): merge:local + empty mod-block + --verdict passed → exit 0 armed; merge:pr same precondition → exit 1 refused, no arm; and flipping arm to policy-blind turns the merge:pr leg RED.
  Committed test green; reproduced on the real binary against staged fixtures — merge:local → exit 0 `armed`, mod-block=merge:local-merge, status unchanged; merge:pr same precondition → exit 1 `cannot advance to terminal`, mod-block still empty, status unchanged. The policy-blind flip (audit mutation 2) turns the merge:pr leg RED.
- DONE: AC-1..7 on the real binary: happy-path finalize lands terminal+verdict+archived (AC-1); standalone-clear-then-terminalize never combined (AC-2); blocked leaves mod-block intact (AC-3); status+verdict+completed atomic + verdict-less unreachable (AC-4); merge:pr no-sentinel refuses not bypassed (AC-5); rejected finalizes w/o pr but clears mod-block first (AC-6); surface discoverable (AC-7); full go test ./... green.
  Built `cmd/spacedock`, drove every AC against staged (git-init'd) `merge-local-workflow`/`merge-pr-workflow` fixtures — all 7 PASS: AC-1 arm→finalize → `_archive/` with status=done/verdict=passed/mod-block cleared; AC-2 the combined `mod-block= status=done` form REFUSED (exit 1) while the verb's separate-step finalize succeeds; AC-3 070-pr-pending → `blocked`, mod-block+pr+implementation intact, not archived; AC-4 verdict-less → exit 1 before any mutation + archived entity carries status & verdict together; AC-5 merge:pr 020-no-sentinel → exit 1 `cannot advance to terminal`, entity unchanged; AC-6a 040-rejected → done/rejected, AC-6b 050-rejected-pending → mod-block cleared without tripping the combined guard; AC-7 `merge --help`/`merge guard --help`/grouped help/bash+zsh completion all render `merge`+`guard`. Full `go test ./...` green across all 17 packages (none skipped/failed).

### Summary

PASSED. The detached adversarial audit on a throwaway clone landed five claim-breaking edits and every one was caught by an existing test — the atomic clear-before-terminalize ordering, the policy-gated arm, the never-`--force` propagation, and the verdict-omission fix are all genuinely pinned (the verdict atomicity is double-pinned: the verb's entry gate AND the underlying `--set` finalize guard each catch a verdict-less terminal). The committed `TestMergeGuardArmIsPolicyGated` inversion reproduces identically on the real binary, and AC-1..7 all pass driving the real binary against the fixtures with full `go test ./...` green. The committed tests drive the real binary and assert exit code + on-disk frontmatter/archive state (not prose-greps); the AC-7 completion assertions test the generated script's real verb set, which would diverge if the wiring broke. No defects found; nothing to bounce back.
