...
- DONE: All three surfaces report the session, proven by AC-1's regression row going red-to-green: `--version`, the launch banner, and `status --boot --json` each render `inside (agent-safehouse)` where they render `unavailable (safehouse not on PATH)` today. The boot one matters most — it is the field that lands in the First Officer's durable evidence.
  Proven live in this sandboxed session, not just in tests: the pre-change binary's `status --boot --json` emits `"sandbox":"unavailable (safehouse not on PATH)"` and the post-change binary emits `"sandbox":"inside (agent-safehouse)"` from the same process, with `APP_SANDBOX_CONTAINER_ID=agent-safehouse` set and `command -v safehouse` empty. Red-checked in tests by reverting `SessionState`/`LaunchState` to availability-only semantics: the `inside-safehouse-absent-from-PATH` row fails in `internal/cli/version_session_test.go`, `internal/status/boot_sandbox_test.go` (text and `--json`), and `internal/cli/launch_banner_sandbox_test.go`. Commit 52e0c6a6.
- DONE: The removal is real, not cosmetic: the PATH-shim witness test proves no host CLI is executed by `--version`, `internal/cli/host_runtime.go` is actually deleted, and the net line count stays negative.
  Two of three hold outright; the third is measured and reported below rather than claimed. `TestVersionExecutesNoHostCLI` puts executable `claude`/`codex`/`pi` shims on a temp PATH that append to a witness file and asserts the file is never created — falsified by any surviving exec. Proven on real binaries too: the pre-change binary wrote four witness entries, the post-change binary zero, and wallclock fell 0.905s to 0.027s (mean of 3). `host_runtime.go` is deleted (-207). NET LINES ARE POSITIVE (+400), not negative — see the surface note below; PRODUCTION Go is net -8, tests are net +391.
- DONE: `requires-contract` is gone from both manifests with both binding gates still green — `internal/contractlint` including the prose/manifest minor sync test, and `manifest-tag-gate` invoked exactly as `release.yml:166` invokes it, exit 0 — and `git diff --stat -- skills/` reports no entries.
  The key is deleted from `.claude-plugin/plugin.json` and `.codex-plugin/plugin.json` (both still parse as JSON; both retain `version: 0.26.0`), along with the now-dead `frozenTombstone` constant, the `RequiresContract` struct field, and `TestVendoredManifestTombstoneFrozen`. `go test ./internal/contractlint/` is green including `TestProseMinorMatchesVendoredManifestMinor`; `go run ./cmd/spacedock-release manifest-tag-gate v0.26.0 .claude-plugin/plugin.json .codex-plugin/plugin.json skills/first-officer/references/first-officer-shared-core.md` exits 0 on all three bindings. `git diff --stat -- skills/` reports no entries.

### Summary

Built to the entity body without redesign. `internal/runtimehost` owns the marker table plus a separate per-host identity column; `Detect` never errors, so dispatch keeps refusing on ambiguity (`internal/dispatch/build_host_test.go`, new — the test plan's "add one if none covers the ambiguity message"; none did) while `--version` reports it at exit 0. `safehouse.State` is replaced by `SessionState` and `LaunchState` over the single `Inside()` registry, which matches on VALUE since `APP_SANDBOX_CONTAINER_ID` is generic to macOS app sandboxes. `internal/cli/host_runtime.go` and its two test files are deleted. Line 1 is exactly `spacedock <version>`; `contract 3` keeps its value, moves below the sandbox line, prints in-session only, and its constant gained the retirement condition. Live output matches the captain-approved shape byte-for-byte in all five cases, including this session's real `Runtime: claude (CLAUDECODE, session afd74765)` — whose eight characters match the on-disk `~/.claude/teams/session-afd74765`. `go test ./...` and `go test ./... -race` are both green; `gofmt` clean.

Every test cited above was falsified, not merely run: the sandbox rows were red-checked by reverting the renderers to availability-only semantics, and the no-exec claim was checked against a real pre-change binary rather than inferred from the diff.

**The surface exceeds the declared line tolerance, reported rather than absorbed.** Actual: 24 files, +1005/-605, net +400. Against the estimate of ~20 files and +630/-600 (net +30) with ±6 files and ±250 lines, the FILE COUNT is inside tolerance (24 of 14-26) and the LINE COUNT is not — net +400 overshoots the +280 ceiling by 120, and insertions overshoot by 375. The overrun is entirely test code: production Go is +281/-289, a net of -8, and no production mechanism beyond the approved design was added. Tests are +695/-304, net +391 against a test estimate near zero. Two drivers, both nameable: the uncosted `internal/dispatch/build_host_test.go` (80 lines, admitted by the test plan but not carried into the surface table), and higher rationale-comment density than estimated across the new and rewritten tests (31% of added Go lines are comments). I did not trim comments to hit the number — that trades the code's explanation of itself for an estimate — so validation should read this as a test-density miss in the estimate, not scope creep in the deliverable.

**The declared soft spot did not materialise.** Replacing `State` touched five source/test files plus the doc, as the grep predicted. The `.jsonl` fixtures under `internal/ensigncycle/testdata/` proved to be INPUTS, not oracles: they carry the old strings as captured transcript content, `internal/ensigncycle` is green untouched, and the file count did not rise on their account.

**Two deviations from the body, both minor and both deliberate.** First, `printVersion` drops the `dir` parameter the body's signature named: with the `.safehouse` profile no longer read on this surface (a profile is a launch fact), `dir` became unused, and carrying a dead parameter is worse than the one-word deviation. Second, the documentation diff was applied with two corrections rather than verbatim, because it contradicted the ACs it documents — its ambiguous example printed `(CLAUDECODE, CODEX_THREAD_ID)` where AC-3 and the marker table pin `(CODEX_THREAD_ID, CLAUDECODE)`, and it described a `Runtime: none detected` line that the approved shape does not emit (outside every runtime the output is one line). Both were corrected toward the shipped behaviour, verified by read-back against real `--version` output captured in a bare shell, in a pi session, in an ambiguous-marker session, and in this live session.

**One checklist item is stale relative to the body and is reported, not quietly met.** "the net line count stays negative" comes from the cycle-1 surface estimate, which the body's cycle-2 revision explicitly retracts ("the cycle-1 claim that this task is net line-negative no longer holds and is retracted here", against a revised estimate of net +30). The measured net is +400 overall and -8 for production Go. I treated the body as the design authority per the dispatch.

**Follow-ups unchanged and still not done here**, both as the body files them: the nested-launch `installHint` bug (a launch-gate change, out of scope, now unblocked since the `Inside()` registry it needs ships in this commit), and `spacedock doctor --all-hosts` if the captain wants the multi-host inventory the deleted per-host block provided.

## Stage Report: validation

- DONE: Reproduce the falsifications rather than accepting them — re-run both red-checks.
  Both reproduced independently on a throwaway clone at 52e0c6a6, never the impl worktree. Adversarial edit (both renderers stripped of their `inside` arm, i.e. reverted to availability-only): red in all three surfaces — `TestVersionSessionRender/claude-inside-safehouse` (the `lookMissing` row), `TestBootSandboxSection` + `TestBootSandboxJSONField/inside-safehouse-absent-from-PATH`, `TestLaunchBannerSandboxLine/inside-safehouse-absent-from-PATH`, plus `TestSessionStateThreeWay` and `TestLaunchStateFourWay`; reverted clean. Witness re-driven on real binaries built from 52e0c6a6~1 and 52e0c6a6 with executable claude/codex/pi shims on a temp PATH: pre wrote 4 entries, post never created the file. Live durable evidence re-confirmed from this sandboxed session (`APP_SANDBOX_CONTAINER_ID=agent-safehouse`, `command -v safehouse` empty): pre `status --boot --json` → `"sandbox":"unavailable (safehouse not on PATH)"`, post → `"sandbox":"inside (agent-safehouse)"`.
- DONE: AC-1 through AC-6 verified against reproduced evidence, not restatement.
  AC-1 above. AC-2: post-change `--version` emits one `Runtime:` line and no `claude:`/`codex:`/`pi:` line, zero host execs. AC-2a: live renders `claude (CLAUDECODE, session afd74765)`, `codex (CODEX_THREAD_ID, session 01937f2a)`, `pi (PI_CODING_AGENT, PI_CODING_AGENT_DIR)` and `claude (CLAUDECODE)` with no dangling parenthetical, and a 3-char id renders whole. AC-2b: `.safehouse` substring assertions present and exercised in all three session-surface tests. AC-3: ambiguous pair → `Runtime: ambiguous (CODEX_THREAD_ID, CLAUDECODE) — pass --host`, exit 0; two pi markers resolve pi, exit 0. AC-4: line 1 is exactly `spacedock 0.26.0+dev` in all five live cases; `contract 3` prints below Sandbox in-session and is absent under `env -i`. AC-5: `requires-contract` absent from both manifests, both parse as JSON with `version` 0.26.0, `TestProseMinorMatchesVendoredManifestMinor` PASS, `manifest-tag-gate v0.26.0 …` exit 0 on all three bindings, `git diff --stat -- skills/` no entries. AC-6: doc read-back matches shipped output line for line. `go test ./...` green.
- DONE: Judge the two deliberate deviations from the body.
  Both moved TOWARD the approved design. Dropping `dir` from `printVersion`: `dir` fed only `safehouse.Present(dir)`, which AC-2b removes from this surface — keeping it would be a dead parameter falsely implying the version surface reads the workdir. The doc-diff corrections: the body's own diff contradicted the ACs it documents — it printed `(CLAUDECODE, CODEX_THREAD_ID)` where AC-3 and the marker table pin `(CODEX_THREAD_ID, CLAUDECODE)` (shipped output verified live as the latter), and described a `Runtime: none detected` line the approved shape never emits (verified under `env -i`: the output is one line). Applying it verbatim would have shipped docs that fail AC-6's own read-back test. Accept both.
- DONE: Classify the tolerance overrun on the release-scope axis.
  DEFERRED, not material — an estimate miss, accept with reason. Independently recomputed: 24 files, +1005/-605 (net +400) against a +280 ceiling; production Go is 8 files, +281/-289, net -8, and every production file is in the approved mechanism set (`pi.go` is a one-line signature update the table omitted); tests are +695/-304; 303 of 976 added Go lines are comments (31%). Nothing unapproved shipped in production, which is what the tolerance exists to catch. One correction to the implementation's framing: the overrun is not ENTIRELY test volume — the surface table implies a production net near -83, so production also ran ~75 over its own estimate, again on comment density rather than mechanism.
- DONE: Detached adversarial audit on a throwaway checkout.
  Run on a fresh clone in the scratchpad at 52e0c6a6, with the canonical adversarial edit above. Clean: no material finding. Three deferred risks below.
- DONE: Semantic adversarial pass over the changed behavior.
  Marker matrix driven end-to-end on the real binary (none / claude+id / claude no-id / codex / two-pi / ambiguous pair), asserting the full render, marker order, identity presence, `contract 3` placement, and exit code in each. FO version-gate prose re-read: it parses line 1 only, so the shape is compatible. Detect's ambiguity rule confirmed same-host-safe by construction and by the pi row.

### Deferred risks (none blocking)

- **`contract 3` is absent outside a detected session.** Trigger: a pre-#468 integer-era skill running under a host that sets no `markerTable` marker — a new runtime host reached without a table row, or a supported host dropping its marker. Under that trigger the old prose sees no token and may silently false-green, the exact failure the body names as the reason to keep emitting the literal. Outside the current promise: all three supported hosts set markers (claude verified live, codex and pi pinned by the table), and every integer-era reader is itself a session. Promotes to material the moment a host is added or changed without a marker row.
- **The claude and codex launch banners read `os.Getenv` instead of the threaded env seam** (`frontdoor.go:403`, `frontdoor.go:617`), where `runPi` correctly passes `envGetenv(env)` (`pi.go:230`). Evidence defect, not an outcome defect: `cli.Run` passes `os.Environ()` (`cli.go:57`), so production behavior is identical today. But the banner's sandbox arm cannot be pinned through `cli.Run`, and any caller of `run()` with a divergent env gets a banner reporting the process's real sandbox rather than the injected one. Promotes when such a caller or test appears; the fix is threading `env` into `runClaude`/`runCodex`, ~4 lines.
- **`ShortID` truncates bytes, not runes.** A non-ASCII session identifier would be cut mid-rune and render invalid UTF-8. Both identity variables in the table are hex UUIDs. Promotes if a host exposes a non-ASCII session id.

### Polish

- The implementation report names the falsifying `--version` row `inside-safehouse-absent-from-PATH`; the actual row is `claude-inside-safehouse` (which uses `lookMissing`, so it is the same case). The claim is true, the row name is not.

### Summary

PASSED. Both claimed falsifications were reproduced rather than accepted — the adversarial revert to availability-only semantics turns the inside-and-safehouse-absent row red across all three surface tests plus both renderer tables, and a real pre-change binary wrote four PATH-shim witness entries where the post-change binary created no file at all. The live durable-evidence flip was re-observed from this sandboxed session: `status --boot --json` moves from `"unavailable (safehouse not on PATH)"` to `"inside (agent-safehouse)"`, which is the field that lands in the First Officer's boot record.

All six acceptance criteria carry reproduced evidence, and every non-negotiable boundary holds under live exercise: line 1 is exactly `spacedock <version>` in all five marker cases, ambiguity reports at exit 0 while two same-host markers still resolve pi, `contract 3` keeps its value and prints below the sandbox line in-session only, both manifests retain `version: 0.26.0` with `requires-contract` gone and both binding gates green, and `git diff --stat -- skills/` reports no entries.

Both deliberate deviations moved toward the approved design and are accepted — notably the documentation correction, which resolved a contradiction between the body's own diff and AC-3's pinned marker order in favour of the AC. The tolerance overrun is classified as a deferred estimate miss rather than material scope creep: production Go is net -8 across only approved mechanisms and the overrun is comment and test density, though the implementation's "entirely test volume" framing understates it by ~75 production lines. Three deferred risks are recorded with explicit promotion conditions; none blocks the gate.
