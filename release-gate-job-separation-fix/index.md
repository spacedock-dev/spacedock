...
- DONE: M1 closed — TestReleaseWorkflowGuardRejectsGoreleaserNeedsJourneyLedger is table-driven over scalar/flow/BLOCK-LIST and all three RED a goreleaser:needs:journey-ledger edge; mutation-confirm (breaking the block-list consumption reds ONLY the block_list subtest); the safe reverse block-list edge (journey-ledger needs goreleaser) stays tolerated
  All three subtests (scalar/flow_sequence/block_list) PASS against the real release.yml (commit bd1fb72b). Mutation-confirm on detached checkout: rewriting the block-list consumption guard `- ` → `ZZZNOMATCH` REDs ONLY block_list (scalar/flow stay green) — not tautological. Throwaway DAG-print probe confirmed the safe reverse edge in block-list form is attributed to journey-ledger (`journey-ledger needs=[goreleaser]`, `goreleaser needs=[]`), guard does NOT trip. Adversarial refutation: 4 extra block-list shapes (quoted entry, multi-job list, deeper indent, extra spacing) all resolve to `goreleaser needs=[...journey-ledger...]` and RED — no residual evasion.
- DONE: No regression — skip-gating, AC-1 job-level skip-consequence, AC-2 release-notes.txt anchor, and the existing mutation controls all intact; internal/release 34/34, offline go test ./... green (1176)
  Named controls re-run green: TestWorkflowsPreserveAndPublishJourneyCosts, TestReleaseWorkflowSkipsLedgerWhenNoProducerRun + its ungated-build adversarial twin, the two comment-only mutation controls, TestAnnotatedTagBodyRoundTrips, TestReleaseYAMLGuardRejectsEmptyBody. The release-notes.txt anchor is load-bearing: only 1 of release.yml's 3 single-line `if [ ... ]; then` conditions (line 131) contains release-notes.txt, so the anchored regex unambiguously selects the empty-body guard despite the cycle-2 download run-id check at line 57. internal/release 34/34 PASS / 0 FAIL; offline `go test ./...` 1176 PASS / 0 FAIL across 15 packages; `go vet ./internal/release/` + `go build ./...` clean.
- DONE: P1 — the download skip-branch tolerates gh-error AND missing-gh as found=false/exit-0 (TestReleaseDownloadSkipBranchToleratesGhError exercises the real script vs a non-zero gh stub AND a no-gh PATH; mutation-confirm removing the guard reds both)
  Both subtests (gh_exits_non-zero, gh_missing_from_PATH) PASS exercising the REAL extracted download script. Mutation-confirm on detached checkout: removing `|| true` REDs BOTH subtests (exit 1 for gh error, exit 127 for missing gh) while TestReleaseDownloadSkipBranchExitsZeroOnEmptyRunList stays green. Restored clean.
...
- DONE: Replace the hand-rolled needs-parser with a yaml.v3 parse so a real goreleaser:needs:journey-ledger in ANY shape (incl. inline-comment M1a + blank-line M1b) is REJECTED by assertGoreleaserDoesNotNeedJourneyLedger
  Deleted `parseNeeds` entirely (0 references remain) and the block-list consumption loop in `parseWorkflowJobs`; added `parseJobNeeds(workflow) map[string][]string` that `yaml.Unmarshal`s the workflow and reads each job's `needs` as a normalized list (scalar via ScalarNode, flow/block via SequenceNode). `parseWorkflowJobs` now sources `needs` from it (keyed by job name) and keeps line-based step attribution. yaml.v3 strips inline `#` comments, tolerates blank lines, and resolves quoting/anchors for free — the whole shape-evasion class is gone, not patched per-shape. workflow_exec_guard_test.go; commit d371b5b6.
- DONE: Extend the adversarial test to cover M1a (all three shapes) + M1b; all RED; mutation-confirm the parse rejects them; the safe reverse edge stays tolerated in every shape
  `TestReleaseWorkflowGuardRejectsGoreleaserNeedsJourneyLedger` is now table-driven over 7 forms: scalar/flow/block-list, all three with a trailing `# comment` (M1a), and a blank-line-split block list (M1b) — all 7 RED. New `TestReleaseWorkflowGuardToleratesSafeReverseEdgeInEveryShape` rewrites the real file's `journey-ledger: needs: goreleaser` into flow/block/comment/blank-line shapes and asserts the guard stays GREEN (direction discrimination holds in every shape). Mutation-confirm: neutering `parseJobNeeds` to `return nil` REDs ALL 7 rejection subtests while the tolerance test + real-file guard stay green — load-bearing, not tautological. journey_workflow_test.go; commit d371b5b6.
- DONE: No regression — skip-gating, AC-1 job-level, AC-2 release-notes.txt anchor, P1 gh-error tolerance, and the existing mutation controls all green; offline go test ./... green
  `release.yml` untouched this cycle (`git diff HEAD -- .github/workflows/release.yml` empty: P1 `|| true`, the release-notes.txt anchor, and skip-gating intact from bd1fb72b). `go test ./internal/release/` 45/45 (was 38); `go vet ./internal/release/` clean; `go build ./...` clean; full offline `go test ./...` 1187 passed across 15 packages.

### Summary (cycle 3)

Took the captain/FO root-fix: deleted the hand-rolled `parseNeeds` (the enumeration trap) and parse `needs:` with `gopkg.in/yaml.v3` — a direct module dep, the same parser the audit used to confirm coupling — so comments, blank lines, scalar/flow/block, quoting, and anchors are all handled by the parser, not a growing shape list. The M1a inline-comment evasions (all three shapes) and the M1b blank-line block-list evasion now RED; a new tolerance test proves the safe reverse edge stays green in every shape; neutering `parseJobNeeds` reds all 7 rejection forms (mutation-confirmed non-tautological). Nothing the audits verified sound was churned — release.yml is untouched this cycle. internal/release 45/45, offline ./... 1187, vet+build clean. Ready for FO re-validation + re-audit before merge.
