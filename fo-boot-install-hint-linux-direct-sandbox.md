        ...
        - id: gate:z3j0tsbr6t3mqd39rhs8bbvq:ideation
          ...
          attempts:
            ...
            - id: gate-attempt:z3j0tsbr6t3mqd39rhs8bbvq-ideation-2
              ...
              provider-evidence:
                result-digest: sha256:2c226357d2d4b3adefcd5f66d33d1909d7b26ae1d0b9a5bb9cb2d99d63b9eaf5
                presented-inventory-digest: sha256:e656af657efa58f108bce13265dcfc471cbd7f28f12e4f78ab17604d10ea8651
              resolution:
                type: Resolution
                id: resolution:binding-1785474244685951000
                briefing: briefing:z3j0tsbr6t3mqd39rhs8bbvq:ideation:attempt-2:revision-1
                by: person:captain
                at: "2026-07-31T05:04:04.685954Z"
                decision: approve
        ...
        - id: gate:z3j0tsbr6t3mqd39rhs8bbvq:validation
          ...
          attempts:
            ...
            - id: gate-attempt:z3j0tsbr6t3mqd39rhs8bbvq-validation-2
              ...
              provider-evidence:
                result-digest: sha256:46353f730b11780a0f407bec7584541613bcd8667e4c3ae8f6f61508cdc4e07c
                presented-inventory-digest: sha256:b04bd3c0c157e5e54042a3765bdc0622603034572865496ecfc5066ea3e99d3b
              resolution:
                type: Resolution
                id: resolution:binding-1785502148393774000
                briefing: briefing:z3j0tsbr6t3mqd39rhs8bbvq:validation:attempt-2:revision-1
                by: person:captain
                at: "2026-07-31T12:49:08.393783Z"
                decision: approve
              application:
                action: advance
                target-stage: done
...
7. `skills/first-officer/references/first-officer-shared-core.md` + NEW `skills/first-officer/references/fo-install-gate.md` — RESTRUCTURED at implementation by captain-ratified design reset (FO chat authorization, 2026-07-31): the approved 45-65-line in-core rewrite could not fit the repo's pinned shared-core byte caps (26754 — the file had zero headroom; growth violated `startup_collapse_test.go` / `fo_function_reference_invariant_test.go`). As implemented: the core carries a LEAN Startup step 1 skeleton — version-gate classes, the OS-aware hint inline (Linux curl|sh lead, macOS brew lead in both forms, source-build fallback, unsupported-OS escape), the every-class sandbox env-registry check sentence (registry name+VALUE: `internal/safehouse/state.go`), the `uname -s` OS-source note (permanently load-bearing, NF4), and the B2 launcher-invariant amendment (a gate failing binary-absent that then succeeds via the approved install performs its ONE launcher resolution at that point) — held at 26746 bytes (8 bytes under cap). The install-offer machinery moves to the deferred reference, read ONLY when the binary-absent class fires outside a sandbox: the direct install-and-resume offer flow, the sentinel-file loop bound (`test -f` on `${TMPDIR:-/tmp}/spacedock-install-attempted-<key>`, identity-env key with the pi cwd-hash/tmp-durable fallback admitted, create-before-run, fallback naming the sentinel path + `rm <sentinel-path>` recovery), `SPACEDOCK_BIN` session-scoped convergence (install.sh stderr parse + `$HOME/.local/bin/spacedock` probe, never persisted — D-2), the sandbox-arm message body (`^Sandbox:` corroboration-only, env wins disagreements, no-install-inside wording), and the permissive/failure branches' message grammar. The core names the deferred read with the ONE literal path and carries it in the deferred-load-points inventory. Core: net +6/-5 lines, byte-neutral; deferred reference: ~22 lines.
...
11. `skills/integration/version_gate_fixture_test.go` + `skills/integration/testdata/version_gate_flow.sh` — AC-2/AC-3 behavior fixtures (test plan required; not enumerated in 1-10 — recorded here at implementation): a deterministic shell mirror of the gate flow driven with captive `curl`/`install.sh`/`spacedock` commands on a minimal PATH; asserts exit codes and observable side effects (sentinel create-before-run, exactly one install run, `SPACEDOCK_BIN` repoint, `--version` re-check, sandbox env-marker detection with NO `--version` output, corroboration agree/disagree arms). Named weakness per N8/NF5: prose-driven mirror; assertions are on shell side effects and semantic content, never byte-for-byte wording.
...
- DONE: ACs 1-4 each get their declared falsifiable test implemented and running GREEN — Go unit tests for the OS-aware remedy (AC-1) and the OS line (AC-4); behavior fixtures for install-and-resume convergence + failure fallback (AC-2, asserting the sentinel file) and sandbox env-marker detection (AC-3) — with exit codes observed, not text claims
  AC-1: `TestTooOldBinaryRemedyOSAware` (pinned goos: linux asserts the curl|sh token + zero brew content; darwin keeps the Homebrew lead + edge-cask block) — removing the Linux branch or leaking brew fails it. AC-4: retargeted `TestVersionSessionRender` (all five cases pin `OS: <goos>/<goarch>` as line 2 in BOTH shapes) + live check (`spacedock --version` built from the branch shows `OS: darwin/arm64` as line 2 in- and outside-session). AC-2: `TestGateFlowInstallAndResumeConvergence` (exit 0; sentinel file observed; install ran exactly once; `SPACEDOCK_BIN` repoint + `gate passed after install` re-check observed) and `TestGateFlowFailureFallbackOneAttempt` (exit 3; exactly one install run across TWO gate entries — sentinel suppresses attempt 2; fallback carries the exact curl token + `rm <sentinel>`). AC-3: `TestGateFlowSandboxEnvMarker` (absent-binary state, marker set → exit 3, install side effect count 0, message carries exact command + "outside the sandbox"; corroboration agrees in the wrong-version class; env wins on disagreement). Plus `TestGateFlowUnsupportedOS` (D-5) and the no-drift + smoke tests below. Fixtures observe exit codes and shell side effects, not prose claims.
- DONE: Every Expected-surface file (1-10) is touched as designed within the ±20% LOC tolerance — cli.go OS line, version_session_test.go + version_message_test.go GOOS-pinned retargets, contract.go goos-threaded remedy, shared-core.md Startup step 1 rewrite (including the launcher-invariant amendment), command-reference.md doc diff (lines 3-26), skill smoke test (full name+value registry rows, sentinel, sandbox env check, invariant text), and the no-drift token test
  `git diff --numstat $(git merge-base main HEAD)..HEAD`: cli.go +11/-5 (16 vs est 6-10+20% — over; NF3-mandated doc/inline comment rewrite), version_session_test.go +22/-5 (27 vs 12-22 over ~2 lines), contract.go +21/-10 (31 vs 15-25 over ~1 line), version_message_test.go +56/-8 (64 vs 20-40 OVER — AC-1 test + retarget bodies), doctor.go/init.go/frontdoor.go 0 (goos threaded at compareNamed; RunDoctor signature unchanged), command-reference.md +6/-4 (10, within 8-14). File 7 RESTRUCTURED by captain-ratified design reset (see below): core +6/-5 lines at 26746 bytes (8 under the pinned 26754 cap) + NEW `references/fo-install-gate.md` (22 lines) + deferred-inventory line; smoke test `version_gate_smoke_test.go` 165 lines vs est 15-30 (OVER — five assertions families incl. full registry rows source-grep); no-drift test 96 lines vs est 10-15 (OVER — tab-section extraction helper + 2 arms). Fixtures recorded as surface file 11: 371 lines (mirror script + Go driver). LOC deviations flagged for the gate rather than hidden.
- DONE: `go test ./...` and `go test ./... -race` both green and `gofmt -w ./cmd ./internal` applied, per repo AGENTS.md — and existing tests are NOT weakened to force green; failures surface as REAL findings, not suppressed
  Both runs green except `TestCodexResolveManifestAgainstInstalledHost`, which fails IDENTICALLY on a stashed baseline (real codex CLI cannot read `~/.codex/config.toml` — EPERM: this machine runs under an agent-safehouse sandbox); pre-existing environmental, unchanged. Suite run with ambient `PI_CODING_AGENT` stripped because `TestVersionAmbiguousMarkersExitZero` leaks ambient markers (pre-existing; also fails unmodified). Retargets keep their teeth: `TestTooOldBinaryRemedyLeadsWithBrew` still asserts the brew lead via RunDoctor on darwin and the curl lead on linux; `TestTooOldBinaryRemedyEdgeChannel` keeps its byte-for-byte pin at pinned goos "darwin". `gofmt -l ./cmd ./internal ./skills` clean. Material finding during implementation surfaced and resolved below, not edited around.
- Material finding → captain-ratified design reset (FO chat authorization, 2026-07-31): the approved 45-65-line in-core Startup step 1 rewrite (file 7) violated the repo's pinned shared-core byte caps (26754; `startup_collapse_test.go`, `fo_function_reference_invariant_test.go`) — zero headroom existed. Resolution (option B ratified): lean in-core skeleton + NEW deferred `references/fo-install-gate.md` carrying the install-offer machinery, read only when the binary-absent class fires outside a sandbox; entity Expected-surface file 7 and smoke-test bullet updated in place. All AC semantics preserved; smoke test asserts the skeleton shape, the deferred trigger + inventory line, and the deferred body's sentinel/convergence/grammar tokens.

### Summary

Implementation of the gate-approved design (resolution:binding-1785474244685951000) is complete on branch `spacedock-ensign/fo-boot-install-hint-linux-direct-sandbox` at commit 0b5409687: `--version` prints `OS: <goos>/<goarch>` as line 2 in both shapes; `tooOldBinaryRemedy(edgeCask, goos)` leads curl|sh on Linux (edge token superseded) and keeps Homebrew on macOS; the FO gate's binary-absent class is offer-and-reserve with a durable sentinel-file bound, session-env sandbox registry check, and a deferred `fo-install-gate.md` carrying the machinery under the boot-residency caps; docs, retargets, no-drift and smoke tests ship with it. The one in-flight conflict (byte caps vs approved in-core rewrite) went through the review-finding checkpoint and came back as a captain-ratified design reset, recorded in the entity body. Residual risks: (1) the fixture/skill surfaces run over the ideation LOC estimates (named above for the gate); (2) the AC-2/AC-3 fixtures mirror prose mechanics in shell — the N8/NF5 weakness is named, not solved; (3) the pre-existing sandbox-blocked codex CLI test stays red on this machine until run outside safehouse.

## Stage Report: validation

- DONE: Each AC's declared test re-run against the candidate with observed exit codes (not report-trust):
  AC-1 `go test ./internal/contract/ -run TestTooOldBinaryRemedy -v -count=1` → `TestTooOldBinaryRemedyLeadsWithBrew` PASS, `TestTooOldBinaryRemedyOSAware` PASS, `TestTooOldBinaryRemedyEdgeChannel` PASS, `ok ... internal/contract 0.574s`, exit 0 (all runs with ambient `SPACEDOCK_BIN`/`PI_CODING_AGENT`/`CLAUDECODE` stripped).
  AC-4 `go test ./internal/cli/ -run 'TestVersionSessionRender|TestVersionMessage|TestVersionAmbiguousMarkersExitZero' -v -count=1` → all six `TestVersionSessionRender` subcases PASS (`outside-every-runtime-is-two-lines` included), exit 0. Live check on a binary built from the branch: outside-session output `spacedock 0.27.0-pre2+dev` / `OS: darwin/arm64` (two lines); in-session adds `Runtime: pi (PI_CODING_AGENT)` / `Sandbox: inside (agent-safehouse)` / `contract 3` — `OS:` is line 2 in both shapes.
  AC-2/AC-3 `go test ./skills/integration/ -v -count=1` → `TestGateFlowInstallAndResumeConvergence` PASS (0.84s), `TestGateFlowFailureFallbackOneAttempt` PASS (0.64s), `TestGateFlowUnsupportedOS` PASS (0.01s), `TestGateFlowSandboxEnvMarker` PASS (0.90s), `ok ... skills/integration 6.618s`, exit 0.
  Guards: `go test ./internal/contractlint/ -v -count=1` → ALL PASS (incl. `TestSharedCoreRemainsBelowPreChangeByteCap`, `TestFOInstructionComponentCaps`, `TestInstallHintNoDrift`, `TestProseMinorMatchesVendoredManifestMinor`, `TestVersionGateSandboxRegistry`), exit 0. Race leg `-race -count=1` on contract/contractlint/skills-integration → all `ok`, exit 0. `gofmt -l ./cmd ./internal ./skills` → empty. Full `go test ./...` → single failure `TestCodexResolveManifestAgainstInstalledHost` (EPERM reading `~/.codex/config.toml` under agent-safehouse); identical failure reproduced on the unmodified main checkout — pre-existing environmental, unchanged.
- Surface re-measured from git: `git diff --numstat "$(git merge-base main HEAD)"..HEAD` — docs/site/reference/command-reference.md 6/4 (10, est 8-14: WITHIN); internal/cli/cli.go 11/5 (16, est 6-10, tol 12: OVER — NF3-mandated doc/inline comment rewrite); internal/cli/version_session_test.go 22/5 (27, est 12-22, tol ~26: marginally OVER); internal/contract/contract.go 21/10 (21 net, est 15-25, tol 30: changed-Lines 31 marginally OVER); internal/contract/version_message_test.go 56/8 (64, est 20-40, tol 48: OVER); internal/contract/doctor.go 0 (est 0-10: WITHIN); internal/cli/init.go,frontdoor.go 0 (est 0: WITHIN); skills/.../first-officer-shared-core.md 6/5 (matches captain-ratified reset: 26746 bytes, 8 under the 26754 cap: WITHIN); skills/.../fo-install-gate.md 22 lines (est ~22: WITHIN); smoke test version_gate_smoke_test.go 165 lines (est 15-30, tol 36: OVER — five assertion families incl. full registry-rows source-grep); no-drift install_hint_drift_test.go 96 lines (est 10-15, tol 18: OVER — tab-section extraction helper + 2 arms); fixture file 11 version_gate_fixture_test.go 253 + version_gate_flow.sh 118 = 371 (recorded outside 1-10 at implementation). DEVIATION FINDINGS (design-reset class, not excused): files 1, 2, 3, 4, 8, 9 exceed the ±20% band; only file 7's restructuring carries a captain-ratified design reset. Implementation flagged all of these in its own report; no new surface files beyond the declared 11 appear in the diff.
- Adversarial falsification attempted on the three high-blast-radius claims — all attacks FAILED to break them:
  (a) AC-1 Linux-remedy purity: worst-case throwaway probe `tooOldBinaryRemedy(edgeCask, "linux")` with edgeCask=false AND edgeCask=true, scanning the full message for `brew`, `@next`, `homebrew`, `cask` (case-insensitive) — zero hits in both arms; curl|sh lead present in both; probes byte-identical across edgeCask values (edge token fully superseded on Linux). Probe run as a temp test, then deleted — no tree change.
  (b) AC-2 one-attempt bound: two-pass live drive of `version_gate_flow.sh` in fresh shells (capt captive `curl`|`install.sh` whose install reports success but drops an incompatible 0.20.0 binary; no `spacedock` on PATH) — pass 1: exit 3, install ran exactly once, sentinel created BEFORE run (`-rw-r--r-- ... spacedock-install-attempted-attack-b`), fallback names sentinel path + `rm` recovery; pass 2 (sentinel pre-existing): `sentinel-blocked`, install side-effect count stayed 1, exit 3. Pre-creating the sentinel manually (mid-run-death simulation) likewise suppresses the install run.
  (c) AC-3 sandbox env-marker detection: absent-binary state with `APP_SANDBOX_CONTAINER_ID=agent-safehouse` → exit 3, install side-effect count 0, NO sentinel created, message carries the exact curl|sh command + "outside the sandbox"; marker set to a DIFFERENT value (`com.example.other`) → correctly NOT inside (install path proceeds once); compatible-binary arm with marker set and NO `Sandbox:` line → "corroboration: DISAGREEMENT ... env check wins", gate passes exit 0; agreement arm corroborates. Note: one attack run was contaminated by the ambient `SPACEDOCK_BIN` (dispatch-session env) resolving the main-checkout binary — detected via an anomalous `0.26.0+dev` version string, re-run with `env -u SPACEDOCK_BIN`, clean result as above.

### Summary

Validation re-ran every declared AC test green (observed exit codes), re-measured the surface from git, and re-ran the repo's load-bearing guards (contractlint caps, no-drift, smoke) green, plus race on the touched Go surfaces and a clean gofmt. The only suite failure is the pre-existing, environmentally-blocked codex CLI test, reproduced identically on unmodified main. All three falsification attacks on AC-1 (Linux remedy brew/edge purity), AC-2 (sentinel-bounded exactly-one-attempt), and AC-3 (absent-binary env-marker sandbox detection with corroboration agree/disagree arms) failed to break their claims. Residual findings for the gate: six surface files (cli.go, version_session_test.go, contract.go, version_message_test.go, smoke test, no-drift test) exceed the ±20% LOC band — design-reset-class findings recorded, not excused (implementation flagged them; only file 7's reset was captain-ratified); the AC-2/AC-3 fixtures remain a shell mirror of prose mechanics (named N8/NF5 weakness); the codex test needs a run outside safehouse.
