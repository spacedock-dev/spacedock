...
- DONE: Implement the id-first cap in `dispatch build` (internal/dispatch/build.go) + the id-aware reconcile resolve (add entityRecord.id; exact-slug-first then sd-b32 id-prefix HasPrefix; ambiguous/no-match → unclassified) exactly per the approved cycle-2 design — short names stay byte-identical, the existing golden/parity suite must remain unchanged.
  `capWorkerName` (build.go) fires only on >64-char overflow; sd-b32 substitutes a fixed 10-char id-prefix, sequential the whole id, slug truncates the head to nameCapTarget(57). `resolveSlugToken` (reconcile.go) + `entityRecord.id` resolve exact-slug-first then `HasPrefix` id-prefix, ambiguous/none → unclassified. Existing 195 golden/parity tests pass unchanged. Commit edbec3a3 (worktree branch spacedock-ensign/dispatch-build-name-cap).
- DONE: Land all six ACs as Go tests over REAL `dispatch build` output + real `Reconcile`/`decompose` (AC3/AC4 bind to a seeded on-disk sd-b32 id set — correct entity, no false Class-A against a live sibling) plus the id-less slug-truncation fallback test; `go test ./internal/dispatch/` green.
  build_namecap_test.go (AC1/2/5/6 + slug-fallback) drives real `dispatch build`; reconcile_namecap_test.go (AC3/AC4) emits capped names via real build, seeds active+_archive sd-b32 entities (shared slug prefix, distinct ids), runs real `Reconcile`. `go test ./internal/dispatch/` = 205 passed (195 prior + 10 new).
- DONE: Apply the single break-glass doc-diff caveat in skills/first-officer/references/claude-first-officer-runtime.md (the only doc change) — keep the worker_key prefix + -{stage} suffix, id-prefix in place of the slug on id-style sd-b32.
  Inline `//` caveat appended to the break-glass `name="{worker_key}-{slug}-{stage}",` template line (around line 109): cap to ≤64 the way `dispatch build` does — prefix+suffix kept, id-prefix for sd-b32, slug-head truncation for id-less. Only doc diff.

### Summary

Capped the dispatch-build worker name at the Agent-tool 64-char ceiling. `capWorkerName` returns short names byte-identical and, on overflow, substitutes an id-first token (sd-b32 fixed 10-char id-prefix / whole sequential id) for the slug or truncates the id-less slug head to nameCapTarget(57) reserving `-cycleN` headroom; worker_key prefix and -{stage} suffix are preserved so `decompose()` round-trips. Reconcile gained `entityRecord.id` and `resolveSlugToken` (exact-slug-first, then sd-b32 id-prefix `HasPrefix` against active+archived, ambiguous/no-match → unclassified) so a capped name resolves to the correct entity with no false Class-A against a live sibling. Six ACs + slug-fallback land as Go tests over real `dispatch build`/`Reconcile`; `go test ./internal/dispatch/` is green (205) and the 195-test golden/parity suite is unchanged. Two small exported accessors added to internal/status (IsValidSDB32ID, IsDigits, SDB32MinPrefix). The unrelated cli `TestCodexResolveManifestAgainstInstalledHost` failure is a pre-existing external codex-host-binary dependency, not touched by this change.

## Stage Report: validation

- DONE: Reproduce every AC's evidence from OUTSIDE the task body: `go test ./internal/dispatch/` green (expect 205, incl. the 10 new), each AC1–AC6 + the id-less slug-fallback verified, AC3/AC4 id-prefix round-trip resolves to the CORRECT entity with NO false Class-A against a live sibling, and the 195-test golden/parity suite byte-unchanged.
  `go test ./internal/dispatch/` = 205 RUN / 205 PASS / 0 FAIL (195 + 10 new: 7 top-level + 3 subtests). Built the worktree binary and drove REAL `dispatch build` over a long-slug sd-b32 fixture: uncapped form 85 chars → emitted `spacedock-ensign-367s2zrbkm-backlog` 35 chars, namePattern-valid, id-prefix `367s2zrbkm` embedded, prefix/suffix kept, name+cycle3=42 (AC1/5/6). Parent commit emits the 85-char name (bug real, fix resolves). AC2/3/4/slug-fallback green via real `Reconcile`/build. Golden/parity unchanged — `git show edbec3a3 --name-only` touches NO golden/testdata and adds only the 2 new test files (no existing test modified).
- FAILED: Detached adversarial audit on a THROWAWAY checkout — try to REFUTE the id-resolution / no-false-Class-A claim; construct an adversarial edit (break resolveSlugToken's ambiguous/no-match→unclassified guard, or make the id-prefix HasPrefix over-match) and confirm a test catches it; route any Material finding back through the feedback flow.
  MATERIAL HOLE FOUND. On a detached `edbec3a3` checkout: edit `count != 1`→`count < 1` (weaken ambiguous guard) keeps all 10 shipped namecap tests GREEN; edit `HasPrefix(rec.id, token)`→`token[:SDB32MinPrefix]` (over-match on 2 chars) also keeps all 10 GREEN. Root cause: the AC3/AC4 fixture's two ids (`367s2zrb…`/`0qv3kaqs…`) share NO prefix (diverge at char 0), so neither the ≥2-match ambiguity guard nor full-token discrimination is exercised. Probes with two ids sharing a sub-10-char prefix FAIL under each edit and PASS against correct code — proving real holes (see Feedback Cycles). Routed back to implementation.
- DONE: Confirm the flagged unrelated failure `TestCodexResolveManifestAgainstInstalledHost` (internal/cli) is genuinely pre-existing/external (codex-host binary dependency), not introduced by this change.
  Reproduced on the PARENT commit (edbec3a3^, before this change) with the identical error: `codex plugin list failed: exit status 1 … Failed to read config file /Users/clkao/.codex/config.toml: Operation not permitted`. External codex-host-binary + config dependency; `edbec3a3` touches NO `internal/cli` file.

### Feedback Cycles

- **Cycle 1 — validation → implementation (detached adversarial audit, Material).** The shipped AC3/AC4 reconcile fixture seeds two sd-b32 ids that share no common prefix, so the id-prefix resolution's two safety properties are never exercised and a regression of either passes the suite:
  - Adversarial edit A — weaken the ambiguity guard `count != 1` → `count < 1` (a token matching ≥2 ids resolves to an arbitrary sibling). Shipped namecap suite stays GREEN. A false Class-A against a live sibling — the exact github#366 hazard — would ship undetected.
  - Adversarial edit B — over-match by comparing only `token[:SDB32MinPrefix]` instead of the full token. Shipped suite stays GREEN. A shortened-prefix regression that mis-resolves the wrong entity would ship undetected.
  - Refutation proof: probes with two ids sharing a long-but-sub-10-char prefix FAILED under each edit and PASSED against correct `resolveSlugToken` (run on the throwaway checkout, reverted before teardown). The production code is correct; the test STRENGTH is the gap.
  - Ask back to implementation: strengthen the AC3/AC4 reconcile fixture (or add one focused `resolveSlugToken` test) so two seeded ids share a sub-10-char common prefix — proving (1) the full 10-char prefix uniquely resolves, and (2) a 2-active-entity shared-10-char-prefix case stays unclassified (ambiguity guard). Re-run validation after.

### Summary

REJECTED. All six ACs + the slug-fallback verify with real `dispatch build`/`Reconcile` evidence reproduced outside the task body (205/205 green, 85→35-char cap proven against both the fix and the parent, golden/parity byte-unchanged), and the flagged cli failure is confirmed pre-existing/external. The detached adversarial audit found one **Material** test-strength hole: the AC3/AC4 fixture's two ids share no prefix, so neither the ambiguity guard nor full-token-length discrimination in `resolveSlugToken` is exercised — two adversarial edits that re-open the false-Class-A hazard both keep the shipped suite green. The shipped code is correct; the test does not guard the property it claims. Routed back to implementation (Feedback Cycle 1) with the exact fixture-strengthening ask. Gate is not clean until closed.

## Stage Report: implementation (cycle 2)

- DONE: Strengthen the AC3/AC4 reconcile fixture (test-only; no `resolveSlugToken`/`capWorkerName` logic change) so two seeded ids share a sub-10-char common prefix, proving both safety properties — full 10-char id-prefix uniquely resolves the CORRECT entity (kills edit B), and two active entities sharing a ≥10-char prefix stay UNCLASSIFIED via the ambiguity guard (kills edit A).
  Added focused `TestResolveSlugTokenSafetyProperties` (reconcile_namecap_test.go) over real `resolveSlugToken` with prefix-sharing 24-char sd-b32 ids: idShareShort/idShareShortSibling share only 6 chars (full-token discrimination case); idShareLong/idShareLongSibling share a 10-char prefix (ambiguity case). Production code (reconcile.go) byte-unchanged — `git diff` empty. Commit 45545b97 on worktree branch.
- DONE: Adversarial refutation — re-apply each edit (A and B) on a throwaway, confirm the strengthened test goes RED under each and GREEN against correct code; record the refutation.
  Edit A (`count != 1` → `count < 1`): ambiguity case RED — token "cccccccccc" resolved to "gamma-slug" instead of unclassified. Edit B (`HasPrefix(rec.id, token)` → compare only `token[:SDB32MinPrefix]`/2 chars): full-token case RED — token "aaaaaa0000" matched both ids → ok=false. Both reverted; reconcile.go restored byte-identical (`git diff --stat` empty). `go test ./internal/dispatch/` green: 209 passed (was 205; +4 focused subtests). `go vet` clean.
