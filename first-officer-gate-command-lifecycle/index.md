...
Require one successful response containing `record`, `validate`, `eligibility`, `consume`, and the record forms/flags `--briefing`, `--result`, `--association`, `--decision`, `--actor`, and `--directive`. This is the minimum current capability fingerprint: all tokens come from the one gate help body, so four subcommand-help calls distinguish no additional stale binary. Cache the pass only in the current FO session for that selected launcher string. Changing `SPACEDOCK_BIN`, selecting a different launcher, or receiving unknown-command/unknown-flag output invalidates the cache and requires one new probe. Do not persist capability state in the workflow.
...
| --- | --- |
| `approve` | Invoke `gate consume` directly. Only exit 0 plus `condition=approved-pending eligible=true consumed=true` and the expected immediate successor is success. Consume itself evaluates the authoritative condition under lock, atomically advances `status`, and marks the application consumed. Commit that mutation, then hand the newly current stage to the existing ordinary reuse-or-fresh dispatch procedure. No separate `status --set` advance is allowed. |
| `revise` | Never consume it as an advance. Invoke the existing `«feedback.route»` procedure; this task does not implement advisory correction rounds. `gate eligibility` remains available only when the condition needs diagnosis. |
| `hold` | Leave the entity at the gate and surface the reason; never consume, advance, or dispatch. `gate eligibility` remains an optional diagnostic read. |
...
After successful consume, `state commit` is the durability barrier: no successor `dispatch build` or host dispatch may occur until the commit contains the exact Resolution, `application.state: consumed`, and successor status. After that barrier, the generic gate lifecycle ends. The ordinary dispatch contract owns build/reuse/fresh dispatch, and each host adapter owns how spawn or completion is observed; this procedure adds no “very next host event,” returned-handle, or common transport requirement.
...
- closed + pending approval: do not re-record the decision; consume, which authoritatively accepts or refuses;
...
| Version-compatible repo-root `./spacedock` lacked the gate subcommands. | In scope: one cached-per-selected-launcher/session `gate --help` fingerprint plus fresh-build/refresh route before mutation. AC-5. Version/doctor-only readiness is explicitly insufficient; repeated subcommand probes are removed. |
...
**AC-1 (VALUE) — A normal FO-approved gated stage cannot dispatch without one durably consumed authorization.** In the real 3k validation-package replay, the FO emits the exact ordered authority trace (briefing record, delegated decision/Result record, consume), commits the retained package before presentation, and commits the consumed successor state before exactly one successful successor `dispatch build`. On-disk state ends at the expected successor with the same attempt's exact Briefing, Resolution, and `application.state: consumed`; the successor stage produces one new durable marker-bearing report/commit that was absent before dispatch. The measured count of advances or dispatches without a matching consumed application commit is **0**, versus the current prose contract's possible manual path of 1. *Verified by:* deterministic real-CLI replay plus one representative engaged live FO journey on each supported host, graded from command log, entity before/after bytes, state Git history, and marker/report cardinality. After the consumed commit, the ordinary dispatch contract—not this lifecycle—owns host handoff.

**AC-2 — The minimum authority sequence is load-bearing and fails closed.** Deleting briefing record, decision/Result record, or consume from the expected trace makes the scenario grader refuse successor dispatch. Removing the former open validate, closed validate, and eligibility reads from the positive trace does not weaken the recorder/consumer guards: malformed open/close writes fail inside `record`, and stale/blocked/wrong-stage/spent approvals fail inside locked `consume`. Revise, hold, and ineligible controls produce zero advance/dispatch effects; revise invokes feedback routing and neither revise nor hold is consumed as advance. *Verified by:* three deterministic source-derived event deletions, direct malformed-record and consume-refusal fixtures, and a red control that requires no diagnostic read in the successful three-command trace.
...
**AC-5 — Readiness and retained-input paths work at the CLI boundary while failures preserve landed write boundaries.** One `gate --help` call rejects a version-compatible but capability-stale executable before room/entity mutation and names refresh/fresh-build remediation; the pass is reused only for the same selected launcher in the current FO session. A repo-root-relative `--briefing` binds the same canonical bytes as its absolute form, and relative `--result`/`--association` inputs reach their semantic validation rather than failing mixed-root normalization. Missing/noncanonical Briefing and invalid association/actor/provenance name the missing artifact/step, return nonzero, preserve entity bytes, and leave no lock residue where promised. Validate/eligibility diagnostics are byte-clean; hold/blocked/repeat-consume refusals are byte-clean; stale consume changes only pending → superseded and never status. *Verified by:* one-help stale shim/counting probe, CLI relative/absolute equivalence fixtures for all retained path flags, and the existing whole-file/tree-hash refusal matrix.
...
**AC-7 — Resume is idempotent and one-use without mandatory diagnostic ceremony.** Open same-Briefing retry creates no duplicate attempt; a closed pending approval is not re-recorded; consumed state never consumes twice; stale state becomes superseded without advancing and requires a replacement Briefing. Across three fresh-process resume passes, the count of successful consumes and resulting stage transitions is exactly 1, whether or not an operator invokes optional `validate`/`eligibility` diagnostics. *Verified by:* fresh-process fixture passes and byte comparisons after each retry.

**AC-8 — The procedure is runtime-portable, fixture-backed, and transport-neutral.** The recorded-gate-lifecycle journey remains one host-neutral scenario with Claude, Codex, and Pi runners. Each host executes one representative engaged route and must satisfy the same observable oracle: three ordered successful gate mutations, a durable consumed successor commit before exactly one `dispatch build`, and exactly one new durable successor effect. After that build, each host follows its ordinary dispatch adapter; the generic lifecycle requires no next-event ordering, returned handle, common spawn schema, or exact-child attribution. Static contract tests prove every gate-entry route funnels through the lifecycle; route-by-host live multiplication is not required. *Verified by:* the three live journeys, AC-2's three deterministic command-deletion controls, `TestFOGateLifecycleOwnsEveryEngagedEntry`, and the existing Claude/Codex/Pi native dispatch fixtures.

## Minimum replay and test plan

The test package copies the real retained 3k validation Briefing and its declared artifacts into a temp folder-form workflow whose gated `validation` stage has a supported nonterminal successor. It preserves the package bytes and uses the historical delegated-approval shape: `--actor agent:first-officer`, `--reason "Captain directive: approved after reviewing the presented 3k validation gate."`, and `--directive "you have the conn toward the sprint goal; authorized to approve gates, PR, relevant CI lanes, and merge; use your judgement."`. The existing command-logging wrapper delegates to the freshly built real binary, records exit/stdout/stderr, and permits the existing dispatch stub only after the complete successful authority trace and consumed state commit.

1. **Baseline live journey (AC-1/AC-3/AC-6/AC-8; high):** launch the real FO, engage the held validation gate, grade its concise evidence review, supply/retain the delegated approval, and assert three ordered gate mutations, atomic advanced+consumed state, a state commit containing that authorization before exactly one `dispatch build`, and one new marker-bearing successor stage report/commit. Run this one representative route once on Claude, Codex, and Pi; ordinary host fixtures continue to own transport.
2. **Minimum-sequence proof (AC-2; medium):** deterministically delete each of briefing record, decision/Result record, and consume from an otherwise complete trace; every arm denies dispatch. Run the same real-CLI positive without open validate, closed validate, or eligibility, and retain direct fixtures showing malformed records fail inside `record` and stale/blocked/wrong-stage/spent applications fail inside `consume`.
...
5. **Resume passes (AC-7; low):** rerun fresh processes over open, pending, stale, and consumed snapshots without mandatory diagnostic reads; exactly one stage transition and consume across three passes. Add an optional-diagnostic arm only to prove reads do not alter the result.
6. **Capability-stale launcher (AC-5; low):** shim a passing `--version` and incomplete `gate --help`; assert one help invocation, pre-mutation halt, and fresh-build remedy. Reuse the pass for a second gate with the same selected launcher, invalidate it when `SPACEDOCK_BIN` changes, then run the baseline green.
...
The primary independent live oracle is durable state plus the command/dispatch log and state Git history. Static command names in skill prose are inspection evidence only. The three source-derived deletion controls prove the authority writes are load-bearing; existing gate-unit fixtures prove diagnostic reads are unnecessary to the write guards.

Reuse the current `recorded_gate_lifecycle_test.go`, command wrapper, 3k fixture, dispatch oracle, state-Git oracle, topology tests, and one live binding per host. Change `recordedGateRequiredEvents` from six to three; remove the two positive validate calls, eligibility call, repeated probe loop, and absolute-path workaround expectation; keep validate/eligibility only in focused diagnostic/refusal tests. Extend existing `internal/cli/gate_test.go` for retained-input normalization. Do not add a second scenario, harness, live matrix, transport parser, or provider runtime.

## Obligation delta

| Obligation | Authority | Bearer | Proof burden |
| --- | --- | --- | --- |
| Three ordered authority mutations; one consumed approval; zero unauthorized advances/dispatches | Canonical AC-1/AC-2 and `fo-gate-lifecycle` | First Officer lifecycle contract plus landed recorder/consumer guards | Real CLI fixture, three deterministic event-deletion controls, malformed-write/consume-refusal fixtures, and each host's representative live command/state trace |
| Exact package committed before presentation; consumed successor committed before dispatch | Canonical AC-1 | First Officer write contract | Ordered state Git history showing both durability barriers and no earlier dispatch build |
...
- **FO contract:** rewrite the existing `skills/fo-gate-lifecycle/SKILL.md` in place and adjust its one shared-core pointer in `skills/first-officer/references/first-officer-shared-core.md`. Remove mandatory diagnostic reads, repeated probes, absolute-path ritual, and runtime handoff prose; do not add a skill, host adapter wording, or presentation template.
...
- **Codified/live proof:** reuse `internal/ensigncycle/recorded_gate_lifecycle_test.go`, its existing 3k fixture/wrapper/oracle, topology/contract tests, and the current Claude/Codex/Pi live bindings. Replace six expected events with three, delete obsolete positive diagnostic calls/probe repetitions/path workaround, and extend existing refusal/resume controls. No second harness. From preserved implementation commit `86bad049` (1,468 added LOC vs `origin/main`), additions are capped at 100 and must be offset by at least 50 deletions; total branch surface is expected at or below **1,518 added LOC**. This is a tolerance boundary, not a prose-compression target.
- **Docs:** revise the already-touched `docs/site/concepts/gates-and-decisions.md` and `docs/site/reference/command-reference.md` to describe three authority mutations, one cached help fingerprint, CLI path normalization, and ordinary dispatch handoff; approximately 8–20 changed lines.
...
- **Rebaseline rule:** after each rebase, set a new informational host total only from committed upstream files outside this task's touched surface, rerun load-set equality/component-cap tests, and preserve this task's previously measured delta. Never absorb a task-owned increase, a shared-core/deferred-cap breach, or an uncommitted concurrent edit into the baseline. Any positive attributable host delta or component-cap breach requires ideation reapproval with the responsible prose/mechanism named; unrelated upstream growth does not trigger semantic compression here.
...
+that package. After the decision, it records the Resolution. An approval advances only
+when `gate consume` rechecks and spends that authorization; consume writes the new stage
+and consumed mark together, and that state is committed before ordinary dispatch begins.
...
+command-capability check. Once per selected launcher/session, `spacedock gate --help`
+must list `record`, `validate`, `eligibility`, and `consume` plus the semantic record
+flags. If it does not, refresh the launcher or build the current checkout and select it
+with `SPACEDOCK_BIN`; do not hand-edit gate frontmatter. Relative retained-input paths
+are resolved from the launch directory, so callers need not manufacture absolute paths.
```
...
In one fixture-backed live First Officer journey, package an exact validation Briefing, record and commit it, record an evidence-bearing delegated approval, consume and commit the application, and only then enter ordinary dispatch. Two controls must fail closed without approval advancement: revise routes through feedback, and hold/ineligible approval does not advance. Removing any of the three authority mutations must make the journey fail; removing diagnostic `validate`/`eligibility` reads from the positive must remain green because their safety checks already live inside the mutation commands.
