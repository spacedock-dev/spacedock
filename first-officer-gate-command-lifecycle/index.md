...
**AC-1 (VALUE) — A normal FO-approved gated stage cannot route without one durably consumed authorization, and only nonterminal targets dispatch.** In the real 3k validation-package replay, the FO emits the exact ordered authority trace (briefing record, delegated decision/Result record, consume), commits the retained package before presentation, commits every successful close before routing, and commits the consumed successor as a descendant. For a nonterminal target, exactly one successor `dispatch build` follows and the stage produces one new durable marker-bearing report/commit. For a terminal target derived from the same fixture, `status --next --json` contains no dispatchable successor, no dispatch build occurs, and the existing merge ceremony receives the current entity. On-disk state retains the same attempt's exact Briefing, Resolution, and `application.state: consumed` in both arms. The measured count of advances or dispatches without a matching consumed application commit is **0**. *Verified by:* deterministic real-CLI nonterminal replay, terminal-target control, and one representative engaged nonterminal live FO journey on each supported host. The nonterminal dispatch pre-HEAD must equal/descend from the consumed commit; terminal state/boot output plus merge-guard fixture prove the merge-only fork.
...
**AC-6 — The Captain receives a concise decision review, not recorder mechanics or raw artifacts.** The live output contains capability/change, test/evidence, reviewed snapshot, findings, one recommendation, and a concrete decision ask; entity/spec/Briefing are linked references. It does not lead with raw JSON/YAML, room listings, or `rebind`/`supersede` ceremony. Pi evidence must be one `assistant` message from the single flat root session after bind and before decision; nested subagent sessions and user/tool messages are ineligible. *Verified by:* the live scenario's structured gate-review grader and retained output artifact; raw-file-dump, nonassistant-role, and nested-session controls fail the grade.

**AC-7 — Resume is idempotent and one-use without mandatory diagnostic ceremony.** Open same-Briefing retry creates no duplicate attempt; a closed pending approval is not re-recorded and is committed before consume if a prior session stopped between close and commit; consumed state never consumes twice; stale state becomes superseded without advancing and requires a replacement Briefing. Across three fresh-process resume passes, the count of close commits, successful consumes, and resulting stage transitions is exactly 1 each. A consumed nonterminal status resumes dispatch; a consumed terminal status resumes the existing merge ceremony, never dispatch. A terminal row with an open PR remains discoverable to the `pr-merge` startup/idle hook only while its `mod-block: merge:pr-merge` records that ceremony; MERGED writes the existing sentinel and re-runs merge guard, OPEN remains blocked, CLOSED returns to the Captain. *Verified by:* fresh-process fixture passes, terminal-target state/boot and merge-guard controls, Git ancestry/hash assertions, and byte comparisons after each retry.

**AC-8 — The procedure is runtime-portable, fixture-backed, and transport-neutral.** The recorded-gate-lifecycle journey remains one host-neutral scenario with Claude, Codex, and Pi runners. Each host executes one representative engaged nonterminal route and must satisfy the same observable oracle: three ordered successful gate mutations, a close commit, its descendant consumed-successor commit before exactly one `dispatch build`, and exactly one later durable successor effect. Pi's review oracle reads only the root assistant session. The logging shim records commit hash and entity snapshot after each state commit and immediately before dispatch, making the ordering falsifiable without transport forensics. The deterministic terminal variant supplies the complementary zero-dispatch/merge-route proof; it is not multiplied across hosts. After build, each host follows its ordinary dispatch adapter; the generic lifecycle requires no next-event ordering, returned handle, common spawn schema, exact-child attribution, or exact built-artifact attribution. *Verified by:* the three live journeys, the shared terminal control, AC-2's three real-binary omitted-mutation controls, `TestFOGateLifecycleOwnsEveryEngagedEntry`, and existing Claude/Codex/Pi native dispatch fixtures.

## Minimum replay and test plan

The test package copies the real retained 3k validation Briefing and its declared artifacts into the existing temp folder-form workflow. Its ordinary form keeps the gated `validation` stage's supported nonterminal successor; a table arm changes only that same fixture's immediate target to the declared terminal stage. It preserves the package bytes and uses the historical delegated-approval shape: `--actor agent:first-officer`, `--reason "Captain directive: approved after reviewing the presented 3k validation gate."`, and `--directive "you have the conn toward the sprint goal; authorized to approve gates, PR, relevant CI lanes, and merge; use your judgement."`. The existing command-logging wrapper delegates to the freshly built real binary and records exit/stdout/stderr. Extend that same log with the state HEAD hash plus entity snapshot digest after each `state commit` and immediately before any `dispatch build`; the existing dispatch stub is permitted only when the real trace and Git ancestry prove the close and consumed barriers.

1. **Baseline and terminal fork (AC-1/AC-3/AC-6/AC-8; high):** launch the real FO on the existing nonterminal fixture, grade its concise evidence review, supply/retain the delegated approval, and assert three ordered gate mutations. The bind commit snapshot is open; the close commit snapshot contains the exact Resolution/pending application; the consumed commit is its descendant and contains advanced+consumed state; dispatch's pre-HEAD equals or descends from consumed; one later commit adds the marker-bearing successor report. Run this representative route once on Claude, Codex, and Pi. In the same deterministic fixture, select terminal as the immediate target, run the same three real commands/commits, then assert consumed terminal state, empty `status --next --json` dispatchability, zero dispatch builds/effects, and entry into the existing merge guard path. The already-recorded development entity is a read-only corroborating sample, not a second fixture or mutation.
...
7. **Runtime lanes and Pi authority (AC-8; high):** keep the host-neutral scenario once with Claude, Codex, and Pi bindings. Each live lane retains command logs, gate review output, before/after entity, state Git history, and the one durable successor effect. Require exactly one flat Pi root session and accept the review only from a root `assistant` row between bind and decision; a nonassistant-row unit control must fail. Host-native fixtures separately prove each adapter's transport event shape; live lanes do not require public event uniformity or every route on every host.
...
Reuse the current `recorded_gate_lifecycle_test.go`, command wrapper, 3k fixture, dispatch oracle, state-Git oracle, topology tests, merge-guard fixture, and one live binding per host. Derive the terminal arm by changing only the existing fixture's immediate target; do not add a second scenario. Delete `TestRecordedGateLifecycleCapabilityStaleLauncherHaltsBeforeMutation` and `probeRecordedGateCapability`; the existing live/fixture oracle already counts one just-in-time successful help call. Narrow the existing Pi JSONL reader/parser rather than adding transport machinery. Extend the current merge-guard fixture/test and one existing structural contract test for the terminal in-flight PR selector. Do not add a new harness, live matrix, runtime protocol, launcher-swap lab, compatibility path, exact-child/artifact attribution, or recorder behavior.
...
| Root-authoritative Pi review | Canonical AC-6/AC-8 | Existing Pi live adapter/parser | Single flat root session, assistant role, bind-before-review-before-decision ordering; nested/nonassistant controls cannot satisfy the grade |
...
Captain rulings of 2026-07-24 remove six non-authority burdens from the generic lifecycle: mandatory open/closed validation reads, mandatory pre-consume eligibility, repeated subcommand help probes, the session executable-identity cache, absolute-path caller ritual, and runtime-specific next-event/handle language. Earlier proof-boundary cuts also remove uniform public exact-child/built-artifact forensics and the route × host live matrix. The reset adds only the target-kind fork, root-assistant Pi evidence, and terminal in-flight PR selector needed to reach the already-owned merge ceremony. No authority outcome is removed: the FO still owes exact package binding, exact decision provenance, locked one-use consume, and durable consumed state before routing. This canonical AC block, test plan, and obligation delta are the sole active proof authority. Later historical reports do not compete with it.

## Expected surface and tolerance

Current code tip `d55ddca3` is 1,500 additions / 63 deletions against its merge base. Cycle 13 is limited to these nine existing files and exact intended amendment:

| File | Intended cycle-13 delta |
| --- | ---: |
| `skills/fo-gate-lifecycle/SKILL.md` | +4 / -4 |
| `skills/first-officer/references/first-officer-shared-core.md` | +1 / -1 |
| `internal/ensigncycle/recorded_gate_lifecycle_test.go` | +36 / -73 |
| `internal/ensigncycle/recorded_gate_lifecycle_pi_live_test.go` | +7 / -16 |
| `docs/site/reference/command-reference.md` | +1 / -1 |
| `mods/pr-merge.md` | +2 / -2 |
| `docs/dev/_mods/pr-merge.md` | +2 / -2 |
| `internal/status/merge_guard_test.go` | +16 / -0 |
| `internal/contractlint/layering_restore_test.go` | +6 / -0 |

The intended amendment is **+75/-99**, a net deletion of 24 lines, yielding 1,575 branch additions and 162 deletions. Additions have a hard stop at **+88** so the branch remains at or below the approved 1,588-addition cap; no tenth file is permitted without re-ideation.

- **FO contract:** replace cached identity prose with one just-in-time probe and route consumed terminal state to merge, nonterminal state to dispatch.
- **Codified/live proof:** delete the 41-line launcher-cache test and 32-line cache helper. Derive one terminal arm from the existing lifecycle fixture; narrow the existing Pi root-session parser; reuse the existing live journeys without a new lane or protocol.
- **Merge resumption:** change only the two `pr-merge` selectors and prove the existing merge guard finalizes terminal+blocked state after the existing sentinel. The selector must not admit arbitrary terminal PR rows without `mod-block: merge:pr-merge`.
- **Docs and contract pin:** replace the cache paragraph in the existing command reference and add one compact structural assertion covering the shipped/development selector. The concepts page already states terminal approval enters merge/terminalization and needs no change.
- **Component budgets and attribution:** shared first-officer core remains **≤26,754 bytes** and deferred lifecycle remains **≤6,600 bytes**. Recompute task-attributable load bytes against the current upstream parent; they must remain non-positive.
- **Stop conditions:** any schema/recorder/application/CLI production change, new fixture/harness/runtime protocol, new gate or merge lifecycle, launcher-swap experiment, compatibility layer, presenter work, exact-child/built-artifact attribution, tenth file, or cap breach returns to ideation before editing.
...
+mark together, and a descendant commit is durable before routing begins. Nonterminal
+targets enter ordinary dispatch; terminal targets enter the existing merge ceremony.
...
+command-capability check. Immediately before every gate lifecycle, freshly resolve the
+launcher and run `spacedock gate --help` exactly once. Help must list `record`, `validate`,
+`eligibility`, and `consume` plus the semantic record flags. Do not cache the result.
+Otherwise refresh or rebuild the launcher; do not hand-edit gates. Relative retained-input
+paths resolve from the launch directory, so callers need not manufacture absolute paths.
```
...
- No schema or recorder/application logic; no gate judgment duplication; no presenter redesign; no advisory correction-round implementation; no blocker evaluator or hold authoring; no arbitrary artifact registry; no provider launch/polling; no new gate verb; no second terminal lifecycle; no hand-authored `gates:` repair; no stable-release work.

## Minimum value demonstration seed

In one fixture-backed live First Officer journey, package an exact validation Briefing, record and commit it, record and commit an evidence-bearing delegated approval, consume and commit the application, and only then enter ordinary dispatch for a nonterminal target. Reuse that fixture with terminal as the target and prove the same durable authority trace yields empty dispatchability and the existing merge ceremony. The log must bind each barrier to a Git hash and entity snapshot. Revise, hold, and refused approval retain their close commit without advancing. Three omission controls run the remaining calls through the real binary and fail dispatch; removing diagnostic `validate`/`eligibility` reads from the positive remains green because their safety checks live inside mutation commands.
