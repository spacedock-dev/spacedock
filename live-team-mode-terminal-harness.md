...
Probe #2 is decisive: team mode IS engaged (not the env-token `Claude API` confound), the FO did the REAL ToolSearch hop (not an init eyeball), and `select:TeamCreate` returns no-match on 2.1.179. Contrast: on 2.1.177 (before the mid-session auto-update) my natural-language drives successfully created `teams/scrubprobe3/config.json`, and the FO independently confirms its live 2.1.177 session resolves TeamCreate via the hop. So the deferred-hop false-negative class is RULED OUT — the 2.1.179 team-tool absence is a genuine upstream regression (#68721 reaching interactive sessions), not a measurement artifact. The `teams/session-<id>/config.json` files that appear are session-team auto-scaffolding, not TeamCreate results. The (pin-2.1.177 + DISABLE_AUTOUPDATER) vs (defer-to-#68721) call stands for the captain.
