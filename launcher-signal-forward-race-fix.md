...
- DONE: Independently reproduce AC-1's divergeable proof — confirm TestLaunchSignalForwardNoDataRace GREEN on fixed code; revert the fix on a throwaway and confirm the SAME test goes RED; run full `go test ./... -race`.
  Fixed worktree GREEN under `go test ./internal/cli/... -race` (and 20/20 deterministic). Throwaway (git archive of fix HEAD e9c82046, separate /tmp checkout) with `forward()` moved BEFORE `cmd.Start()` → RED, exit 1, `WARNING: DATA RACE` read at host_launch_unix.go:52/:53 by the pump goroutine (created pre-Start) vs `cmd.Start()` write at host_exec.go:301 — deterministic 10/10 RED. Full `go test ./... -race`: all packages ok (internal/cli 27.6s).
- DONE: Detached adversarial audit on a THROWAWAY checkout — construct claim-breaking edits the new test must catch; a GREEN under any is a HOLE.
  Ran on the throwaway (never the impl worktree). Edit A (goroutine-before-Start): RED via -race (forwarding still fired, race alone failed it — proves the race leg is load-bearing). Edit B (`forward` made a no-op): 0 races but FAIL `exit=124, want 143` (proves the AC-2 forward leg is load-bearing). Edit C (post-Start read kept, `Signal()` dropped): 0 races but FAIL `exit=124, want 143` (forward leg catches a silent break with no race). No claim-breaking edit stayed GREEN — no HOLE found; clean audit, refuted nothing material.
- DONE: Verify AC-2 and AC-3 with evidence; confirm diff scope internal/cli only; produce PASSED/REJECTED.
  AC-2: TestLaunchResidentParent / TestLaunchExitCodePropagation / TestLaunchPTYTerminalSignals all PASS under -race (incl. "external SIGTERM forwarded to host"); early-signal drain proven independently by a standalone arm→deliver→drain-after program (3/3 PASS under -race). AC-3: independent sweep finds exactly one production goroutine (host_launch_unix.go:48, now post-Start/HB-ordered); the only off-main Start-written-field read is `cmd.Process` at :52/:53; `cmd.ProcessState` (host_exec.go:305) is read on main after Wait; single Start site. Diff scope = internal/cli only (host_exec.go, host_launch_other.go, host_launch_test.go, host_launch_unix.go). gofmt/vet clean, windows cross-build ok.

### Summary

PASSED. Independently reproduced AC-1's RED→GREEN divergence at the exact claimed read/write sites and confirmed it is deterministic, not timing-stress (fixed 20/20 GREEN, reverted 10/10 RED); full `go test ./... -race` is green. The detached adversarial audit (throwaway checkout) found no hole: the new test has independent teeth on both legs — the race detector catches a reintroduced ordering, and the `exit==143` assertion catches a silently-broken forward (proven by the no-op and dropped-Signal edits, both 0 races but FAIL). AC-2 holds (existing signal tests green under -race; early-signal-drain mechanism proven standalone) and AC-3 holds (one launcher-startup goroutine, now HB-ordered; no second antipattern; diff confined to internal/cli). Per the captain's merge gate this internal/cli fix merges on the DETERMINISTIC + `-race` proof delivered here; the tagged-commit live E2E remains the authoritative live check.
