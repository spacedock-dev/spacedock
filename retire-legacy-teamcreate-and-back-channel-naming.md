...
- DONE: Implement BOTH q4 and 9q4 in ONE commit per both ideations' co-edit maps (q4: delete using-legacy skill + :9 override line, rename back-channel->inter-agent communication per the verbatim prose diff, rewrite BareModeAdvisory + PresentFalseHint; 9q4: remove ## Degraded Mode from fo-dispatch-recovery, replace the claude-fo-dispatch.md trigger with the ## Dispatch Failure Retry rung + :68 carve-out + durable ### Dispatch Retries note, reword shared-core:49, ship the net-new default-tagged AC-1 bounded-retry offline oracle). NEVER q4-alone-first.
  One joint commit 3b1323d5 (32 files, +292/-2047) on the shared worktree. All mapped co-edits applied. AC-1 oracle `assertBoundedRetryObservables` (positive + no-retry RED + third-attempt RED controls) ships with the rung; retargeted the degraded-bare oracle to `assertBareReachableObservables` + `TestLiveBareReachable`. AC-3 review grep: no "back-channel" on the shipped FO surface; the `claude-binding` preservation anchor now resolves `## Inter-Agent Communication`.
- DONE: Re-baseline per-host foHostLoadBaselineBytes; TestFOHostPromptLoadRatchet + TestFOHostPromptLoadRatchetDiscriminates green.
  claude 111183->95378 (AC-2: FO load shrinks 15805 B >= 14 KB net of the rename, read from FO_FUNCTION_METRICS). DEVIATION from the ideation's "pi UP ~13 B / codex UNCHANGED": the 9q4 shared-core:49 reword shrinks the SHARED load, so codex 74608->74593 and pi 70725->70723 (rename +13 offset by the reword -15) were BOTH touched and re-baselined to measured (zero-slack — what AC-4 actually requires). Both ratchet tests green.
- DONE: go build ./..., go test ./... (incl. ./internal/contractlint/...), and go test ./... -race all green as the SINGLE joint commit; the AC-1 offline oracle ships in the same commit as the rung; stay within tolerance.
  All three green (15 ok packages, race clean; default + //go:build live tags compile). Tolerance: 32 files — the mapped ~11 q4 + ~7 9q4 PLUS 3 surprise bindings the contractlint-only ideation spike missed and the captain's widen-scope ruling (see Summary). The claude surface shrank 15805 B — no net-positive breach.

### Summary
Landed q4+9q4 as one green commit (3b1323d5). Three bindings the ideation spike (which ran only `./internal/contractlint/...`, not `./...`) did not surface, all mechanically forced by the deliberate changes: 5 status boot goldens + 1 dispatch parity golden carried the old PresentFalseHint/BareModeAdvisory strings (regenerated via -update; only those lines changed); and `internal/ensigncycle/teardown_marker_consistency_test.go` os.ReadFile'd the deleted using-legacy skill for the legacy TERMINAL_TEARDOWN_BOUNDED marker. On escalation the captain RULED widen-scope: retire the orphaned legacy TeamDelete teardown-grade machinery (teardown_marker_consistency_test, teardown_grade_test, teardown_grade_watcher_test, the `terminalTeardownMarker` const + grade fns in streamwatch, 3 teardown-exclusive fixtures), keeping go-forward coverage. Per the guardrail I KEPT `sonnet_teamdelete_hang` (a SHARED go-forward stream fixture used by streamsink/streamwatch_regression/shallow_boot_measure) and the whole go-forward stream watcher. ONE judgment call flagged: the live `TestLivePtyEnsignCycleTeamTeardown` accepted the legacy marker as one of two termini — I surgically dropped that retired terminus, keeping its go-forward dispatch-to-terminal proof (a `//go:build live` test I compile-checked both tags but could not execute here).

## Stage Report: validation

Covers BOTH q4 and 9q4 against the single joint commit `3b1323d5` (worktree branch `spacedock-ensign/retire-legacy-and-retry-rung`, tree clean).

- DONE: Verify EVERY AC on BOTH entities against joint commit 3b1323d5 with reproduced evidence; mutation-test each automated guard.
  q4 AC-1: skill dir gone, no `select:TeamCreate`/`using-legacy` token on claude-fo-dispatch.md; `TestLegacyTeamCreatePathFullyRetired`+`TestNormalPathContractNamesNoLegacySkill` green — re-adding the override line REDs both (mutation confirmed). q4 AC-2 VALUE: `FO_FUNCTION_METRICS claude_bytes=95378`, 15805 B below the pre-change 111183 (parent `ca136f83` measured=111183 with using-legacy in the claude path) ≥14000 → PASS. q4 AC-3: FO-surface `back-channel` grep=0, heading `## Inter-Agent Communication`; `TestFOFunctionNormalizationPreservationSuite/claude-binding` green — renaming the heading back REDs it (mutation confirmed). q4 AC-4: all 3 baselines == measured (claude 95378 / codex 74593 / pi 70723, zero-slack); ratchet+discriminator green; adding inert prose to a measured claude file REDs the ratchet (mutation confirmed). q4 AC-5: goldens carry the literal advisory/hint — reverting `PresentFalseHint` REDs 5 status goldens; reverting `BareModeAdvisory` REDs `TestBuildParityCrossProduct` (both mutations confirmed). 9q4 AC-1: `assertBoundedRetryObservables` is a real Agent-name parser; dropping its upper bound REDs the third-attempt control, dropping its lower bound REDs the no-retry control (both mutations confirmed). 9q4 AC-3: retry rung prose holds one entity reversibly + "session dispatch mode NEVER changes"; the three dissolved triggers carry no session-wide state (review read).
- DONE: Detached adversarial audit on a THROWAWAY checkout; widen-scope teardown-grade retirement removed ONLY legacy grading; pty terminus (a) proven by on-disk assertions.
  All four assignment-named claim-breaking edits RED a specific guard (legacy override→AC-1 tests; loose-below/inert-prose→ratchet; PresentFalseHint "run TeamCreate"→status+build goldens; retry-bound drop→oracle controls). Retired symbols `terminalTeardownMarker`/`gradeTerminalTeardown`/`markerEmittedByAssistant` = 0 in Go source; `sonnet_teamdelete_hang` fixture + `newStreamWatcher` SURVIVE and are exercised green (StreamSink/streamwatch-regression/shallow-boot, all default-tagged). pty `TestLivePtyEnsignCycleTeamTeardown` terminus (a) asserts `^## Stage Report: \S` + `^status:\s*done\s*$` + `someCommitNamesOnly` (path-scoped commit) — dispatch-to-terminal proven without the retired marker; live-tag compiles clean (`go test -tags live` exit 0).
- FAILED: 9q4 AC-2 (VALUE) bare no-team live drive not reproduced here.
  `TestLiveBareReachable` (//go:build live) WOULD spend live budget (`~/.claude/benchmark-token` present → OAuth/sonnet, nested `spacedock claude` -p FO drive). I did not initiate live model spend from a nested validation subagent (an agent-message instruction is not the captain's consent for a costly/outward-facing action). Offline shape-oracle `assertBareReachableObservables` + its 4 RED controls (bare-Agent-required, retired-report-rejected, recovery-load-rejected, team-shape-rejected) are default-tagged and GREEN — the bare-reachability SHAPE is guarded in CI, but the VALUE proof is pending. Captain-run command: `go test -tags live -run TestLiveBareReachable ./internal/ensigncycle -v -count=1`.
- DONE: Semantic adversarial pass; classify findings; confirm the joint commit green across ./... incl contractlint + ensigncycle + -race.
  `go build ./...` exit 0; `go test ./...` exit 0 (contractlint 0.851s, piruntime fresh); `go test -race ./...` exit 0 (release 29.9s, status 53.9s fresh). Findings below.

### Findings

- F1 (deferred / evidence — NOT material): 9q4 AC-2's live proof is un-run (captain-gated by budget). Not an outcome or mechanism defect — the retirement doesn't touch bare mode, this very session runs SendMessage-without-TeamCreate, and the offline oracle is green. Promotes to material only if the captain-run drive fails. Gate must obtain it (command above) or consciously accept the offline evidence before final sign-off.
- F2 (deferred / polish — NOT material): 4 residual "back-channel" strings in internal Go comments (`internal/dispatch/build.go:70,314`, `internal/cli/frontdoor.go:60,79`), OUTSIDE AC-3's verified "FO surface" boundary (its Verified-by grep is over the FO surface, which is clean). Trigger to promote: captain intends a repo-wide (code-comment) rename; sweep as follow-up.
- F3 (informational): AC-4's ratchet is upper-bound-only — a baseline set ABOVE measured (loose) does NOT red (mutation confirmed PASS). The DELIVERED state is verified zero-slack (baseline==measured via FO_FUNCTION_METRICS), and loosening is a deliberate reviewable edit, so no VALUE AC is at risk; noted so the gate reads AC-4's "green proves ==" as "green proves ≤; == established separately by the metric."

### Summary

Recommendation: PASSED, with one required captain-run follow-up — the 9q4 AC-2 bare no-team live drive (`go test -tags live -run TestLiveBareReachable ./internal/ensigncycle -v -count=1`). Every other AC across both entities (q4 AC-1..5, 9q4 AC-1/AC-3) is reproduced against commit 3b1323d5 and each automated guard is mutation-verified non-tautological; the detached adversarial audit is clean (all four claim-breaking edits red their guard); the widen-scope teardown retirement removed only legacy grading while the go-forward stream watcher + shared fixture survive and stay green; the tree is green across `./...` and `-race`. No material or mechanism defect found — do NOT route through an implementation feedback cycle (nothing to fix). F1 is a captain-gated VALUE-proof step, not a code defect; F2/F3 are deferred/informational with the revisit conditions above. The AC-4 pi/codex direction deviation from ideation (both shrank via the shared-core reword rather than pi-up/codex-flat) is a benign re-baseline-to-measured, which is what AC-4 requires.
