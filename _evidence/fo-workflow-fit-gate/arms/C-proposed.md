...
A fit failure is not repaired by adding a shippable mechanism. If the work does not belong in this workflow, it does not belong at any shape: reshaping it until it satisfies the workflow's proof policy buys admission with machinery the workflow never needed. Re-shape only after fit passes.
