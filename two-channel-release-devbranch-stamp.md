...
2. **Channel-aware `release.yml` version-stamp branch (M2 finding — k6d owns this).** The "Stamp plugin manifests to the release version" step hardcodes `git switch next` / `git push origin next` (`release.yml:161,172`). Make the stamp branch derive from the release channel: a stable `v*` cut on `main` stamps+pushes the plugin manifest `version` to `main`; an edge cut to `next`. Today's hardcoded `next` would land the displayed plugin version on the edge channel while the stable plugin on `main` stays un-stamped. This is the M2 carve-audit finding (pj's flip checklist:128) explicitly tagged "naturally k6d's two-channel mechanism" — **k6d owns it**, confirmed below.
...
- the channel-aware `release.yml` version-stamp branch (stable→`main` / edge→`next` — the M2 finding in pj's flip checklist:128, confirmed k6d's here);
...
- **AC-b (channel-aware release.yml version-stamp branch).** The `release.yml` "Stamp plugin manifests to the release version" step derives its stamp/push branch from the release channel — a stable `v*` cut stamps+pushes plugin-manifest `version` to `main`, an edge cut to `next` — so a stable cut never lands the displayed version on the edge branch.
  - *Verified by:* a NEW `internal/release` workflow-guard Go test that parses `release.yml`, locates the version-stamp step, and asserts its branch is channel-derived (a stable-channel parse yields `main`, the edge path `next`) — and FAILS on the current hardcoded `git switch next` / `git push origin next`. The test's expected value comes from the channel input, not from the step's own text, so it is a real relationship check, not a tautology. (Today no guard test pins this step's branch — confirmed: `workflow_exec_guard_test.go` covers only the journey-ledger separation + goreleaser ordering. The guard is net-new.)

- **AC-c (two-channel brew, edge lane reused).** A `goreleaser` run emits a stable `spacedock` cask and an edge `spacedock@next` cask, each pinning its channel's archive url+sha; the edge publish cadence is the UNCHANGED `next-publish.yml`.
  - *Verified by:* `goreleaser release --snapshot --clean` producing both cask artifacts in `dist/` (the generated `.rb` cask files for `spacedock` and `spacedock@next`), inspected for distinct names and per-channel archive ids. The "reuse" half is verified by `next-publish.yml` carrying no diff in k6d's change set (`git diff` over the PR shows it untouched) — a negative check on a real file, not body prose.
...
| b  | NEW `internal/release` workflow-guard test parsing `release.yml`, channel-derived branch assertion (RED on hardcoded `next`) | Go unit (workflow parse) | low |
| c  | `goreleaser release --snapshot` emits both casks in `dist/`; `git diff` shows `next-publish.yml` untouched | live build + diff check | low–medium |
