...
pr: pr-merge:450
verdict: passed
completed: 2026-06-30T20:13:55Z
archived: 2026-06-30T20:13:55Z
---
...
- **RESOLVED (cycle 1 closed).** Impl ensign reverted Sites D + B and kept A + C (commit `813ae998`, rebased to `a811a162` onto current main); true diff vs `origin/main` is exactly 2 lines (Sites A & C). `audit-82k-contract` re-reviewed the fixed cores and returned **CLEAN** — both original proofs demonstrably closed against the now-prescribed text (inline-report.md: `status --read` last `## Stage Report` = offset 11 → full report, all items seen; fenced-report.md: offset 27, the fenced heading at line 33 correctly skipped; Site B `total_lines`=8 vs the old `wc -l`=7). FO independently re-ran AC-2 on `a811a162` (not the ensign's self-report): `dispatch TestBuild` goldens byte-identical, `contractlint` + `ensigncycle` green. AC-1 is unaffected (the B+D revert restores load-bearing reads, so adoption-not-regress is at least as well supported). Gate re-entered and approved.

## Stage Report: implementation (cycle 2)

- DONE: Revert Sites B and D to their originals per the cycle-1 audit; keep Sites A and C trimmed
  Commit `813ae998` on branch `spacedock-ensign/read-guidance-redundant-with-grep`. Site D (`first-officer-shared-core.md` `## Completion and Gates` step 1) restored to the fence-aware `status --read <ref> --json` last-`## Stage Report` heading read; Site B (`ensign-shared-core.md` `## Stage Report Protocol` append bullet) restored to the newline-robust `status --read <entity-path> --json` `total_lines`. Sites A (ensign `## Working` item 1) and C (FO Prefer-Grep bullet) unchanged — both keep grep-primary + the fence-safe `--read --json` fallback. Net `git diff` vs base = the two core `.md` files only, 2 insertions / 2 deletions.
- DONE: Re-run the AC-2 behavioral checks after the partial revert — goldens byte-identical and contractlint/ensigncycle green
  `go test ./internal/dispatch/ -run TestBuild -count=1` `ok` (fresh run, goldens byte-identical → the partial revert is still runtime-loaded skill prose, not dispatch text); `go test ./internal/contractlint/ ./internal/ensigncycle/` `ok`. No prose-grep test added.

### Summary

Cycle-1 audit was correct on both findings, so I reverted Sites B and D and kept only Sites A and C — the two sites where grep-primary plus a fence-safe `status --read --json` fallback genuinely suffices. Site D's bare `grep -n '## Stage Report'` was unanchored and dropped the fence-safe fallback (it could latch onto an inline-code/fenced body mention and miscount the gate's done/skipped/failed); Site B's `wc -l` undercounts by 1 on a no-trailing-newline file. Both reverted to the load-bearing `status --read --json` form. AC-2's behavioral surfaces stay green on a forced fresh run, and the net contribution against my base is now exactly the two clean core-`.md` edits (2 lines). Commit `813ae998`.
