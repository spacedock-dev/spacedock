...
## Stage Report: validation (FO-recorded from validator run 4333f38b, 2026-06-20)

**Verdict: PASSED** (independent fresh validator, run `4333f38b`; result file `/tmp/spacedock-dispatch/eq-val-result.md`). One merge-safety caveat (qn↔eq integration) — merge-time resolution required, not a breach.

- DONE (AC-1 live): `~/.pi/agent/settings.json` `packages` contains the Spacedock worktree entry (persistent from implementation); resolved package root has `package.json` (name=spacedock, pi.skills, pi.extensions) + `.pi/extensions/spacedock.ts` + `skills/ensign/SKILL.md` + `skills/first-officer/SKILL.md`. Built binary run from `/tmp` (non-repo cwd): `spacedock doctor --host pi` → `OK Spacedock package: <worktree>` + `Pi runtime ready.` exit 0.
- DONE (AC-2 live precondition): replicated pi-subagents' own `discoverAvailableSkills("/tmp")` + `skillsWarning` via a deno probe importing the real `skills.ts`. `discoverAvailableSkills("/tmp")` → 12 skills incl `ensign ( user-package )` + `first-officer ( user-package )`, no cwd/symlink dependency. `skillsWarning("/tmp", ["ensign"])` → `undefined`; `["ensign","first-officer"]` → `undefined`; control `["nonexistent-skill-xyz"]` → warning (proves probe logic; inverse of `0637e2ed`). AC-2b nested-probe constraint recorded honestly — validator has no `subagent(...)` tool in its child context; the load-bearing `skillsWarning` precondition is proven; FO (parent) to run the nested `subagent(... skill:["ensign"])` probe from `/tmp` for full AC-2b closure.
- DONE (AC-3 behavior-tested): `go test ./internal/cli/ -run TestPiRuntimeConfigRetiresSkillFlagsAndCwdFallback -v` → all 4 subtests PASS (cwd_fallback_removed, dev_override_retained, launch_args_have_no_retired_skill_flags, doctor_gates_on_spacedockPackageOK_from_non-repo_cwd). `go test ./internal/cli/ ./internal/piruntime/ -race` green.
- DONE (AC-4 live dev-override): added marker `skills/val-ac4-probe/SKILL.md` → fresh-process `discoverAvailableSkills("/tmp")` lists it as `user-package` WITHOUT reinstall; removed → gone. Local path read live. Marker cleaned up.
- DONE (non-regression): `go test ./...` green except pre-existing `internal/status` `TestMigrationCheckFixturesParseConsistently` (verified on eq's parent baseline `25caff38` — identical failure; eq's commit doesn't touch `internal/status` or the debrief fixture). 2 pre-existing dirty files NOT in eq's commit. gofmt clean on eq's Go files.
- DONE (textual region discipline — NOT breached): eq's commit `4f254748` touches ONLY its owned regions (`runInitWithPi`/`piRuntimeConfigFromEnv`/`checkPiRuntime`/`parsePiSetupArgs`/`printPiDoctorReport` + new `PiInstall`/`SpacedockPackageStatus`/`piSpacedockPackageStatus` + the D4 `--skill` retirement inside the `runPi` argv literal). Does NOT touch qn's `parsePiFrontDoorArgs`/`runPi`-wrap-assembly/`setPiHelp`.
- **MERGE-SAFETY CAVEAT (load-bearing for the merge phase)**: validator attempted the live `eq`↔`qn` merge. One benign comment-adjacency conflict in `runPi` (both sides added a comment before the `argv` literal — resolvable by keeping both; `go build` succeeds after). BUT the merged tree FAILS 4 of qn's safehouse-wrap tests (`TestPiFrontDoorWrapsWhenKnobPresent`/`PlainWhenNoTrigger`/`WrapsWhenSafehouseProfileAlone`/`WrapInnerEqualsUnwrapped`) — root cause: eq's D5b raises `piRuntimeLaunchReady` to require `spacedockPackageOK`, but qn's tests build an isolated fake pi env with NO Spacedock package, expecting the launcher to proceed to the wrap path; the new gate bails with "Pi runtime is not ready" before reaching qn's wrap logic. This is a real integration-level collision but NOT a region-discipline breach and NOT a regression on eq's own branch (qn's tests don't exist on eq's branch — eq is based on `25caff38`). MERGE-TIME RESOLUTION REQUIRED: qn's 4 safehouse-wrap tests need to stub `spacedockPackageOK = true` (canned package status via the existing fake seam) OR eq's check needs a test seam qn's tests can use, so the launcher proceeds to the wrap path under test. FO adjudicates ownership of that resolution at the merge boundary.

### Summary

Validation PASSED. All 4 ACs reproduce live from a non-repo cwd. Region discipline clean. One merge-time integration task flagged: qn's 4 safehouse-wrap tests must be updated for eq's raised `piRuntimeLaunchReady` gate when the two branches integrate (stub `spacedockPackageOK` in qn's tests, or add a shared test seam). Not a breach, not a regression — a normal merge-time integration the FO sequences into the merge phase.
