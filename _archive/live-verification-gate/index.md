...
- DONE: AC-1/AC-4 the gate — terminal `--set` guard REFUSES an uncited runtime-observable `Verified by: live <ref>` AC under `require-external-proof: true`
  `TestLiveRunGuardRefusesUncitedRuntimeAC` PASS (yy-shape placeholder → exit 1, frontmatter stays `status: implementation`, diagnostic names "live run"). Driven end-to-end through real `NativeRunner.Run`→`runSet` guard (native_harness_test.go), not a unit shortcut.
- DONE: AC-4 three-way resolveLiveCitation — real→pass, 404→refuse, connectivity/auth→INDETERMINATE not a refusal
  `TestResolveLiveCitationThreeWay` PASS (citedAndReal / definitivelyAbsent / indeterminate via injected stub) + `TestResolveLiveCitationSession` + `TestResolveLiveCitationPlaceholder` PASS. Indeterminate exits non-zero but is a labeled tooling error (`could not be checked … not a refusal … --force`), classified distinct from definitivelyAbsent — a blip cannot masquerade as a missing live run.
- DONE: AC-1 offline ACs NEVER refused + guard inert when opt-in off
  `TestLiveRunGuardNeverRefusesOfflineAC` PASS (offline AC → done) and `TestLiveRunGuardInertWhenOptInAbsentOrFalse` PASS (absent + false subcases → done).
- DONE: Prove strength — reproduce the implementer's two adversarial probes
  never-classify break-edit (`classifyLiveACs` returns nil) REDs `TestLiveRunGuardRefusesUncitedRuntimeAC` (exit 0 not 1) + `TestClassifyLiveACs/yy-shape` — under-gate caught. over-gate break-edit (drop `live\b` from `liveLeadRe`) REDs `TestLiveRunGuardNeverRefusesOfflineAC` (offline AC got exit 1) + offline classifier — over-gate caught. Both directions red; worktree restored clean (HEAD b8f03087, no diff).
- DONE: AC-2 offline gradable core — a broken durable outcome reds the grade
  `TestScenarioGradesBrokenOutcomeNegative` PASS (gate-advanced-to-done AND no-gate-review-observed both red the grade, even with a gate-shaped final message — state-oriented, not transcript-echo).
- DONE: AC-2 LIVE half — `go test -tags live -run TestLivePrimitiveRunsAgainstClaudeAdapter ./internal/ensigncycle`
  PASS against real Claude (79.7s run 1; 51.6s persisted run 2) on existing `~/.claude/benchmark-token` — NO 401, no rotation needed. A real agent ran the gate-held scenario through the promoted `internal/livescenario` primitive: gated entity stayed `status: review` (durable state held), observed output presented "Gate review … Decision". Persisted session artifact at `session:/tmp/spacedock-live-artifacts/live-verification-gate/claude-shared-scenarios/livescenario-primitive/claude-stream.jsonl` (session_id 0640c458) RESOLVES `citedAndReal` through the real `resolveLiveCitation` — AC-2's own `Verified by: live …` would PASS p4's gate at terminal `--set`. p4 demonstrably eats its own gate.
- DONE: AC-3 dev-template live-scenario recommended-practice block present
  `TestLiveScenarioRecommendedPracticePresent` PASS; `### Live scenario for runtime claims` (development.md:133) carries the recording-proves-the-watcher / text-check-proves-the-words distinction + the {runbook, setup, durable-outcome} triple beside External-proof / Detached-audit.
- DONE: Full offline `go test ./...` green + `go vet ./...` + `go vet -tags live`
  `go test ./...` 1020 passed / 13 packages; `go vet ./...` clean; `go vet -tags live ./internal/ensigncycle/` clean; live-tagged adapter compiles.

### Summary

PASSED. Every AC's cited evidence reproduced from a check outside the task body (tests, exit codes, on-disk state, a real live run). The gate's strength is proven, not assumed: I reproduced both implementer probes and confirmed the suite REDs in both directions (under-gate misses the yy slip; over-gate refuses offline ACs). AC-2's recursion is closed for real — the live primitive ran against a real Claude agent (no 401, existing token), held the gate on durable state, and produced a persisted session artifact that resolves `citedAndReal` through the actual resolver, so p4 eats its own gate. The three-way resolver correctly classifies a connectivity blip as indeterminate (a labeled tooling error), distinct from a definitive refusal. Note for the FO: the detached adversarial audit on the status-mutation/guard + shipped-template surface is the FO's parallel in-flight item; this validation is necessary-and-strong but the gate is presented clean only once that audit records no material findings.

### Feedback Cycles

**Cycle 1 (2026-06-04) — detached adversarial audit, 1 material.** The audit (`audit-live-verification-gate`, checkout b8f03087) confirmed classification/opt-in/inert/reuse are strong (both implementer break-edits red), but found a material hole at the crux: `ghCiRunResolver`'s `notFoundRe = /(?i)HTTP 404|Not Found/` (the real connectivity-vs-404 discriminator) has ZERO tests and is unsound — the loose `Not Found` substring false-refuses on (1) a proxy/gateway 5xx body containing "Not Found" and (2) a masked-404 from a private/unscoped-token repo (GitHub masks 403→404), refusing a run that exists with misleading remediation. One-directional (false-refuse only; indeterminate never passes). Routed to implementation: anchor the regex to `(HTTP 404)`, extract a pure exit+output→three-way classifier with a unit test over literal gh error strings (genuine-404→absent; bad-creds/rate-limit/DNS/502-with-Not-Found-body→indeterminate), and make the masked-404 message name token-scope as well as cite-a-run.

Implementation fix (commit on `spacedock-ensign/live-verification-gate`): (a) `notFoundRe` anchored to the parenthesized `\(HTTP 404\)` — gh's own definitive 404 token, not a bare substring. (b) Extracted pure `classifyGhRunOutput(runErr, combined) liveResolutionKind` (live_proof.go) that `ghCiRunResolver` now delegates to; `TestClassifyGhRunOutput` feeds literal gh strings — genuine `(HTTP 404)`→definitivelyAbsent; `Bad credentials (HTTP 401)` / `rate limit (HTTP 403)` / DNS `no such host` / a 502 body containing "Not Found"→indeterminate. Adversarial: the collapse-everything-to-absent break-edit REDs the new test (rate-limit/DNS/502 cases); reverting to the OLD loose regex REDs the gateway-502 case — proving the anchoring is load-bearing in both directions. (c) The masked-404 refusal diagnostic now names BOTH remediations — "Cite a resolvable ci-run:<id> or session:<path>, OR if the run exists check your token's repo scope (a private/unscoped-token repo returns a masked 404)" — locked by an added `scope` assertion in `TestLiveRunGuardRefusesUncitedRuntimeAC`. Full offline `go test ./...` 1028 passed / 13 packages; `go vet ./...` + `go vet -tags live ./internal/ensigncycle/` clean; `gofmt -l` clean.
