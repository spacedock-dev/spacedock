...
- DONE: help_test.go is byte-restored to origin/main — the brittle help-OUTPUT grep (nine flag strings, rule-fragment wording, example command lines) is GONE and NO replacement help-content grep was added anywhere in the diff. Confirm `git diff origin/main -- internal/dispatch/help_test.go` is empty and grep the whole diff for any surviving assertContainsAll/strings.Contains over --help/usage/stdout.
  `git diff origin/main -- internal/dispatch/help_test.go` = 0 bytes. Branch diff vs origin/main is only dispatch.go (printBuildUsage hunk) + build_input_mode_test.go; help_test.go is absent. Grepped the whole dispatch package: no assertContainsAll/strings.Contains over --help/usage rendered output in the diff. The lone `strings.Contains(ex, "--advance")` (build_input_mode_test.go:141) is a coverage guard ensuring the advance example is in the executed set, not a help-prose assertion; the AC-2 stderr check asserts the parser's real error contract (exit 2 + exact message), not help wording.
- DONE: AC-1/AC-2/AC-3 stay verified BEHAVIORALLY by the surviving tests: AC-2 (TestDispatchBuildAdvanceInputMode — exit/stdout/stderr on the flag-file --advance success and the stdin+--advance exit-2 failure) and AC-3 (TestDispatchBuildHelpExamplesParse — EXECUTES every rendered help example against the real parser; independently reproduce the drift-catch by breaking one example → FAIL, restore). Parser/input-mode selection byte-unchanged (direction a).
  AC-2 PASS: flag/file `--advance` → exit 0, empty stderr, envelope has schema_version/prompt/model and omits subagent_type/name; stdin JSON + `--advance` → exit 2, empty stdout, stderr == `error: flag/file input requires --entity-path, --stage, and --checklist-file`. AC-3 PASS: renders help, extracts 3 examples (≥1 `--advance`), runs each via runNative asserting exit 0. Independent drift-catch on a throwaway checkout: dropped `--checklist-file` from the reuse-advance example → TestDispatchBuildHelpExamplesParse FAILED exit=2 with the exact trio-required error, then restored (clean tree, throwaway removed). Direction (a): dispatch.go diff is one hunk inside printBuildUsage; parseBuildOptions/loadBuildFields/hasRequestFlags/isBuildRequestFlag appear 0× in the diff and build.go is absent from it.
- DONE: gofmt/vet clean; go test ./internal/dispatch/ and -race green; go test ./... green except the named pre-existing environmental internal/cli test. Produce PASSED/REJECTED.
  `gofmt -l` clean on both changed files; `go vet ./internal/dispatch/` clean; `go test -count=1 ./internal/dispatch/` green (27.9s); `go test -race -count=1 ./internal/dispatch/` green (26.8s); `go test ./...` green except `internal/cli` TestCodexResolveManifestAgainstInstalledHost (declared environmental: `Failed to read config file /Users/clkao/.codex/config.toml: Operation not permitted`; touches no dispatch code). Verdict: PASSED.

### Summary

VERDICT: PASSED (cycle 2). Fresh independent re-review after the captain flagged the brittle help-OUTPUT prose grep. The revert is clean: `git diff origin/main -- internal/dispatch/help_test.go` is empty, no replacement help-content grep exists anywhere in the diff, and the surviving coverage is genuinely behavioral — AC-2 pins exit/stdout/stderr for both the accepted flag/file `--advance` and the rejected stdin+`--advance` form, and AC-3 executes every rendered `--help` example through the real parser. I independently reproduced the AC-3 drift-catch on a throwaway checkout (broke the reuse-advance example → FAIL exit 2, restored). Direction (a) holds: only printBuildUsage changed; the four parser/input-mode functions are byte-unchanged. gofmt/vet clean; dispatch tests + `-race` green; `go test ./...` green except the declared environmental internal/cli Codex-config-read failure. No material findings; no deferred risks.
