...
## Stage Report: validation (FO-recorded from validator run bb75eff9, 2026-06-20)

**Verdict: PASSED** (independent fresh validator, run `bb75eff9`; result file `/tmp/spacedock-dispatch/b2-val-result.md`). The validator ran Phase-1 tests in a fresh detached worktree at `cf8e3a5c` to avoid the worktree's stray codex-test remnants skewing results — methodologically sound.

- DONE (AC-2 live round-trip — PROVEN, independently corroborated): the validator confirmed the Phase-2 stage report quotes the `contact_supervisor need_decision` escalation text + the FO's `intercom reply` text verbatim, and that the ensign resumed within the 10-min window. Crucially, the validator cross-checked with the FO (parent, `subagent-chat-019ee237`) via `intercom ask` — the FO confirmed (1) receiving the `need_decision` from `subagent-worker-e8a69a88-...-1`, (2) replying with decision (a) as quoted, (3) within the 10-min timeout with the ensign resuming. The AC-2 evidence is NOT self-referential (verbatim FO text + independent FO source).
- DONE (AC-3 dual completion signal — PROVEN): both signals observed — the subagent return value + the explicit inbound done-message sent via `contact_supervisor`/`intercom send` before return. The FO independently observed the done-message preceding the subagent return. The stage report is the file-verify gate for both.
- DONE (AC-4 ensign talkback — PROVEN, subsumed by AC-2): the `need_decision` is the behavioral exercise of the `## Clarification` protocol Phase-1 added to `pi-ensign-runtime.md` — proof by behavior, not prose.
- DONE (AC-5 worker identity capture — PROVEN): identity captured at spawn (`subagent-worker-e8a69a88-...-1`); FO addressed the still-running worker by that captured identity; reply received mid-run. The `worker-identity-capture` schema fields are declared in the Pi adapter binding.
- DONE (AC-1 structural contractlint — Phase 1, PASS): `TestCapabilityBinding` PASS (independently re-confirmed by FO on the b2 branch). 7 named capabilities match across the core + all 3 adapters. Dual-extraction + set comparison — legitimate structural contractlint, not prose-grep.
- DONE (AC-6 runtime-neutral regression — offline gate PASS): `go test ./internal/cli/ ./internal/contractlint/ ./internal/ensigncycle/ ./internal/dispatch/ ./internal/hostneutrality/ ./internal/piruntime/ ./internal/contract/ ./skills/integration/` all green incl `-race`. `hostneutrality` green (zero host tool calls in the host-neutral core). `claude-live`/`codex-live` live lanes run in CI on the PR (API keys + env-approval gate, not runnable from worktree); offline gate is the validator bar, CI confirms.
- DONE (non-regression + region discipline): `go test ./...` fully green on the clean `cf8e3a5c` checkout (incl. `internal/status` — the debrief-fixture fix landed on main). b2's only code commit is Phase 1 (`cf8e3a5c`, 6 files: 1 new test + 5 skill `.md`); NO Phase-2 code edits (Phase 2 was live-proof only). gofmt clean on b2's Go file; the 2 gofmt-flagged files (`prose_function_routing_test.go`, `section_read.go`) are pre-existing baseline drift on main, not b2.

### Open items (non-blocking)
- **Worktree cleanup**: the b2 worktree has stray untracked/unmerged `internal/ensigncycle/codex_*` files from a concurrent/aborted prior session (not b2, not main). FO to clean (`git reset --hard cf8e3a5c && git clean -fd internal/ensigncycle/`) before/after terminalization — does not affect the verdict. (FO note: these are the same codex-test remnants preserved once already from qn's worktree; will re-preserveclean.)
- **AC-6 live lanes**: `claude-live`/`codex-live` are the FO's CI step on the PR; offline gate green, CI confirms.
...
- DONE: Renames. `worker-back-channel`→`«addressable-worker»` with `inbound-message-service` folded in as the worker→FO half + the listen call on the `→` line; `worker-identity-capture`→`«worker-identity»`; `context-budget-probe`→`«context-budget»`. Kept `«async-dispatch»`/`«completion-signal»`/`«roster-reconcile»`. 7 capabilities → 6.
- DONE: M1 ABSENT-honest. The Claude `«addressable-worker»` `→` segment is `Claude: ABSENT — Claude Code 2.1.183 exposes no SendMessage/TeamCreate … the back-channel substrate is Pi-only`. Removed the `claude-fo-dispatch.md` "a ToolSearch(select:TeamCreate) no-match … is the normal path where the named-background-Agent + SendMessage back-channel works" equivalence (was line 99 under `## Degraded Mode`).
- DONE: AC-1 contractlint replaced. `internal/contractlint/capability_binding_test.go` now binds the «fn» structure: it extracts the DEFINED capability set (`## «name»:` headings whose body carries a per-host `→` line) and the body-CALLED set (`«name»` references beyond the heading), asserts they are equal (a dead definition or a dangling call reds), and asserts every `→` line names all three hosts (`**Claude:**`/`**Codex:**`/`**Pi:**`). Two independent divergeable sources + per-host coverage; empty-set guards; NOT prose-grep. Mutation-verified RED on: dropped Pi `→` segment, dead definition (renamed heading), dangling call (undefined cited name), flipped host label.
- DONE: Proof green. `go build ./...` clean; `go test ./internal/contractlint/` green incl. the host-neutrality scanners (`TestDispatchCoreHasNoClaudeTeamImperative`, `TestDispatchCoreHasNoClaudeModelToken`, `TestEventLoopCoreHasNoPRScan` + their discriminators) and `TestCapabilityBinding`. Full offline gate green (`./internal/cli/ ./internal/ensigncycle/ ./internal/dispatch/ ./internal/hostneutrality/ ./internal/piruntime/ ./internal/contract/ ./skills/integration/`). gofmt clean on the test file.
- DONE: Scope held to Approach A (capability contract surface). Did NOT touch the Pi model-stamping wiring (Approach B / member 2 `pi-dispatch-model-stamping`): the Pi `### Model Resolution` + `### Canonical Model Space` sections keep member 2's behavior; only their dangling `worker-identity-capture` references were retargeted to `«worker-identity»`. The Claude-enum `sonnet`/`opus`/`haiku` was NOT inlined on the `→` line (it would trip `TestDispatchCoreHasNoClaudeModelToken`); the Claude `«worker-identity»` segment defers to "the Claude enum declared in `claude-fo-dispatch.md`".
- FLAGGED (net-delta determination — needs FO adjudication): The checklist requires the net line delta vs origin/main be NEGATIVE. It is NOT: working-tree vs origin/main is **+162** (189 ins / 27 del). My rework vs the prior #409 HEAD (the +172 state the FO reviewed) IS a cut: **−10** (140 ins / 150 del). The cumulative stays positive because origin/main had NO capability layer and NO `capability_binding_test.go` at all — the required additions (the consolidated per-host `→` lines + the 134-line non-vacuous test) exceed the contract prose removable within Approach A's scope. Going cumulative-negative would require gutting the per-adapter back-channel **operational** prose the checklist's "NET CUT" clause names — but the codex `## Awaiting Completion`/`### Foreground wait`/idle-wake-observation prose and `claude-fo-dispatch.md`'s `## Worker Back-Channel`/`## Spawn Call` machinery carry load-bearing await/reuse contract NOT captured by the terse `→` lines. Cutting them to hit a line count is the SB2-class loss-of-contract risk; reconciling the whole Claude file to ABSENT-honest is the larger M1 effort the checklist deliberately scoped OUT (it asked only for `→ Claude: ABSENT` + the line-99 removal). I held scope rather than gut load-bearing prose. FO: confirm whether (a) −10 vs the reviewed +172 state satisfies the intent, or (b) the cumulative-vs-origin/main must go negative — in which case the per-adapter operational-prose cut is a separate, larger pass that needs its own scoping (it touches the codex/claude await machinery, not just the capability layer).
- NOTE (runtime): this rework ran on a **Claude Code** session that exposes **no SendMessage tool** (confirmed via ToolSearch — no top-level or deferred SendMessage). The dispatch prompt's completion-signal `SendMessage(to="team-lead", …)` is therefore not emittable from this runtime. This is itself live corroboration of the M1 finding I implemented (Claude `«addressable-worker»` = ABSENT). Completion is signaled via this committed stage report (the file-verify gate) + the final assistant message; the FO should verify against this entity file, not an inbox message.

### Summary

Feedback Cycle 1 rework complete on the capability contract surface (Approach A). The 7 named capabilities are now 6 «fn» prose-functions the dispatch-core body calls by name, with per-host realizations on their `→` lines; the `## Named Capabilities` registry and all three `## Capability implementations` tables are dissolved; the Claude `«addressable-worker»` binding is ABSENT-honest and the line-99 TeamCreate-equivalence is removed; the `## Awaiting Completion` guardrails are byte-untouched. AC-1's contractlint is rewritten to the «fn» structure (dual-source, mutation-discriminating, per-host coverage). `go build ./...` + `go test ./internal/contractlint/` green; full offline gate green. Net delta is −10 vs the reviewed +172 state but +162 cumulative vs origin/main — FLAGGED for FO adjudication, since a cumulative cut requires gutting load-bearing per-adapter await/reuse operational prose (an SB2-class risk and a larger out-of-scope pass), which I declined to do unilaterally.

## Stage Report: implementation (Claude flag-gated DETECT refinement — 2026-06-19)

Branch `spacedock-ensign/pi-back-channel-dispatch`, code commit `3149b736` (2 files, +2/−2). This refines the prior Feedback Cycle 1 rework: the Claude `«addressable-worker»` binding was made flat ABSENT, but the flag-gated back-channel IS available when `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` is set (captain-confirmed live this session). Flat ABSENT was wrong; the correct binding is DETECT-based.

- DONE: Refined the Claude `«addressable-worker»` `→` segment in `fo-dispatch-core.md` from flat ABSENT to DETECT-based. Now: `Claude: PRESENT when CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS is set (probe SendMessage availability at boot — ToolSearch/enabled-tools — since the flag gates the back-channel substrate), else ABSENT (fresh one-shot + task_notification-observed completion + TaskStop teardown, no reuse)`. The `**Codex:**` and `**Pi:**` segments are byte-untouched; the 3-host `→`-line shape is preserved so `TestCapabilityBinding`'s per-host coverage stays green.
- DONE: Reconciled the `claude-fo-dispatch.md` `## Worker Back-Channel` prose. It said Claude PROVIDES the back-channel UNCONDITIONALLY ("with no separate setup step") — which contradicted the new flag-gated core framing. Reconciled to the flag-gated/detected framing: PROVIDED "when the team-mode opt-in is enabled — detect it at boot by probing SendMessage availability … present when enabled, else fall back to fresh one-shot dispatch". Did NOT re-introduce the line-99 'no-TeamCreate = normal SendMessage path' equivalence (it was removed in Feedback Cycle 1 and must stay removed). Did NOT use the literal `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` token in this file — `TestNormalPathContractInlinesNoLegacyMachinery` (`deletedFromMergedFloor`) requires that token ABSENT from `claude-fo-dispatch.md` AND the legacy skill; expressed the gate as "team-mode opt-in" + the `SendMessage` probe instead.
- DONE: `## Awaiting Completion` guardrails byte-untouched. `git diff` shows the only `claude-fo-dispatch.md` change is the single `## Worker Back-Channel` opening line (1 ins / 1 del); the premature-reap ban and idle guardrail are unchanged.
- DONE: Proof green. `go build ./...` clean (exit 0); `go test ./internal/contractlint/` green incl. `TestCapabilityBinding`, the host-neutrality scanners (`TestDispatchCoreHasNoClaudeTeamImperative`, `TestDispatchCoreHasNoClaudeModelToken`, `TestEventLoopCoreHasNoPRScan`), and the legacy-layering scanners (`TestNormalPathContractInlinesNoLegacyMachinery`, `TestLegacyConsumerRetiredButPathLives`, `TestNormalPathContractNamesLegacySkill`). Broader offline gate green (`./internal/contractlint/ ./internal/cli/ ./internal/dispatch/ ./internal/hostneutrality/ ./internal/piruntime/ ./internal/contract/ ./skills/integration/`).
- DONE: Diff minimal — exactly the two named sites (the Claude `→` segment + the reconciled note). `git diff --stat`: 2 files, +2/−2.
- NOTE (runtime corroboration): this refinement ran on a Claude Code session that DOES expose `SendMessage` (loaded via `ToolSearch(query="select:SendMessage")`). The prior cycle's stage report (line 434) recorded a session with NO SendMessage tool. The two sessions differing on `SendMessage` availability is exactly the variance the flag-gated DETECT framing captures — flat ABSENT could not represent it; DETECT-based PRESENT/ABSENT does. The completion signal `SendMessage(to="team-lead", …)` IS emittable from this session.

### Summary

Refined the Claude `«addressable-worker»` binding from flat ABSENT to flag-gated DETECT (PRESENT when `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` is set, probed via `SendMessage`/`ToolSearch` at boot; else fresh one-shot), and reconciled the contradicting `claude-fo-dispatch.md` `## Worker Back-Channel` prose to the same detected framing without re-introducing the line-99 equivalence and without the banned flag token (which `TestNormalPathContractInlinesNoLegacyMachinery` forbids in that file). The 3-host `→` shape and the `## Awaiting Completion` guardrails are preserved byte-untouched. Diff is minimal (2 files, +2/−2). `go build ./...` and `go test ./internal/contractlint/` green incl. `TestCapabilityBinding` and the host-neutrality + legacy-layering scanners. Code commit `3149b736` on `spacedock-ensign/pi-back-channel-dispatch`.

## Stage Report: validation (Feedback Cycle 1 «fn» reframe + DETECT — 2026-06-19)

Re-validation of the Feedback Cycle 1 rework (commits `01fc4333` «fn» reframe + `3149b736` Claude flag-gated DETECT) on branch `spacedock-ensign/pi-back-channel-dispatch` (HEAD `3149b736`). Scope: the capability contract surface (Approach A). AC-2/3/4/5's Pi-host live proof was validated in the prior cycle (`bb75eff9`) and is not re-driven; this cycle verifies the reframe did not regress it and that the new «fn» structure is sound.

- DONE: Independently verify the «fn» reframe + detect binding — `go test ./internal/contractlint/` green from the worktree.
  `TestCapabilityBinding` + host-neutrality scanners (`TestDispatchCoreHasNoClaudeTeamImperative`/`...ModelToken`/`TestEventLoopCoreHasNoPRScan` + discriminators) + legacy-layering scanners (`TestNormalPathContractInlinesNoLegacyMachinery`/`TestLegacyConsumerRetiredButPathLives`/`TestNormalPathContractNamesLegacySkill`) ALL PASS (fresh `-count=1 -v`).
- DONE: Structural confirmation — body CALLS «fn»s; registry + tables GONE; every `→` names all 3 hosts; Claude DETECT-based.
  `grep -rn "Named Capabilities\|Capability implementations" skills/` → NONE (registry + all 3 per-adapter tables dissolved). 6 `## «fn»:` defs (`addressable-worker`/`async-dispatch`/`worker-identity`/`completion-signal`/`context-budget`/`roster-reconcile`), each `→` line carries `**Claude:** … · **Codex:** … · **Pi:** …`. The core body calls each by name (lines 25/36/37/40/142/144). Claude `«addressable-worker»` `→` = `PRESENT when CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS is set (probe SendMessage … ToolSearch/enabled-tools), else ABSENT` — DETECT, not flat. NB2's `intercom address` leak is closed (renamed to neutral "worker address" at core :37/:87); `hostneutrality` green confirms zero host imperative leak.
- DONE: `## Awaiting Completion` guardrails byte-intact vs origin/main.
  `diff` of the extracted `## Awaiting Completion` section in `claude-fo-dispatch.md` (origin/main vs HEAD) → IDENTICAL. The only `claude-fo-dispatch.md` changes are the `## Worker Back-Channel` opening line (flag-gated framing) + the line-99 TeamCreate-equivalence removal under `## Degraded Mode` — not `## Awaiting Completion`.
- DONE: LIGHT LOCAL LIVE E2E (Claude PRESENT path).
  Flag set this session (`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`). `ToolSearch(select:SendMessage)` returns the tool (PRESENT path confirmed). Spawned a throwaway `Agent(run_in_background=true)` worker; it sent a mid-run `PROBE_PING` to its lead (worker→FO half, observed via `task_notification`); I replied `token=BACKCHANNEL-OK-7Q2X` via `SendMessage(to=name)` (FO→worker half, which resumed the worker); the worker echoed `RECEIVED: BACKCHANNEL-OK-7Q2X`. Full round-trip proves the Claude `«addressable-worker»` PRESENT/DETECT binding describes actual behavior, not prose.
- DONE: DETACHED ADVERSARIAL AUDIT (throwaway checkout `/tmp/b2-audit-*`, NOT the impl worktree; removed after).
  Mutated the «fn» binding 4 ways; each turned `TestCapabilityBinding` RED, restored tree GREEN: (1) dropped Pi `→` segment on `«context-budget»` → `capability «context-budget» → line does not bind host "Pi"`; (2) dangling `«phantom-capability»` body call → set mismatch (called⊋defined); (3) dead definition (renamed `«roster-reconcile»` heading) → set mismatch; (4) flipped host label `**Pi:**`→`**Py:**` on `«async-dispatch»` → does-not-bind-host. AC-1 guard is non-vacuous.
- DONE: AC cross-check (all 6 ACs reproduced; scope held to Approach A).
  AC-1 PROVEN (`TestCapabilityBinding` green, strengthened over the prior table-based version). AC-2/AC-4/AC-5 Pi-host live proof preserved from prior validation (`bb75eff9`, FO-corroborated, run `e8a69a88…`); the reframe is contract-prose only and does not touch the proven Pi host behavior; the Claude-host back-channel equivalent re-proven live this session. AC-3 CORRECTED by the reframe (SB2 fix — Pi completion is PRIMARY subagent-return + optional `intercom send` advisory; the `contact_supervisor` done-fiction dropped; file-verify gate intact in `pi-first-officer-runtime.md` `## Awaiting Completion`). AC-6 offline gate fully green (`contractlint`/`hostneutrality`/`cli`/`dispatch`/`piruntime`/`contract`/`skills/integration`); live `claude-live`/`codex-live` lanes are the FO's env-gated CI step. Scope confirmed Approach-A-only: Pi adapter diffs are capability-name retargets (`worker-identity-capture`→`«worker-identity»`) + the SB2 completion correction + the Q13 null-model narrative; the Pi model-stamping behavior (`intercom list` parent-model read, canonical-model-space declaration) is byte-preserved — no Pi-code, no Approach-B behavior change.

### Summary

PASSED. The Feedback Cycle 1 rework is sound: the 6 capability «fn»s are defined-and-called with all-3-host `→` lines, the registry + per-adapter tables are dissolved, the Claude `«addressable-worker»` binding is flag-gated DETECT (PRESENT live-proven this session via a real `SendMessage` round-trip; flag set + tool present), and the `## Awaiting Completion` guardrails are byte-intact vs origin/main. AC-1's contractlint is non-vacuous (4 adversarial mutations all RED on a detached checkout). All 6 ACs have reproduced evidence; AC-3 is corrected (SB2 done-fiction dropped); no AC lacks evidence; scope held to Approach A with no Pi-code/Approach-B behavior change beyond retargeted references. The offline gate is fully green and `go build ./...` is clean; the `claude-live`/`codex-live`/`pi-live` lanes remain the FO's env-gated CI gate on #409. Recommendation: PASSED.
