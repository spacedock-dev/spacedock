# Briefing: feedback-round convention — ideation gate, third presentation

**Chosen direction (revised per your annotations):** the command deferred ITSELF via this entity's own ordering — the first cut is a documented entry-format convention, prose-only: a specified `### Feedback Cycles` entry format (timestamp, reviewer/loop, verdict, surface vs estimate with %, AC unchanged-or-narrowed note), hand-appended by the FO and the in-stage loop; actuals from one documented git one-liner; deviation is arithmetic any reader does. The falsifiable rebuttal test for keeping the command returned nothing: tamper-evidence guards an adversary that never appeared; auto-capture replaces a one-liner whose mistyping IS the drift the format catches. Command/binary/AC-hash/escalation machinery all deferred, shipped only if live drives show hand-authored entries drifting.
**Declared expected surface:** prose-only — ~3 doc files (~16 lines: two skill-prose swaps + dev-README diffs for the ideation estimate line and the in-stage round convention), 1 fixture + 1 offline check for the e6j value proof, 0 Go source. Tolerance: 2×, plus a hard self-check — any product code in the first cut trips a reconfirm.
**New ACs per your annotations:** AC-6 — any similar-shape in-stage reviewer loop (roborev re-panel and detached-audit as the dev instances, phrased generically) appends a conforming entry to the same section. AC-7 — a validation-time landing audit: generic principle in the skills, dev specifics in docs/dev/README.md, neither leaking.
**FO recommends approve.**
Value proof unchanged: the e6j replay (deviation vs declared estimate screams at round 2; real history silent to round 10), now a fixture + offline check.
Decision: approve = recorded pending advance to implementation + unlocks lane 1's next dispatch (az). Reject = bounce with your asks.

---
## Full revised design follows
---
