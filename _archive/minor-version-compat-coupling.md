...
pr: pr-merge:468
verdict: passed
completed: 2026-07-03T02:10:44Z
archived: 2026-07-03T02:10:44Z
---
...
- DONE: Every **AC-N** (AC-1..AC-5) verified by REPRODUCING its cited evidence, not trusting the report.
  AC-1: `TestStampVersionFlipsCompatibilityFloor` re-run PASS; read the test — it restamps ONLY the release version on the original fixture copies (manifest via `StampVersion`, prose via `StampProseVersion`) and the same fixed binary 0.24.5 flips Compatible→TooOldBinary on both gates; no integer anywhere in the path.
  AC-2: `TestCompare`/`TestParseMajorMinor` PASS covering both directions, patch/prerelease interchange, and every spiked real shape. Real-binary check reproduced from scratch: downloaded the published v0.22.0/v0.23.0 darwin_arm64 release tarballs, built the new binary from the worktree — both integer-era binaries abort the tombstone fixture with "Upgrade the binary" exit 1; the new binary (`spacedock 0.24.0-pre1+dev (contract 3)`) rejects the real v0.22.0/v0.23.0 manifests (fetched from their tags) as too-old-plugin exit 1 and accepts its own checkout's manifest exit 0 — byte-identical messages to the implementation report.
  AC-3: grep confirms no `CONTRACT_VERSION` constant, no `ParseRange`/`MalformedRange`/`PluginPredatesContract`, no `--skip-contract-check`; exactly two frozen sentinels (`">=3,<4"` in both vendored manifests, `frozenContractToken = "(contract 3)"` in cli.go); `internal/contractlint` PASS; the FO shared-core carries the new gate with the single stamped `0.24` literal (grep: one occurrence across skills/).
  AC-4: `TestGateHostStaysSilentForTooOldPlugin`, `TestFrontDoorTooOldPluginAutoRefreshes` (claude+codex), `TestFrontDoorTooOldPluginNoInstallRefuses` (claude+codex), `TestFrontDoorTooOldPluginSecondMissRefuses` all PASS; read the tests — real behavioral tests through the hostOps seam asserting announce text, no mismatch print on the heal arm, install-exactly-once, and no blind launch.
  AC-5: `TestUnstampedSourceBuildReportsCheckoutVersionPlusDev` PASS — a real `go build` of cmd/spacedock whose `--version` line 1 is checked against the manifest read independently from disk. Full `go test ./... -count=1` green (16 packages).
- DONE: Detached adversarial audit on a THROWAWAY checkout, never the implementation worktree — sentinel freeze, prose/manifest drift window, dev-build minor claim, auto-heal re-gate-once each attacked; the deliverable's own tests caught EVERY edit.
  Throwaway clone of the branch under the session scratchpad. (1) Tombstone bumped to `">=4,<5"` → `TestVendoredManifestTombstoneFrozen` RED. (2) `frozenContractToken` bumped to `"(contract 4)"` → RED via `dev_version_test.go:77`'s independent `(contract 3)` literal (see accounting below). (3) Prose minor bumped with manifests fixed AND manifest bumped with prose fixed → `TestProseMinorMatchesVendoredManifestMinor` RED in BOTH directions; live `manifest-tag-gate v0.24.0` with drifted prose refuses exit 1 with the stamp remedy. (4) Stale-checkout dev build (manifest set to 0.23.0, `go build`) reports `spacedock 0.23.0+dev` and is rejected by a 0.24 manifest as too-old-binary exit 1 with the rebuild/upgrade remedy. (5) Re-gate-once deleted from `resolveHealableGate` (launch blind after install) → `TestFrontDoorTooOldPluginSecondMissRefuses` RED ("exit = 0, want non-zero"). The audit refuted nothing material; minor findings below.
- DONE: A PASSED/REJECTED recommendation with honest accounting: the contractlint sync test binds two INDEPENDENT divergeable values.
  Verified by reading and by attack (3) above: the prose side comes from `release.ProseMinor` over the stamped FO shared-core file, the expectation from each vendored manifest's own `version` parsed by `contract.ParseMajorMinor` — two artifacts a release stamps together, red on either side diverging. Not prose-grep, not an echo of one file. Honest accounting of discounted/flagged evidence: (a) `internal/cli/cli_test.go`'s token tests (:90, :107) assert `--version` output against `frozenContractToken` ITSELF — self-referential, discounted as freeze proof; the effective freeze guard is `dev_version_test.go:77`'s independent literal, which my audit proved reds on a bump. AC-3's "pinned by the sync test" is strictly true only for the tombstone; the token's pin lives in internal/cli. Advisory hardening: add an independent `(contract 3)` pin to contractlint (or de-self-reference cli_test). (b) The implementation report's "zero remaining CONTRACT_VERSION references outside docs/roadmap" overclaims: stale comment-only mentions survive in three untouched test files (codex_plugin_dir_test.go:183, decoupling_behavior_test.go:100, install_behavior_codex_test.go:162) and docs/dev/_proposals — prose only, the mechanism itself is gone. (c) Cosmetic edge: a re-gate that lands too-old-binary prints the mismatch message twice (gateHost's default arm, then printHealableRemedy). None of (a)-(c) blocks.

### Summary

Recommendation: PASSED. All five ACs reproduce from scratch — including re-downloading the published v0.22.0/v0.23.0 binaries for the cross-era matrix and re-running the AC-1 divergeable stamp-then-gate flip — and the full suite is green. The detached adversarial audit on a throwaway checkout attacked the sentinel freeze, the prose/manifest drift window (both directions, plus the live tag gate), the stale dev-build minor claim, and the re-gate-once heal path; every adversarial edit turned a shipped test red. Three minor, non-blocking findings recorded: cli_test's self-referential token pin (real guard is dev_version_test's independent literal), an overclaimed "zero references" grep in the implementation report (stale comments survive), and a double-printed remedy on the rare heal-then-too-old-binary path.
