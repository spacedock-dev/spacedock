...
- DONE: Re-validation item 1 — confirm `survey_sync_codex_test.go` drives the REAL `agentsview sync` (not a fixture injection), runs `codex-presence` from queries.sql against the SYNCED DB, asserts the matched count, and is NON-VACUOUS (reds on a Codex-blind sync). Run it.
  Read the test: it writes synthesized Codex rollout jsonl, `git init`s two same-basename-different-root cwds (`repoA/proj`, `repoB/proj`), drives `exec.Command(agentsview, "sync")` HOME-isolated, then runs the `codex-presence` query loaded from `queries.sql` against the synced `sessions.db`. `go test -run TestSurveyCodexPresenceThroughSync -count=1` → PASS (0.09s, uncached real-binary run, not a skip). Non-vacuous: pointed `CODEX_SESSIONS_DIR` at an empty dir (Codex-blind sync) → reds `got "0" (a Codex-blind sync would yield 0)` — the exact phantom-0 failure mode the fixture-injection AC-2 could not catch. Reverted; re-verified green; worktree clean.
- DONE: Re-validation item 2 — AC-1/AC-2/AC-3 still green at 396534cc; full `go test ./skills/integration/` + `go vet ./...` + `go build ./...` clean.
  `go test ./skills/integration/ -count=1` → `ok` (1.169s). Survey subtests at this HEAD: query-smoke 6/6 (scoping `3|0|span`, codex-presence `2|2`, codex-not-folded-into-scope sessions=3), probe 2/2 (present silent/exit-0; absent sole sentinel), sync-e2e 1/1. `go vet ./...` exit 0; `go build ./...` exit 0.
- DONE: Re-validation item 3 — the SKILL.md §1 change mandates a fresh sync each run (no longer invites a stale persisted `SURVEY_DB_DIR`) and the query/mechanism is unchanged.
  §1 line 57 now reads **"Always run this sync as part of the survey — never query a pre-existing `SURVEY_DB_DIR` without re-syncing first"** (replacing the prior "persists/incremental" framing that invited skipping), and line 34 documents the narrowing is Claude-only while Codex syncs unscoped from `~/.codex/sessions` (what populates codex-presence). Confirmed via `git show --name-only 396534cc` that the fix touched ONLY `survey_sync_codex_test.go` + `SKILL.md` — `queries.sql`, `fixture-sessions.sql`, `survey_probe_test.go`, `survey_queries_test.go` are UNCHANGED, so AC-1/2/3 carry forward intact (and re-confirmed green above).

### Recommendation (cycle 2)

**PASSED.** The cycle-1 fix is sound and the new sync→codex-presence e2e test is non-vacuous — it drives the real `agentsview sync` and reds on the exact Codex-blind/stale-DB path that produced the captain's phantom 0, which AC-2's fixture-injection structurally could not exercise. The §1 prose now mandates the sync every run, closing the "query a stale persisted dir" gap at the recipe level. AC-1/2/3 artifacts are untouched by the fix and re-verified green at 396534cc; full integration suite, `go vet`, and `go build` are clean. AC-4/AC-5 are captain-accepted (re-drove AC-4 with `~/.codex` allowed → correct 61-Codex hint + state-the-fact SCAFFOLD + no folded-keys clause). Recommend PASSED. Verified SHA: **396534cc**.

### Summary (cycle 2)

Re-validated the feedback-cycle-1 fix at HEAD 396534cc by exercising behavior. The new `survey_sync_codex_test.go` is a genuine end-to-end test (real `agentsview sync` over a HOME-isolated synthesized Codex source → `codex-presence` query against the synced DB) and is provably non-vacuous: it reds with `0` on a Codex-blind sync, catching the precise failure the fixture-only AC-2 missed. The SKILL.md §1 prose now mandates a fresh sync each survey, removing the stale-persisted-dir trap that caused the captain's phantom 0; the query/fixture/probe mechanism is unchanged (fix touched only the new test + §1 prose). Full integration suite (query-smoke 6 + probe 2 + sync-e2e 1), `go vet ./...`, and `go build ./...` are clean; the worktree is clean at HEAD after the non-vacuousness probe was reverted. Recommend PASSED.
