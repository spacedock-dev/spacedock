...
pr: pr-merge:446
verdict: passed
completed: 2026-06-30T19:45:01Z
archived: 2026-06-30T19:45:01Z
---
...
- DONE: Ship the stderr advisory (`claudeteam.LegacyTeamNameAdvisory`) on the `!bareMode && host=="claude" && teamName!=""` complement of `mergedMode` (stderr-only; envelope untouched) + the `--help` Flags-block additions (`--host`, `--team-name`, `--bare-mode`), exact pinned wording, aligned in both places.
  `claudeteam.LegacyTeamNameAdvisory` added beside `BareModeAdvisory` (verbatim pinned text); called from `runBuildFields` (build.go) on the pinned gate; `printBuildUsage` (dispatch.go) gained the three flags with the pinned wording. Advisory + `--help` both name "legacy TeamCreate-registry dispatch shape" and "auto-team is the default". Commit d833cd5a.
- DONE: Behavioral proof green — two-arm advisory test (marker PRESENT on `--team-name` arm, ABSENT on merged arm; legacy envelope re-asserted on the `--team-name` arm: name+team_name present, run_in_background absent) + a `--help` golden asserting the three flags and the legacy/auto-team phrasing; existing `TestBuildLegacyModeUnchanged` stays green as the frozen envelope baseline.
  New `TestBuildLegacyTeamNameAdvisory` (AC-1+AC-3) and `TestBuildHelpDocumentsShapeFlags` (AC-2) PASS; `TestBuildLegacyModeUnchanged` PASS unchanged; full repo suite green (15 pkgs). Commit d833cd5a.

### Summary

Implemented per the pinned design (exact advisory + `--help` wording, same gate, stderr-only, AC-3 envelope untouched). PLACEMENT DECISION (FO-approved, option B): the advisory fires on the SUCCESS path — just before the envelope emit, after every error guard — NOT "immediately after the mergedMode line" as the ideation prose stated. Rationale: the pinned spot is before the error guards, so every `team_name` build that ERRORS would emit advisory-then-error; that warns about a legacy envelope that was never emitted (purposeless noise) AND would force weakening two existing prefix-assertion tests (`TestBuildTeamNamePathSafety`, `TestBuildTeamNameCombinedLengthCap`, which assert `HasPrefix(stderr, "error: ")`). Success-path placement fires the advisory exactly when the legacy envelope materializes; this implementation finding supersedes the ideation body's placement prose. The 14 `build-error-*` goldens and the two prefix tests stay BYTE-IDENTICAL (no weakening). Golden churn limited to 17 success-path `team_name` goldens; each diff verified as +1 line in the `stderr` section only — stdout (JSON envelope), exit code, and dispatch body unchanged in every one.

## Stage Report: validation

- DONE: MEASURE each AC's end-value by running the behavior (not a prose-grep): AC-1 advisory present on the `--team-name`+claude arm and absent on the merged arm; AC-2 `--help` stdout lists the three flags; AC-3 envelope unchanged.
  All proven by EXERCISING a binary built from `d833cd5a` (`go build ./cmd/spacedock`), not by trusting source/tests alone. AC-1: two-arm live build — legacy arm stderr = exactly 1 WARN line, merged arm stderr empty (both exit 0). AC-2: live `dispatch build --help` stdout lists `--host`/`--team-name`/`--bare-mode` with "legacy TeamCreate-registry dispatch shape" + "auto-team is the default", exit 0, stderr empty. AC-3: `jq` on both envelopes — legacy = name+team_name present, `run_in_background` absent (not even a null key); merged = team_name absent, `run_in_background:true`. Implementer tests `TestBuildLegacyTeamNameAdvisory`, `TestBuildHelpDocumentsShapeFlags`, `TestBuildLegacyModeUnchanged` also green.
- DONE: Confirm the placement-B decision held — advisory fires ONLY on the success path, error builds byte-identical, error goldens + the two `HasPrefix(stderr,"error: ")` tests unchanged, each ~17 success goldens carries `team_name` and gained exactly +1 advisory line.
  Source: advisory sits at `build.go:701-703`, after `out.RunInBackground` and every error guard, just before `emitBuildJSON`. Live error-path exercise (host=claude, `--team-name=../escape`) → exit 1, stderr = clean `error: ...`, 0 advisory lines, empty stdout. Commit `d833cd5a` touches 0 error goldens and 0 of `build_teamname_path_test.go`/`build_merged_mode_test.go` (both prefix tests + `TestBuildLegacyModeUnchanged` unmodified, all green). All 17 changed goldens: `team_name` present, `run_in_background` absent, +1 line each, zero deletions — the only added line is the advisory.
- DONE: Detached-audit assessment; `go test ./internal/dispatch/ ./internal/claudeteam/` green; PASSED/REJECTED recommendation.
  `go test ./internal/dispatch/ ./internal/claudeteam/ -count=1` → both `ok`. No detached audit run: routine, low-blast-radius — a stderr-only WARN line plus 3 lines of `--help` text, no envelope/contract/on-disk-format/error-path change (AC-3 + the unchanged error goldens prove the byte surface is frozen). Recommendation: **PASSED**.

### Summary

VALIDATED — recommend PASSED. All three ACs proven by running the behavior against a freshly built binary (live `--help`, live two-arm build, live error-path build) and observing stdout/stderr/exit, not by prose-grep; the implementer's Go tests independently corroborate. The FO-ratified placement-B held: the advisory fires only when the legacy envelope is actually emitted (success path), so error builds stay byte-identical and no existing assertion was weakened. One immaterial nit: the impl summary says "14 `build-error-*` goldens" but there are 18 — the claim's intent (all error goldens byte-identical) is in fact stronger, since the commit touches none of them. Detached audit waived (low blast radius, frozen byte surface); rationale recorded above.
