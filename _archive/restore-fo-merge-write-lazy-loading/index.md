...
Verified by: the non-terminal `rejection-flow` trace reaches its gate with no merge-core read; `merge-hook-guardrail` reads the exact merge core before terminal mutation and preserves its refusal; read-only `shallow-boot` leaves the PR-bearing entity unchanged with live discovery deferred; and both Claude and Codex `merge-mod-block-recovery` fixtures begin with a non-empty `mod-block=merge:*`, then normalize to successful engage/convergence → observed mod-block → write-core read → merge-core read → first merge-guard attempt while preserving done/PASSED/cleared-mod-block/archive/clean-state assertions. Planted eager, missing, and post-guard reads fail. The cycle-2 live spike proves the recovery path and durable outcome are reachable on both real hosts before implementation.
...
5. **Live behavior (expensive, implementation-owned).** Strengthen the Claude/Codex `gate-guardrail`, `filing`, `rejection-flow`, and `merge-hook-guardrail` runners; keep read-only `shallow-boot` separate from the outcome-oriented `merge-mod-block-recovery` journey. Prove the interactive greet-and-stop default through an actual interactive transport rather than instructing a headless model to emulate it. Require recovery to observe successful engage/convergence before mod-block → write-core read → merge-core read → first merge-guard attempt, while retaining done/PASSED/cleared-mod-block/archive/clean-state assertions. The implementer must run the relevant exact-head local Codex live journeys (and the corresponding Claude journeys where supported) before requesting Roborev; local SKIP is not green evidence. Roborev is the final adversarial implementation check, not a substitute for these gates. Pi remains compile/static-covered because its shared live scenario runners are separately backlog-scoped.
...
Captain process correction (2026-07-14): Roborev belongs at the end of
implementation, after the implementer has independently made the ordinary gates
and all relevant exact-head local live journeys green. A red Roborev or missing
live evidence means implementation is incomplete and must not advance to an
independent validator. Route this entity back to implementation; the implementer
owns both in-scope evidence repairs and relevant live green, then requests a
task-delta Roborev before validation.
