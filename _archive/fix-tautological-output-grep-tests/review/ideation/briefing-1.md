# Briefing: fix 8 tautological output-grep tests — ideation gate

**Chosen direction:** six deletions (each covered by a named, read-and-cited behavioral sibling) + two narrowings (keep dynamic assertions — the computed peer-SHA, the filing round-trip — drop only prose-mirror greps, two of which copy source sentences verbatim and one greps the test's own fixture). Test-only, no scope growth beyond the 8.
**Declared expected surface:** 6 _test.go files, six deletions + two narrowings, ~-135/+0 LOC, no production code. (Computing this per your directive caught a self-error — an earlier draft said 5 files; fixed.)
**FO recommends approve.**
Checklist: 2 done, 0 skipped, 0 failed. AC-1 (value): a mutation-survival matrix — for every delete, a real behavior mutation must still turn a retained sibling RED; a green mutation = lost coverage = AC fails. AC-2: exact edits land, no new output-wording greps, suite green. No spike needed, recorded. Bonus finding: third-shape debt confined to this CLI-output set, not codebase-wide.
Decision: approve = recorded pending advance to implementation (worktree at Commander drive). Reject = bounce with your asks.
