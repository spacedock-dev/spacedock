...
- DONE: Independently reproduce the cycle-1 audit's Mutant 6 in a throwaway worktree (remove/break the merge-hook `cannot advance to terminal` guard-error check at shared_assertions_impl_test.go:55) and confirm the new negative case in shared_scenarios_negative_test.go now goes RED (and the whole ensigncycle suite reds under the mutant), GREEN on the shipped assertion.
  Reproduced the audit independently, NOT by trusting cycle-2's claim: created a detached throwaway worktree at impl HEAD 77e1258e (`/tmp/audit-mutant6-real-shared`, never the impl worktree), removed the lines-55-57 guard-error `if`-block from `assertMergeHookGuardHeld`. Under the mutant, `TestMergeHookGuardrailNegativeBypass` reds at exactly `shared_scenarios_negative_test.go:121` (the new isolating case — observed mentions a merge hook, omits the terminal-guard refusal, so it fails ONLY on the line-55 check) and the WHOLE `internal/ensigncycle` suite reds (previously stayed green under Mutant 6 — cycle-1's hole). Reverted byte-identical (`git diff --stat` empty, clean status), re-ran GREEN on the shipped assertion. Throwaway worktree removed (`git worktree list` shows 0). Confirmed the fix is pure test-strength: `git show --stat 77e1258e` = +14 lines to shared_scenarios_negative_test.go ONLY; the shipped assertion file is untouched.
- DONE: Confirm go test ./... green and no AC regressed; re-confirm AC-1/4/6/7 hold and AC-2/3 remain by-construction-pending a valid-auth live CI run (flag, do not block).
  `go test ./...` all 10 packages `ok` (internal/ensigncycle ran fresh, 2.098s, not cached). AC oracles re-reproduced this cycle: AC-1 `TestSharedRuntimeScenarioDefinitions` PASS (host-neutral table rejects host-named fields); AC-5 all three negative suites PASS incl. the new merge-hook isolating case; AC-6 `TestSharedScenarioDocsContract` PASS; AC-7 `TestExtractClaudeFinalMessage` 5/5 subcases PASS (result/success, fallback, 401-launch-failure, non-401 error, no-message error). AC-4 `.github/workflows/runtime-live-e2e.yml` still wires claude-live → `TestLiveEnsignCycle`+`TestLiveClaudeSharedScenarios` (lines 173/184) and codex-live → `TestLiveCodexSharedScenarios` (line 302), env-gated + artifacts. AC-2/3 structural guard `TestSharedScenarioRunnerCoverage` PASS (live tag, no-spend, bidirectional). Live-tagged code `go vet -tags live` clean (exit 0) and compiles (build-only run, no tests). BY-CONSTRUCTION-PENDING-LIVE (flag, not a block): AC-2 (real Codex gate/rejection/merge-hook behavior) and AC-3's live gate-hold assertion need a valid host credential — the local OAuth token is expired (cycle-1 already proved the 401 path live as `errClaudeLaunchFailed`); CI `CI-E2E`/`CI-E2E-CODEX` approval is the oracle.

### Summary

Cycle-2 re-validation = re-audit of the cycle-1 detached-audit MATERIAL finding. Independently reproduced Mutant 6 in a throwaway detached worktree (not by re-reading cycle-2's report): with the line-55 `cannot advance to terminal` guard-error check removed, the new isolating negative case reds at `shared_scenarios_negative_test.go:121` AND the whole ensigncycle suite reds — exactly the regression that previously slipped through green. GREEN on the byte-identical shipped assertion; the fix is +14 test-only lines, shipped assertion untouched. Full `go test ./...` green across all 10 packages; AC-1/4/5/6/7 oracles re-reproduced clean; AC-2/3 structural coverage guard passes, their live behavior by-construction-pending a valid-auth CI run (flagged, not blocking — local token expired, CI approval is the oracle). RECOMMENDATION: PASSED — the cycle-1 Material finding is closed and independently confirmed; no AC regressed; the two live lanes are the only remaining oracle and are correctly flagged as pending CI, not gating this merge.
