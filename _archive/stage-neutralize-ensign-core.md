...
pr: pr-merge:451
verdict: passed
completed: 2026-06-30T20:49:51Z
archived: 2026-06-30T20:49:51Z
---
...
- DONE: MEASURE AC-1 (stage-neutrality guard): run `go test ./internal/contractlint/` green, and independently reproduce non-vacuity — defeating the scanner regex must RED both discriminator controls, re-inserting either parenthetical into the real core must RED the absence check, and incidental English ("Signaling done") must pass.
  3/3 green (Enumerates/Discriminates/RedsOnReinsertion). On a detached throwaway worktree at `ca73544d`: defeating `stageEnumParenRe` REDs both paired controls (Discriminates + RedsOnReinsertion) while the bare absence check passes vacuously — exactly what the controls catch; re-inserting `(implementation, validation)` AND `(ideation, backlog)` into the real core each REDs the absence check at line 36; shipped core has 0 scanner hits and line-51 "Signaling done" is not matched.
- DONE: MEASURE AC-2 (occupancy): `wc -c` exactly 49 bytes LESS than origin/main, `git diff origin/main` shows ONLY the two parentheticals deleted with nothing added (rule prose verbatim).
  origin/main (13e19398, current after `git fetch`) = 8941 B, worktree = 8892 B, delta = −49 B; diff is a single-line replacement deleting only `(implementation, validation)` / `(ideation, backlog)`, rule prose unchanged.
- DONE: MEASURE AC-3 (no behavior loss): `go test ./internal/dispatch/` (delivery test) green; assignment carries the show-stage-def fetch line not inlined prose; show-stage-def returns the sentinel-carrying subsection while the core does not; perturbation returns no sentinel. `go test ./...` green.
  TestBuildStageDisciplineRidesFetchNotInlineAssignment green (count=1): body carries `Skill(skill="spacedock:ensign")` + `show-stage-def --stage ideation`, no inlined DEV-DISCIPLINE-SENTINEL; show-stage-def ideation returns the sentinel, validation (unplanted) returns none. Corroborated on the real dev workflow: show-stage-def validation returns `### validation`, core carries 0 "Detached adversarial audit". Full `go test ./...` exit 0, 0 FAIL, 17 packages.
...
- DONE: AC-1 guard widening (audit cycle 1) — broaden the parenthetical-stage-enumeration scanner so `,` `/` `;` and whitespace are equivalent separators, flagging ≥2 dev stage names joined by any of them; the comma-only form let `(implementation/validation)`, `(implementation; validation)`, `(implementation validation)` re-leak with CI green.
  `internal/contractlint/ensign_core_stage_absence_test.go`: separator class widened to `[,/;\s]+`. Commit 45fbbf59.
- DONE: Discriminator mustFlag extended with the slash, semicolon, and bare-whitespace forms (comma cases retained); "Signaling done" stays in mustPass (no over-flag); stage-neutral strip result still passes.
  Discriminator green; the single-token incidental case unflagged.
- DONE: By-design tradeoffs documented in a test comment — single-name parentheticals (e.g. `(implementation)` alone) and non-dev (ticket/experiment/survey) stage vocab are intentionally NOT caught; the parenthetical must start with a stage name and be a pure stage-name list, so incidental "done" in prose does not engage the scanner.
  Comment on `stageEnumParenRe`.
- DONE: Non-vacuity re-confirmed by exercising — a slash-form parenthetical re-inserted into the REAL core reds the absence check (file reverted byte-identical, md5 match, 8892 bytes); defeating the scanner reds all 5 mustFlag forms (incl. the 3 new separators). `go test ./internal/contractlint/` and `./internal/dispatch/` green.

### Summary

Cycle-1 feedback addressed: the AC-1 guard now treats comma, slash, semicolon, and whitespace as equivalent separators inside a stage-name parenthetical, closing the natural-shorthand re-leak the adversarial audit found. The scanner stays conservative (parenthetical must start with a stage name and be a pure ≥2 stage-name list), so the incidental "Signaling done" line and single-name parentheticals remain unflagged — documented as by-design tradeoffs. The ensign-core neutralization (−49 bytes) and the AC-3 dispatch behavior-loss control are unchanged. Non-vacuity exercised live (slash-form re-insertion reds the real-file absence check; scanner-defeat reds all five mustFlag forms).
