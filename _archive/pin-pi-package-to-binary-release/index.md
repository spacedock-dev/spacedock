...
- Cycle 4: REJECTED at validation on combined tip 989e193de — AC-4 failed because an install error or ineffective repair could still launch Pi, and AC-5 failed because a dirty checkout pseudo-version was treated as a release ref. Both Material outcome defects were owned by this task and authorized `fix`; correction commit 284299e3b now refuses launch after a spent failed/ineffective repair and keeps dirty pseudo-versions on the dev sentinel. Surface +309 net / 3 files vs estimate +295±80 / 3±1; AC unchanged.
...
- DONE: cut the deliverable down to the re-scoped surface in the worktree (branch spacedock-ensign/pin-pi-package-to-binary-release, deliverable commit 178661933): delete the fake-seam flow test suite (internal/cli/pi_package_repair_test.go) and replace it with the kept pure-function tables (piReleaseRefFrom derivation table incl. pseudo-version float rows, piGitSourceRef/piPinnedSource classifier table, piPackageNeedsRepair trigger table) in one small test file with no fake-runtime scaffolding; delete internal/ensigncycle/pi_frontdoor_pin_live_test.go and revert its docs/runtime-live-ci-registry.md registration; keep the production code (pi_package_source.go, pi.go repair call site) unchanged
  Commit `ecf097656`: `pi_package_repair_test.go` deleted (the 335-line fake-seam flow suite), replaced by `pi_package_source_test.go` (+94, three pure tables, zero scaffolding); `pi_frontdoor_pin_live_test.go` deleted; `pi_frontdoor_test.go` and `runtime-live-ci-registry.md` reverted to origin/main (the statusAfterInstall seam +9 and registration +18 gone); `pi_package_source.go` and `pi.go` byte-unchanged (`git diff 178661933 HEAD -- internal/cli/pi.go internal/cli/pi_package_source.go` is empty).
- DONE: verify the pure-function tables still pin every retained falsifier class (proxy-build pins, @next pseudo-versions float, wrong-ref triggers, git/non-git classifier, dev sentinel) and that the hand-executed scenario matrix named in the test plan is actually runnable by the validator (each scenario: the exact commands, fixture setup, expected exit code, stderr, and resulting settings.json state)
  All three tables pass verbosely (`-run 'TestPiReleaseRefDerivation|TestPiPinnedSourceAndRefParsing|TestPiPackageNeedsRepairTable'` 3/3 PASS) and each row's falsifying change is named in the table comment; the hand-run matrix's seams verified present in production: the front door reads `PI_CODING_AGENT_DIR` for the agent dir (pi.go:550) so a fixture settings.json is seedable, and pi binary discovery goes through `LookPath` (pi.go:59) so the PATH-shim recorder technique works for the install-failure arm.
- run gofmt on changed dirs, go test ./..., and go test ./... -race; commit path-scoped to the worktree branch, and write ## Stage Report: implementation (cycle 2) per the ensign discipline with each DONE's evidence on a separate indented line
  gofmt clean; full suite `go test ./...` ok (rc=0, ambient runtime markers unset); race ok for the changed packages (cli 191s, ensigncycle 295s); measured surface vs origin/main = 298 insertions/1 deletion = +297 net across 3 files (54+150+94, −1), inside the +295±80 / ±1 re-baseline. Commit `ecf097656` pushed to spacedock-ensign/pin-pi-package-to-binary-release.
