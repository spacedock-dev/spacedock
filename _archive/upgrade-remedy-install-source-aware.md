...
completed: 2026-07-22T09:07:38Z
verdict: passed
score:
...
## Stage Report: validation (cycle 2 — @next-only trim)

- DONE: Verify the trimmed @next-only fix against the branch (37a872e2) — @next names `brew upgrade spacedock@next` and NO bare stable (word-boundary); every other install's 3-line block byte-for-byte UNCHANGED; reproduce both; mutation-test the guard.
  `TestTooOldBinaryRemedyEdgeChannel` (green): `tooOldBinaryRemedy(true)` contains `brew upgrade spacedock@next`, regexp `` `brew upgrade spacedock(\s|$)` `` ABSENT (probed: `@next` never matches; bare stable at `\n`/EOF does), keeps `spacedock install`; `tooOldBinaryRemedy(false)` == the pinned 3-line block byte-for-byte — reds if the @next switch drops or the false-block drifts. LIVE: built binary staged under a real `Caskroom/spacedock@next/…` path emitted `brew upgrade spacedock@next` through its own `spacedock doctor`; the same binary from a non-Caskroom path and via a `Caskroom/spacedock/` stable-cask PATH-symlink both emitted bare `brew upgrade spacedock`. Mutation A (@next arm → bare stable) reds `TestTooOldBinaryRemedyEdgeChannel`.
- DONE: Verify detection (edgeCask from resolved Caskroom token, threaded through compareNamed/ManifestVerdict/RunDoctor, Compare passes false — signature unchanged); detached adversarial audit on a THROWAWAY checkout through frontdoor.go gateHost.
  `Compare(host,pluginVersion,binaryVersion)` signature UNCHANGED (passes `false`); `contract_test.go` empty diff, its 11 `TestCompare` subtests + `TestCompareMessageShape`/`TestCompareHostSubstitution` green. `TestIsEdgeCaskPath` (green) + boundary probe: `spacedock@beta`, `spacedock@next-extra` (prefix), Caskroom-last, `@next`-not-after-Caskroom → all false; relative + token-last → true (exact-match holds). Audit on a detached throwaway worktree at 37a872e2: Mutation A reds the remedy guard; Mutation B (exact-match→`HasPrefix`) reds `TestIsEdgeCaskPath/stable_cask`; the gateHost-equivalent threading is proven LIVE — staged @next binary → @next remedy through the real `runDoctor`, and severing the `init.go`/gateHost thread to hardcoded `false` (rebuilt) flips the live output to bare stable.
- DONE: go test ./... + -race green; gofmt/vet clean; semantic adversarial pass; classify findings; recommend; flag rebase-onto-main concern.
  `gofmt -l` clean; `go vet ./internal/cli ./internal/contract` clean; `go build ./...` clean; `go test ./...` green; `go test -race ./internal/cli ./internal/contract` green (disk at 99% but no ENOSPC this run). Semantic pass: os.Executable anchor proven path-dependent (edge / non-Caskroom / stable diverge); EvalSymlinks proven via the real PATH-symlink-into-Caskroom topology (the captain's exact env); empty/missing-Caskroom table+probe covered. Rebase: `git rebase --onto main dceede3a` applied CLEANLY; only `frontdoor.go` overlaps main (commit `ecffcede`, TeamCreate/Degraded-Mode region — does NOT touch gateHost/ManifestVerdict/runningEdgeCask); the rebased tree builds and its remedy/detection tests pass. (Main is at `fcf968960`, past the `2377876e` the dispatch named — no new interaction.)

### Summary

PASSED. The captain's regression is fixed at the value level and proven end-to-end: a binary installed as the `spacedock@next` cask, run via its real Homebrew PATH-symlink-into-Caskroom (the captain's exact topology), emits `brew upgrade spacedock@next`; every other install (stable cask, non-brew, unresolved) emits the byte-for-byte-unchanged 3-line block. Each value claim is mutation-tested — the @next arm and the detection each red a specific guard on a claim-breaking edit — and `Compare`'s signature stayed stable (contract_test.go empty diff, all cases green). The rebase onto current main is clean and does not touch the remedy path. No material findings.

Deferred risk (non-blocking, evidence not outcome): no COMMITTED automated test reds if a future edit severs the `runningEdgeCask()` wire at `gateHost` or the three `init.go` arms — Mutation C (hardcoded `false`) kept the whole cli+contract suite green. The delivered code IS correct (proven live: staged @next binary → @next; severed rebuild → bare stable), so no value AC fails now; AC-1's cited proof (`tooOldBinaryRemedy(true)`) and AC-3's (`isEdgeCaskPath`) are reproduced green and the AC text does not promise a front-door e2e test. Promotion condition: land a committed end-to-end doctor test staging the binary under a fake `Caskroom/spacedock@next/` path (the exact live exercise run here). Observation for the FO: the local branch has diverged from `origin/…` (reset to origin/main for cycle 2 while origin still carries the four-source tip) — a merge/push note, not a code concern.
