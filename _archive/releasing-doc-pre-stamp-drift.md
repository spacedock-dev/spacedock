...
pr: pr-merge:432
verdict: passed
completed: 2026-06-22T04:19:15Z
archived: 2026-06-22T04:19:15Z
---
...
- DONE: Verify the AC-3 fix in the INTEGRATED (rebased) doc — `grep main:refs/heads/stable docs/releasing.md` NONE, doc narrates `$RELEASE_COMMIT:refs/heads/stable` matching release.yml:249 + TestStableRefGuardRejectsMainSource; rebase integration coherent.
  Confirmed rebase: merge-base = origin/main 7338d556 (#431), both task commits replayed (d3ff3617, f864bc1f), clean tree. `grep main:refs/heads/stable docs/releasing.md` → NONE; doc L32 narrates `git push origin "$RELEASE_COMMIT:refs/heads/stable"` == release.yml:249. Integration coherent: #431's branch-per-channel model present (source.ref=stable, spacedock-edge, channel-in-marketplace-NAME, binary adds branch source) AND this task's moving-`stable`/e2e-gate reconciliation present; #431's false "stable entry repointed to the released tag" / "Repointing the stable entry is a commit on the repo root" prose GONE (verified against the 7338d556 original) — "repoint" now appears only in negating sentences (L29, L86). No conflict markers.
- DONE: Reproduce every AC's cited evidence + go test ./... green + manifest-tag-gate CI wiring intact.
  AC-1: e2egate.go:58 `run.HeadSha == releaseCommit` == doc L46; TestE2EGatePassesForGreenRunOnReleaseCommit PASS / TestE2EGateBlocksGreenRunOnWrongCommit BLOCK. AC-2: ran the CLI over REAL tag manifests — v0.20.0 (manifest 0.19.9) exit 1/BLOCK, v0.22.0 (0.22.0) exit 0/PASS (independent artifacts, demonstrably diverged). AC-3: above. AC-4: mirror is a 120000 symlink → `diff` empty. AC-5: deferral note docs/releasing.md L168-171. `go test ./...` green, 0 FAIL across all 16 packages. CI wiring intact: gate step release.yml:138 (`!contains(github.ref,'-')` skip L135), goreleaser `needs: e2e-gate` L141; TestReleaseWorkflowGatesGoreleaserOnManifestTag + dropped-step/dropped-needs/pre-release-skip twins all PASS (non-cached).
- DONE: DETACHED adversarial audit on a THROWAWAY checkout (never this worktree) over the integrated doc + CI-wired guards.
  Cloned the branch tip into /tmp/spacedock-audit.* (since removed), baseline green. Edit 1 — regress the stable push to `git push origin main:refs/heads/stable` (the exact AC-3 regression the doc warns against): TestStampStepAdvancesStableRefToTaggedCommit REDs (src=="main" not $RELEASE_COMMIT) and TestStableRefGuardRejectsMainSource REDs (precondition gone). Edit 2 — drop the manifest-tag-gate CI invocation (the cycle-1 advisory-only hole): TestReleaseWorkflowGatesGoreleaserOnManifestTag REDs ("no job runs manifest-tag-gate"). Restore → all green. The deliverable's tests catch the regressions the doc documents. Clean audit, no new material findings; worktree + state checkout untouched.

### Summary
PASSED. Cycle-3 closed the cycle-2 AC-3 BLOCKER inside the rebase: the integrated doc narrates the tagged-SHA stable advance (`$RELEASE_COMMIT:refs/heads/stable`) matching release.yml:249, and the false `main:` form is gone (grep NONE). The rebase integration is coherent — #431's branch-per-channel marketplace model and this task's moving-`stable`/e2e-gate reconciliation read as one doc, with #431's false hand-edit-repoint claim removed. All five ACs reproduced from independent evidence (AC-2 against real v0.20.0/v0.22.0 history); full suite green; manifest-tag-gate CI-enforced. Detached adversarial audit on a throwaway checkout confirmed both guard families RED on the regressions and GREEN on restore — clean.
