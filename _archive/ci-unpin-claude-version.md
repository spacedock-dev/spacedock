...
- DONE: Reproduce AC-2 offline: `go test ./internal/release/... ./internal/contractlint/...` green AND `go test ./...` green; then reproduce the discriminator — mutate a SPACEDOCK_PINNED_CLAUDE_VERSION + ${CLAUDE_VERSION:-$SPACEDOCK_PINNED_CLAUDE_VERSION} fallback back into the real runtime-live-e2e.yml fixture and confirm both TestClaudeLiveWorkflowFloatsToLatest and TestLegacyConsumerRetiredButPathLives go RED with their discriminating messages, then revert and re-confirm green.
  Targeted suites green + full `go test ./...` exit 0 (no FAIL/panic, 15 pkgs OK). Mutated the REAL fixture (added `SPACEDOCK_PINNED_CLAUDE_VERSION: "2.1.177"` + the `${CLAUDE_VERSION:-$SPACEDOCK_PINNED_CLAUDE_VERSION}` fallback): both guards RED — float guard "pin must be gone, not hidden", legacy guard "still pins SPACEDOCK_PINNED_CLAUDE_VERSION ... consumer is retired"; `git checkout --` reverted clean (no diff) and all three guards re-PASS.
- DONE: AC cross-check AC-1/AC-2/AC-3 with evidence: AC-2 fully reproduced (above); AC-1 design-coherence (the install step resolves ${CLAUDE_VERSION:-latest}, no SPACEDOCK_PINNED_CLAUDE_VERSION var anywhere, DISABLE_AUTOUPDATER retained, Show-tool-versions records without an enforcement RED) and AC-3 coherence (the legacy pty lane SKIPs on the merged floor via m4's capability gate). Flag that AC-1/AC-3's LIVE CI proof (a green unpinned run reporting ≥2.1.178 + merged scenarios green + legacy pty SKIPPED) is MERGE-GATED — coupled to m4's skip-gate landing first — and is therefore a merge-boundary verification, not runnable in this offline validation.
  AC-1 coherent: `${CLAUDE_VERSION:-latest}` (yml:144), no `SPACEDOCK_PINNED_CLAUDE_VERSION` anywhere, `DISABLE_AUTOUPDATER: "1"` retained (yml:109), Show-tool-versions has no `exit 1` and still records `claude --version` to `$GITHUB_STEP_SUMMARY`, override hatch `CLAUDE_VERSION: ${{ inputs.claude_version }}` present (yml:137), yaml parses clean (`yq eval`). AC-3 coherent: the legacy pty tests are NOT on this branch (m4 owns them), the `ToolSearch(select:TeamCreate)`-empty ⇒ skip mechanism is the documented contract (`claude-fo-dispatch.md:9`, `m4-readiness.md` step 2), merge-order sequencing recorded in the body Test plan + `m4-readiness.md` step 3/4. AC-1/AC-3 LIVE CI proof flagged MERGE-GATED (coupled to m4's skip-gate landing first) — a merge-boundary verification, not runnable offline.
- DONE: Detached adversarial audit (CI/release machinery — high-stakes surface): on a throwaway checkout, confirm the new claude_version_float_guard_test.go and the rewritten legacy_teamcreate_layering guard are REAL discriminators — they RED on a re-introduced pin AND (for the legacy guard) on the using-legacy-claude-team skill or its dispatch load line going missing — not always-green; record the adversarial edits tried and that the guards caught them.
  Ran on a detached `git worktree` (`/tmp/ci-unpin-audit.*`, torn down after; impl worktree never mutated). Baseline green there, then 5 adversarial edits ALL caught: (A1) pin var + wired fallback → float guard RED + legacy guard RED; (A2) lingering pin var declared but NOT wired into the fallback → float guard RED (the "re-pin waiting to be wired back" hole — stronger than minimally required); (B) emptied `using-legacy-claude-team/SKILL.md` → legacy guard RED "must be RETAINED"; (C) removed the `spacedock:using-legacy-claude-team` dispatch load line → legacy guard RED + `TestNormalPathContractNamesLegacySkill` RED; (D) dropped the override-hatch `CLAUDE_VERSION <- inputs.claude_version` binding → float guard RED. Each restored → green. No always-green guard found; clean audit.

### Summary

VERDICT: PASSED. AC-2 fully proven now (targeted + full suites green; the discriminator demonstrated by mutating the REAL `runtime-live-e2e.yml` back to a pin — both guards RED with discriminating messages — then reverting to green). AC-1 design-coherence holds against the real fixture (float resolution, no pin var anywhere, `DISABLE_AUTOUPDATER` retained as freeze-for-run, Show-tool-versions records without enforcement RED, override hatch survives, yaml parses clean). AC-3 coordination is coherent (the legacy pty lane lives on m4's branch and SKIPs on the merged host via the documented `ToolSearch(select:TeamCreate)`-empty capability gate; merge-order sequencing recorded). AC-1/AC-3's LIVE CI proof (a green unpinned run reporting ≥2.1.178 with merged scenarios green and legacy pty SKIPPED) is MERGE-GATED — coupled to m4's skip-gate landing first — and is a merge-boundary verification, not runnable in this pre-merge offline validation, per the dispatch's load-bearing-coupling note. The detached adversarial audit on a throwaway checkout caught all five adversarial edits (re-pin wired, lingering pin decl, emptied legacy skill, removed dispatch load line, dropped override hatch); both guards are real discriminators, not always-green.
