...
completed: 2026-06-16T19:09:34Z
verdict: PASSED
score: 0.30
worktree:
issue:
...
**Firehose-fixture correction (FO catch — verified, decisive).** The FO flagged that my fixture hid the REAL firehose: the live runner's watcher is `func(line string){ t.Log(line) }`, so every host stream-json line prints via `t.Log` even on PASSING tests under `-test.v` — arbitrary text with no `=== RUN`/`--- PASS` prefix. Added `TestPassingWithStreamFirehose` (always passes, t.Logs 40 marked stream lines) to the fixture and measured the two mechanisms over it: the cycle-1 grep clean view leaked all 40 firehose lines to stdout (1179 bytes — it would FAIL AC-1), while gotestsum stdout is 39 bytes with ZERO firehose lines (the 40 lands in the `--jsonfile` archive). This is exactly why a scaffolding grep is insufficient and gotestsum's structural per-test-output routing is required — and it confirms the FO's premise that my cycle-1 approach did NOT meet AC-1 for the live steps. AC-1 (green) now asserts the firehose marker is ABSENT from stdout; AC-2 asserts it is PRESENT in the archive (the triage-vs-root-cause split). Non-tautology proven: the same fixture leaks 40 markers through the old grep vs 0 through gotestsum.

## Stage Report: validation

- DONE: Close the firehose concern EMPIRICALLY — pinned `gotestsum --format pkgname` over a `t.Logf` multi-KB stream blob; assert blob ABSENT from stdout, PRESENT in `--jsonfile`.
  Ran the live CI shape over `cleanoutputfixture` (gotestsum v1.13.0, sha256-installed): green stdout = 4 lines, `STREAM_FIREHOSE_LINE` count = 0; jsonl archive = 72 lines/16774 bytes, marker count = 40. The 40 t.Log stream lines land ONLY in the archive. Proves `--format pkgname` structurally suppresses the live-runner's `t.Log(line)` Claude stream (the real 143KB firehose), not just `=== RUN` scaffolding.
- DONE: Non-tautology of the firehose assertion (the failmod fixture could not show this).
  Perturbation: old `go test -v` leaks the marker 40x to stdout (the 143KB-class firehose); gotestsum 0x. Swapping the AC-1 runner to `go test -v` REDs `TestLiveCIStepStdoutIsCleanOnGreen` (scaffolding + 40 firehose lines leak); with gotestsum it passes. The assertion discriminates real suppression, not an absent-marker artifact.
- DONE: Detached adversarial audit on a throwaway checkout (high-stakes CI/release) — sha256 fails-closed, exit preservation, artifact upload, no surviving firehose/`@latest`, non-tautological graders.
  sha256 FAILS CLOSED (behavioral): tampered checksum → `shasum` FAILED, exit 1, bin dir empty, nothing installed (the `set -e` aborts before `tar`/`install`). Exit preserved natively through gotestsum: green 0 / red 1. Every `*-detail.jsonl` carried in upload-artifact lists (claude live-e2e + claude-shared; codex-shared; pi-coverage + pi-front-door-smoke). Install step in all 3 live jobs + the offline job. No surviving `transcript.txt` firehose; line-181 "transcript" is the out-of-scope agent stream-json. Guard perturbation: reintroducing `-v | tee transcript.txt` REDs both `TestLiveWorkflowHasNoFirehoseRegression` and `TestLiveWorkflowStepsUseGotestsumOneRunShape`. Refuted nothing material.
- DONE: AC cross-check AC-1..AC-6 each carry reproduced evidence; confirm the entity's win holds over the t.Log-bearing case.
  AC-1: green clean (0 `=== RUN`, 0 firehose) + red failures-only with `a_test.go:28`/`a_test.go:38` + `=== Failed` recap — reproduced. AC-2: jsonl non-empty, carries `"Action":"fail"` + `TestGammaFails` + `compute() = 7, want 42` + the 40 firehose lines — reproduced. AC-3: counter==1 (one run) — TestLiveCIStepRunsSuiteOnce PASS. AC-4: exit 0/1 preserved — reproduced. AC-5: install script pins `VERSION="1.13.0"`, no `@latest`, `shasum -a 256 -c`, concrete per-platform checksums incl. linux_amd64 (CI runner); fails-closed proven behaviorally. AC-6: shared-core "Reading a live CI result" note (line 109) + README Testing-Resources row (line 220) present; doc/workflow contract guards (journey/e2egate/workflow-exec/contractlint/auto_continue_pi_live) green.

### Summary

PASSED. The one open concern — that the real 143KB firehose is the live-runner's `t.Log(line)` Claude stream, which the failmod fixture never exercised and the cycle-1 grep could not strip — is now closed empirically: the committed `TestPassingWithStreamFirehose` fixture (40 `t.Logf` stream lines, always passing) under pinned `gotestsum --format pkgname` shows the firehose marker 0x on stdout and 40x in the `--jsonfile` archive, and the same fixture leaks 40x through `go test -v`; swapping the AC-1 runner to `-v` REDs the green assertion, so it is non-tautological. The detached adversarial audit on a throwaway checkout refuted nothing material: the sha256 install fails CLOSED behaviorally (tampered checksum → exit 1, nothing installed), gotestsum preserves the green/red exit natively, every `*-detail.jsonl` rides the upload-artifact lists across all five live runs, no firehose or `@latest` regression survives, and the structural guards RED under perturbation. AC-1..AC-6 each reproduce their cited evidence; the full offline suite (15 packages) is green with gotestsum installed (mirroring CI's offline job). Boundaries respected: did not touch the TestLiveEnsignCycle flake, the cycle-1 commit history, or push/PR — Commander handles merge.
