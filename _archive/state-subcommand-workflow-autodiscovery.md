...
pr: pr-merge:461
verdict: passed
completed: 2026-07-02T05:53:44Z
archived: 2026-07-02T05:53:44Z
---
...
- DONE: M1 — added `TestStateCommitExplicitWorkflowDirOverridesDiscovery` (internal/cli/state_from_root_test.go): two live standalone split-root workflows (docs/dev, docs/ops) on disk; `state commit task-b --workflow-dir <B>` run with cwd inside workflow A (whose walk-up alone resolves A). Asserts the commit lands in B's checkout (task-b.md in `git log`) and A's checkout has zero commits.
  RED/GREEN proof: reverted `resolveWorkflowDir` (state_sync.go:418) to the audit's exact mutant — check `status.DiscoverWorkflowDir(dir)` first and return it unconditionally on a hit, before the explicit-flag check. New test failed (exit 1, "no entity task-b under .../docs/dev/.spacedock-state" — the mutant redirected the commit into A). Full `go test ./internal/cli/... ./internal/status/...`: only the new test failed, every other test in both packages stayed green, matching the audit's "stays GREEN suite-wide" finding exactly. Reverted the mutant; suite green again.
- DONE: M2 — `TestStateSweepFromRootResolves` (internal/cli/state_from_root_test.go:44) now asserts the JSON `state_branch` field equals the real nested workflow's branch (`spacedock-state/dev`), not just that the envelope is well-formed and HEAD is unchanged.
  RED/GREEN proof: applied the audit's exact mutant in `discoverWorkflowDownward`'s single-match case (native_runner.go:570) — `case 1: return root, 0` instead of `return workflows[0], 0`. Test failed, showing the wrong `state_branch` value `"spacedock-state/hostA"` (derived from the git toplevel's basename) instead of the expected `"spacedock-state/dev"` — proving the new assertion is strictly stronger than a bare non-empty/presence check. Reverted the mutant; test green again.
- DONE: Scope fence held — only internal/cli/state_from_root_test.go changed (`git diff --stat` against the prior implementation commit shows exactly one file, 69 insertions/3 deletions); no product code touched.
  Full repo `go test ./...`: all 16 packages ok. Commit cf58dff1 on branch spacedock-ensign/state-subcommand-workflow-autodiscovery.

### Summary

Closed both test-strength holes the cycle-1 audit surfaced, without touching any product code. Each fix was verified against the audit's exact mutant: RED with the mutant applied, GREEN after reverting, and (for M1) confirmed the mutant leaves every other test in internal/cli and internal/status green, matching the audit's "stays GREEN suite-wide" observation precisely. Full suite passes.

**Amendment (PR #461 CI fix):** `TestStateCommitExplicitWorkflowDirOverridesDiscovery`'s scratch checkouts were bare `git init` with no `user.name`/`user.email`, so the product code's real `git commit` only worked on hosts whose git falls back to an OS-derived identity (true on this macOS dev box, false on the CI runner — "fatal: empty ident name"). Added `git config user.email "t@t"` / `user.name "t"` right after each checkout's `git init`, matching commissionSplitWorkflow/twoHostStateWorkflow's existing pattern. Reproduced RED locally by forcing `user.useConfigOnly=true` (the plain `HOME=$(mktemp -d)` recipe doesn't reproduce on macOS since git derives identity from the OS user database regardless of HOME/env) — confirmed RED before the fix (`fatal: no email was given and auto-detection is disabled`) and GREEN after. Commit 0b9cd9cf, pushed to spacedock-ensign/state-subcommand-workflow-autodiscovery.

## Stage Report: validation (cycle 1)

Re-verified the cycle-1 test-only fixes narrowly in the worktree at cf58dff1. Suite green as shipped: fresh `go test ./... -count=1`, all 16 packages ok. Both audit mutants reproduced by hand and observed RED: M1 (resolveWorkflowDir, state_sync.go:418, discovery-first returning a walk-up hit before the explicit-flag check) made `TestStateCommitExplicitWorkflowDirOverridesDiscovery` fail with the mutation redirected into workflow A (`no entity "task-b" under .../docs/dev/.spacedock-state`), and a full `go test ./internal/cli` under the mutant showed it as the ONLY failure — confirming the pre-fix suite really did stay green suite-wide; M2 (`discoverWorkflowDownward` single-match returning the git toplevel, native_runner.go:571) made `TestStateSweepFromRootResolves` fail on its `state_branch` assertion, emitting `"spacedock-state/hostA"` (toplevel-basename-derived) instead of `spacedock-state/dev`. Both mutants reverted; `git status`/`git diff HEAD` clean and all 7 FromRoot/OverridesDiscovery tests green again. Commit cf58dff1 is test-only: `git show --name-only` lists exactly internal/cli/state_from_root_test.go. The PASSED recommendation stands.
