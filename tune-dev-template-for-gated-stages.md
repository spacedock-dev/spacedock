...
- DONE: Land as the next stack layer using your own designed mechanics (gh pr create --base onto the current top, then gh stack link); base worktree is the stack tip
  Worktree branch `spacedock-ensign/tune-dev-template-for-gated-stages` is based on `d1d8f745f`, the head of `stack27/11-self-contained-contracts` and the last unmerged layer. Created with `gh pr create --base stack27/11-self-contained-contracts` carrying the reviewed title and `--body-file`, then joined with `gh stack link`; no `gh stack submit` and no `gh api PATCH` in the record. PR number reported to the first officer, which owns the `pr:` field.
