...
- FAILED: the `gh stack link` join clause of AC-6 — attempted twice, not verifiably in effect
  `gh stack link 699 ... 711` printed `⚠ failed to update base branch for PR #709 to stack27/09-trim-version-output: HTTP 422 PullRequest.base is invalid`, then printed `✓ Updated stack to 12 PRs (stack #707)`. The stack record did not change: `gh stack view 707 --json` returns 11 branches, topping out at `stack27/11-self-contained-contracts`, with #711 absent; `gh stack view 711` resolves to Stack #707 but renders the same 11. A second, narrower `gh stack link 707 711` then reported `ℹ PR #711 is already in stack #707 — skipping`, so gh's membership check and its own enumeration disagree. Root cause is upstream of this task: #709 was restacked onto `main`, `stack27/09-trim-version-output` is now an ancestor of `main` (`git merge-base --is-ancestor`, true), and GitHub rejects resetting #709's base to it, which aborts the stack-record rewrite before #711 is appended while gh still prints success. Held unchanged pending first-officer authorization — see the finding below.
...
### Finding — worker proposal, awaiting first-officer authorization
...
- Trigger evidence: the 422 on #709 quoted in the FAILED item above, plus the 11-vs-12 disagreement between `gh stack link` and `gh stack view --json`, both reproducible now against stack #707.

Proposal, as three separate facts. Materiality: **Material** — a released workflow's documented step silently no-ops. Ownership: **not this task** — the trigger is #709's base drift onto `main`, which belongs to `retire-prose-grep-tests`, and the remedy touches neither file in this task's approved surface. Disposition: **Needs decision** — restacking #709, accepting an ungrouped top layer, or amending the mod prose to warn that `gh stack link` can report false success are three different scope calls, and the mod wording was approved verbatim at the ideation gate. Candidate bytes and `HEAD` are unchanged; no reviewer rerun started. Holding for the captain.
