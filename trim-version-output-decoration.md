    ...
    records:
        ...
        - id: gate:x8g3dnqndfa1m85d8ga2cgem:ideation
          ...
          attempts:
            ...
            - id: gate-attempt:x8g3dnqndfa1m85d8ga2cgem-ideation-3
              briefing:
                id: briefing:x8g3dnqndfa1m85d8ga2cgem:ideation:attempt-3:revision-1
...
- DONE: Independently re-exercise the ACs against worktree commit d8fe1e74d, never by reading the report: build the binary - OS line present and byte-identical to merge-base, no session segment, no pass-host suffix, Sandbox prints the sandbox name or none; byte measures 130-to-103 in-session and 148-to-123 ambiguous
  Both binaries rebuilt from clean trees (`git archive` of b7b5b33c3 — verified as both origin/main merge-base and HEAD's parent — and of d8fe1e74d); the outside shape is byte-identical between eras (38 bytes), so line 1 and the OS line are byte-identical; `, session `, `pass --host`, and `inside` absent from every cut shape; all three Sandbox arms driven live (`agent-safehouse` / `none (safehouse available)` / `none (safehouse not installed)`); sandboxed-arm bytes measure 130→103 in-session and 148→123 ambiguous, exactly as declared, line counts 5/5/2 unchanged.
- DONE: Zero files under skills/integration touched (out of bounds by construction); the install-gate prose consumer unmodified; cli.go:853 comment now names version_session_test as the pin; contract-3 token untouched
  `git diff --name-only b7b5b33c3..d8fe1e74d` lists 9 files — none under `skills/integration/`, `fo-install-gate.md` absent; cli.go:853 reads "pinned by internal/cli/version_session_test.go"; `frozenContractToken` const and its emission are byte-unchanged, `grep -rn "contract 3|frozenContractToken" internal/contractlint/` returns nothing, and the cut binary prints `contract 3` last in every in-session shape.
- DONE: Suite differential against the merge-base baseline; reproduce any failure on the clean base before attributing; verdict PASSED or REJECTED with per-AC citations
  `go test ./...` and `go test ./... -race` in the worktree: 19 packages ok each; the sole failure (`TestCodexResolveManifestAgainstInstalledHost`, sandbox denies `~/.codex/config.toml`) reproduces byte-identically on the unmodified b7b5b33c3 extraction — environmental, pre-existing. `gofmt -l` clean. **Verdict: PASSED.**

Per-AC evidence, each re-derived rather than read:

- AC-1: `wc -lc` matrix over five env shapes on both binaries; every in-session shape shrinks (−27 in-session, −25 ambiguous, sandbox-arm invariant), outside is byte-identical.
- AC-2: `TestVersionSessionRender` + `TestVersionSandboxLineNamesTheSandbox` green at d8fe1e74d, then falsified by mutation in a throwaway `git archive` tree: reverting `SessionState` WITH every `want` literal regenerated in step still fails all three arms by name and trips the absence loop on `inside`; reintroducing ` — pass --host` fails the absence loop by name plus `TestVersionAmbiguousMarkersExitZero`. A coordinated rename cannot green either.
- AC-3: `fo-install-gate.md:22` anchors on `^Sandbox: ` and env-wins, no value literal; independent repo sweep found the only value classifier is `version_gate_flow.sh:44` (`inside*` glob — the zvk9-doomed file, untouched here); `go test ./skills/integration/` green unmodified; no skill parses the boot JSON `sandbox` field.
- AC-4: OS line is line 2 in both shapes; ambiguous arm exits 0 driven live; `dispatch build` under two markers refuses with the complete three-value remedy, byte-identical base vs cut (live stdin-JSON drive); pi two-marker case resolves non-ambiguous; contract-token placement test green.
- AC-5: the finding section records the #468 boundary, both channel queries, the named blocking population, and the earliest retirement point; the corrected attribution matches the empty contractlint grep.
- AC-6: suites as above, exception applied only to the clean-base-reproduced environmental failure.

Findings, recommended for FO disposition (neither blocks the gate):

1. Evidence defect, recommend Polish — the implementation report's AC-1 "correction" mis-attributes the ambiguous-baseline discrepancy. It claims ideation's declared 148→123 omitted the 11-byte `contract 3\n` line; the measured matrix shows 148→123 is exactly the sandboxed-Sandbox-arm measurement (reproduces byte-for-byte, consistent with the in-session 130→103 the same report accepts), while the implementation's 159→134 is the safehouse-available arm — the two arms differ by exactly 11 bytes, a numeric coincidence with the token line. Delivered behavior and AC-1 are unaffected; the error lives only in report prose.
2. Deferred risk — `version_gate_flow.sh:44` still classifies on the stale `inside*` glob. Trigger is outside current promises: the harness drives captive scripts, never the real binary, and the file is deleted whole by captain-approved zvk9 (b77bfc942). Promote-to-material condition: zvk9 fails to land AND the script is driven against a real post-rename binary; even then the documented env-wins rule keeps the install verdict correct.

### Summary

All six ACs re-exercised independently at d8fe1e74d against merge-base b7b5b33c3: fresh binaries, a five-shape byte matrix, live drives of both keep-boundary commands and `status --boot`, two falsifying mutations in a throwaway tree, and both suites with the one failure reproduced on the clean base. The declared AC-1 byte pairs (130→103, 148→123) reproduce exactly once both shapes are measured in the sandboxed arm, which also resolves the implementation report's baseline-correction narrative as a mis-attribution — recorded as a Polish finding since no behavior or AC is affected. Verdict: PASSED, with one deferred risk (the zvk9-doomed `inside*` glob) carrying its promote condition.
