...
- FAILED: (pre-existing, not introduced) two internal/cli test failures observed during the run
  TestVersionAmbiguousMarkersExitZero fails only with this subagent session's `PI_CODING_AGENT` env set (passes with `env -u PI_CODING_AGENT`); TestCodexResolveManifestAgainstInstalledHost fails identically on the clean HEAD (verified via git stash) — codex manifest resolution, untouched by this entity.
