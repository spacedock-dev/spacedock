...
- DONE: gate record --round refuses an open log byte-clean (tree digest unchanged) and its error names the recovery; appending the correction's entries then records successfully
  `roundLogClosed` + call site in `internal/gates/round.go` (commit d125351b4). `TestRoundNoFindingsAndPreflightRefusals` gains three open shapes — reviewer-verdict tail, dangling disposition Annotation, hold-verdict tail — each asserting the tail the message names AND an unchanged `treeDigest`; `open-log refusal leaves a recordable round` asserts the "not closed" token, byte-cleanliness, then records the SAME round after appending the correction's entries (5-entry summary). Falsifier executed: neutralizing the call site (`if false && …`) reds exactly those four cases and nothing else in the package; restoring it greens `internal/gates`.
- DONE: A durably incomplete round room grades status fail with codes exactly [rejection-round-incomplete], never rejection-round-missing
  `TestRejectionTruncatedRoundRoomReportsItsOwnCode` records the complete round, overwrites the room log with `rejectionReviewerLog`, and asserts `gradeLive` → status `fail`, codes exactly `[rejection-round-incomplete]`, detail naming "holds 2 log entries, want 4" plus the retained ids; restoring the complete log grades `pass`. Fixing only the stream regex, or leaving the count inside the identity composite, leaves that state under `rejection-round-missing` and reds it. `TestRejectionNonRecordFileMutationReportsItsOwnCode` pins the split — README and `candidate.txt` → `rejection-workflow-file-mutated`, room-log tamper → the round oracle's code; ordering the non-record checks back after `ValidateRoundFile` reds the candidate case (the reachability falsifier). Extractor: the live `entries=2` result must satisfy `claudeRecordedRejectionRound`, while a `cycle=2` result must not — re-pinning `entries=4` reds the first, and the second proves genericizing the count loosened nothing else.
- DONE: AC-6's targeted local live rejection-flow run executed against the built change, reporting observed codes or a NAMED unavailability
  Attempt 4 PASSED with no findings — no `rejection-round-missing`, no `rejection-round-incomplete`, no `rejection-workflow-file-mutated`, which is AC-6's expected observation. One `--round validation/1` call, `is_error=false`, `entries=4` (stream lines 489-490); 8 routing events across both cycles. Artifacts: `.live-artifacts-zf7-rejection-4/claude-shared-scenarios/rejection-flow/`. Three earlier attempts, named rather than hidden: (1) the FO ended its turn after the first dispatch, so the journey never reached the record — a model-adherence stop, and notably NOT `rejection-round-incomplete`; (2) "API Error: Connection closed mid-response"; (3) HTTP 429 session limit. Per AC-6 the last two are provider errors and never grades.

### Summary

Shipped the combined fix: `gate record --round` now refuses a round log that has not been answered (closed = last entry is a Resolution that either sits past the verdict or is an `approve` verdict), derived from `parseReviewLog`'s Reviewer model and `validateResolution`'s approve/revise-hold dichotomy, so no entry count or actor identity enters the predicate. The refusal is byte-clean, names which open shape it saw, and names the recovery in contract order; `ValidateRoundFile` is untouched, so legacy rooms stay readable and the honest read-side codes (`rejection-round-incomplete`, `rejection-workflow-file-mutated`) remain as the backstop for legacy or bypassed state.

What stays unproven live: the refusal path itself never fired in the passing run, because the FO followed the record-after-correction order — the conforming outcome AC-6 predicted. So the live evidence proves the change does not break the journey and that the journey grades green; the refusal and its recovery are proven offline in `internal/gates`, and live adherence to the recovery remains a model-adherence residual for the metrics instrument.

Surface, disclosed against the estimate: +278/-15 across 3 files, net +263 vs the declared +150 with ±60 tolerance — about 175% of estimate, over tolerance by 53 net lines. Causes, none of them a design change: 86 of the 278 added lines are comments or blanks (this file's falsifiability-comment style); and the grading half needed 121 code lines rather than ~85 (three split-boundary cases, the truncation test with its restore control, the shared record helper, and the moved non-record byte block). The declared hard boundaries held: the product change is confined to the round-record admission path — `internal/gates/round.go` +35 (18 code) is exactly the admission predicate, its call site, and the refusal message with its tail helper. No CLI grammar, no new flags or verbs, no gate-ceremony change, no read-time (`ValidateRoundFile`) enforcement, no fixture-text change, no skill-prose change.

Tests: `go test ./...` on this exact code is green in 22 packages except one pre-existing, machine-state failure — `TestCodexResolveManifestAgainstInstalledHost` (`internal/cli`), reproduced identically on the unmodified `main` checkout (a locally installed codex plugin cache the test expects absent). `-race` is green on `internal/gates`; the `internal/ensigncycle` and `internal/cli` race passes did NOT complete — they timed out in unrelated git-heavy and timing-sensitive tests (`TestDurableKeepMovingBatchMotion`, `TestCodexProcessActivityResetsQuietBudget`) while this box carried a load average above 400 (source unattributed — process enumeration is blocked in this sandbox, and the 15-minute average was already elevated before these runs began). A full rerun launched after committing made 2 of 22 packages in 25 minutes under that load and was abandoned. That race gap is a named blocker, not a result: it needs a re-run on an unloaded machine or CI.

## Stage Report: validation

- DONE: Reproduce AC-1 through AC-5's cited evidence, including running the named falsifiers and confirming each reds exactly the cases claimed and nothing else
  All five reproduced; all four named falsifiers executed on a throwaway checkout of `d125351b4`, never the implementation worktree. AC-1: neutralizing the call site (`if false && !roundLogClosed(…)`) reds exactly four subtests of `TestRoundNoFindingsAndPreflightRefusals` — revise-verdict tail, dangling-Annotation tail, hold-verdict tail, and `open-log refusal leaves a recordable round` — and nothing else in a full `internal/gates` run that is otherwise 59 PASS / 0 FAIL. AC-2: folding the count back into the identity composite regrades the truncated room `[rejection-round-missing]` with the raw `%#v` struct dump — the exact lying label and useless message this entity exists to kill — and only that test reds. AC-3: re-pinning `entries=4` reds only "Claude extractor graded the live early-record result as never invoked"; its `cycle=2` sibling stays green, proving genericizing the count loosened nothing else on the line. AC-4: moving the non-record byte block back after `ValidateRoundFile` reds only `reviewed candidate artifact`, under `rejection-round-missing` — so the ideation's reachability correction was real, and the README half is order-independent. AC-5: `internal/gates` (123s) and `internal/ensigncycle` (227s) are both fully green with the precondition ON, which is simultaneously the over-refusal control — the lone-`approve` no-findings round and the advisory decline-triage close still record.
- DONE: Confirm AC-6's passing live run from its retained artifacts, and weigh the three named earlier attempts as disclosed rather than hidden
  Attempt 4 confirmed first-hand: exactly one `gate record --round validation/1` in the stream (lines 490-491), `is_error:false`, `entries=4`, the result enumerating reviewer Annotation + reviewer `revise` Resolution + ensign Annotation + ensign `revise` Resolution — record-after-correction, so no `rejection-round-*` code is reachable; topology 8 routing events across two cycles. All three earlier attempts are retained on disk and each matches its disclosure exactly: `.live-artifacts-zf7-rejection` holds a one-event topology and the final message "Dispatched the implementation ensign … ending this turn"; `-2` terminates `api_error` / "Connection closed mid-response"; `-3` terminates `api_error_status:429` / "You've hit your session limit". Disclosed, not hidden — the two provider errors are exactly what AC-6 says are never grades. Refinement the report did not make: the grading half DID run live (the live runner compiles `assertRejectionRecordedRound` from this worktree and wires it at `claude_live_runner_test.go:438`), so the new codes' sites, the moved byte block, and the genericized regex all graded the run's real durable end state and produced no code. Only the admission half's refusal went unexercised live.
- DONE: Judge the disclosed 175% surface overrun and the named race-suite gap: is the overrun's causal account honest, and is the race gap a real blocker or dischargeable by CI
  Overrun: honest, and every declared hard boundary held. The diff touches exactly the three declared files; `internal/gates/round.go` is +35/-0 and I counted it by hand — 18 code lines (4-line call site, 7-line `roundLogClosed`, 7-line `roundOpenTail`) and 17 comment/blank, so "+35 (18 code) is exactly the admission predicate, its call site, and the refusal message with its tail helper" is exact. `ValidateRoundFile` is genuinely untouched: round.go has zero deletions and that function appears only as diff context. No CLI grammar (`recordRoundLockedWith` is reachable only from `operation.go:115`, inside the `input.Round != ""` branch; the `--decision` path never reaches it), no read-time enforcement, no fixture-text change, no skill prose. The comments are the file's index to its own falsifiers, not padding — three of them state exactly the falsifying edits I then executed, and each claim held. One arithmetic correction, running against the worker's own excuse: the comment/blank share of added lines is 94 (17 + 18 + 59), not 86; its per-file figure of 121 code lines out of 180 for the ensigncycle file is exact. The 53-line tolerance breach is real, disclosed unprompted, and confined to test volume with no new mechanism or file — a captain-visible deviation the gate must carry, not grounds for a design reset. Race gap: CLOSED here, not merely dischargeable by CI. Machine load fell from 527 to single digits mid-validation, so I ran the full `go test ./... -race` on the throwaway checkout: 19 packages ok, zero `DATA RACE`, `internal/ensigncycle` ok in 495s and `internal/gates` ok in 59.7s. The only red is `TestCodexResolveManifestAgainstInstalledHost` in the untouched `internal/cli`, confirmed from this run rather than inherited: the resolver returns `/Users/clkao/.codex/plugins/cache/spacedock-local/spacedock/0.27.0-pre7/.codex-plugin/plugin.json`, a stale cache dated 2026-08-17 that falsifies the test's "not installed" premise. The full non-race `go test ./...` matches exactly — same 19 packages ok, same single `internal/cli` red. `gofmt -l ./cmd ./internal` and `go vet` on both changed packages are clean.

### Review-finding disposition (validation, cycle 1)

- **Deferred risk — the refusal path never fired live.** AC-6's verification condition, as written, is a green journey with no rejection-round code, and that was met; the entity's cost and spike sections separately promised AC-6 would prove live adherence to the recovery, and it did not. The residual is structurally unforceable: firing the refusal needs the live FO to attempt an early record, which the entity itself called "not spikeable … model adherence, not mechanism". Its sharpest sub-risk is now closed empirically rather than by source-reading — the worry was that a refused-then-recovered run would count a second publication and red `rejection-round-publication-count`, but attempt 4's own stream (line 290) shows Claude Code flags an exit-1 Bash command `is_error:true`, and `claudeRejectionRoundPublications` counts only `is_error` false-or-absent results, so refuse-then-recover grades exactly one publication on the real runtime. Worst case of what stays unproven is an FO treating exit 1 as a terminal stop, which grades `rejection-round-missing` — an honest red, strictly better than today's silent durable truncation under a lying label. Promote-to-material: a post-fix live lane observing `rejection-round-missing` from a fired-but-unrecovered refusal, or any live `rejection-round-incomplete` (which would mean the precondition was bypassed). Tracked by the metrics instrument.
- **Polish — the refusal's recovery clause assumes a rejection context.** Probed on the throwaway checkout: an `approve` verdict followed by a dangling Annotation is refused with "Route the correction, append that round's entries…" though there is no correction to route. The operative instruction (append a closing Resolution) is still right and the refusal is byte-clean; zero observed occurrences; no value AC fails.
- **Polish — `roundOpenTail` indexes `Entries[len-1]` with no empty guard** while `roundLogClosed` has one. Unreachable today: probed empty, blank-only and Annotation-only logs are all refused by `parseReviewLog` first ("review log must be non-empty UTF-8 JSONL…", "review log entry 1 is not JSON", "review log has no Resolution"), so nothing panics. A future caller that skips the parse would.
- **Boundary checked, behaves as designed, no finding.** A trailing non-verdict `hold` Resolution closes the round (a hold IS a response to the verdict, and `hold` is an authorized disposition); pointer identity against `review.Reviewer` correctly distinguishes the verdict from a later Resolution with identical content.

**Recommendation: PASSED.** No material finding. All six ACs have reproduced, non-self-referential evidence; each named falsifier reds exactly its claimed cases and nothing else; the full race suite is green with zero data races. Deferred risk and polish above are listed for the gate, not blocking.

### Summary

Reproduced every AC by execution on a throwaway checkout rather than by re-reading the report, and ran all four named falsifiers: each reds exactly the cases claimed and nothing else, including the ordering falsifier that proves the `candidate.txt` byte check would otherwise be dead code behind `ValidateRoundFile`'s artifact-digest check. The surface overrun's causal account is honest and every declared hard boundary held, with one arithmetic slip that understates rather than inflates the worker's own excuse. The race gap is closed rather than deferred: load dropped mid-validation and the full `-race` suite ran green with zero data races, leaving only a pre-existing `internal/cli` failure whose stale-codex-cache cause I confirmed from this run. The live residual — the refusal never firing — is a deferred risk, not a gap: it is unforceable by design, its worst unproven case is an honest red, and its sharpest sub-risk (double publication counting on recovery) is now settled empirically from the retained stream.
