...
**Tag-the-green-tip invariant + the `next` freeze window (why the order is what it is):** the green e2e run, the force-pushed `main` tip, and the tagged `v0.20.0` commit MUST all be the SAME commit. The wrinkle: `runtime-live-e2e.yml`'s `workflow_dispatch` has NO per-SHA input (its inputs are `claude_version`/`codex_version`/`effort`) — a dispatch runs against the selected ref's TIP at dispatch time, not the recorded `$PREPARED`. Because `next` keeps moving (dev-stays-on-`next`, AC-8), a concurrent dev push between recording `$PREPARED` and the e2e run would make the green run's `headSha` ≠ `$PREPARED`. Therefore **`next` is FROZEN from the moment `$PREPARED` is recorded (step 3) through the tag (step 7)** — branch-protection lock or a captain-announced dev quiesce for the flip window. And after the e2e goes green (step 4), the green run's `headSha` is RE-VERIFIED to equal `$PREPARED` before proceeding; a mismatch means the freeze leaked — re-record `$PREPARED` and re-dispatch (burning another live run). Nothing may advance the tip between the green run and the tag. The only commit that lands after the tag is `release.yml`'s own post-tag stamp step (`k6d`), which is downstream of the gate and does not affect the tagged SHA. The calendar-bump is deliberately NOT in the pre-flip fold (it would open a stale-payload window — see AC-7); it lands on `main` post-flip (step 9).

1. **Pre-cut antipattern audit** on the prepared `next` line (the 0.20.0 content) before anything outward fires. (Internal.)
2. **Fold the 0.20.0 content that must ride the tagged commit onto the prepared `next` line.** Each is a normal commit/PR onto `next`, reviewed before it goes green: (a) marketplace ref flip + paired test edit in ONE commit — `.claude-plugin/marketplace.json` ref `next`→`main` + `marketplace_manifest_test.go:67-68` expected `next`→`main`, `go test ./skills/integration/` green (AC-6); (b) doc reconciliation — `AGENTS.md:28` + `docs/releasing.md` against the real post-flip `release.yml`/`.goreleaser.yaml` (AC-4). (Internal — on `next`, not yet outward.) *(Note: the marketplace ref reads `main` while the commit still lives on `next`; that is intentional — it is the serving channel the flip is about to create, asserted green by the paired test. The calendar-bump is NOT folded here — it lands post-flip in step 9, see AC-7's transient-window reasoning. With the calendar key unchanged, the dormant `ref:main` on `next` fires no `@next` re-pull during the window.)*
...
8. 📡 **Calendar-bump on `main` (post-flip).** Apply the stable calendar-key bump on `main` now that `main` carries the flipped line — this is the trigger that makes existing-HOME `@…` users' `claude plugin update` re-pull `main`. Held until here precisely to avoid the stale-payload window (AC-7). *Captain gate if it pushes to `main`.*
