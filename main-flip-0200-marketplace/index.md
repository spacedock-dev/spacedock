...
**Tag-the-green-tip invariant (why the order is what it is):** the green e2e run, the force-pushed `main` tip, and the tagged `v0.20.0` commit MUST all be the SAME commit. So every commit that belongs in 0.20.0 — the marketplace ref+test flip, the calendar-bump, the doc reconciliation — is folded into the prepared line BEFORE the live e2e runs. Nothing may advance the tip between the green run (step 4) and the tag (step 7). The only commit that lands after the tag is `release.yml`'s own post-tag stamp step (`k6d`), which is downstream of the gate and does not affect the tagged SHA.

1. **Pre-cut antipattern audit** on the prepared `next` line (the 0.20.0 content) before anything outward fires. (Internal.)
2. **Fold all 0.20.0 content onto the prepared `next` line — these become the tip that gets tagged.** Each is a normal commit/PR onto `next`, reviewed before it goes green: (a) marketplace ref flip + paired test edit in ONE commit — `.claude-plugin/marketplace.json` ref `next`→`main` + `marketplace_manifest_test.go:67-68` expected `next`→`main`, `go test ./skills/integration/` green (AC-6); (b) calendar-bump producing a changed `marketplace.json` calendar key (AC-7); (c) doc reconciliation — `AGENTS.md:28` + `docs/releasing.md` against the real post-flip `release.yml`/`.goreleaser.yaml` (AC-4). (Internal — these are on `next`, not yet outward.) *(Note: the marketplace ref reads `main` while the commit still lives on `next`; that is intentional — it is the serving channel the flip is about to create, asserted green by the paired test.)*
3. **Record the SHAs.** `OLD_MAIN=$(git rev-parse origin/main)`; `PREPARED=$(git rev-parse origin/next)` AFTER step 2's commits land. `$PREPARED` is the single commit the green run, the flip, and the tag all reference. (Internal — pins AC-1's archive target and AC-2's CAS.)
4. 📡 **Run the live e2e on `$PREPARED` and wait green.** `workflow_dispatch` of `runtime-live-e2e.yml` against `next` at `$PREPARED` (it is still on `next` at this point); approve each spending environment; confirm all 5 legs `success`. This green run's `headSha` == `$PREPARED` is what `release.yml`'s `e2e-gate` will match at the tag. (Feeds AC-3 via tag-the-green-tip.) *Captain gate: authorize the live API spend.*
...
7. 📡 **Cut 0.20.0 — tag the green tip.** Tag `$PREPARED` (now `origin/main`'s tip, the green-run commit) `v0.20.0`, annotated; push the tag. `release.yml`'s `e2e-gate` resolves the tagged SHA == `$PREPARED`, finds the matching green run from step 4 → goreleaser publishes the Release + bumps the brew tap; the channel-aware stamp (`k6d`) commits the stable stamp on `main` AFTER the tag (downstream of the gate, does not alter the tagged commit). (AC-3.) *Captain gate: the actual release.*
8. **Post-flip verification** — fresh-HOME install resolves `0.20.0` from `main` (AC-3b); existing-HOME `plugin update` re-pulls `main` against the bumped calendar key (AC-7); the three upgrade journeys incl. the 5th (AC-5); record dev-stays-on-`next` (AC-8). *(Some release-machinery issues only manifest once the tag fires — verify after.)*
