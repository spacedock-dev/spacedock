...
**What still ships from this half:** the eligibility read *honors* any `blockers` entry or active `execution-hold` that is present — fail-closed, ineligible — even though nothing in this layer authors one. If a hold is ever needed it is a `gates:` write through the recorder binary's existing surface, not a new command here.
...
**AC-A4 (second-pending-application refusal — RED FIXTURE).** A current gate attempt binds at most one application. Creating a second pending application while one already exists (pending or consumed) on that attempt is refused, not appended; the record still holds exactly one application. *Test:* red fixture — call the create-application verb twice on the same closed attempt; the second is rejected and the record is unchanged.
