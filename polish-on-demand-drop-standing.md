...
## Stage Report: implementation (cycle 3, rework 1 — AC-2 live scenario registered)

- DONE: AUTHOR the AC-2 live scenario test (//go:build live) — boot a real FO against a fixture workflow declaring a standing comm-officer mod, reach first dispatch (where spawn-standing-all injects), and ASSERT comm-officer PRESENT in the team config.json members (residency proof; reds if residency breaks).
  `internal/ensigncycle/live_standing_residency_test.go`: `TestLiveStandingResidencyInjectsCommOfficer` reuses the `TestLiveEnsignCycle` front-door harness (isolatedClaudeEnv + withBinaryOnPath + streamWatcher TeamCreate→dispatch-close), stages the realistic 3-stage fixture PLUS `_mods/comm-officer.md` (standing), then globs `{CLAUDE_CONFIG_DIR}/teams/*/config.json` and asserts a member named comm-officer — the same members[] shape MemberExists reads. Oracle is the on-disk roster, not a contract grep; absent comm-officer fails (the cycle-1/2 regression). Commit 4d0de768.
- DONE: REGISTER it into runtime-live-e2e.yml's `-run` selector (fold into the existing live-cycle step's alternation).
  `.github/workflows/runtime-live-e2e.yml` line 174 (the `Run live ensign cycle` step's real `run:` block, not a comment): `-run 'TestLiveEnsignCycle|TestLiveStandingResidencyInjectsCommOfficer'`. Runs behind the CI-E2E / CI-E2E-OPUS approval gates on every PR to main. Commit 4d0de768.
- DONE: VERIFY it's actually invoked + offline gate green.
  `go test -tags live -list 'TestLiveEnsignCycle|TestLiveStandingResidencyInjectsCommOfficer' ./internal/ensigncycle/` selects `TestLiveStandingResidencyInjectsCommOfficer`; the name appears in the yaml's real `run:` line 174 (grep-confirmed, not a comment). `go vet -tags live ./internal/ensigncycle/` exit 0; `go build ./...` exit 0; `go test ./...` exit 0 (15 pkgs ok, live test compiles out of the default surface). Binary + shed + AC-1/AC-4 unchanged from cycle 3.

### Summary

Validation cycle-1 rework: closed the one blocking defect — AC-2 (the load-bearing residency proof) was authored-as-intent but not registered/runnable in CI. Authored `TestLiveStandingResidencyInjectsCommOfficer` (//go:build live) mirroring the `TestLiveEnsignCycle` live front-door harness: it boots a real FO against a fixture workflow declaring a standing comm-officer mod, watches TeamCreate→dispatch-close (spawn-standing-all runs before the first team-mode Agent() dispatch), then reads `{CLAUDE_CONFIG_DIR}/teams/*/config.json` and asserts comm-officer is in the members roster — the independent on-disk oracle, reds if residency breaks. Registered it into runtime-live-e2e.yml's live-cycle `-run` alternation (line 174, real run: block) so it gates at PR CI behind the CI-E2E approval, and verified invocation via `go test -tags live -list` + the yaml grep. Binary, contract shed, and AC-1/AC-4 are unchanged from cycle 3; `go test ./...` + `go vet -tags live` green. The note in the prior cycle-3 report that AC-2 "gates at PR CI" is now TRUE (it was the gap validation correctly rejected).

## Stage Report: validation (cycle 2)

- DONE: AC-2 live residency scenario REGISTERED + runnable + reds-if-residency-breaks.
  `internal/ensigncycle/live_standing_residency_test.go` defines `TestLiveStandingResidencyInjectsCommOfficer` (`//go:build live`). `go test -tags live -list 'TestLiveStandingResidencyInjectsCommOfficer' ./internal/ensigncycle/` selects it. Wired into the REAL `run:` step in `runtime-live-e2e.yml:174` — `-run 'TestLiveEnsignCycle|TestLiveStandingResidencyInjectsCommOfficer'` (under `set -o pipefail`, not a comment). Read the test: boots the real `spacedock claude` front door against a fixture declaring a standing comm-officer mod, awaits TeamCreate + dispatch-close (spawn-standing-all runs before first team-mode Agent dispatch), then globs `{CLAUDE_CONFIG_DIR}/teams/*/config.json` and `t.Fatalf`s if comm-officer is absent from every roster. Oracle is the on-disk roster the live harness wrote (parsed with the same `members[].name` shape `MemberExists` reads), NOT a contract grep. Reds if residency breaks — the cycle-1/2 regression guard.
- DONE: Binary + shed + AC-1/AC-4 UNCHANGED from the cycle-3 clean reproduction.
  `git diff --stat 776a5f82 4d0de768` = exactly two files (the live test +1 yaml `-run` line); the dispatch binary, contract shed, mod, and the 4 spawn-standing-all fixtures are byte-identical to the prior PASS reproduction. Additive-only.
- DONE: Offline + vet gates green.
  `go test ./...` exit 0 (live compiles out; dispatch + ensigncycle ok). `go vet -tags live ./internal/ensigncycle/` exit 0.

### Feedback Cycles

- **validation cycle 2 — re-validation of the cycle-1 fix (commit 4d0de768):** The cycle-3 REJECT (AC-2 unregistered in the CI live gate) is CLOSED. The fix registers `TestLiveStandingResidencyInjectsCommOfficer` — a genuine live scenario whose load-bearing assertion is the on-disk team `config.json` roster, reds if comm-officer residency breaks — into `runtime-live-e2e.yml`'s real live-cycle `-run` selector. The fix is additive-only (live test + one yaml line); no regression to the previously-clean binary/shed/AC-1/AC-4. The AC-2 live residency scenario now gates at PR CI behind the CI-E2E approval gate — the load-bearing residency proof. (Skip-on-no-credential matches `TestLiveEnsignCycle`'s established discipline; the credential is provided behind the gate.)

### Summary

PASSED. The cycle-3 rejection is fully closed and nothing else regressed. AC-2's load-bearing residency proof is now a registered, runnable, residency-reds-on-break live scenario (`TestLiveStandingResidencyInjectsCommOfficer`) wired into `runtime-live-e2e.yml`'s real `-run` alternation at line 174 — verified selectable via `-list`, with the on-disk `config.json` roster as the independent oracle (not a contract grep). The fix is strictly additive (2 files: the live test + one yaml line); the binary driver, contract shed, mod self-declaration, and AC-1/AC-4 fixtures are byte-identical to the prior clean reproduction. `go test ./...` exit 0 (live compiles out) and `go vet -tags live` exit 0. The AC-2 live residency scenario gates at PR CI as the load-bearing residency proof. All four ACs proven; recommend merge.
