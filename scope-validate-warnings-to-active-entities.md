...
They raised one adjacent finding, verified here rather than taken on report, and it is stronger than they stated. In the main tree `SummaryFileDiagnosticsAt` has exactly two consumers: `internal/cli/cli.go:337` and `SummaryFileAt` (io.go:200), which discards the warnings slice with `_`. `SummaryFileAt` in turn is reached only by `SummaryFile` (io.go:196) and a test; `SummaryFile` is reached only by `application_test.go:365`. So once cli.go:337 goes, the whole `SummaryFile`/`SummaryFileAt`/`SummaryFileDiagnosticsAt` trio is production-unreachable, not merely warning-blind — which undercuts the approved keep-boundary that retains the reader because `SummaryFileAt` consumes it. That belongs to neither entity's scope; both of us recommend filing it separately rather than widening either cut.
