...
- 121 are `unknown gate application field` (validate.go:238-241); 4 are `field 'verdict' value "..." is not one of [PASSED REJECTED]` (field_conformance.go:100-104).
- The run exits 0 and stdout is `VALID`. Captured output is 51457 bytes.
- Active-scope warn lines: zero.

The value chain is broken at the fix step. Archived scope is publish-only and terminal, so no tool-mediated write can ever clear these findings — the alarm fires identically on every invocation, forever, and a signal that cannot change carries no information. The 4 verdict warnings are residue of the writer that `6c45fd59c` already fixed; that commit's ruling was explicit that already-archived terminal entities must not be hand-migrated to silence a diagnostic about a value the tool itself wrote. The same reasoning applies to the 121 application-field warnings, which is why this is the identical pathology rather than a new one.
