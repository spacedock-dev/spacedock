...
**AC-1 (value, measured against a baseline that moves the wrong way) — the interactive greet is deterministically light.** On a heavy fixture (≥2 discoverable workflows; the chosen workflow holding ≥2 ready `gate: true` entities), a live interactive boot on the branch renders ZERO `present-gate` reviews at the greet and asks NO workflow-pick question, versus the current-contract baseline which renders one `present-gate` review per ready gate and asks which workflow. Measured quantity: present-gate-attributable tool calls captured at the greet-stop drop from ≥2 (baseline, one per ready gate) to 0 (target); the greet's terminal prose changes from a workflow-pick question to a `Use engage` hint.
- *Tested by:* a live interactive FO drive on the same fixture under both the `origin/main` contract and the branch; each session transcript snapshotted at the greet-stop (the FO's first yield for captain input, before any direction); parse `tool_use` and assert baseline present-gate count ≥ ready-gate count while target == 0, and baseline greet contains a pick question while target contains the engage hint with no pick. (Per the spike: categorical signals at a controlled greet-stop, not a raw total count.)
