...
- DONE: `--version` keeps `spacedock <ver> (contract <N>)` as line 1 (must satisfy `^spacedock .* \(contract \d+\)$`), then appends a `Sandbox:` line + per-runtime block; the launcher banner gains a `Sandbox:` line; `status --boot` gains a `SANDBOX:` section and `--boot --json` gains a `sandbox` field (AC-1, AC-2, AC-3).
  Banner: `launchBanner` gains a `Sandbox:` line after the workflow line (frontdoor.go; pi.go passes its own lookPath). Boot: `gatherBoot` computes `d.sandbox`, `printBoot` appends `SANDBOX:` after STATE_BACKEND, `bootJSON` appends the `sandbox` key. Version: `printVersion` appends `Sandbox:` + claude/codex/pi lines; first-line regex guarded by TestVersionFirstLineUnchanged. Live binary verified: all three states render (enabled / available, not enabled / unavailable).
- DONE: claude `enabled` field added to the plugin-list read so enablement reflects `enabled` not mere presence (AC-4); a host whose enablement probe ERRORS renders `installed, enablement unknown`, never silent `not installed` (AC-5); all five ACs proven by Go tests with injected probe seams — no live host CLI in the test path.
  `pluginListEntry.Enabled` added (host_exec.go); `claudeEnablement` reads it (false/absent → not-enabled). `runtimeProbe` seam with `execRuntimeProbe` (production) + `fakeRuntimeProbe` (tests); probe-error → `enablementUnknown` → `installed, enablement unknown`. Proven by TestClaudeEnablementFromListJSON (AC-4), TestVersionEnablementUnknown (AC-5), TestVersionPerRuntimeBlock (AC-3), TestBootSandboxSection/JSONField (AC-2), TestLaunchBannerSandboxLine (AC-1).
- DONE: `go test ./internal/cli/ ./internal/status/` green over the WHOLE packages (internal/cli is the hot shared package). ZERO .md edits — gj's doc-diff delegated to yw. `git show --stat` no *.md.
  `go test ./internal/cli/ ./internal/status/ ./internal/safehouse/` → 770 passed; full module `go test ./...` → 1275 passed; gofmt/vet clean. `git show --stat` on commit 0f381834: 21 files, all `internal/**/*.go`, zero `*.md`.

### Summary
Implemented the three sandbox surfaces with the three-way state strings unified in the new `safehouse.State(selected, available)` so banner/boot/version read identically. The claude `enabled` field was added to `pluginListEntry` and read by `claudeEnablement`; per-runtime detection runs behind an injected `runtimeProbe` seam (claude/codex via plugin-list, pi via `checkPiRuntime`) so a probe error renders `enablement unknown` and tests never shell a live host CLI. The PATH-dependent boot SANDBOX line is stripped from the byte-compared goldens exactly like STATE_BACKEND (new `stripSandbox` helper), with its state-from-inputs behavior pinned independently in boot_sandbox_test.go; the goldens regenerated byte-identical. Committed on branch `spacedock-ensign/startup-sandbox-status` (0f381834); zero `.md` edits per the anti-collision constraint (gj doc-diff delegated to yw).

## Stage Report: validation

- DONE: Reproduce `go test ./internal/cli/ ./internal/status/` GREEN over the WHOLE packages; confirm the five AC tests exist and pass — banner Sandbox line (AC-1), boot SANDBOX section + --boot --json sandbox field (AC-2), version per-runtime block (AC-3), claude `enabled` field read (AC-4), probe-error renders `enablement unknown` not `not installed` (AC-5) — all via injected probe seams, no live host CLI.
  `go test ./internal/cli/ ./internal/status/` → 750 passed (770 with safehouse, matching the implementer's report). The five AC tests: TestLaunchBannerSandboxLine (AC-1, real launchBanner over 4 (selected,available) corners), TestBootSandboxSection + TestBootSandboxJSONField (AC-2, runNative drives the built binary against a real .safehouse file + a real safehouse executable on a pinned PATH), TestVersionPerRuntimeBlock (AC-3, exact per-runtime lines from injected probe), TestClaudeEnablementFromListJSON (AC-4, fixture JSON the independent oracle), TestVersionEnablementUnknown (AC-5, probe-error → `installed, enablement unknown` plus an explicit negative guard against `not installed`). All assert independent test-supplied expected values; none reads the string back from the render under test.
- DONE: ADVERSARIALLY verify AC-3's first-line guard is NOT a tautology... temporarily break it, run the test, see it RED, then revert. Also confirm AC-4's claude-enabled reader returns not-enabled when the fixture's `enabled` is false.
  Refuted twice. (1) Reordered printVersion so the Sandbox line prints above the version line → TestVersionFirstLineUnchanged FAILED at version_runtime_test.go:53 (`first line "Sandbox: unavailable..." does not match`). (2) Reformatted the version line to `[contract %d]` (brackets) → FAILED again (`"spacedock dev [contract 1]" does not match`). Reverted via `git checkout`; `git diff HEAD` now empty, printVersion line 1 restored to `spacedock %s (contract %d)`, clean tree re-runs 750 GREEN. The guard catches both reorder and format-break — not a tautology. AC-4 enabled-false: `claudeEnablement` (host_runtime.go:64-78) reads the `enabled` boolean and the `enabled-false` subtest passes, returning enablementNotEnabled; the reader is genuine field-reading (the fixture, not the render, is the oracle).
- DONE: Confirm zero `.md` edits (gj's --version/banner doc-diff is delegated to yw); recommend PASSED/REJECTED.
  `git show --name-only` on 0f381834: 21 files, all `internal/**/*.go`, zero `.md` and zero non-`.go`. The absent doc edit is the intended yw delegation, not a defect. No live host CLI in any AC test path. Live `./spacedock --version` sanity passed (first line matches `^spacedock .* \(contract \d+\)$`). RECOMMENDATION: PASSED.

### Summary
Verified the deliverable, did not re-implement. Both required packages are GREEN (750; 770 with safehouse) on the clean worktree, all five AC tests exist and assert against independent oracles, and the commit carries zero `.md` edits (all 21 files `.go`), confirming the yw doc-diff delegation. The highest-stakes check — the AC-3 first-line guard the FO/ensign skills parse for the contract token — is genuinely strength-tested: it went RED under both a reorder (Sandbox line above the version line) and a format-break (`[contract N]` brackets), and was reverted to a clean tree. AC-4's claude-enabled reader correctly resolves `enabled:false` to not-enabled from the fixture oracle. gj is low-blast-radius UX with the inline refutation serving as the audit-class check (no separate detached audit). Recommendation: PASSED.
