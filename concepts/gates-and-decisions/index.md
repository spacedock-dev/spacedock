...
doc_version: "0.20.2"
last_updated: "2026-08-09 19:51:41"
---
...
After completion verification, a gate with no current-stage authority remains `validating` until gqs's mechanical report checks pass. It then appears as `needs-preparation` on boot and every machine scheduler read. Engage performs semantic report review. A concrete `report-incomplete:` veto stops without mutation; otherwise the First Officer calls `gate prepare` exactly once with its question, Artifact, summary, and References, commits the emitted binding, re-reads `awaiting-captain`, and presents it. Open, withdrawn, stale, closed, and spent attempts retain their existing lifecycle routes.
