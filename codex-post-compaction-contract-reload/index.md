...
| Subject | Creation and identity | Typed state | Required binding |
| --- | --- | --- | --- |
| Worker obligation | Before spawn/reuse, persist workflow/entity/stage/cycle, assignment epoch, report baseline, continuation, revision, and exact next action. | `await_completion`, `verify_report`, `route_feedback`, `rerun_review`, `present_gate`, or `terminalize`. | Permit binds obligation key/revision and assignment epoch. Pre-spawn `await_completion` is unbound with exactly one authorized `spawn_initial`; after spawn it binds returned worker identity and becomes `wait`. |
| Standalone session action | With zero unresolved obligations, under lock require `needs_rehydrate=false`, no active standalone action, and no permit; persist action ID, session identity, class/target digest, generation, revision 1, before/after evidence, successor, and empty result. | `planned`, `issued`, `executing`, `indeterminate`, `consumed`, or `blocked`. | Permit binds session identity plus action ID/revision and MUST omit worker, obligation, and assignment fields. Valid only for `state_mutation`, `archive`, or `merge`. |

Standalone creation is a real state transition, not a shortcut around the ledger. The gateway validates the canonical target and expected before/after evidence, writes `planned` revision 1, increments checkpoint generation, and fsyncs before it may issue a permit. Each later transition increments both revisions. Success fills result and successor fields; ambiguity fills the typed blocker and observed evidence. Terminal standalone records remain for the session so an old nonce is still recognized as consumed/blocked, then clean-session teardown removes them.
...
Resume never silently repairs or deletes uncertainty. A mismatched nonce/host/workflow/schema/owner is quarantined; an `executing` permit is interpreted as indeterminate; a crash between pre-spawn record and effect retains the unbound `spawn_initial`; a crash after a visible effect requires the action-specific evidence table. Age cannot mark a subject complete. A maintenance command may list or garbage-collect terminal/quarantined records, but unresolved state needs explicit recovery.

Spacedock-owned dispatch, state mutation/archive, and merge paths go through the action gateway before effects. `needs_rehydrate=true` blocks subject creation, permit issue, and permit execute. Raw host actions are covered only where a client interceptor is live-proved.
...
| Transition | Durable requirements | Result and refusal rules |
| --- | --- | --- |
| subject -> `issued` | Under lock reconcile exact next action; store one permit; increment subject revision and checkpoint generation; atomic write plus file/directory fsync. | Wrong subject/action/target, active permit, `needs_rehydrate`, or priority conflict refuses with zero effects. |
...
| `executing` -> `consumed` | Release lock for effect. On exact returned result, reacquire; verify subject/permit; fill result/successor; consume; increment revisions; fsync. | Preserve any later `needs_rehydrate`. Duplicate use or stale revision has zero effects. |
| `executing` -> `blocked` or new permit | Crash, timeout, lost response, or partial result becomes indeterminate. Restart never invokes it directly; reconcile action-specific durable evidence. | Exact success consumes without replay. Conclusive no-effect may authorize a new permit only under the table below. Missing/partial/multiple/mismatched evidence blocks. |
...
Automatic continuation is bounded: at most one directive injection and one watchdog-started turn per `(compaction_epoch, checkpoint_generation)`. Stop refusal is not durable progress. A watchdog runs only for a non-wait typed action with no operator turn pending; repeated state, failed fence, or no generation advance records a blocker and stops. Wait timeouts preserve the same worker/epoch; they never imply completion or redispatch.

## Crash recovery by action class

| Action | Exact-success evidence | Conclusive no-effect evidence | Ambiguous/partial result |
| --- | --- | --- | --- |
| `spawn_initial` | Exactly one durable acknowledgement keyed by permit nonce/assignment epoch plus matching roster/mailbox identity: bind it and commit `wait`, never spawn again. | Only an authoritative gateway/idempotency query proving the dispatch key was never accepted may authorize a new permit. Absence alone is insufficient. | Missing/unqueryable, multiple, mismatched, or cross-epoch evidence -> `indeterminate_spawn`. |
...
`compaction_ready` is prevention, not recovery. It is true only at a declared logical phase boundary when every obligation is durable, every dispatched obligation is bound to worker/epoch, every report baseline and typed next action exists, no standalone action is nonterminal, and no permit is issued/executing. Launcher/client compaction refuses false readiness and names missing fields. Observed automatic compaction still fences; unobserved zero-subject automatic compaction remains outside the guarantee. Pressure thresholds only suggest checking readiness.
...
| --- | --- |
| Filesystem | Session directory `0700`; files `0600`; reject symlinks, non-regular files, owner mismatch, traversal, nonce/identity mismatch, unknown schema, partial/regressing records. Threat model excludes malicious unrestricted same-UID processes. |
...
| AC-3 | Checkpoint + contracts + workflow + roster/mailbox reconstruct entity, stage, cycle, epoch, baseline, continuation, and next action without summary state. | Replace summary with stale executable prose and compare reconstructed typed state. |
...
| AC-7 | Invalid paths/ownership/schema/header/digest/count/size/resume fail closed; crash-retained subjects resume only after validation and become indeterminate without replay. | Boundary/one-over/corruption fixtures and crash adoption tests. |
...
| AC-9 | Pre-spawn persists unbound `await_completion`, exactly one `spawn_initial` binds returned worker then `wait`; timeout/interruption preserves worker; feedback completes repair, verification, re-review, gate. | Full pre-spawn path plus wait/interruption and rejected-fix replay. |
...
| AC-14 | Every external effect has fsynced executing intent; restart recognizes exact success, proves no-effect, or blocks ambiguity without duplicate replay. | Three kill points for all four action classes; durable fake external ledger; effect count <= 1. |
...
Exercise serialization, atomic write/fsync/rename, CAS, adoption, quarantine, and cleanup. Drive the four lifecycle rows and readiness predicate term by term. Test each exact and one-over size/count/path/string bound, corrupt magic/checksum/digest/length/count combinations, all-zero-count/nonzero-body rejection, quiescent IO/allocation counts, and held-lock `checkpoint_busy`. Confirm crashes retain every nonterminal subject/permit while clean exit removes only eligible state.

#### Suite 3 — Subjects, permits, and concurrency

Drive the full obligation pre-spawn path and zero-obligation standalone creation for all three non-worker actions. Table-test wrong subject kind, action, target, session, action/obligation revision, assignment epoch, generation, and duplicate nonce. Use deterministic barriers for writer-before/writer-after executing intent, two workflows, reverse completion order, priority, stale completion, and generation conflict. Every refusal asserts a zero external-effect counter.

#### Suite 4 — Crash reconciliation

Run each action in a subprocess and kill at all three barriers. Restart from the on-disk checkpoint plus a durable fake external ledger keyed by permit nonce. Cover exact success, authoritative no-effect, missing result, partial result, multiple effects, mismatched identity/hash/commit/ancestry, and ambiguous API response. Reconciliation must never call the effect directly; every run proves total effect count at most one and the exact successor or blocker.
...
- DONE: Compress within budget.
  Replaced the 9,229-word amendment stack with one current decision/design/proof artifact using the required outline and compact tables; the pre-commit result is 3,525 words and 182 source lines.
- DONE: Preserve invariant and AC coverage.
  Retained the completion/report/successor invariant, summary distrust, epoch/CAS isolation, obligation/standalone subjects, exactly-one unbound spawn, fsynced executing intent, no-blind-replay reconciliation, lifecycle/readiness boundary, non-authoritative hooks, bounded continuation, all exact limits, and AC-1 through AC-17 with proofs.
