...
- zero or more standalone session action intents for guarded non-worker effects.
...
A zero-obligation `state_mutation`, `archive`, or `merge` uses a standalone action intent, never a synthetic worker obligation. Its record contains a random action ID, session nonce/host identity, action class, exact target/argument digest, checkpoint generation at creation, its own monotonic action revision, state (`planned`, `issued`, `executing`, `indeterminate`, `consumed`, or `blocked`), expected before/after evidence, explicit successor fields, observed result fields, permit nonce when issued, and typed blocker when applicable. It contains no worker identity or assignment epoch.

Creation is normative: under the checkpoint lock, require `needs_rehydrate=false`, zero unresolved obligations, no other active standalone action, and no outstanding permit; validate the non-worker action class and before/after evidence; write revision 1 in `planned`; increment checkpoint generation; atomically persist and fsync file plus directory. Reconciliation may then issue one permit whose subject is this action ID/revision. Each transition increments both checkpoint generation and the action's own revision. Success fills result/successor fields and marks `consumed`; ambiguous recovery fills blocker/result evidence and marks `blocked`. Consumed/blocked records remain durable for the session and are removed only by the normal clean-session lifecycle, so an old permit nonce remains rejectable as already terminal.
...
- Prefix the checkpoint with a checksummed fixed 4 KiB guard header containing magic, schema, total/body length, generation, `needs_rehydrate`, compaction/rehydrate epochs, nonce and host identity, bounded canonical launcher/contract-root paths, aggregate source digest, obligation count, standalone-action count, outstanding-permit count, and body digest. Header paths are capped at 1,024 bytes each. Cap the whole checkpoint at 1 MiB including the header (body at most 1 MiB minus 4 KiB), obligations at 128, standalone action records at 128 with at most one active, outstanding permits at one, and any body string at 4 KiB. When all three counts are zero, the atomic writer compacts the body to length zero, so the header is the complete inactive checkpoint. The inactive path performs one `stat` plus one `pread` of at most 4 KiB, decodes zero body records, and uses at most 16 KiB transient allocation. Bad magic/checksum/length/count/digest, truncation, excess size, or zero counts with a nonzero body fails closed.
...
- Create the record after launcher/contract-root resolution. On clean session exit with zero obligations and no nonterminal standalone action/permit, remove the session directory, including terminal session-action audit records. On crash or exit with an obligation, nonterminal standalone action, or permit, retain it for explicit resume. Resume adopts it only after nonce/thread/workflow validation; mismatched or stale records are quarantined and reported, never silently reused or deleted.
...
An unresolved obligation is not a blanket prohibition. The reconciler compares the requested action with the deterministic priority order and the obligation's typed next action. A permit is a discriminated union with `subject_kind=obligation|standalone` plus a random nonce, action class, exact target/argument digest, resulting checkpoint generation, and permitted successor transition. An obligation permit additionally requires obligation key/revision and assignment epoch. A standalone permit instead requires session nonce/host identity plus standalone action ID/revision and MUST omit worker identity, obligation key, and assignment epoch. Issuance is an atomic checkpoint mutation that stores the permit, advances the selected subject to `issued`, increments its revision and checkpoint generation, and stamps the permit with those resulting revisions. Wrong subject kind, action/target, generation, subject revision, obligation epoch when applicable, second use, or revocation is rejected before effects. Independent dispatch remains permitted only after higher-priority worker obligations are accounted for.
...
Concretely, the guarded commands read `${SPACEDOCK_FO_CHECKPOINT}` and ask one launcher-owned action gateway for a permit before emitting a dispatch spec or performing any filesystem/git mutation. The initial action classes are `spawn_initial`, `spawn_replacement`, `state_mutation`, `archive`, and `merge`. `needs_rehydrate=true` always refuses creation or issuance until rehydration succeeds. Worker actions reconcile an obligation and issue only its exact typed next action; this explicitly permits the pre-recorded unbound `spawn_initial`. When there are zero obligations, a requested `state_mutation`, `archive`, or `merge` must first follow the standalone creation procedure above and then issue against that record. It may not borrow an old obligation or fabricate an assignment epoch.

Permit issuance and execution are separate. The action gateway acquires the checkpoint lock and rechecks common permit fields plus the selected subject: obligation key/revision/assignment epoch for worker actions, or session identity/action ID/action revision for standalone actions. It also checks issued status and `needs_rehydrate`. It then changes permit and subject to `executing`, records the action-specific reconciliation key and expected before/after evidence, increments the subject revision and checkpoint generation, and atomically persists plus `fsync`s the checkpoint file and directory. That durable executing transition is the action's linearization point. The gateway then releases the lock before invoking the external effect. A concurrent context-loss or ledger writer therefore serializes before the executing commit and stales/refuses the permit, or after the action has durably entered recovery-visible execution.

If the effect returns, the gateway reacquires the lock, verifies the same executing permit/subject and observed result, fills the subject's explicit result/successor fields, marks subject and permit consumed, increments subject revision and checkpoint generation, and fsyncs before returning. Only an error that conclusively occurred before any external visibility may consume the permit as no-effect and authorize reconciliation toward retry. A timeout, lost response, partial result, or any other ambiguous error keeps the executing intent and enters the same indeterminate reconciliation path as a crash. A crash cannot perform any post-effect write; after restart, the persisted `executing` permit and subject are interpreted as `indeterminate`, block all conflicting actions, and are never replayed merely because the successor is absent. Raw host calls that the gateway cannot own remain outside the enforcement claim.
...
Context pressure is a suggestion signal, never permission to compact mid-obligation or action. The launcher computes `compaction_ready` from durable state. It is true only when every active obligation has been written to the checkpoint, every dispatched obligation has its worker identity and assignment epoch bound, every report baseline is stored, every obligation has one typed next action, no standalone action is nonterminal, and no permit is issued or executing. A context-size or tool-count threshold may suggest compaction only at a logical phase boundary and only when this predicate is true.
...
**AC-10: Inactive cost.** On the uncontended, quiescent no-action probe with no observed compaction, unresolved obligation, standalone action record, or outstanding permit, guard evaluation acquires/releases the lock within its 250 ms ceiling, performs one `stat` and one `pread` of at most 4 KiB, decodes zero body records, uses at most 16 KiB transient allocation, and performs no roster/mailbox query, contract stream, or checkpoint write. Contention beyond 250 ms returns `checkpoint_busy` with zero authorization/effect; no latency/read bound is claimed for that contended path. Oversized, corrupt, or inconsistent headers fail closed. Trace, allocation, and held-lock fixtures assert these bounds.

**AC-11: Compaction readiness prevents unsafe voluntary compaction.** `compaction_ready` is false whenever any active obligation lacks durable persistence, bound dispatch identity/epoch, stored report baseline, or typed next action, any standalone action is nonterminal, or any permit is issued/executing; launcher/client-initiated compaction is refused with the exact missing fields. Readiness becomes true only when all obligations satisfy that predicate, no standalone action/permit is active, and the current point is a declared logical boundary. Table tests toggle every term independently, and an observed automatic-compaction fixture proves that bypassing this prevention layer sets `needs_rehydrate`; an unobserved fixture is classified under AC-4's limit.
...
**AC-13: Typed permits are action-scoped, single-use, and race-free.** Every permit authorizes only its subject kind, exact action class/target digest, checkpoint generation, and subject revision. Obligation permits additionally bind obligation key and assignment epoch; standalone permits instead bind session identity and action ID and contain no worker fields. Wrong-subject/action/target, stale generation/revision/epoch when applicable, and duplicate-use attempts cause zero effects. A writer that changes generation or sets `needs_rehydrate` before the under-lock `executing` commit makes the permit fail; a writer after that fsynced commit observes an already-linearized action and cannot erase its indeterminate/reconciliation state. Successor commit preserves any later `needs_rehydrate` flag.
...
3. Add checkpoint serialization, private-path validation, atomic-write, compare-and-swap, incomplete-bind, digest, lifecycle, and stale-resume tests under a proposed `internal/rehydrate/` package. Generate exact-boundary and one-byte/one-record-over fixtures for the 4 KiB header, 1 MiB total checkpoint, 1,024-byte header paths, 128 obligations, 128 standalone action records/one active, one outstanding permit, and 4 KiB body strings; corrupt magic/checksum/digest/length/count and all-zero-count/nonzero-body fixtures fail before allocation or action. On an uncontended no-action probe, instrument one `stat`, one `pread <= 4096`, zero body decodes, and `-benchmem <= 16,384 B/op`. Hold the lock beyond 250 ms and assert typed `checkpoint_busy`, zero authorization/effect, and no fast-path latency claim. Add table tests that toggle each `compaction_ready` term independently, including a nonterminal standalone action and issued/executing permit, require a logical boundary, report missing fields, and refuse unsafe launcher/client compaction.
...
6. With zero obligations, table-drive `state_mutation`, `archive`, and `merge` through standalone creation (`planned` revision 1 with explicit empty result), issuance, fsynced executing, exact success/result/successor, and consumption. Assert that every permit has `subject_kind=standalone`, session/action identity and action revision, and no obligation/worker/assignment fields. For every class assert zero effects on wrong action, wrong target, stale standalone revision/generation, and duplicate use; ambiguous evidence blocks the record. Re-run the three kill boundaries from step 8 on this zero-obligation path.
7. Add deterministic generation/compaction race tests around the executing-intent linearization point for both subject kinds. If a writer increments generation and/or sets `needs_rehydrate` before the gateway's under-lock executing commit, the permit is rejected and the effect counter remains zero. If the executing commit fsyncs first, the writer sees the executing subject, may set `needs_rehydrate` afterward, and cannot erase it; the effect result/successor commit preserves that flag. Cover two workflows, out-of-order completions, stale epochs, generation conflicts, and matching/malformed reports.
8. For obligation-backed `spawn_initial` and standalone `state_mutation`, `archive`, and `merge`, run a subprocess fixture with kill barriers (a) after executing-intent fsync but before effect invocation, (b) after the effect becomes externally visible, and (c) after result observation but before successor/permit-consumption commit. Restart reconciliation from disk and a durable fake external ledger keyed by permit nonce. Assert no automatic replay and total effect count <= 1. Exercise exact-success recognition, conclusive no-effect recovery where the action rules permit it, and partial/missing/multiple/mismatched/ambiguous evidence producing the named typed blocker. Spawn absence without a conclusive idempotency query must block, not redispatch.
9. Add remaining concurrent-ledger and state-machine tests for hook failure, no-progress watchdog, and bounded wait timeout. Inject stale executable-looking summary text and assert that transitions depend only on typed durable inputs.
