...
The guard itself must preserve the dispatch protocol it protects. Because `await_completion` is written before spawn, “any unresolved obligation blocks dispatch” deadlocks the required first spawn. A guard that checks state and releases its lock before the effect also permits compaction or another ledger write to invalidate the decision. Finally, an unbounded “small read” is not an inactive-cost contract. The design therefore needs typed action authorization, a linearizable validation/effect boundary, and quantitative checkpoint limits.
...
The FO records `await_completion` before spawning or routing work, with an unbound worker, a new assignment epoch, the report baseline, and typed next action `spawn_initial`. The reconciler may authorize exactly that initial unbound transition; after the spawn returns, the launcher binds the returned worker identity and changes the typed next action to `wait` in the same guarded action transaction. A crash before spawn leaves `spawn_initial`; a crash after an externally visible spawn but before bind records an indeterminate typed blocker and requires roster/mailbox reconciliation, never blind redispatch. A completion from epoch N cannot satisfy epoch N+1. Clearing requires the matching assignment epoch, a newer committed report, checklist verification, and a durably recorded successor state.
...
Permit issuance and execution are separate, but execution is linearizable. The action gateway acquires the checkpoint lock, rechecks permit nonce, action/target digest, generation, obligation revision, assignment epoch, unused status, and `needs_rehydrate`, then holds the lock through the side effect and its ledger transition. For spawn, the gateway invokes the proved client interceptor, receives the worker identity, binds it, advances `spawn_initial -> wait`, marks the permit consumed, and increments generation before releasing the lock. For filesystem/git actions it records the typed successor and consumption under the same lock. A side-effect failure consumes the permit and records a typed retry/blocker; retry requires reconciliation and a new permit. A concurrent compaction/ledger writer serializes before the recheck (making the permit stale) or after the committed side effect—never between validation and effect. Raw host calls that the gateway cannot own remain outside the enforcement claim.

The inactive gateway path uses only the 4 KiB guard header bound above. It acquires the same checkpoint lock before reading; if the header says zero obligations, zero permits, and `needs_rehydrate=false`, it holds that lock through the non-worker side effect as a non-persisted fast-path allowance, without decoding the body. Thus a context-loss writer linearizes before the header read and blocks the action, or after the completed effect; there is no inactive check-then-act window. An initial spawn must first create its unbound obligation and therefore always uses the permit path. Any active count/flag requires the bounded full decode and typed reconciliation. Oversized, corrupt, inconsistent, or generation-regressing input fails closed before permit issuance. The inactive path performs no roster/mailbox query, contract stream, or write.
...
**AC-10: Inactive cost.** With no observed compaction requiring reload, no unresolved obligation, and no outstanding permit, a guarded command performs one `stat` and one `pread` of at most 4 KiB, decodes zero body records, uses at most 16 KiB transient allocation, and performs no roster reconciliation, mailbox query, contract streaming, checkpoint write, or repeated suggestion work beyond the existing pressure-bucket check. Oversized, corrupt, or inconsistent headers fail closed. A trace/allocation fixture asserts these quantitative bounds.
...
5. Add permit/state-machine tests for the full pre-spawn path: persist unbound `await_completion` plus report baseline, assignment epoch, obligation revision, and `spawn_initial`; reconcile and issue the exact permit; execute spawn through the gateway; bind the returned worker; commit `wait`; consume the permit. Assert zero side effects for wrong action class, wrong target digest, stale generation, stale obligation revision, wrong assignment epoch, and duplicate use. Simulate a side-effect failure and require a consumed permit plus typed retry/blocker and a newly reconciled permit.
6. Add deterministic race tests with barriers. For the permit path, pause between issuance and gateway execution; a competing writer increments generation and/or sets `needs_rehydrate`, the under-lock recheck rejects the stale permit, and the fake side-effect counter remains zero. Reverse the order and prove the side effect plus successor transition commit before the writer acquires the lock. Repeat on the inactive fast path by pausing while the gateway holds the lock: the writer cannot set `needs_rehydrate` between header validation and effect and commits only afterward. Also cover two workflows, out-of-order completions, stale epochs, generation conflicts, and matching/malformed reports.
7. Add remaining concurrent-ledger and state-machine tests for hook failure, no-progress watchdog, and bounded wait timeout. Inject stale executable-looking summary text and assert that transitions depend only on typed durable inputs.
...
9. Extend integration tests for each Spacedock-owned guarded command to prove an observed flag blocks before side effects, a matching typed permit allows only its action, successful rehydrate unlocks permit issuance, and the zero-obligation/no-event path meets the quantitative inactive bounds. For raw collaboration and final-stop actions, assert only the action classes the live client spike proves; record uncovered classes rather than calling them eventless fallback. Assertions use tool/event traces and workflow state, not transcript phrasing.
10. Run AC-14 and AC-15 live and retain app-server JSONL, plugin hook log, `PLUGIN_DATA` audit, checkpoint/readiness/permit transitions, and the temp workflow git log as CI artifacts.
