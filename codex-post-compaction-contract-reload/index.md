...
Concretely, the guarded commands read `${SPACEDOCK_FO_CHECKPOINT}` and call one launcher-owned `guardCheckpoint(action_class)` before emitting a dispatch spec or performing any filesystem/git mutation. The initial action classes are `dispatch`, `state_mutation`, `archive`, and `merge`. If `needs_rehydrate` is true or any obligation is unresolved, the guard returns a stable non-zero result with the checkpoint generation and required `rehydrate` action; the command has made no side effect. Only a successful `spacedock dispatch rehydrate` transition can unlock it. The inactive path performs one bounded metadata read and predicate evaluation; it performs no roster/mailbox query, contract stream, or write. This guard does not cover raw collaboration tools, assistant prose, or final response unless the live client spike proves and installs an interceptor for that action class.
...
**AC-4: Detection and interceptor coverage.** Event-capable fixtures deduplicate `postCompact`, `contextCompaction`, and legacy `thread/compacted` into one epoch and set `needs_rehydrate` before returning control. Failed-hook fixtures with an observed event remain fenced. Eventless fixtures with an existing obligation are blocked on Spacedock-owned guarded commands by the durable ledger, without relying on summary prose. A zero-obligation eventless fixture proves the honest limit: no epoch/flag transition occurs, an uncovered raw host action is not claimed safe, and the result is reported as unsupported rather than passing a fallback assertion.
...
**AC-10: Inactive cost.** With no observed compaction requiring reload and no unresolved obligation, a guarded command performs at most one bounded checkpoint metadata read plus predicate evaluation, and performs no roster reconciliation, mailbox query, contract streaming, checkpoint write, or repeated suggestion work beyond the existing pressure-bucket check. A trace fixture asserts those operation counts.

**AC-11: Compaction readiness prevents unsafe voluntary compaction.** `compaction_ready` is false whenever any active obligation lacks durable persistence, bound dispatch identity/epoch, stored report baseline, or typed next action; launcher/client-initiated compaction is refused with the exact missing fields. Readiness becomes true only when all obligations satisfy that predicate, and compaction is allowed only when readiness is true and the current point is a declared logical boundary. Table tests toggle every term independently, and an observed automatic-compaction fixture proves that bypassing this prevention layer sets `needs_rehydrate`; an unobserved fixture is classified under AC-4's limit.
...
7. Extend integration tests for each Spacedock-owned guarded command to prove an observed flag or existing obligation blocks before side effects, successful rehydrate unlocks it, and the zero-obligation/no-event path incurs no roster, contract-stream, or checkpoint-write operations. For raw collaboration and final-stop actions, assert only the action classes the live client spike proves; record uncovered classes rather than calling them eventless fallback. Assertions use tool/event traces and workflow state, not transcript phrasing.
8. Run AC-13 and AC-14 live and retain app-server JSONL, plugin hook log, `PLUGIN_DATA` audit, checkpoint/readiness transitions, and the temp workflow git log as CI artifacts.
