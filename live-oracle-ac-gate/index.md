...
title: Encode the live-oracle requirement — gate runtime-observable ACs on a live-run artifact + promote 8y's shared scenarios to a first-class live-scenario primitive
status: backlog
source: captain (2026-06-03, session 11) — after yy's fix passed offline + 3 detached-audit cycles but FAILED its live AC-1, the captain noted the live-oracle rule is prose-only (yy slipped) AND that agents don't intuitively reach for "descriptive runbook → run a real agent → grade durable outcomes" live tests
score: "0.40"
worktree:
started:
completed:
...
The requirement that a **runtime-observable AC** (one whose truth can only be decided by running the real producer) be confirmed by a **live run** is encoded only as advisory prose — the README validation stage and the merge-on-proof bar in handoffs. There is no gate that refuses to mark such an AC PASSED until a live-run artifact is on the record. `yy` (sonnet-live-ci-flake, #282) was marked `by-construction-pending-live` and merged with AC-1 unconfirmed; the post-merge live retrigger then revealed the fix doesn't actually close the flake. This violates the workflow's own Working Principle ("prefer a code gate over a prose-only rule; a prose-only rule has a ceiling of wording-present, which is not behavior").

## Problem

Two coupled gaps:

1. **The live-oracle rule is prose-only and unenforced.** A runtime-observable AC can reach terminal/PASSED on offline proof alone (unit tests, a recorded-stream fixture, a prose oracle). But a recording proves the *consumer* (the streamwatcher localizes a hang in a frozen stream), never the *producer* (does the real model, reading the contract, actually exit?) — the fix produces a *different, future* stream you can only get live. yy passed every offline oracle + 3 adversarial audit cycles and was still wrong.
...
1. **AC classification + a real gate.** At ideation, classify each AC as *code-oracle* (offline test/fixture sufficient) or *runtime-observable* (only a live run decides). Extend the existing AC-evidence cross-check (`status --validate` / a `gate verify`) so the terminal gate **refuses** a runtime-observable AC marked PASSED unless it cites a live-run artifact (a CI run id or the archived session JSONL). This turns `by-construction-pending-live` from a hopeful label into an enforced state.
...
3. **Surface the pattern in the dev template.** Add to "choose proof at the claim's altitude": *if the claim is what an agent/model DOES at runtime, the proof is a scripted live scenario graded on durable state/output — a recording proves the watcher not the producer; a prose check proves the words not the behaviour.*

**Riskiest unknown — spike first:** the gate's classification + citation mechanism. How does the binary KNOW an AC is runtime-observable (an explicit tag? a `Verified by: live …` convention?) and how does it verify a live-run-artifact citation is real (a run id that exists / a present JSONL)? Pin this before designing the prose, since a sloppy classifier either over-gates (blocks code-oracle ACs) or under-gates (lets runtime ACs through — the status quo).
...
Verified by: a test driving the gate on a fixture entity carrying a runtime-observable AC — RED (refused) when no live-run artifact is cited, GREEN when one is; a code-oracle AC is unaffected.
...
Verified by: a presence oracle over the dev template confirming the proof-altitude guidance names the live-scenario pattern and the recording-proves-the-watcher / prose-proves-the-words distinction.

**AC-4 — The yy slip would now be gate-caught.**
Verified by: a fixture reproducing yy's shape (a runtime-observable AC-1 marked PASSED with only offline proof) is REFUSED by the gate from AC-1.

## Test plan

- Spike the AC-classification + citation-check mechanism FIRST (the riskiest unknown).
- Go tests for AC-1/AC-4 (gate behaviour over fixture entities) and AC-3 (template presence oracle).
...
- Motivated by yy's live AC-1 failure (session 11): offline + 3 audit cycles passed, live failed — the strongest possible case for the gate. `8y` built the substrate; `at` will be the first entity that *needs* this primitive (its AC-1 is a live scenario).
