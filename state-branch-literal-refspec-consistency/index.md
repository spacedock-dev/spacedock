...
- **Cycle 3 — validation → captain escalation (2026-07-15):** Exact-head Roborev job 1472 and independent command-level reproduction rejected commit `cda550e0fd054f8ea22d1bd9b302d749a4761d50`. Outcome/data-integrity defect serving AC-1: an allowed spelling such as `thing/index.md` can be a directory, and the explicit selector accepts it so path-scoped Git recursively commits `thing/index.md/unexpected.md`. Automatic feedback routing stops at this cycle. Proposed narrow correction: require the matched checkout-derived path to be a regular entity file before return and add flat/folder directory regressions proving nonzero exit with unchanged HEAD and index; do not expand into e6j or lifecycle machinery.
