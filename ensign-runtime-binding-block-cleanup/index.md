...
- DONE: Reproduce the AC-3 byte measurements on the worktree tip independently — codex ≤2390, pi ≤1768, claude + ensign-shared-core byte-unchanged vs v0.22.0; confirm the value gate holds
  Independently re-measured on the worktree tip AND on a fresh throwaway `git worktree --detach c79268f6`: codex 1522 (gate 2390), pi 1333 (gate 1768), claude 2556=2556, ensign-shared-core 8829=8829. The two byte-locked files show an EMPTY `git diff v0.22.0` (content-identical, not just same count). Value gate holds with margin.
- DONE: Confirm every removed section is genuinely owned by ensign-shared-core (no host-specific binding silently dropped); each host adapter still carries its host-specific concerns
  Each removed discipline grep-confirmed present in the shared core: worktree, split-root, frontmatter, path-scoped commit, Proving-your-work, Stage Report Protocol (incl. cycle-N redo), assignment reading, no-worktree case (shared core L36). Host-specific concerns preserved: codex keeps Clarification/Completion signal/Captain communication/Shutdown response; pi keeps Clarification/Completion signal (no captain comms or mailbox shutdown to bind). The codex "Feedback Interaction" section was not lost — its host-specific routing survives in the Completion-signal/Shutdown bullets via `«addressable-worker»`, and its generic redo behavior is shared-core Stage Report Protocol.
- DONE: Assess the implementer-flagged drift (shared-core Fetch-on-Demand → `## Completion Signal` H2 now a binding bullet on codex/pi) — does the cross-reference resolve, and should it block
  Real-but-cosmetic; does NOT block. The H2 still exists on the byte-locked claude adapter (resolves exactly); on codex/pi the `Completion signal ->` bullet carries the same channel under a near-identical label (resolves semantically). No code parses the adapter `## Completion Signal` heading — the `### Completion Signal` in `internal/dispatch/build.go` is the dispatch-file section, unrelated. The clean fix (soften the pointer to "completion-signal binding") lives in byte-locked `ensign-shared-core.md`; applying it now would breach the AC-3 net-zero guardrail, so deferring it is correct. Suggest folding the one-word softening into a future shared-core touch when the byte-lock is not in force.

### Summary

PASSED. All six ACs verified with independent evidence, not the implementer's numbers: AC-3 byte gates reproduced on a throwaway checkout (codex 1522≤2390, pi 1333≤1768, byte-locked pair unchanged via empty diffs); AC-1/AC-2 confirmed by diffing each removed section against `ensign-shared-core.md` and confirming the host-specific bindings survive; AC-4 `go test ./internal/contractlint/...` green and the re-targeted #417 pi guard proven non-tautological (it FAILS when the `Completion signal` binding bullet is removed); AC-5 `go test ./...` all green and `gofmt -l ./cmd ./internal` clean; AC-6 docs/runtime-support.md gains an ensign binding-block subsection matching the landed compact-per-host approach. The detached adversarial audit ran on a fresh `git worktree --detach c79268f6` (not this worktree) and reproduced identical bytes and green gates. The flagged Fetch-on-Demand cross-reference drift is cosmetic, resolves on all three hosts, and is correctly left unfixed because the fix would breach AC-3's byte-lock — recorded as a deferred shared-core cleanup, not a blocker. One note for the gate: the #417 contractlint guard proves the binding-block PROSE label is present (a genuine regression guard), not the runtime BEHAVIOR; the behavioral structural claims rest on the independent diff-vs-shared-core cross-check plus the green piruntime/livescenario suites.
