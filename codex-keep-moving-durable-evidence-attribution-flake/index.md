...
- Cycle 4: REJECTED — PR CI/Codex and Sonnet live; surface 10 files/+875/-1,449 vs approved head 10 files/+875/-1,449 (100%); AC unchanged. Codex's supported `approved-gate` journey atomically persisted `started`, the implementation report, and only its newly bound room subtree because inline `state commit` is a no-op. Sonnet durably started `ready-one` and `ready-two` with the questioned rework in one path-scoped batch, then separately started `approved-gate` before terminalization; the complete-set batch predicate rejected both ready tickets even though the overall assertion independently requires all three journeys. Route only these Material task-owned attribution false reds: accept the exact atomic-room episode and credit only valid expected tickets in a known-ticket dispatch batch while leaving any omitted ticket to fail its own journey; retain adjacent preexisting-room, modified-room, foreign-path, slug-prefix, and genuinely missing-dispatch negatives. Do not revisit declined archive, split-terminal, verdict, transcript, provider, parser, or observer work.
