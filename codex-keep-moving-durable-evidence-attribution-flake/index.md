...
- Cycle 4: REJECTED — PR CI/Codex live; surface 10 files/+875/-1,449 vs approved head 10 files/+875/-1,449 (100%); AC unchanged. The supported `approved-gate` journey prepared a new same-ticket gate room in an inline workflow whose `state commit` is a no-op, so the first worker commit atomically persisted `started`, the implementation report, and only that newly bound room subtree. Route only this Material task-owned attribution false red: accept that exact combined episode with adjacent preexisting-room, modified-room, foreign-path, and slug-prefix negatives; do not revisit declined archive, split-terminal, verdict, transcript, or provider work.
