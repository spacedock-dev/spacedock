...
1. **Detect the presenter before any side effect.** Resolve the TUI binary (`SUBSPACE_TUI_BIN` or `subspace-tui` on `PATH`) and version-gate it (its `--version` against the expected version, overridable by env as the interim ritual does) *before* creating the retention directory or any other side effect. Absence or a version mismatch exits non-zero with a one-line message that names **both** the install remedy and the fallback (present the gate in chat) — zero side effects, no retention directory created. See `## No-subspace fallback` below.
2. **Validate the explicit briefing package before launch.** Parse the briefing JSON, verify each Artifact and supporting Reference revision/digest, and derive the canonical pane title in the provider's title grammar (`<short-id> · <stage> · (approve|revise|hold)?`, ≤40 columns).
3. **Announce that workflow state will not advance**, then launch `subspace-tui` as **one blocking child process** through the caller's terminal transport, binding a caller-owned `--result` path.
...
5. **Completion is exit + validated result + retention, never pane creation or a wait timeout.** The command returns success only after the child exits, the returned result validates against the bound briefing, and retention completes. The gate-attempt ensign that invokes the command stays unresolved and addressable for the whole blocking call; the First Officer waits with `wait_agent({timeout_ms:300000})` and re-waits on timeout rather than treating a timeout as completion.
6. **Normalize the provider envelope briefing id to the attempt briefing id after digest validation**, as specified in `gate-resolution-frontmatter-contract.md` — see below.
7. **Never mutate entity frontmatter.** Recording the resolution into `gates:` is the recorder's job (3k); computing blockers, holds, eligibility, and applying the action is h1's. This command hands the First Officer a validated, retained result and stops there.

## No-subspace fallback

The captain's attempt-1 revise asked what happens when the user has no Subspace installed. The practice already exists — it is named here, not invented.

- **Detection.** The command resolves the TUI binary (`SUBSPACE_TUI_BIN` or `subspace-tui` on `PATH`) and version-gates it before any side effect. An absent binary or a version mismatch exits non-zero with a single line naming both the install remedy (`brew install spacedock-dev/tap/subspace-beta` and the plugin install, per the contract) and the fallback (present this gate in chat). Nothing is launched, no retention directory is created, and the entity is untouched.
- **Fallback: chat presentation.** The First Officer presents the gate in conversation per the present-gate template, and the captain's chat-rendered resolution is recorded through the recorder (3k) exactly as every 0260 gate was recorded. The record model is presentation-agnostic by construction: a resolution's provenance names its channel, and the recording-identity ruling (`docs/roadmap/durable-decisions/index.md` Constraints, 2026-07-21) applies unchanged — the captain's identity attaches because the captain rendered the decision over content the captain saw (presented content approved in chat). No Subspace-specific field is required to record a chat resolution.
- **Shape.** A missing presenter is an ordinary condition selecting the chat path — not a mode, not a degraded state, and it never blocks the gate itself. This mirrors the teams-unavailable-selects-bare-dispatch ruling shape (`dispatch-failure-retry-rung.md`: an unavailable capability "is not a mode at all — it is the ordinary condition selecting [the alternate path], evaluated where dispatch happens"). The gate proceeds; only the channel changes.

## What ships spacedock-side now vs. the subspace-tui dependency

**Ships spacedock-side now (this task, fully testable with a fake subspace-tui child):** presenter detection + version-gate with the remedy-and-fallback message, briefing-package validation, canonical title derivation, retention-directory allocation, the blocking child launch, stderr→diagnostics capture, launch-metadata recording, post-exit result validation, provider id normalization, and atomic retention on every exit path. None of this needs the real TUI to be exercised — the launch is a subprocess with a `--result` path and an exit code, so the whole lifecycle is driven in fixtures against a stub child (proven by the spike below).
...
**AC-5 — The briefing package is validated and the title derived before launch; an absent or version-mismatched presenter fails cleanly to the named chat fallback with zero side effects.** A valid multi-artifact briefing yields a canonical pane title in the provider's grammar and confirms each Artifact and supporting Reference revision is present. An absent `subspace-tui`/`SUBSPACE_TUI_BIN` or a version mismatch exits non-zero with a one-line message naming **both** the install remedy **and** the chat fallback, launches nothing, and creates **no retention directory** — the entity and working tree are byte-unchanged. *Test:* a valid-briefing fixture (assert title derivation + Reference presence); a missing-binary fixture and a version-mismatch fixture (assert non-zero exit, a message containing both the install remedy and the named fallback, zero launch, and that no retention directory was created).
...
4. **Pre-launch validation + no-subspace fallback (AC-5).** Valid multi-artifact briefing → canonical title + Reference presence; missing binary and version-mismatch → non-zero exit, one-line message naming both the install remedy and the chat fallback, zero launch, and no retention directory created (entity byte-unchanged).
...
+
+If Subspace is not installed (no `subspace-tui`/`SUBSPACE_TUI_BIN`, or a version mismatch), the
+command exits without side effects and names the fallback: the First Officer presents the gate
+in chat and records the captain's decision through the recorder the same way. A missing presenter
+changes only the channel; it never blocks the gate.
```
