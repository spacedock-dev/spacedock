...
## Stage Report: validation (guard-fix cycle)

- DONE: Confirm the audit's empty cases (newline-only body, subject-only annotated tag, lightweight tag) now make the guard FIRE where the old `[ ! -s ]` passed; a real body still passes.
  Reproduced INDEPENDENTLY in my own scratch repo (`/tmp/sdr-guard`) against BOTH literal guard strings (not via the test). `git tag -l --format='%(contents:body)'` yields a 1-byte `\n` (verified via `od -c`) for all three empties — never zero-byte, which is exactly why the OLD `[ ! -s ]` (zero-byte test) PASSED them (the dead guard). The NEW `[ -z "$(tr -d '[:space:]' < release-notes.txt)" ]` strips whitespace → empty → FIRES (reject) for `light`/`subjonly`/`emptybody` and PASSES the real 29-byte body. Before/after table: light 1B old=pass new=REJECT; subjonly 1B old=pass new=REJECT; emptybody 1B old=pass new=REJECT; realbody 29B old=pass new=pass.
- DONE: A real tool-cut tag body still PASSES the guard and reaches goreleaser `--release-notes`.
  Live: built the branch binary, ran real `notes 2.0.0` (confirm=y, no claude) in a fresh repo — the production `cutAnnotatedTag`→`AnnotatedTagArgs` cut `v2.0.0`; `%(contents:body)` = `674bd81 feat: a genuine user-facing change` (non-empty, `dispatch:` noise filtered); the NEW guard PASSES → reaches goreleaser `--release-notes`.
- DONE: The guard-logic check exists and is real; full `go test ./...` green + gofmt/vet clean; release.yml otherwise unchanged (only guard + comment); the 2 release-notes commits intact.
  `TestReleaseYAMLGuardRejectsEmptyBody` (notes_extract_test.go:60-118) reads the REAL guard via regex from release.yml — I independently confirmed the greedy regex extracts the FULL condition `[ -z "$(tr -d '[:space:]' < release-notes.txt)" ]` (the inner `'[:space:]'` brackets do not truncate it) — and runs that exact string in `sh -c` against the four tag shapes. Mutation proof: reverting release.yml to `[ ! -s ]` makes the test FAIL on all three empties (`guard fired=false, want reject=true (body="\n")`); restoring passes — so it executes the guard, not a substring check. `go build ./...` OK; full `go test ./...` = 718 passed in 12 packages (the +1 over cycle-3's 717 IS the new guard test), no failures (codex test passes this env); gofmt clean; vet clean. Guard-fix commit `6a88e770` touches release.yml in EXACTLY the one `if` line + 3 comment lines (`git show` confirmed); trigger `v*`, Run goreleaser `--release-notes`, and Stamp plugin manifests steps unchanged. Prior commits `fb70a090` + `94cf3bcd` intact (unchanged SHAs); branch now `3 ahead`, diff = the same 5 release-notes files.

### Per-AC evidence table (guard-fix cycle)

| AC | Verdict | Evidence |
|----|---------|----------|
| AC-1 notes clean (filter + prompt + no-claude fallback) | PASS | release unit tests green; live no-claude `notes 2.0.0` emitted filtered log, `dispatch:` noise dropped, `feat:` kept |
| AC-2 captain review before tag, no live CI prompt | PASS | confirm/decline injected-IO tests + live confirm cut the tag; CI reads tag only |
| AC-3 notes land on Release; build still on tag push | **PASS** | live tool-cut `%(contents:body)` non-empty passes the new guard → goreleaser `--release-notes`; trigger/builder/stamp intact |
| Empty-body guard fires on empty bodies (audit fix) | **PASS** | independent scratch-repo before/after: new guard REJECTS the 3 newline-only shapes the old `[ ! -s ]` passed; `TestReleaseYAMLGuardRejectsEmptyBody` runs the real guard string and a `[ ! -s ]` revert fails it |

### Summary

PASSED. The dead-guard hole the audit found is genuinely closed, verified by reproduction — not on faith. I confirmed in my own scratch repo that `%(contents:body)` returns a 1-byte `\n` for a lightweight tag, a subject-only annotated tag, and an empty second `-m`, so the old `[ ! -s ]` (zero-byte) guard passed all three (publishing a blank Release), while the new `[ -z "$(tr -d '[:space:]' …)" ]` guard FIRES on all three and still passes a real body. A live tool-cut tag (real `notes 2.0.0` → `cutAnnotatedTag`/`AnnotatedTagArgs`) produces a non-empty body that passes the new guard and reaches goreleaser `--release-notes`. The implementer's `TestReleaseYAMLGuardRejectsEmptyBody` is a genuine guard-logic check: it reads the actual guard expression out of release.yml (I confirmed the regex extracts the full condition) and executes it, so a revert to the dead `[ ! -s ]` form fails the test on the three empties — proving it catches the regression rather than spell-checking. Full suite green (718 passed, the +1 being the new test), gofmt + vet clean; the guard-fix commit `6a88e770` changes release.yml in exactly the one `if` line plus its comment, the prior 2 release-notes commits are intact, and the safe tool-cut path is untouched. Release still deferred (no merge/tag/push).
