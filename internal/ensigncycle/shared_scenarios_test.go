...
func sharedRuntimeScenarios() []sharedRuntimeScenario {
	return []sharedRuntimeScenario{
		...
		},
		{
			...
			intent:        "FO performs three authority mutations—bind a retained Briefing, record delegated approval, and consume exactly once—commits each durable barrier, treats validation as an optional diagnostic, and only then dispatches the successor.",
		},
		{
			name:          "rejection-flow",
