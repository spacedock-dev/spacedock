...
func haikuLoopPrompt(fx haikuLoopFixture) string {
	return strings.Join([]string{
		...
		"",
		"«dispatch»(slug, stage) -> (1) build the worker dispatch artifact with `spacedock dispatch build --workflow-dir " + fx.definitionDir + " --entity-path <entity_dir>/<slug>.md --stage <stage> --checklist-file - --bare-mode`. Feed literal checklist lines using a quoted heredoc: TODO: append the exact note line " + haikuBuiltMarker + " to the entity body; TODO: append a ## Stage Report: <stage> section (one item per line). (2) Spawn the worker with a bare Agent(...) call whose prompt is EXACTLY the `prompt` field from the dispatch build output — do NOT hand-assemble your own worker prompt. (3) Wait for the Agent to return; its return IS the worker's completion — do NOT re-dispatch the same stage.",
		"",
