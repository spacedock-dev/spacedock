...
// TestDrainToExitReturnsFullTranscriptOnExit: drainToExit (the predicate-free
// sibling expect for the shared-scenario runners) accumulates EVERY drained line
// and returns the full transcript once the proc exits — the whole stream the
// shared assertions consume, not just a tail.
func TestDrainToExitReturnsFullTranscriptOnExit(t *testing.T) {
	src := &fakeLineSource{}
	proc := &fakeProc{}
	w := newTestWatcher(src, proc)

	src.push(toolUseLine("Bash", `"command":"one"`))
	src.push(toolUseLine("Bash", `"command":"two"`))
	go func() {
		time.Sleep(2 * w.pollInterval)
		src.push(toolUseLine("Bash", `"command":"three"`))
		proc.setExited(0)
	}()

	out, err := w.drainToExit(w.quietBudget, "shared scenario x")
	if err != nil {
		t.Fatalf("drainToExit on a clean exit returned error: %v", err)
	}
...
// TestDrainToExitResetsDeadlineOnActivity: a stream that keeps emitting lines past
// the quiet budget does NOT trip — the deadline resets per line — and drainToExit
// returns cleanly once the proc exits, despite total elapsed exceeding the budget.
func TestDrainToExitResetsDeadlineOnActivity(t *testing.T) {
	src := &fakeLineSource{}
	proc := &fakeProc{}
	w := newTestWatcher(src, proc)
	...
	go func() {
		_, err := w.drainToExit(w.quietBudget, "shared scenario noisy")
		done <- err
	}()
	...
	case err := <-done:
		if err != nil {
			t.Fatalf("a progressing stream must not trip the quiet budget; got %v", err)
		}
	case <-time.After(2 * w.quietBudget):
		t.Fatal("drainToExit did not return after the proc exited")
	}
	if proc.wasKilled() {
		t.Error("drainToExit must NOT kill a progressing-then-cleanly-exiting proc")
	}
}

// TestDrainToExitKillsStalledStream: a stream that goes silent past the quiet
// budget before the proc exits trips stepTimeout AND kills the proc (so a hung
// host does not outlive the test) — the per-stage hang guard for the shared
// runners, the SAME no-progress discipline expect uses.
func TestDrainToExitKillsStalledStream(t *testing.T) {
	src := &fakeLineSource{}
	proc := &fakeProc{} // never exits — models a hung host
	w := newTestWatcher(src, proc)

	src.push(toolUseLine("Bash", `"command":"first"`))

	start := time.Now()
	out, err := w.drainToExit(w.quietBudget, "shared scenario stalled")
	elapsed := time.Since(start)

	var st *stepTimeout
	if !errors.As(err, &st) {
		t.Fatalf("a stalled stream must trip *stepTimeout, got %T: %v", err, err)
	}
	if st.label != "shared scenario stalled" {
		t.Errorf("stepTimeout.label = %q, want the scenario label", st.label)
	}
	if !proc.wasKilled() {
		t.Error("drainToExit must kill a hung host on the quiet-budget trip")
	}
	if !strings.Contains(out, "first") {
		t.Error("drainToExit must return the partial transcript captured before the stall")
	}
	if elapsed > 3*w.quietBudget {
