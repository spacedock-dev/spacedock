...
func TestGateValidateReportsApplicationExtensionWarnings(t *testing.T) {
	...
	if code != 0 {
		t.Fatalf("gate validate exit=%d stdout=%q stderr=%q", code, out.String(), errOut.String())
	}
	if !strings.Contains(out.String(), "gate=gate:task:ideation") {
		t.Fatalf("gate validate lost summary: %q", out.String())
	}
	for _, field := range []string{"action", "blockers", "feedback"} {
		if !strings.Contains(errOut.String(), "Warning: unknown gate application field '"+field+"'") || !strings.Contains(errOut.String(), filepath.Join(root, "task.md")) {
			t.Fatalf("gate validate missing warning/path for %s: %q", field, errOut.String())
		}
	}
}

func TestGateValidateRejectsBadRetainedRequestBinding(t *testing.T) {
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "README.md"), []byte("---\nid-style: slug\nstages:\n  states:\n    - name: ideation\n      initial: true\n      gate: true\n    - name: implementation\n---\n# Workflow\n"), 0o644); err != nil {
		t.Fatal(err)
	}
