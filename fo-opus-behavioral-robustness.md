...
- DONE: Every `**AC-N**` verified by REPRODUCING its cited evidence, not trusting the report: AC-3's guard table re-run with Spike C's sequence red-first and every sanctioned path green (merge-guard atomic finalize, reject-then-archive, verdict-less non-terminal archive, bare dispatch-into-terminal, --force); AC-2's falsifiability re-demonstrated (fixture passes current assertion, fails pre-#433 logic); AC-1 per the body's own ordering — offline suites first, the single sanctioned-path live opus spot-check (~$2) before committing to the 5-rep measurement (≈$15, artifacts attached to the task); any AC whose evidence cannot be reproduced is flagged, not excused.
  AC-3 red-first reproduced on a throwaway clone of pre-gate main with only the guard test file applied: Spike C's `--set 002-vendor-script completed verdict=PASSED worktree=` exited 0 and mutated the entity, and the archive mirror passed a `verdict: PASSED` + `status: implementation` entity through; on implementation commit 66750ea9 all 8 guard-table cases pass and `go test ./internal/status ./internal/cli -count=1` is green uncached (merge.go:281-289 confirms merge-guard finalize routes status+verdict+completed through the gated runSet, then runArchive with force=false). AC-2 reproduced: checked-in fixture is sha256-identical (1ab6bae3b2014f…) to a fresh `gh api` re-download of run 27841539602 artifact 7756226361 (`spacedock/spacedock/live-artifacts/claude/claude-opus-4-8/claude-shared-scenarios/filing/claude-stream.jsonl`, 102,607 bytes); swapping in `git show 6ea56f95~1:…shared_filing_test.go` reds `TestAssertClaudeFilingViaNewAcceptsRealPR399Attempt1Stream` with the original false-red message, restored clean. Full `go test ./...` green in the worktree.
  AC-1 FLAGGED — blocked, not excused: the sanctioned-path spot-check (`SPACEDOCK_LIVE_MODEL=claude-opus-4-8 go test -tags live ./internal/ensigncycle -run 'TestLiveClaudeSharedScenarios/filing|TestLiveMergedTeamModeDispatch'`) failed with API 401 before any FO work ($0 spent, num_turns=1, harness itself labels it "credential/launch error, not a behavior failure"); a direct `claude -p` call injecting the same `~/.claude/benchmark-token` (mtime Jul 1) 401s identically, so the credential is invalid, not the harness. The 5-rep measurement was therefore never started (spot-check-first ordering honored); no live artifacts exist to attach. Escalated to the FO with remediation options (refresh benchmark-token, or CI-lane runs under environment approval).
- DONE: Detached adversarial audit on a THROWAWAY checkout, never the implementation worktree (status mutation/guard paths are a Proof-policy high-stakes surface): construct claim-breaking edits — e.g. weaken the runSet finalize-status condition, drop the runArchive mirror, loosen the fixture assertion — and confirm the deliverable's own tests catch EACH; record material findings as `### Feedback Cycles` or record "refuted nothing material".
  Fresh clone in the session scratchpad; 4 edits, each applied/reverted: (1) runSet gate condition weakened to fire only on empty status → caught by TestFinalizeStatusGateRefusesSpikeCSequence; (2) runArchive mirror block deleted → caught by TestFinalizeStatusGateArchiveRefusesNonTerminalVerdict; (3) capturedLauncherFilesViaNew var-capture branch disabled → caught by the PR399 real-stream fixture test; (4) capturedLauncherFilesViaNew made to accept unconditionally → caught by the `$EDITOR new` negative case. Refuted nothing material.
- DONE: A PASSED/REJECTED recommendation with honest accounting: reject any self-referential evidence, any test that proves obsolete or over-specified behavior, and any prose-grep offered as behavioral proof.
  Recommendation: REJECTED, solely because AC-1's live evidence does not exist yet (credential-blocked); AC-2/AC-3 are fully verified with no self-referential evidence — the guard tests run the binary and observe exit codes and on-disk frontmatter, and the fixture test runs the assertion over independently-provenanced captured data that demonstrably diverges under the pre-#433 logic. No prose-grep was offered or accepted as behavioral proof. Once credentials are restored, only the AC-1 spot-check + 5-rep measurement remains; no implementation rework is indicated by any evidence gathered here.
