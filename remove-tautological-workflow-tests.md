...
Re-review of the correction round (commit 356c4ccac) against the two material findings from cycle 1. Findings re-proven on a throwaway `git archive HEAD` checkout, never the implementation worktree; candidate bytes and HEAD unchanged by this stage.

- DONE: Re-verify finding 1 yourself on the real file: inject each banned surface into the on-disk workflow one at a time and confirm RED, then revert and confirm GREEN. The implementer reports 3-for-3; prove it rather than reading it.
  Proven, not read. Injected each surface into the real on-disk `.github/workflows/runtime-live-e2e.yml` one at a time: `CODEX_HOME: ${{ runner.temp }}/codex-home` → RED "Codex HOME must be initialized in a step, not with the unavailable job-level runner.temp context"; `PI_OPENAI_CODEX_AUTH_JSON` → RED "Pi lane must not use the dedicated PI_OPENAI_CODEX_AUTH_JSON secret"; `SPACEDOCK_PI_LIVE_CHILD_MODEL` → RED "Pi lane must derive its provider model from auth mode". Each failure is `TestRuntimeLiveWorkflowSecretBansHoldOnRealFile` itself at runtime_live_evidence_workflow_test.go:79, not a bystander test. Reverted after each; GREEN each time; workflow `cmp`-identical to pristine at the end.
- DONE: Check the new real-file test is not merely a duplicate of the adversarial path — it must fail when the real workflow is dirty and pass when it is clean, independently of the discriminator at line 108.
  Three separate proofs. (1) Independence: deleted `TestRuntimeLiveWorkflowNamedEvidenceMutationControls` outright — the new test still passes clean and fails dirty with the discriminator gone entirely. (2) Non-duplication, the converse: deleted only `TestRuntimeLiveWorkflowSecretBansHoldOnRealFile`, kept the discriminator, injected `CODEX_HOME` into the real file — the whole `internal/release` package stays GREEN, so the new test is the sole carrier of real-file coverage and reproduces cycle 1's Finding 1 on this HEAD. (3) Fail-closed: renaming the `codex-live` job away REDs with "workflow lacks codex-live job" rather than silently passing on an absent job.
- DONE: Confirm the AC-3 ledger's four new rows describe real losses and that the rewritten row 4 no longer claims coverage that was absent, then re-issue a verdict on the whole task.
  Rows 5-8 re-proven on this HEAD by injection, all four still unguarded: `if: always()` on journey-delta-comment (row 5), the journey-delta CLI `run:` swapped for `echo` (row 6), the artifact upload path repointed (row 7), `--jsonfile` dropped from a live step (row 8) — each leaves `internal/release` + `internal/contractlint` green. Row 8's stated nuance also holds: abandoning gotestsum entirely still REDs via `TestRuntimeLiveCommonSuiteTimeouts`' workflow-vs-docs run-shape agreement. Row 4 no longer asserts uninterrupted coverage; it records the one-round disconnect, names the fix, and keeps cadence exclusivity flagged as still unenforced offline. Accurate as written.

### Verdict: PASSED

Both material findings are closed on evidence I generated rather than accepted. The full kept-ban injection matrix is now **11 of 11 RED** against the real files, up from 8 of 11 in cycle 1: goreleaser↛journey-ledger edge, `pty-team-mode`, `inputs.effort`, offline controls under `-tags live`, the `-v | tee` firehose, `gh run list --branch next`, the `SPACEDOCK_PINNED_CLAUDE_VERSION` re-pin, an unowned `-run` selector, and the three formerly-orphaned secret bans. Measured by process exit code with `-count=1`; grep-based pass detection over a multi-package `go test` is unreliable and produced one false all-green sweep I discarded after it contradicted a result already proven.

Hygiene re-verified on the corrected HEAD: `go test ./...` green, `go test ./... -race` exit 0, `gofmt -l ./cmd ./internal` empty, surface 60 insertions / 925 deletions across 9 files (net -865, inside the declared -890 ± 150 / 9 ± 2), all 19 deleted test functions still absent from `go test -list`, and `TestRuntimeLiveWorkflowSecretBansHoldOnRealFile` present. AC-1: the 16-file audited set is complete and the deletion judgment holds. AC-2: arm (a) reproduced independently via the node24 oracle bump (12 pin sites, 5 workflow files, `.github` md5-identical); arm (b) now satisfied by every kept ban, including the three that previously had a discriminator but no live application. AC-3: `go test ./...` green and the gap ledger now names all 8 losses.

**Deferred risks, for the captain's decision — the AC-3 ledger's 8 rows.** Each is a real wiring whose only offline check this task removes; none fails a value AC under normal use, because in every case the predicate behind the wiring stays unit-tested and the loss is the textual check that the workflow still invokes it. Promote-to-material condition, uniform across all 8: any one of them fires in a real run — a dropped `needs:` edge, an inert journey-delta step, a missing candidate binary in a run's artifacts, or an absent `-detail.jsonl` archive during an FO root-cause fetch. Rows 5-8 in particular were invisible until this validation injected them; they are recorded now, not fixed, per this task's "adding new tests is out of scope" boundary.

**Polish, not re-raised as a finding per FO routing.** AC-1's literal count is 2 rather than 0 (`journey_delta_workflow_test.go:32`, `live_registry_reconciliation_test.go:338`); 0 under the ideation-approved S1-S4 taxonomy. The captain's reading to make.

### Summary

The correction round did exactly what the findings asked and nothing more: one test function, +6 lines, plus ledger prose. I re-proved Finding 1's fix end-to-end on the real on-disk workflow (3-for-3 RED with distinct ban errors, revert to GREEN), and established the new test is neither a duplicate of nor dependent on the adversarial discriminator — with the discriminator deleted it still carries the load, and with the new test deleted a dirty real file goes unnoticed. Finding 2's four new ledger rows are accurate on this HEAD by injection, including row 8's gotestsum nuance, and row 4 now states the true post-fix coverage. Recommending PASSED with the ledger's 8 rows carried forward as recorded deferred risks.
