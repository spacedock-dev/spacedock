...
- DONE: (a) Account for the spacedock-injected `--plugin-dir <dir>` prefix so it does not shadow the real prompt.
  Added `--plugin-dir` to both hosts' `valueTakingHostFlags`; parseFrontDoorArgs re-injects only `--plugin-dir` into fd.passthrough (safehouse knobs live in the separate fd.safehouseFlags field, booleans are consumed) so this is the complete injected-prefix set. Session-12 case now names `@/tmp/handoff.md`, never `/co`. Commit 571493c4.
