...
Survey follow-ups deferred from vh (0.19.8) to 0.19.9, on the agentsview Codex theme. **D2 (the silent-0 honesty caveat) was DROPPED from scope by the captain (2026-06-08)** — 47rx is now **D1** (Codex body-surfacing via the `workdir` heuristic) **+ D3** (source-health detector, still deferred/upstream-gated). **D1's scope (count-only vs full content-surfacing) is PENDING captain review** after the rendered content views below. **0.19.9 candidate per captain; roadmap/sprint assignment is the captain's.**
...
## Scope decision: D1 attribution is PROVEN; D1's content depth is PENDING captain review; D3 is upstream-gated

**D2 is dropped (captain, 2026-06-08).** 47rx is D1 + D3.

**D1 — Codex body-surfacing: attribution is PROVEN buildable; the CONTENT-DEPTH call is PENDING the captain's review of the rendered views below.** The `$.workdir` discovery (Spike Part 1) cleanly scopes Codex to this repo, sibling-free — that blocker is gone. What remains is a scope choice the captain reserved: how much Codex CONTENT the body should surface. The rendered views (Spike Part 2, below) show three candidate depths:
  - **(i) COUNT only** — a workdir-attributed Codex session count line. Trivially buildable; DB-derivable.
  - **(ii) WORKSTREAMS + tool-call mapping** — Codex's workstream clusters (from `first_message`) and its tool-call activity (`exec_command`/`update_plan`/`spawn_agent`, which map coherently). DB-derivable; the rendered codex-only view (below) shows this is coherent, NOT garbage.
  - **(iii) FULL WORK-BY-AREA / file edits** — which files Codex changed (`internal`/`skills`/`docs`). Recoverable ONLY by parsing `apply_patch` markers in the RAW ROLLOUTS — agentsview does not ingest structured Codex file edits (it stores them as empty `write_stdin` rows). So depth (iii) needs out-of-band rollout parsing in the survey skill OR an upstream `kenn-io/agentsview` ingestion change.

  **I am NOT locking D1's depth.** The rendered views are below for the captain to choose (i), (ii), or (iii). My read, offered not decided: depth (ii) is the sweet spot — it surfaces real, coherent Codex content (workstreams + activity) entirely from the agentsview DB, with no brittle rollout parsing; depth (iii) is where the upstream gate bites. But this is the captain's call after seeing the views.

- **D3 — source-health detector: UPSTREAM-GATED, deferred.** Distinguishing a denied/disabled Codex source from a genuinely-empty one is not cheaply achievable today. Confirmed against agentsview v0.32.1: neither `agentsview sync`, `agentsview projects` (lists project→count but not agent/source), nor any other subcommand exposes a per-source count or source-health signal; an agent-side `[ -r ~/.codex ]` probe faces the same Seatbelt FS-access denial that forced vh's AC-5 `command -v`→`agentsview --version` swap. D3 needs an upstream source-health signal (a `kenn-io/agentsview` change). Deferred.

**Ship shape.** D1 ships as one task (this entity) at whatever depth the captain selects; D3 defers as upstream-gated. The AC + test plan below are written for the DB-derivable depths (i)/(ii) and flag where depth (iii) would change them — they will be refined in the FO's post-views feedback round once the captain picks a depth.
...
**Combined verdict: coherent and additive at every depth — the question is purely how deep to go.** The captain's call: (i) count, (ii) count + workstreams + activity (DB-only, my suggested sweet spot), or (iii) + file-level work-by-area (adds rollout parsing). All three are honest and useful; depth iii is the only one that touches the upstream/rollout gate.
...
## Proposed approach (firmed where settled; D1 DEPTH pending captain)

The shared foundation, settled at all depths: a labeled `-- name: codex-scoped` query in `queries.sql` that scopes `agent='codex'` sessions by `exec_command.$.workdir` under the repo-root prefix (the workdir analogue of the Claude cwd-prefix scope — counts this repo's Codex, excludes a same-basename sibling). This is the de-risked Part-1 mechanism every depth builds on; it STAYS alongside the existing project-NAME-only `codex-presence` flag (the superset that may include siblings).

On top of that foundation, the captain picks the body depth (rendered views above):
- **Depth (i) — Codex COUNT line.** Render the workdir-attributed count as a body line, distinct from the name-only `codex-presence` hint. Pure-DB, smallest.
- **Depth (ii) — + WORKSTREAMS & activity (my suggested sweet spot).** Add a Codex workstreams cluster (from `first_message`) and an activity summary (`update_plan`/`exec_command` tallies). Pure-DB, coherent (B-1/B-2), no rollout parsing.
- **Depth (iii) — + file-level WORK-BY-AREA.** Add Codex's `internal`/`skills`/`docs` edit buckets (B-3). Requires parsing `apply_patch` markers from raw `~/.codex/sessions/*.jsonl` (NOT the agentsview DB) — a new rollout-read the survey doesn't do today, or an upstream agentsview ingestion change. This is the only depth that touches a gate.

The AC + test plan below cover the settled foundation + depth (i)/(ii); the FO's post-views feedback round firms the chosen depth (and adds the rollout-parse AC if the captain picks iii).
...
- **AC-3 — the live survey report renders the workdir-attributed Codex body content at the chosen depth.** Driving `/spacedock:survey` end-to-end on a repo whose synced DB carries `$.workdir`-attributed Codex sessions, the body shows the Codex content for the captain-selected depth: depth (i) a workdir-attributed COUNT line (distinct from the name-only `codex-presence` flag); depth (ii) additionally a Codex workstreams cluster + activity summary derived from `first_message`/`update_plan`/`exec_command`; depth (iii) additionally a Codex work-by-area. Verified by: the live-drive rendered report (the survey's proof bar) — a grep over SKILL.md does NOT satisfy this; expected values come from the real survey output. (Depth pending captain; this AC is finalized in the post-views feedback round once depth is chosen.)
...
- **Live drive (AC-3) — minutes; the ONLY proof for the rendered Codex body content.** ONE end-to-end pass: seed a survey DB via the SKILL.md §1 recipe (this repo's real Codex history lands `$.workdir`-attributed — 57 sessions observed in the spike), drive `/spacedock:survey`, observe the body's Codex content at the chosen depth. Reuses the spike's sync recipe; agentsview v0.32.1 present. Per ideation, the live drive may escalate to a captain-run drive (as vh's AC-4/AC-5 did). Cost: LOW–MEDIUM. (Depth-(ii) adds workstream/activity-cluster assertions; depth-(iii) adds a rollout-parse query-smoke — both firmed post-views.)
...
- **Estimated total cost/complexity: LOW (depth i/ii) to MEDIUM (depth iii adds rollout parsing).** One new labeled `codex-scoped` query + fixture `exec_command` rows + test assertions + the step-4 fence change (the Codex body content at the chosen depth) + SKILL.md prose, plus ONE live drive. Depth (i)/(ii) need no agentsview/binary changes; depth (iii) adds a raw-rollout read.

## Notes

Provenance: vh feedback cycle 1 + the captain's 2026-06-08 live-drive findings, plus THIS ideation's D1 spike + rendered content views. D2 (silent-0 caveat) was DROPPED by the captain (2026-06-08); 47rx is D1 + D3. The D1 spike here OVERTURNS the task body's original premise that D1 was upstream-gated: the `exec_command.$.workdir` field makes the attribution heuristic buildable in 0.19.9 without the upstream Codex-cwd change. The rendered views (Spike Part 2) show Codex content is coherent at every depth; D1's depth (i/ii/iii) is the captain's pending call. Per the survey discipline, a grep over SKILL.md never satisfies a behavioral AC — D1's rendered Codex content bottoms out on a live drive; the workdir-attribution query bottoms out on the query-smoke against the extended fixture.
