...
- **One resolver (`sr`'s `resolveTrunk(workflowDir)`, unexported in `internal/dispatch`):** reads `status.ParseFrontmatter(README)["trunk"]`, returns `main` on empty/absent. Both callers live in `internal/dispatch` — `reconcile.go`'s `classD`/`classE` consume its return, and the `dispatch trunk` command (`dispatch.go`) prints its return — so one same-package symbol serves both, no export needed (confirmed with `sr`: `reconcile.go:296` already calls `ParseFrontmatter`, so no new import either). This task CONSUMES the same `resolveTrunk` — it does NOT define a parallel resolver. The single symbol IS the one-source proof: reconcile and the command cannot diverge.
...
**The sole load-bearing contract between the two halves: `dispatch trunk` stdout is a BARE branch name.** `sr` pins AC-5 to emit exactly the branch name with no trailing newline beyond the one `$( )` strips and no extra lines; this task's mod consumes it as `BASE=$(spacedock dispatch trunk --workflow-dir docs/dev)` and uses quoted `"$BASE"` at every site (`gh pr create --base "$BASE"`, the draft, the diff stat, the fallback merge target). This task's body assumes nothing else about the stdout shape — bare-name only. (Confirmed with team-lead at the staff review.)
...
**(1) Does `status.ParseFrontmatter` surface a top-level `trunk:` key, and does absence yield empty (so the resolver owns the `main` default)?** This is the parser `sr`'s `resolveTrunk` reads and that the `dispatch trunk` command and reconcile (`reconcile.go:296`) call.
