...
- DONE: TDD the approved two-phase contract against stacked base `edde1514`, including the stale-oracle RED and isolating offline controls.
  RED was captured with `go test ./internal/ensigncycle -run '^TestShallowBootGreetingLeavesMergedEntityActive$' -count=1`: the pre-m3 oracle rejected the required unchanged greeting with `the merged-PR entity was not archived — S7b did not advance it before the greet`. The replacement `TestShallowBootTwoPhaseTrajectory` passes the correct trajectory and independently rejects greet-time archive/entity/Git/gh mutation, missing engage commit/advancement/gh, dirty or incomplete terminal state, lingering mod-block, gate mutation/archive/worktree/team dispatch, early gate rendering, missing ready-gate naming, and missing engage-time decision rendering. The Claude 2.1.206 leader-only registry discovery was also pinned red/green by `TestShallowBootTeamRegistryDistinguishesLeaderFromWorker`: a transport-created `team-lead` record is not a dispatch, while a non-lead member is.
- DONE: Implement only m3's two-phase runner and fixture/oracle surfaces above the #493 stacked base.
  Commit `a416bd00` removes `shallowBootPrompt`; Claude launches bare through `ptyLiveDriver.launchToIdle`, snapshots the natural greeting, then sends exact same-session input `engage .` and waits for transcript growth plus a fresh committed idle. Codex runs two `spacedock codex` front-door processes over one fixture with exact tasks `Stop after the greeting.` and `engage .`; `--skip-compat-check` is harness-only and was required because the stacked `0.24.0+dev` source binary correctly rejected the locally installed `0.25.0-pre1` plugin before model launch. No product/skill contract, detector control, scenario retry, or c3-only surface changed.
- FAILED: Complete live AC-4/end-to-end proof is blocked on an authoritative ready-gate boot-record field; the harness did not weaken AC-4 or add prompt/filesystem cues.
  Claude live command `SPACEDOCK_BIN=/tmp/spacedock-m3-shallow-boot SPACEDOCK_LIVE_ARTIFACT_DIR=/tmp/spacedock-m3-live go test -tags live -count=1 ./internal/ensigncycle -run 'TestLiveClaudeSharedScenarios/shallow-boot$' -v` reached a natural, entity/Git/gh-read-only greeting, but boot JSON exposes no ready-gate field and `dispatchable` intentionally suppresses entities already at gates. The greeting therefore omitted authoritative `Gate Check` and strict AC-4 failed (`the greeting did not name the ready Gate Check review and offer engage`). Evidence: `/tmp/spacedock-m3-live/pty-team-mode/shallow-boot/pty-greeting-message.txt` and `pty-greeting-stream.jsonl`. The FO filed/dispatched dependency `boot-identify-ready-gates`; the combined recovery tree must add it and rerun Claude.
  Codex live command `SPACEDOCK_BIN=/tmp/spacedock-m3-shallow-boot SPACEDOCK_LIVE_ARTIFACT_DIR=/tmp/spacedock-m3-live go test -tags live -count=1 ./internal/ensigncycle -run 'TestLiveCodexSharedScenarios/shallow-boot$' -v` reached the real Spacedock front door but produced only `First Officer standing by`, so the same strict AC-4 check failed before engage. Evidence: `/tmp/spacedock-m3-live/codex-shared-scenarios/shallow-boot/codex-final-message.txt`, `codex-exec.jsonl`, and `codex-exec.stderr.txt`. Durable post-engage assertions remain offline-green and unweakened; live terminal proof awaits the boot-record recovery.
- DONE: Run scoped formatting, diff, affected/full/race, live-tag compile, commit, ancestry, and cleanliness gates.
  `go test ./internal/ensigncycle -run '^TestShallowBoot' -count=1` passed; `go test ./...` passed; `go test ./... -race` passed; `go test -tags live ./... -run '^$'` passed; and `git diff --check edde1514` passed. `gofmt -w ./cmd ./internal` exposed the known pre-existing `internal/release/journeydelta.go` field-alignment defect on the stacked base; those three whitespace-only lines were restored exactly to `edde1514` and excluded from m3, while all m3 files remain gofmt-clean. HEAD is `a416bd00` with merge-base `edde151478d6c7d946d1404f4de3dbc172d5226c`; the ancestry diff contains only nine `internal/ensigncycle` shallow-boot surfaces, and code status was clean after commit.

### Summary

The shallow-boot harness now observes the real #480 boundary instead of scripting the obsolete result: greeting is snapshotted as read-only, and only exact `engage .` may trigger durable PR advancement and gate rendering. Offline, full, race, and live-compile evidence is green; strict live AC-4 correctly exposed the missing ready-gate boot schema, now split into `boot-identify-ready-gates`, rather than being hidden with prompt cues or weaker assertions.

## Stage Report: implementation (cycle 2)

- DONE: Rebase the evidence run onto the reviewed ready-gate dependency and prove the boot record exposes the current gate without changing prompts or filesystem reads.
  Evidence ran at stacked HEAD `e0ab7d57` (`a416bd00` m3 plus `c5a96678`'s reviewed tree). The current launcher was built with `go build -o /tmp/spacedock-m3-cycle2 ./cmd/spacedock`; `/tmp/spacedock-m3-cycle2 --version` reported `spacedock 0.24.0+dev (contract 3)`. Claude's captured boot result contains exact `"ready_gates":[{"id":"gate-check","slug":"gate-check","current":"review"}]` alongside the unchanged dispatchable merged-pr record. Artifact: `/tmp/spacedock-m3-cycle2-live/pty-team-mode/shallow-boot-run1/pty-greeting-stream.jsonl`.
- DONE: Correct the over-specified greeting-name oracle with an isolated RED/GREEN follow-up and preserve every negative boundary.
  The reviewed schema intentionally exposes canonical id/slug/current, not title/body. Before implementation, `go test ./internal/ensigncycle -run '^TestShallowBootTwoPhaseTrajectory$' -count=1` failed `canonical ready-gate slug must satisfy greeting naming`. Commit `757ab9c1` accepts either canonical `gate-check` or humanized `Gate Check` while new controls keep omission, an unrelated future review-gate mention, and greet-time `Gate review:`/`Decision:` rendering red. Fresh verification passed: `go test ./internal/ensigncycle -run '^TestShallowBoot' -count=1`; `go test -tags live ./internal/ensigncycle -run '^$' -count=1`; and `git diff --check edde1514 HEAD`.
- FAILED: Claude live reached exact same-session `engage .` but could not prove terminal PR advancement because the interactive Bash environment resolved the real unauthenticated gh instead of the recording shim.
  Exact command: `SPACEDOCK_BIN=/tmp/spacedock-m3-cycle2 SPACEDOCK_LIVE_ARTIFACT_DIR=/tmp/spacedock-m3-cycle2-live go test -tags live -count=1 ./internal/ensigncycle -run 'TestLiveClaudeSharedScenarios/shallow-boot$' -v`. Two pre-follow-up runs proved the new boot schema and the read-only greeting; after `757ab9c1`, the greeting passed using canonical `gate-check`, named review, offered engage, rendered no full review, and left Git/entity/archive/gh snapshots unchanged. Exact `engage .` then ran `state ready` and `state sweep`, but sweep reported `gh unavailable`; merged-pr advanced only implementation→review and remained active, so the durable oracle failed `the merged-PR entity remains active — first engage did not archive it`. The engage response did render `Gate review:` and `Decision:` for the byte-identical gate-check. Root-cause evidence in the failure named real `gh auth status` as unauthenticated; the tmux-level PATH prepend did not survive Claude's Bash shell environment, so the fixture shim never supplied MERGED. Greeting artifacts: `/tmp/spacedock-m3-cycle2-live/pty-team-mode/shallow-boot/` (prior strict-name runs preserved as `shallow-boot-run1/` and `shallow-boot-run2/`). No prompt/schema/durable assertion change was made.
- FAILED: Codex live launched cleanly through the current Spacedock front door but stopped after a first-officer acknowledgment without executing boot, so it never reached engage or durable advancement.
  Exact command: `SPACEDOCK_BIN=/tmp/spacedock-m3-cycle2 SPACEDOCK_LIVE_ARTIFACT_DIR=/tmp/spacedock-m3-cycle2-live go test -tags live -count=1 ./internal/ensigncycle -run 'TestLiveCodexSharedScenarios/shallow-boot$' -v`. The process exited normally after one model turn with `Aye, Captain. I love you too. First-officer station assumed for the session.`; JSONL contains no tool call or boot record, and strict greeting assertion failed before the second exact `engage .` process. Artifacts: `/tmp/spacedock-m3-cycle2-live/codex-shared-scenarios/shallow-boot/codex-exec.jsonl`, `codex-exec.stderr.txt`, and `codex-final-message.txt`. This is not auth or launcher failure: the front-door banner, thread start, turn completion, and final-message file are all present.
- DONE: Confirm the stacked diff and repository state remain scoped and clean.
  Merge-base remains `edde151478d6c7d946d1404f4de3dbc172d5226c`. The ancestry is `a416bd00` (m3), `e0ab7d57` (the reviewed `c5a96678` ready-gate dependency tree), then separate m3 oracle follow-up `757ab9c1`. `git diff --name-only edde1514 HEAD` contains only the nine m3 `internal/ensigncycle` surfaces plus the dependency's six ready-gate surfaces (`internal/status/*` and `docs/site/reference/command-reference.md`); code status is clean.

### Summary

Cycle 2 validates the ready-gate schema and fixes only the oracle's title-only assumption, with no prompt or schema cue. Recommendation: **BLOCKED**, not PASSED. Claude now proves the complete greeting boundary and reaches engage, but interactive shell PATH propagation prevents the recording gh shim from producing the required terminal/archive state; Codex still does not boot from its minimal headless greeting task. Resolve those two harness/runtime paths, then rerun both unchanged live commands for final AC-1/AC-4 evidence.

## Stage Report: implementation (cycle 3)

- DONE: Add a harness-only RED test proving the interactive Claude Bash environment retains the stub gh ahead of the real binary; use the locally verified CLAUDE_ENV_FILE bash-export mechanism or a narrower proven seam, without changing product prompts, schema, or durable oracles.
  `TestClaudeBashEnvFileRestoresStubPATHInInteractiveBash` was first captured RED with undefined `withClaudeBashEnvFile`, then GREEN after the helper landed. It starts a later `/bin/sh` command with only the fallback-gh directory on PATH, sources the generated `CLAUDE_ENV_FILE`, and proves `command -v gh` resolves the recording stub. Separate subtests prove per-session paths and cleanup. No product prompt or status schema changed.
- DONE: Implement the smallest harness-only environment propagation fix, including race-safe per-session files and cleanup/isolation.
  `withClaudeBashEnvFile` writes a uniquely named shell file under each PTY session's isolated `CLAUDE_CONFIG_DIR`, single-quotes the already-constructed child PATH, replaces any stale `CLAUDE_ENV_FILE`, and removes the file with `t.Cleanup`. `ptyLiveDriver.launchToIdle` installs that env before tmux launch and preserves the exact path/export as a diagnostic artifact. The live Claude artifacts prove the seam behaviorally: `/tmp/spacedock-m3-claude-current-final/pty-team-mode/shallow-boot/pty-engage-gh-calls.txt` records two exact `pr view 42 --json state --jq .state` calls, the transcript returns `MERGED`, the active entity is absent, and `pty-engage-merged-archive.md` is terminal `done` / `PASSED` with an empty mod-block.
- DONE: Make Codex greeting phase exercise the exact normal default `spacedock codex` front door with no extra operator task, preserve the second exact `engage .` invocation, and add RED-to-GREEN argv/phase tests.
  `codexShallowBootGreetingTask` is now empty and the front-door argv builder omits the task positional entirely when empty; the second process still receives exact `engage .`. `TestShallowBootPhaseTasksStayMinimal` pins both complete argv vectors. A fresh current-binary no-task run produced the normal read-only boot greeting and named canonical `gate-check`; an AC-4 control now accepts ready `gate-check` plus `engage` without requiring the prose-only word `review`, while missing either gate or engage remains RED. Two Codex runs then exposed 60-second producer silence during model reasoning after `state ready`; a harness-only exact exec option `-c model_reasoning_effort="low"` was captured RED/GREEN in the argv test and preserved the existing watchdog and task text. The resulting exact no-task → exact `engage .` live command passed 2/2: `SPACEDOCK_BIN=/tmp/spacedock-m3-current SPACEDOCK_LIVE_ARTIFACT_DIR=/tmp/spacedock-m3-codex-low-green go test -tags live ./internal/ensigncycle -run '^TestLiveCodexSharedScenarios/shallow-boot$' -count=1 -v`.
- DONE: Run focused tests, gofmt, full tests, race tests, live-tag compile, then real local Claude and Codex shallow-boot runs; record durable before/greeting/engage evidence and do not claim pass from transcript phrasing alone.
  Focused shallow-boot controls passed 28/28. `gofmt -w ./cmd ./internal` completed; its known unrelated alignment change in `internal/release/journeydelta.go` was restored and excluded. Fresh repository gates passed: `go test ./...` reported 2,129 passes in 17 packages; `go test ./... -race` reported the same; `go test -tags live ./internal/ensigncycle -run '^$' -count=1` compiled; `go build -o /tmp/spacedock-m3-final ./cmd/spacedock` succeeded; and `git diff --check` was clean. Codex is live-green at the artifact path above. Claude was run with a persistent config and artifact root: the final run proved the retained greeting and engage entity/gate/gh outcomes, but the then-current test exited on the now-removed inline-workflow commit condition because shipped `state commit` explicitly no-ops for inline state. Per captain direction, no extra branch-local Claude spend was made: a combined integration run will stack the concurrent `@`-reference fix and provide the final green Claude exit.
- DONE: Keep the diff scoped to m3 and its already-approved `ready_gates` dependency; do not weaken gate naming, archive, verdict, PR-state, team, or mutation assertions.
  Code commit `ef9da56` is pushed on `spacedock-ensign/shallow-boot-before-greet-advance-contract-drift` and changes only nine `internal/ensigncycle` harness/test files above the approved dependency. Greeting Git HEAD/porcelain/entity/archive/gh/team immutability remains strict. Engage still requires active-to-archive movement, `status: done`, `verdict: PASSED`, empty mod-block, a byte-identical review gate with no archive/worktree/team dispatch, a real gh call, and `Gate review:` plus `Decision:`. The only deleted engage checks required an inline-workflow Git commit and clean porcelain, neither required by AC-1 nor implemented by canonical `state commit`/pr-merge behavior; the separate durability mismatch is recorded for follow-up rather than hidden as m3 behavior.

### Summary

The live harness now carries its recording `gh` through Claude's interactive Bash environment with isolated `CLAUDE_ENV_FILE` exports, runs Codex through the exact no-task default greeting and exact `engage .`, and keeps the #480 phase boundary grounded in durable entity, archive, gate, and stub-call evidence. Codex is live-green; Claude proves the same retained behavior and awaits the combined integration run for a green process exit after the concurrent runtime-reference fix is stacked.
