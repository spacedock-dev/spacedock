...
0.20.5 ships when, merged to `main`:
...
- `v0.20.5` cut after the pre-cut antipattern audit is clean.
...
- **Hand-off protocol shape.** Is the level-3 teammate a standing member (like `comm-officer`) spawned at first gate, or on-demand per judgment call? What exactly routes (gate adjudication, design, scope) versus stays mechanical? The boundary is the whole game and the spike informs it.
