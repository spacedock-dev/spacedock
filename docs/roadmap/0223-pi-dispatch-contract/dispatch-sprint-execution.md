...
## The four members (gate-approved: 1-3 captain-approved; 4 capstone captain-approved; the 4th member `qn` added 2026-06-19 after captain approved the safehouse-flag-parity scope — its ideation gate is pending, drive it only after the captain approves its gate)

| # | slug | id | deliverable |
|---|------|----|-------------|
| 1 | `pi-install-managed-skill-placement` | `eqrcrxcyye56nfwm997bj33d` | Ship Spacedock as a pi package; `spacedock install --host pi` actually installs; both parent + child discover skills (D1–D5) |
...
| 3 | `pi-safehouse-flag-parity` | `qn5sg36exf6apjjxymbfthgj` | Pi front-door `--safehouse-*` flags + safehouse wrapping (mirrors claude/codex `frontdoor.go`); unblocks member 1's install verification on sandboxed sessions |
| 4 | `pi-back-channel-dispatch` (capstone) | `b23y61pgk93ph44pz506m2wy` | Harden `fo-dispatch-core.md` to 7 runtime-neutral named capabilities; wire Pi adapter bindings for frictions 1–6; ensign talkback via `contact_supervisor` |

## Sequencing

- **Parallel start: members 1, 2, 3.** Independent code surfaces (1: `pi.go` `runInitWithPi`/`piRuntimeConfigFromEnv`/`checkPiRuntime` + `package.json` + `.pi/extensions/`; 2: `pi-first-officer-runtime.md` prose; 3: `pi.go` `parsePiFrontDoorArgs`/`runPi` launch wrapping + `help.go`). No inter-member dependency. **Note: members 1 and 3 both touch `internal/cli/pi.go` — coordinate so they don't collide on the same regions** (member 1 owns the install/resolution/check functions; member 3 owns the front-door parsing + launch wrapping + help).
- **Member 3 lands before or alongside member 1's AC-1 verification** — member 1's `pi install` writes to `~/.pi/agent` (outside the repo); on a sandboxed session the Commander needs member 3's `--safehouse-add-dirs ~/.pi/agent` (Q14). Break-glass until 3 lands: `safehouse --add-dirs ~/.pi/agent -- spacedock pi ...`.
- **Member 4 (capstone) Deliverable A (core rewrite) can start in parallel** with 1-3 — prose-structural reorganization of `fo-dispatch-core.md`; the `claude-live`/`codex-live` regression (AC-6) can run as soon as the core rewrite is draft.
- **Member 4's pi-live drive (AC-2/AC-3/AC-4/AC-5) requires members 1 + 2 landed.** The drive needs a dispatched ensign that loads the ensign contract (member 1's install-managed discovery) running on the parent's live model (member 2's stamping) from a non-repo cwd (exercising member 1's no-cwd-dependency). Do NOT attempt the capstone's pi-live drive until Q12's preflight confirms 1+2 landed.
- **Merge order:** 1, 2, 3, then 4. Each to `main` via PR-merge; state commits concurrency-safe (Q4, Q5).
