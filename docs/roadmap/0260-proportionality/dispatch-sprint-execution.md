...
4a. **If a Commander drive is already in progress** — read `docs/dev/.spacedock-state/_debriefs/2026-07-20-02-0260-commander.md` IN FULL before acting. REQUIRED, not optional, and this includes a post-compaction continuation of the same session: the compacted summary's claim of having read it is not the reading. It carries the captain directives issued in chat (the roborev-before-validation rule, the corrected grep ruling and why the FO's first formulation was too restrictive, each-member-pays-its-own-way, the bw/02av split), the exact remaining work for the unfinished members with paths and measured numbers, seven recorded conduct findings against the FO, the per-host remedy data from z7's lure matrix, and two systemic findings for the next sprint's shaping. Do not re-ask settled gates — they are in each entity's `gates:` frontmatter. Check whether Degraded Mode was tripped; if it was, it is irreversible for that session (sequential bare dispatch, no background workers, no `SendMessage` to any pre-trip name).
...
| 02av | ensign-finding-triage-disposition | — | LEFT THE SPRINT 2026-07-21 (captain decision): returned to ideation for the advisory-resolution reframe, next train. Land NOTHING of it — no standing block, no README bullet, no findings field |
...
3. **Wave 3 — bw lands ALONE (captain decision 2026-07-21; supersedes the composed landing).** bw ships its own surface only: the entry-format convention with surface/estimate/AC-drift fields, steps 2-3, the description line, the shared-core lines, the README ideation + in-stage bullets. **02av has LEFT THE SPRINT (captain decision 2026-07-21) — land nothing of it: no standing block, no README implementation bullet, no findings field.** The shaping FO already returned it to ideation and cleared its worktree pointer; it is outside the drivable query and needs no Commander boundary action. Its reframe (round dispositions, including the decline, as ADVISORY resolutions under the gate-recorder model) is next-train work; the reset and reframe brief are recorded in the entity. The index DoD line for the decline replay is marked moved accordingly.
