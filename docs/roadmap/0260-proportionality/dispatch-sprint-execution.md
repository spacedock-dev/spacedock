...
3. **Wave 3 — bw + 02av as one composed change with one shared implementation/validation gate.** They co-edit `feedback-rejection-flow/SKILL.md`, the implementation stage-def in `docs/dev/README.md`, and the same `### Feedback Cycles` entry shape. The composed entry MUST carry bw's surface/estimate/AC-drift fields AND 02av's findings disposition field, including the all-declines case. Record the shared gate as two per-entity attempts with the same briefing — the gates schema is per-entity; "shared gate" is one presentation, two records.
4. **Wave 4 — 2ae last.** Its "verbatim" sources are the LANDED post-wave-2/3 text of `docs/dev/README.md` and the contract files, not the ideation-time copies quoted in its body: re-anchor its Pieces 1-2 before/after diffs against the landed wording, then validate the assembled README before copying from it.
