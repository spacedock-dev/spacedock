...
**Second re-lock (2026-07-20, adopted from an independent codex-FO cross-review):** four merges (the falsifiable-test estate absorbs the output-grep shape with its gate-vs-no-gate contradiction to resolve once; the template member absorbs the proof-policy port; the runtime-semantics pair becomes one; the AC-2 fixture absorbs the re-anchor scenario); deferred members leave the sprint label entirely (no second roster, stamped or otherwise); the verification member defers its minting-elaboration block (compact captain rule stays boot-resident); the cycle-record member trims to a narration-first core (command + two skill-prose swaps; refusal machinery, gate rendering, template and docs lines deferred until live drives show narration ignored). The cross-review counts as the sprint-wide preflight's first pass — one pre-cut audit remains, no review ceremony added. New evidence recorded: three codex-FO corroborations and the 0.25.1 AC-narrowing incident (see the synthesis addendum) — an AC edit weakening the value claim after a rejection is a design-reset event, and proof lives at the seam where the failure lives.
**Re-lock (2026-07-20):** the captain applied the sprint's own thesis to the roster. Essence restated: (1) the feedback loop learns to reframe instead of repair; (2) evidence must be able to fail; (3) disciplines that already exist gain reach — into packets, reviewers, and the template. The stakes member reduces from a new ontology to a read-through of existing declared posture; the design-reset routing merged into the cycle-record member; the smallest-sufficient sharpening folded into the verification member; estate cleanups not killing an incident class carry `sprint-readiness: defer`. Drivable set: `spacedock status --workflow-dir docs/dev --where sprint=0260-proportionality --where 'sprint-readiness != defer'`.
...
- ~~An ensign triages review findings against stakes before fixing; a correct-but-disproportionate finding gets a recorded decline, not a dutiful fix~~ — **moved to the next train** (captain decision 2026-07-21): the triage member reframes its record mechanism onto the gate recorder; rule text and spike stand, banked in the entity with its reframe brief.
...
|---|---|
| Spacedock contract (skills + binary) | reframe routing + the narration-first cycle-record core; the cheapest-check-that-can-fail ordering + infra consent + fan-out surfacing + bare-ordinal rule; "5/5 passed is sufficient" removal. (The ensign finding-consumption rule + decline disposition moved to the next train with 02av, 2026-07-21) |
...
- MOVED to the next train (captain decision, 2026-07-21): the seeded correct-but-disproportionate-finding decline replay left this release with the finding-triage member (02av), whose decline record reframes onto the gate-recorder model before landing. 0.26.0 ships the feedback-cycle record convention and design-reset barrier without the ensign-side decline mechanism.
...
- [x] **⚠️ Preflight staff review (sprint-wide)** — independent reviewer, refute the sprint as a whole → `staff-review.md` — DONE 2026-07-20 as a two-seat panel: codex seat (`staff-review.md`, 4 material + 3 polish, verdict READY AFTER FOLDS) + fable seat (`staff-review-fable-delta.md`, 12 material additions + recorded declines); all material folds captain-approved and applied; independent closure pass verified CLOSED
