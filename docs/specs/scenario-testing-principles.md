...
- `merge-mod-block-recovery` — after a read-only boot, engage makes a resumed FO recognize an in-flight merge mod-block and reach the correct durable terminal/archive outcome without repeating already-recorded merge work.
- `filing` — the FO files a new seed entity via the atomic `spacedock new <slug>` path, not the drift-prone `--next-id` + hand-write pair.
- `shallow-boot` — a freshly-booted FO greets from local state, leaves live PR discovery and advancement to engage, creates no team, dispatches no worker, and stops for input without mutation.
