...
- `merge-mod-block-recovery`: performs read-only boot, then engages and resumes an already in-flight merge from its durable sentinel and mod-block; it asserts terminal status, cleared block, archive placement, and a clean workflow checkout without prescribing the recovery command.
