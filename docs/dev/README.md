...
- When an in-stage review round (a roborev re-panel, a detached-audit pass, or any similar in-stage reviewer loop) returns findings, append a conforming `### Feedback Cycles` entry — `- Cycle {N}: {verdict} — {reviewer}; surface {files}/{LOC} vs estimate {declared} ({P}%); findings {none | {F} fixed, {D} declined: <ref · class · why not material · promotes when>}; AC {unchanged | narrowed: <note>}` — with the actuals from `git diff --numstat "$(git merge-base main HEAD)"..HEAD`. Read the deviation against the ideation estimate from the entry; past the declared tolerance, record a design-reset decision (see the bullet above) before another pass. In-stage and cross-stage rounds share one section and one convention, the one `feedback-rejection-flow` carries.
- When consuming an in-stage review round's findings, triage before fixing. The committed finding-triage taxonomy is the release-scope classification in the `validation` stage-def (material / deferred risk / polish / needs decision) plus `.roborev.toml`'s four-field evidence record: released user and workflow; observable harm; affected value AC or non-negotiable boundary; trigger evidence. Fix only material findings; record a decline for a correct-but-disproportionate one — its class, why it is not material, and what would promote it — in the `findings` field of the same entry that logs the round; escalate a needs-decision finding to the FO. Narrowing a value AC to make a finding pass is a design-reset event for the captain, not a fix (see the entry's AC-drift note).
