...
- **Outputs:**
  ...
  - A recommendation of “narrow fix” must label an **outcome defect** (the delivered behavior/state fails the value AC) or an **evidence defect** (the test, harness, or observation boundary cannot validly establish the AC), cite the affected AC, and name the exact failing boundary. If the correction spans both paths, or an evidence fix requires another controller or lifecycle layer, it is a design reset, not a narrow fix.
  - Before treating any finding as release work, run **release-scope triage**: name the released user and normal workflow, the observable harm, the value AC or non-negotiable safety, security, data-integrity, or compatibility boundary it violates, and evidence that the trigger is a common or explicitly promised workload. Choose the least invasive action that preserves the release outcome.
  - If no named AC or non-negotiable boundary fails within that evidence, label the finding a **deferred risk**. Record its concrete trigger and revisit condition; do not turn it into a narrow fix or design reset. An unconstrained input or theoretical worst case does not alone widen release scope. If the intended workload is unknown, measure or probe it before escalating.
