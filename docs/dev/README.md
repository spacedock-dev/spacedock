...
- Material means the finding cannot be silently ignored; it does not mean this task owns the fix. A Material finding the task did not cause and whose remedy lies outside its approved scope is Needs decision — preserve the evidence, name the blocked claim, and stop.
