...
- **Outputs:** A fleshed-out task body with problem statement, proposed approach, acceptance criteria, and a test plan
  ...
  - The acceptance-criteria section heading in the task body must be exactly `## Acceptance criteria` — scanner-exact, no qualifiers or suffix (the `--ac-scan` reader matches only that literal heading; decorated variants like "… (entity-level, with proof owner …)" fail the scan and force a withdraw-and-re-prepare cycle).
  - Never place test files under `.pi/extensions/` — pi loads that directory as project extensions on every session launched from the repo root; a test file there (e.g. importing `bun:test`) kills extension loading and with it every worker dispatch from the repo root. Put TS tests under `test/`.
