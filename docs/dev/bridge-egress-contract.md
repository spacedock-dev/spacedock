...
```
{"timestamp":"<rfc3339>","ts":"<rfc3339>","host":"<claude|codex|pi>","event":"<host lifecycle event>","session_id":"<«session-id»>","agent_id":"<id, ensign subagents only>","agent_type":"<e.g. spacedock:ensign, empty for the main FO>","actor_id":"<host-scoped actor id>","detail":{"tool":"<tool name, when available>","source":"<event source, when available>"}}
```
