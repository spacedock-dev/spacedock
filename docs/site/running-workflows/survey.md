...
The match is conservative: a fork is dropped only on a confident repo match, because a false "still open" is a cheap nudge while a false "shipped" silently hides real open work. When no repo signal is available — not a git repo, or the lookups fail — the frontier degrades to transcript-only and every open fork is flagged `unverified` rather than presented as authoritative.
