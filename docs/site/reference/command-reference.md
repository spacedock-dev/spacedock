...
For source-checkout or retained development launchers, version compatibility is not a command-capability check. Immediately before every gate lifecycle, freshly resolve the launcher and run `spacedock gate --help` exactly once; do not cache the result. It must list `record`, `validate`, `eligibility`, `consume`, and the semantic record flags. If it fails, refresh the installed launcher or build the current checkout and select that executable with `SPACEDOCK_BIN`; do not hand-edit gate frontmatter as a fallback. Relative Briefing, Result, and association inputs resolve from the launch working directory.
