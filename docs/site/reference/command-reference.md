...
| `spacedock gate prepare-review <entity> [--publish --question TEXT --artifact INPUT.md --summary TEXT --recommendation TEXT [--reference INPUT.md ...]] --json` | Inspect mode emits committed gate candidates, stage prose, launch cwd, and publish argv without mutation. Publish accepts only those candidate paths plus First Officer prose, commits the entity binding and two-file room atomically, synchronizes state, and emits checklist, acceptance, and presenter inputs. `pre-commit`, `sync-pending`, and `projection-pending` phases are exact-replay recovery points; replay never creates another attempt. |
