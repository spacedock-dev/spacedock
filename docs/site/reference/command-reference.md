...
| `spacedock gate record <entity> --decision approve\|revise\|hold --actor ID [--reason TEXT]` | Record a chat decision and its derived one-use application. Delegated First Officer decisions require an evidence reason; the recorder does not accept or authenticate Captain-message text. Recording never advances status or dispatches. |
