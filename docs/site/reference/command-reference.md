...
| `spacedock gate record <entity> --result FILE --association FILE --actor ID [--adoption-note TEXT]` | Verify and adopt an exact provider Result. The retained association must cover the Result bytes, provider/canonical Briefings, and complete presentation mapping. |
...
| `spacedock gate validate <entity>` | Validate gate pointers, attempt chains, briefing bindings, frozen closures, and resolution shape; report the current recorded decision. |
