...
For source-checkout or retained development launchers, version compatibility is not a command-capability check. Before gate work, one `spacedock gate --help` probe must list `record`, `validate`, `eligibility`, `consume`, and the semantic record flags. A successful result may be cached only for the canonical executable target plus its content digest; replacement, symlink/PATH retargeting, or an unknown command or flag requires a new probe. If it fails, refresh the installed launcher or build the current checkout and select that executable with `SPACEDOCK_BIN`; do not hand-edit gate frontmatter as a fallback. Relative Briefing, Result, and association inputs resolve from the launch working directory.
...
| `spacedock gate record <entity> --briefing PATH/briefing.json` | Bind a complete retained package manifest whose basename is exactly `briefing.json`. Other basenames fail before mutation. The recorder derives the current-stage logical gate and whether this opens, rebinds, or supersedes an attempt. |
| `spacedock gate record <entity> --result FILE --association FILE --actor ID [--adoption-note TEXT]` | Verify and adopt an exact provider Result. The recorder derives the complete artifact inventory from the frozen canonical Briefing bytes, then requires the retained association to cover the Result, identities, actor, and full presentation mapping. |
| `spacedock gate record <entity> --decision approve\|revise\|hold --actor ID [--reason TEXT] [--directive TEXT]` | Record a chat decision and its derived one-use application; delegated decisions require the quoted directive. Recording never advances status or dispatches. |
