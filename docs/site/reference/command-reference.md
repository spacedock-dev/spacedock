...
| `spacedock gate record <entity> --operation FILE [--briefing FILE]` | Apply a pointer-checked `open`, `rebind`, `close`, or `supersede` operation to the entity's durable gate-resolution records. Recording changes only `gates`; it does not advance status, create an application, or dispatch. |
