...
A gate decision is recorded in the entity's versioned `gates` collection before any stage transition or worker dispatch. The `spacedock gate record` verb is the sole writer for the gate/attempt/briefing/resolution portion of that collection; it preserves application-owned subtrees without interpreting them. Each attempt stores one briefing reference and digest: replaceable while open and frozen when closed. `spacedock status --fields gate-state,gate-decision,gate-resolution` surfaces the current recorded result without changing the default status table.
