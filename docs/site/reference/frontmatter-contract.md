...
The known `application` subtree is approval-only. Canonical writes contain exactly
`target-stage` and `state`; read-only compatibility may observe arbitrary unknown
application keys as warnings. Unknown application extensions are ignored and are never
authority. Revise and hold carry no application.

A recorded approval carries a one-use application. Status surfaces it as `approved-pending`,
`consumed`, or `superseded`. Hold and revise carry no application; their Resolution
remains the durable decision. The application is never consumed twice.
