...
There is one captain and one first officer per session. There are as many ensigns as there is dispatchable work — the first officer dispatches one per entity per stage.
...
**Shaping is defining what good looks like before the work runs.** You set the mission, the stages, and the bar each stage must clear — declared in the workflow README, not negotiated mid-task. You commission a workflow with [`/spacedock:commission`](../running-workflows/commission.md), and you make the calls at gates: approve, redo with feedback, or reject. Some gates you answer yourself; others resolve through a delegated agent review. That is the whole of the captain's standing job.
...
- [Workflows and entities](workflows-and-entities.md) — the directory and files the roles operate on.
- [Stage lifecycle](stage-lifecycle.md) — how an entity moves backlog → ideation → implementation → validation → done.
- [Gates and decisions](gates-and-decisions.md) — what a gate review carries, the three calls you make, and the detached adversarial audit.
