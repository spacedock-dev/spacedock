...
completed:
verdict:
worktree: .worktrees/spacedock-ensign-gate-release-on-e2e
issue:
sprint: 0200-flip
...
- DONE: Add the `e2e-gate` job to `release.yml` (goreleaser declares `needs: e2e-gate`; the job resolves the tagged commit SHA and gates on a `conclusion:success` Runtime Live E2E run whose `headSha` equals it) plus the pure SHA-match decision as a `spacedock-release` subcommand (mirroring `journey-costs`) and the `SPACEDOCK_E2E_GATE_WAIVER` escape hatch — AC-1/AC-2/AC-3.
  `release.yml` gains an `e2e-gate` job (checkout fetch-depth:0/fetch-tags, Go, `git rev-list -1 "$GITHUB_REF_NAME"` → `go run ./cmd/spacedock-release e2e-gate "$RELEASE_COMMIT"`); goreleaser now `needs: e2e-gate`. Pure predicate `EvaluateE2EGate` in `internal/release/e2egate.go`; subcommand `runE2EGate`/`ghRunListForCommit` in `cmd/spacedock-release/e2e_gate.go` wired into main.go's switch + usage. Waiver via `SPACEDOCK_E2E_GATE_WAIVER` (`vars.` in the workflow), short-circuits before the run list, audited to `$GITHUB_STEP_SUMMARY`. Commit 6681b91a.
- DONE: Land the M2 co-edit in the SAME change as the `release.yml` edit (four byte-literal anchors + the no-`needs` assertion) so `go test ./internal/release/` is green over the WHOLE package (AC-4's real bar).
  Same commit 6681b91a re-aligned all four anchors in `journey_workflow_test.go` to the new `  goreleaser:\n    needs: e2e-gate\n    runs-on: macos-latest` header (the two RejectsGoreleaserNeedsJourneyLedger tables now inject `[e2e-gate, journey-ledger]` re-couplings; `insertGoreleaserCarrier`'s const anchor updated) and flipped `TestReleaseWorkflowJobGraphMatchesGitHubActions` to require goreleaser needs exactly `["e2e-gate"]`, keeping the journey-ledger separation. `go test ./internal/release/` green over the whole package (was 0/14 with the bare edge).
- DONE: Commit offline proof — workflow-guard test (needs-edge + SHA-gated query) WITH paired adversarial variants the guard REJECTS, plus predicate unit tests over fixture `gh run list` JSON for pass / block(parked,wrong-SHA,empty) / waiver. No live run.
  `internal/release/e2egate_workflow_test.go` parses real `release.yml` for the needs-edge + SHA-bound gate step, with adversarial dropped-`needs:` and weakened-SHA (`RELEASE_COMMIT=""`, gate-call removed) variants the guard rejects. `internal/release/e2egate_test.go` (9 predicate cases) + `cmd/spacedock-release/e2e_gate_test.go` (4 subcommand cases) cover pass/parked/wrong-SHA/empty/waiver(set+unset)/malformed. Built the binary and exercised it offline against a stubbed `gh` on PATH: empty list → exit 1, green-for-commit → exit 0, waiver → exit 0 w/ audited reason, missing arg → exit 2.

### Summary

Added the release-time e2e gate: `release.yml`'s new `e2e-gate` job resolves the tagged commit SHA and the `spacedock-release e2e-gate` subcommand blocks the cut unless a `conclusion:success` Runtime Live E2E run exists for that exact commit (waivable via `SPACEDOCK_E2E_GATE_WAIVER`, audited to the step summary); goreleaser `needs: e2e-gate`. The SHA-match decision is the pure `EvaluateE2EGate` predicate. The M2 co-edit landed in the same commit (6681b91a) — the four byte-literal anchors track the new goreleaser header and the no-`needs` assertion requires goreleaser needs exactly `["e2e-gate"]` — so `go test ./internal/release/` and `./cmd/spacedock-release/` are green; the guard + adversarial twins catch a dropped-edge or weakened-SHA edit. Pre-existing `internal/dispatch` launcher test fails on the clean baseline too (env-dependent `spacedock` binary on PATH); my diff touches no dispatch files. No live run at implementation time, per the assignment.

## Stage Report: validation

- DONE: Reproduce each AC-1..AC-4's cited evidence independently: `go test ./internal/release/ ./cmd/spacedock-release/` GREEN over the whole package (AC-4 bar); guard REJECTS dropped-`needs`/weakened-SHA variants (AC-1); predicate covers pass / block(parked,wrong-SHA,empty) / waiver(set,unset) (AC-2/AC-3); `go test ./...` from root with every failure accounted.
  `go test ./internal/release/ ./cmd/spacedock-release/` → 80 passed (commit 6681b91a on spacedock-ensign/gate-release-on-e2e). AC-1: `TestReleaseWorkflowGatesGoreleaserOnE2E` + adversarial twins `...RejectsDroppedNeedsEdge` / `...RejectsWeakenedShaMatch` (2 subcases) all PASS. AC-2: 9 predicate cases (`TestE2EGate*`) PASS incl. parked/wrong-SHA/empty block + pick-among-many. AC-3: waiver set passes & records reason, unset enforces, waiver short-circuits malformed JSON — PASS. AC-4: `TestReleaseWorkflowJobGraphMatchesGitHubActions` asserts goreleaser needs exactly `["e2e-gate"]` (never journey-ledger) — PASS; whole package green. `go test ./...` → ONLY `internal/dispatch/TestLauncherCommandFallsBackToPathWhenSpacedockBinUnsetEmptyOrUnusable` fails.
- DONE: Confirm the disclosed `internal/dispatch` launcher failure is PRE-EXISTING on the clean baseline (env-dependent `spacedock`-on-PATH) and not introduced here.
  Ran that test in a throwaway `--detach` checkout of the parent `85404bdd` (clean baseline): FAILS identically (exit status 2, dispatch usage banner). The diff `git show --name-only 6681b91a` touches NO dispatch files. Root cause: `/opt/homebrew/bin/spacedock` is the stale `0.19.4` binary, which predates the `dispatch` CLI shape the test invokes — pure env artifact, unrelated to this change. Not a rejection ground.
- DONE: Detached adversarial audit (MANDATORY — CI/release machinery): on a THROWAWAY checkout of the merge result (6681b91a), try to REFUTE the gate; confirm the deliverable's own tests catch each weakening.
  Throwaway `--detach` worktree of 6681b91a (never the impl worktree); 7 adversarial edits, each apply→test→revert, checkout left clean and re-green. ALL caught RED: A1 drop `needs: e2e-gate` (guard+job-graph red); A2/A2b weaken SHA (`RELEASE_COMMIT=""` / drop gate call); A3 predicate drops `conclusion=="success"` (parked passes → 3 reds); A4 predicate drops `headSha` match (wrong-commit passes → 2 reds); A5 subcommand returns 0 on block; A6/A6b re-couple goreleaser→journey-ledger (separation guard + job-graph red); A7 gate call only in a comment (guard red — `executableShellCommands` strips comments). Detached audit: NO material findings.
- DONE: File a PASSED/REJECTED recommendation citing reproduced evidence per AC; record the audit outcome.
  Recommend PASSED — see Summary. No Material findings; no feedback cycle opened.

### Summary

PASSED. All four ACs reproduced independently against commit 6681b91a: the guard (real-YAML parse, not prose-grep) proves goreleaser `needs: e2e-gate` and the e2e-gate step resolves `git rev-list -1 "$GITHUB_REF_NAME"` then runs the SHA-bound subcommand; the pure `EvaluateE2EGate` predicate passes ONLY a `conclusion:success` run whose `headSha` equals the release commit and blocks parked/wrong-commit/empty; the waiver bypasses only when set and is audited; the whole `internal/release` package is green with goreleaser needing exactly `["e2e-gate"]` (journey-ledger separation intact). The mandatory detached adversarial audit refuted nothing material — seven weakenings (dropped edge, weakened SHA, parked-passes/wrong-commit-passes predicate, exit-0-on-block, ledger re-coupling, commented-out gate) are each caught RED by the deliverable's own tests. The only `go test ./...` failure (`internal/dispatch` launcher) is confirmed pre-existing on the clean baseline `85404bdd`, env-dependent on a stale `spacedock 0.19.4` on PATH, and untouched by this diff — not a rejection ground.
