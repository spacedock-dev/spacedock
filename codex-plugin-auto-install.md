...
completed: 2026-06-08T18:29:48Z
verdict: PASSED
worktree:
issue:
...
- DONE: `execHost.Install` gains a codex arm + a codex install-sequence helper: emits `plugin remove spacedock@spacedock` and `plugin marketplace remove spacedock` (both TOLERATED — marketplace remove exits 1 "not configured" on a fresh box), then `plugin marketplace add <source> --ref <devBranch>` and `plugin add spacedock@spacedock` (both FAIL-FAST); `--ref` and its value OMITTED when branch=="". The claude arm stays byte-identical. Branch threaded is the shared `devBranch`, NOT a hardcoded `next`. (AC-1)
  `codexInstallArgvSequence` + host-dispatch switch in `Install` (commit 7ab6c84b); claude `installArgvSequence` byte-identical (`git diff 487d9e6a` shows no change to it). 6 stub tests (order, both cleanup-tolerance arms, both pin fail-fast arms, empty-branch omits `--ref`) in `install_tolerance_codex_test.go` — stub-echoed argv is the oracle; branch=`devBranch` flows from `runCodex`/`runInit`, never hardcoded.
- DONE: `runCodex` switch mirrors `runClaude`: `NoPluginFound → ops.Install("codex", marketplaceSource, devBranch) → launch`; `--no-install` opts out (refuse-and-instruct: non-zero, no install, no launch, stderr carries the codex plugin remedy); a contract mismatch still fails fast. Invert `frontdoor_test.go:414` (`TestCodexFrontDoorNoPluginFailsFastWithoutInstalling`) — migrate its no-install/no-launch assertion to the `--no-install` arm. (AC-2/3/4)
  `runCodex` gate replaced with the `runClaude` switch (commit 07e1cf1e). Old fail-fast test inverted into `TestCodexFrontDoorNoPluginAutoInstalls` (AC-2, asserts `installCmds == {codex, source, branch}` + launch) and `TestCodexFrontDoorNoPluginNoInstallRefuses` (AC-3); `TestCodexFrontDoorFailFastOnMismatch` (AC-4) stays green. Codex safehouse short-circuit test migrated onto the `--no-install` refuse path, mirroring the claude analog.
- DONE: FIX (do NOT add around) the now-false comments/error-strings: `host_exec.go:271-273` ("only supported for claude"), `host_exec.go:32-34` ("Codex 0.136.0 has no --json"), `frontdoor.go:314-316` ("codex has nothing to auto-install") — rewrite each to describe the new behavior. Include the AC-5 host-native test (`go test`, isolated `CODEX_HOME`, local-path marketplace, branch=""); any live launch in tests `export SPACEDOCK_BIN=<dev binary>` first.
  All three stale strings rewritten; grep for them over non-test source returns NONE (commits 7ab6c84b/07e1cf1e/0b714fac). `TestCodexPluginInstallIsHostNative` (AC-5) runs the real `execHost{}.Install("codex", <local-marketplace>, "")` under isolated `CODEX_HOME`, asserts `plugin list --json` installed:true + cache manifest at the resolver path; passes against codex-cli 0.137.0, skips cleanly when codex off PATH.

### Summary

Implemented the codex front-door plugin auto-install as the codex analog of #311, TDD per AC. `execHost.Install` now dispatches by host: the new `codexInstallArgvSequence` mirrors the claude shape in codex's verb vocabulary (`plugin remove`/`add`) and pins via codex's own `--ref <branch>` flag (omitted when branch=""), with the same tolerance asymmetry (both cleanup steps tolerated, both pin steps fail-fast). `runCodex` adopts `runClaude`'s switch so a missing plugin auto-installs then launches while `--no-install` refuses and a mismatch fails fast. All three Commander-flagged stale comments/error-strings were rewritten (not built around). The `spacedock install --host codex` prose path (init.go) was left untouched per the ideation out-of-scope note. Verification: full module `go test ./...` = 1166 passed, `go vet ./...` clean; AC-5 exercised the real codex-cli 0.137.0 unattended. No live-launch test was needed (the launch is the `fakeHost.launchedArg` assertion), so the SPACEDOCK_BIN-export caveat did not arise in this stage's tests; it remains a note for validation's host-native AC-5 live drive.

## Stage Report: validation

- DONE: Reproduce AC-1..AC-5 independently. AC-1: codex install-sequence stub tests pass; stub-echoed argv proves order + tolerance asymmetry (both `plugin remove` AND `marketplace remove` tolerated; both `marketplace add`/`plugin add` fail-fast) and `--ref` omitted when branch=="". AC-2/3/4: NoPluginFound auto-installs `{codex, source, devBranch}` then launches; `--no-install` refuses (non-zero, no install/launch, codex-plugin remedy on stderr); mismatch fails fast. AC-5: real host-native `execHost{}.Install("codex", <local-marketplace>, "")` under isolated `CODEX_HOME` → `plugin list --json` installed:true + cache manifest at resolver path. `go test ./...` + `go vet ./...` green.
  AC-1: 6/6 codex stub tests PASS (`TestCodexInstallIssuesSequenceInOrder`, both `…Tolerates…RemoveFailure`, both `…FailsFastOn…Add`, `…OmitsRefWhenBranchEmpty`) — stub-echoed argv is the oracle. AC-2/3/4: `TestCodexFrontDoorNoPluginAutoInstalls` (install seam `[codex spacedock-dev/spacedock next]` + launch), `…NoInstallRefuses`, `…FailFastOnMismatch` all PASS. AC-5: `TestCodexPluginInstallIsHostNative` PASS in 0.17s (ran, NOT skipped) against real codex-cli 0.137.0. `go test ./...` exit 0, 0 FAIL lines; `go vet ./...` exit 0. (Build from `./cmd/spacedock`; the one full-suite RED I first saw was MY exported `SPACEDOCK_BIN` leaking into `internal/dispatch`'s `…FallsBackToPath…/unset` subcase via `os.Environ()` — green with `env -u SPACEDOCK_BIN`, not a deliverable defect.)
- DONE: DETACHED ADVERSARIAL AUDIT (high-stakes front door — REQUIRED). On a separate throwaway checkout of the merge result, try to REFUTE the suite; probe (a)–(e); tier Material vs Polish.
  Detached `git worktree add --detach /tmp/codex-audit.* 0b714fac` (NEVER the impl worktree); baseline green; all five probes REFUTED, each turning the suite RED, then reverted clean: (a) tolerating `plugin add` → `TestCodexInstallFailsFastOnPluginAdd` RED; reordering add/install → `…IssuesSequenceInOrder` RED ("out of order"). (b) hardcoding `--ref next` (ignoring threaded branch) → `…OmitsRefWhenBranchEmpty` RED (missing bare-source + leaked `--ref`). (c) dropping `--no-install` refuse → `…NoPluginNoInstallRefuses` + `…PluginGateShortCircuitsBeforeSafehouse` RED ("install invoked despite --no-install"). (d) mismatch `default:` proceeds → `…FailFastOnMismatch` RED ("exit = 0, want non-zero"). (e) claude `installArgvSequence` `plugin install`→`plugin add` → `TestInstallArgvSequence` + both claude tolerance tests RED. No material findings; no polish findings. Throwaway worktree torn down.
- DONE: Channel-tracking + scope discipline: confirm the branch threaded is the shared `devBranch` end-to-end (NOT a hardcoded `next`); the three stale comments/error-strings are gone from non-test source (rewritten); the `spacedock install --host codex` prose path correctly left out of scope. PASSED/REJECTED with verified commit SHA.
  Channel-tracking PASSES: only `"next"` literal in non-test source is `var devBranch = "next"` (frontdoor.go:49, the channel default); `runCodex`/`runInit`→`Install("codex", marketplaceSource, devBranch)`→`--ref <devBranch>` thread the var, never a hardcoded `next` (probe (b) proved a test guards this). z9 does NOT own channel selection (next→main retarget stays the flip's k6 work). Stale strings GONE from non-test source — grep for "only supported for claude", "0.136.0"/"has no --json", "nothing to auto-install" returns NONE (the `state.go:22` "nothing to init" hit is unrelated). Scope: `runInit`'s codex arm still `printCodexInstallProse` (init.go:57), correctly out of scope per the ideation note. Verified at commit `0b714facc9bd74095b77e4083ac91a658fe0cb0d`.

### Summary

PASSED. All five ACs reproduce independently against sources outside the changed file (stub-echoed argv, fakeHost recorded seam, exit codes, and real codex-cli 0.137.0 on-disk state) — AC-5's host-native install RAN (0.17s, not skipped). The required detached adversarial audit refuted all five mandated probes (a–e) plus two bonus probes on a throwaway checkout, each turning the suite RED then reverted; no Material or Polish findings. Channel-tracking is clean (`devBranch` threaded end-to-end, not a hardcoded `next`), the three stale comments/error-strings are gone from non-test source, and the `install --host codex` prose path was correctly left out of scope. `go test ./...` exit 0 (1166+ tests, 0 FAIL) and `go vet ./...` exit 0. Recommendation: PASSED at commit `0b714facc9bd74095b77e4083ac91a658fe0cb0d`.
