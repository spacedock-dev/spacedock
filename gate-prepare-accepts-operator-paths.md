...
Observable semantics declared by this task: command grammar is unchanged; stored gate/Briefing formats and authority are unchanged; absolute and uniquely resolving state-relative behavior stay unchanged; the CLI makes a uniquely resolving launch-cwd-relative Artifact or Reference succeed by supplying `LaunchDir`; an omitted `LaunchDir` adds no candidate and preserves direct-package behavior; multiple distinct cleaned lexical candidates now refuse even when they alias one file; a spelling with no candidate names all attempted roots instead of reporting only the state-prefixed seek path. Downstream readable-file, symlink, commit, and repository-boundary errors retain their current owners and safety effect; the resolver neither follows symlinks nor de-duplicates by file identity.
...
Verified by: fixtures commit different files at the launch-dir and state-root candidates for the same relative spelling and assert nonzero refusal names `--artifact`/`--reference`, original spelling, both cleaned lexical paths, and absolute disambiguation; zero-match cases name attempted roots and accepted forms. A lexical-alias leg proves distinct spellings are not collapsed by `EvalSymlinks`/`SameFile`; a sole symlink leg reaches the existing `gitsource.Inspect` non-symlink refusal. Entity bytes, gate records, room tree, and repository status remain unchanged. A guard table feeds absolute, cwd-relative, and state-relative non-regular/unreadable or foreign-repository candidates through the resolver and asserts the existing inspector refusals rather than fallback. Falsified by cwd-first selection, file-identity de-duplication, post-lock ambiguity detection, or bypassing `gitsource.Inspect`.
...
- Add AC-3 tables for `{unique launch, unique state, ambiguous lexical paths, absent}` and for downstream safety guards. Include a symlink-alias shape to prove the resolver does not collapse candidates by file identity, then a sole symlink to prove `gitsource.Inspect` still owns rejection. Assert errors and byte/tree/status non-mutation. Cost: medium; ordinary Go fixtures. The changes that must make tests fail are restoring silent root priority, defaulting omitted `LaunchDir`, or calling `EvalSymlinks`/`SameFile` in candidate de-duplication.
