...
- Cycle 2: REJECTED — validation re-review (same reviewer, reused); surface 8 files/net +674 vs estimate ≤12 files/net +260±40% (259%); AC unchanged, finding narrowed not closed. Cycle 2's fix is real — the three probe shapes that carried cycle 1's finding now proceed, the ideation six-entity spike reproduces against shipped code for the first time, both really-merged entities resolve cleanly (both `R100`), and nothing cleared in cycle 1 regressed. But the replay retargets only on `--name-status` R-lines while git's `--follow` also retargets on C-lines, so a flat→folder conversion git records as a COPY still hard-errors permanently — the same single live entity that exposed the original finding, one commit deeper. The cycle-2 regression test misses it because it deletes the flat file in the conversion commit, which is exactly what turns the copy into a rename.
...
- **Reviewer (validation cycle 2, 2026-08-18) — Material: the rename replay retargets only on `R`-lines, so a flat→folder conversion git records as a COPY still refuses permanently.** This is the unfixed remainder of cycle 1's material finding, not a new defect and not a regression. Released user and normal workflow: the First Officer running `spacedock merge guard <slug> --verdict passed` on an entity whose folder form was materialized by adding `<slug>/index.md` without removing the flat `<slug>.md` in the same commit. Observable harm: `git log --follow` retargets on that event and emits `C<score>\t<old>\t<new>`, but `entityPathsAtCommits` only advances `currentPath` when `strings.HasPrefix(fields[0], "R")`, so every commit older than the conversion is still read at the folder path and `git show` exits 128 — the same `merge guard: failed to read git history for {slug}`, exit 1, no terminalization, no archive, no recovery in the message, no bypass flag, permanent because history is append-only. Affected authority: `value-ac[AC-2]` — "A healthy run is unaffected … Fails if the guard refuses a correctly-ordered run, which would block every normal merge." Trigger evidence: the one live entity that failed under cycle 1 still fails under cycle 2, one commit deeper — `stakes-declaration-read-through/index.md` died at `73170770a` before, dies at `92f163a7a` now. Its follow event is `C099 stakes-declaration-read-through.md → stakes-declaration-read-through/index.md` at commit `828b1b7e6`, which added the folder index while leaving the flat file in place (`git ls-tree 828b1b7e6` lists both paths). This is git's DEFAULT behavior, not a local config artifact: `git config --get diff.renames` is unset in the state checkout and the C-line persists under an explicit `diff.renames=true`. Reproduced synthetically on a throwaway clone — git emits `C100 010-task.md → 010-task/index.md` and the guard exits 1. Breadth is unchanged from cycle 1: 1 of 186 live entities, scanned by driving the shipped `entityBlobHistory` over every live entity in the real checkout. The cycle-2 regression test for this shape cannot catch it because it `os.Remove`s the flat file in the conversion commit, which makes git record an `R`-line instead. Defect kind: outcome defect. Narrow fix, same function, no AC or surface change: retarget on `C`-lines as well as `R`-lines, since git's own `--follow` retargets on both; more robustly, take each commit's path from its own status line's new side rather than replaying a running pointer, which covers `R`, `C`, `A`, `M` and `D` uniformly.
- **Reviewer (validation cycle 2, 2026-08-18) — Deferred risk: `--follow` lists the commit that DELETED the tracked path, which has no blob to read.** Released user and normal workflow: an entity deleted from the state checkout and later recreated at the same path — an accidental deletion reverted by re-adding rather than by `git revert`. Observable harm: the delete commit appears in the walk with a `D` line; its tree has no blob at that path, so `git show` exits 128 and the same permanent refusal fires. Affected authority: `value-ac[AC-2]` — not violated today; no live entity is in this state. Trigger evidence: reproduced synthetically (delete, recreate at the same path, report, finalize → exit 1 on `failed to read git history`), and it fails identically on `1b14c09bc`, so it is pre-existing from cycle 1 and was missed by cycle 1's review, not introduced by cycle 2. A scan of all 186 live entities found zero whose `--follow` history carries a `D` line on the followed path. Promotes to material on the first such occurrence; the narrow fix is to skip a commit whose blob is absent rather than hard-erroring, which the status-line-per-commit form above gives for free.

## Stage Report: validation (cycle 2)

- DONE: Attack the refusal for false positives — build the matrix the implementation did not: folder-form entities, entities renamed or archived mid-history, feedback-cycle entities carrying "## Stage Report: {stage} (cycle N)" as the only report, an entity whose completing stage differs from its live status, and a report commit that also touches unrelated paths. Each must proceed or refuse correctly, with the reason stated.
  Re-ran my own cycle-1 probes against `27c69c0ba`, not the implementer's tests. All three shapes that carried the material finding now PROCEED: slug rename, flat→folder via `git mv`, unarchive. Everything I cleared in cycle 1 still behaves — folder-form, unrelated-paths report commit, multi-stage entity, uncommitted terminal flip, `(cycle N)`-only report all proceed; no-report and batched report+flip still REFUSE, including across a rename, so the replay does not launder the defect the entity exists to catch. New replay probes: two-hop rename chain, rename plus content change in one commit, and an unrelated sibling rename all proceed correctly. FALSE POSITIVE REMAINING: a flat→folder conversion git records as a COPY, and a delete-then-recreate at the same path (findings above).
- DONE: Test the deliberate-coalescing case specifically. The First Officer intentionally batches some state writes. Determine whether any SANCTIONED coalesced shape now refuses — if the FO's own normal ceremony can produce a single commit carrying both the report and the flip, this guard breaks delivery and that is a material finding.
  Settled in cycle 1 and not disturbed by cycle 2, which changed only how a blob's path is resolved, never the ordering rule or the terminal-state predicate. Re-confirmed the load-bearing half directly: batched report+flip still refuses with "terminal state does not follow the committed report", and it refuses on a renamed history too.
- DONE: Test the overrun claim: net +527 against a declared +260 with +/-40% tolerance (max ~371). The implementation attributes it to the approved 7-item test plan plus internal/cli fallout the estimate never scoped. Verify no scope beyond the approved design landed, and confirm the 7 test-plan items are each genuinely covered rather than padded.
  Cycle-2 growth is confined to the authorized fix. `git diff --numstat 1b14c09bc..HEAD` is +64/-11 in `merge.go` and +94 in the existing `merge_guard_report_order_test.go`, 2 files, no new files; reading the diff, `merge.go` changes only `entityBlobHistory`'s internals plus two new unexported helpers (`entityPathsAtCommits`, `isCommitHashLine`), and the test file gains exactly the three rename regressions. Nothing else came with it — no new command grammar, flag, field, stored format, or AC edit. Cumulative is net +674 across 8 files, 259% of the approved +260 and past the +371 ceiling, matching the implementation's own figure; file count stays at 8, within the declared "up to 12".

### Checks run

Throwaway clone of `27c69c0ba`, never the implementation worktree. `go test ./...` green except `TestCodexResolveManifestAgainstInstalledHost`, which fails identically on unmodified main at the merge-base `a108559c` — machine-local codex install state. `go test -race ./internal/status/` clean, no data races. Six-entity spike claim reproduced myself by driving the shipped `entityBlobHistory` over the real state checkout: all six PASS, each with the report strictly before the first terminal commit — `pre0-cut-idempotent-on-rerun` (12 commits, report 9 < terminal 11), `remove-tautological-workflow-tests` (13, 7 < 12), `collapse-duplicate-edge-marketplace-routes` (21, 12 < 20), `prerelease-ships-stable-stamped-default-artifact` (10, 7 < 9), `run-rejection-journey-in-team-mode` (32, 17 < 31), `red-auto-continue-gate-bypass` (18, 13 < 17). The two entities this repo really merged after a conversion now resolve with zero `git show` failures at their real pre-archive commits — `fix-tautological-output-grep-tests` (17 commits) and `merge-guard-arm-not-a-stopping-point` (26); both conversions were `R100`, which is why the R-only replay covers them. The implementer's three new tests are honestly falsifiable: copied onto `1b14c09bc` unchanged, all three go red with the exact exit-128 symptom, and all three are green on `27c69c0ba`.

### Acceptance criteria

- **AC-1** VERIFIED, unchanged from cycle 1 and re-confirmed under the new path resolution: both observed shapes still refuse with nothing mutated and no archive, including on a renamed history, and the uncommitted-report shape still recovers.
- **AC-2** STILL NOT SATISFIED, but far narrower. Correctly-ordered histories crossing an `R`-line rename now finalize, and the ideation's six-entity evidence finally reproduces against shipped code. A correctly-ordered history whose conversion git records as a `C`-line copy still refuses permanently — one live entity, the same one that failed cycle 1.
