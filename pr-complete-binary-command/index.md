...
## FO decision — command name (captain, 2026-06-04)

**The command is `spacedock dispatch advance {slug} --to done` — the TERMINAL case of `dispatch advance`, NOT a separate `dispatch complete`.** This supersedes every `dispatch complete` reference above. Rationale: the captain flagged that `dispatch complete` reads as "the dispatch is complete" (cleanup) rather than "advance the entity toward its end state"; the command's true job is driving the entity to its `done`+archived terminal, which IS an advance to the terminal stage. This reverses cycle-2's A4 rejection — the unified "advance to a stage, terminal included" mental model wins; the heavy terminal ceremony branches INTERNALLY on `--to done` (mid-flow advances stay light, the terminal advance carries the mod-block-clear → terminalize → archive → worktree/branch teardown → push ceremony with all the guard-honoring/idempotency from the ACs above). This UNIFIES p2 with roadmap #2 (`dispatch advance`): implement the `--to done` terminal slice here; #2's mid-flow `--to {stage}` is the sibling. #6's Class C action becomes "run `dispatch advance {slug} --to done`". ACs above stand; only the command spelling changes. (Implementation 0.19.6, serialized on first-officer-shared-core.)
