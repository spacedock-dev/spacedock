        ...
        - id: gate:ebgwr177kjjs6w5thhywz408:ideation
          ...
          attempts:
            ...
            - id: gate-attempt:ebgwr177kjjs6w5thhywz408-ideation-2
              ...
              resolution:
                type: Resolution
                id: resolution:spacedock:ebgwr177kjjs6w5thhywz408:ideation:2
                briefing: briefing:ebgwr177kjjs6w5thhywz408:ideation:attempt-2:revision-1
                by: person:captain
                at: "2026-08-15T03:55:57.81631Z"
                decision: approve
                reason: 'Captain ruling 2026-08-15 (approve all except x8): approved into implementation'
              application:
                target-stage: implementation
                state: consumed
        - id: gate:ebgwr177kjjs6w5thhywz408:validation
          stage: validation
          attempts:
            - id: gate-attempt:ebgwr177kjjs6w5thhywz408-validation-1
              briefing:
                id: briefing:ebgwr177kjjs6w5thhywz408:validation:attempt-1:revision-1
...
- DONE: Re-apply the spiked removal; reducer test re-pointed to gate-readiness; archived provider Result test added
  Commit `f6f8c6ba1` on `spacedock-ensign/trim-dead-gate-model-surface`. All four removals landed with every keep-boundary intact: `gates.Read` and its `doc`/`gateErr` results stay at `discover.go:216`, only the nine `fields[...]` assignments and their `if gateErr == nil` block went. Two projection-only tests deleted, four repaired per the ideation dispositions, plus the now-unused `gates` import and `decodeSingleStatusEntity` helper (grep confirmed both were confined to the deleted test). `TestStatusProjectsSharedGateReadinessReducer` now runs `--fields id,gate-readiness` and is AC-4's proof — it still asserts all five readiness values across the seven-entity split-root corpus, so it fails if the removal disturbs `gates.Read`, the reducer, or the readiness projection. AC-5's `TestArchivedProviderResultDecodesWithoutAnnotationProseFields` (`internal/gates/archived_authority_test.go`) drives the real retained Result copied to `testdata/archived-provider-result.json` through `decodeProviderResult` and `verifyProviderResolution`; it guards the fixture still carries `target`, `kind`, `body`, and `selectors` first, so it fails if the fixture is ever sanitized into a tautology, and fails if authority decode is made strict.
- DONE: Doc sentence replaced and proven; AC-2 verified by both grep and compile
  `frontmatter-contract.md:13` final sentence now names `--fields gate-readiness`. Both halves proven against the binary built from this commit, not asserted: the new command prints a populated `GATE-READINESS` column (`superseded`, `not-applicable`, `validating`, `approved-awaiting-m…`, `needs-preparation`); the old `--fields gate-state,gate-decision,gate-resolution` prints three empty columns at exit 0. AC-2 grep over `cmd`, `internal`, `skills` returns no consumer of any removed name; the six residual hits are all non-columns and named keep-boundaries — ABOUTME prose in `application.go`/`model.go`/`ac2_reanchor_test.go`, the `gate-briefing.json` room artifact in `gate_test.go:486`, the `gate-state: preserve-me` byte-preservation fixtures in `internal/ensigncycle`, and a `"gate":"gate:a"` member of a gate-room-request JSON literal. `go build ./...` and `go vet ./...` are clean, which is the only verifier for `Summary.Condition`/`Summary.Eligible` since `Eligibility` keeps live fields of those names.
- DONE: Delta no shallower than -80 per AC-1; -timeout 40m; calibrate to the merge-base baseline
  Calibrated to `git merge-base main HEAD` = `4d1912a69`: **10 files, +40 / -137, net -97** — negative and inside AC-1's -80 floor. Against the declared 8 files / -124 this is +2 files and 27 shallower, both accounted for by the AC-5 test (31 lines) and its fixture, which the tolerance admits explicitly. Declared semantic change 1 was measured on both sides by building a merge-base binary and diffing `--all-fields --json` on the same live workflow: ten `gate-*` keys before, `gate-readiness` alone after.
- DONE: AC-3 suite green under both plain and -race at -timeout 40m
  `go test ./... -timeout 40m` and `go test ./... -race -timeout 40m` both green on `f6f8c6ba1` except the declared carve-out. `TestCodexResolveManifestAgainstInstalledHost` fails identically on an untouched merge-base checkout with the same `Failed to read config file ~/.codex/config.toml: Operation not permitted` sandbox error — reproduced, not assumed. See the Summary for two additional failures that appeared only in the plain run and were falsified as contention flakes.

### Summary

Landed the spiked removal unchanged as `f6f8c6ba1`: 10 files, +40 / -137, net -97 against merge-base `4d1912a69`. Every acceptance criterion was proven by exercising the behavior — the doc sentence by running both commands against a binary built from this commit, the `--all-fields` shrink by diffing two built binaries on the same live workflow, and `Summary.Condition`/`Summary.Eligible` by the compiler rather than grep, since `Eligibility` keeps live fields of those names.

One thing the gate should see. The plain `go test ./...` run also failed `TestCodexProcessActivityResetsQuietBudget` and `TestCodexProcessRecognizesTerminalTurnBeforeOSExit` in `internal/ensigncycle`, which are outside AC-3's declared carve-out. Both assert against a hard-coded 250 ms no-progress wall-clock budget on a codex subprocess. Four checks falsify a regression: both pass in isolation on this branch; both pass on the untouched merge-base baseline; the full `internal/ensigncycle` package passes on both baseline (645s) and this branch (315s); and the whole package passes under `-race` on this exact commit. The failing run took 1312s for that package — 2x to 4x the clean runs — because a concurrent `internal/cli` run and sibling ensign suites were saturating the machine, and a 250 ms budget cannot survive that. The removal touches status field projection and two gate struct definitions, neither of which is on the codex subprocess supervision path. I did not change these tests; the fixed 250 ms budget is a pre-existing load sensitivity worth its own entity, not this one's to widen.

## Stage Report: validation

- DONE: Independently re-exercise the ACs against worktree commit f6f8c6ba1, never by reading the report: net -97 within the -124 plan and no shallower than -80, grep plus compile for AC-2 (Eligibility carries live same-named fields, so the compiler is the verifier), all-fields output loses the nine gate-* keys with gate-readiness intact
  AC-1: measured `git diff --numstat 4d1912a69..f6f8c6ba1` myself — 10 files, +40/-137, net -97, deeper than the -80 floor. AC-2: grep over cmd/internal/skills shows no consumer (every residual hit is a `gate-attempt:` ID prefix, the `gate-briefing.json` room filename, ABOUTME prose, or the `gate-state: preserve-me` keep-boundary fixtures); `go build ./...` and `go vet ./...` clean; struct inspection confirms Summary lost Condition/Eligible while Eligibility retains both, Annotation lost Target/Kind/Body, io.go aliases gone, `gates.Read` intact at discover.go. All-fields: built binaries from f6f8c6ba1 and clean 4d1912a69, diffed `--all-fields --json` and text headers on the live docs/dev corpus — exactly the nine gate-* keys disappear, `gate-readiness` intact with identical values on all 25 entities, no other key changes; a removed name under `--fields` prints an accepted blank column at exit 0 in text and JSON, matching declared semantic change 2.
- DONE: The reducer test re-pointed to gate-readiness must assert all five readiness values; the archived provider Result test must decode and verify under the reduced Annotation; the doc sentence's new command proven true and old proven false against the built binary
  `TestStatusProjectsSharedGateReadinessReducer` asserts all five values (validating, awaiting-captain, withdrawn-awaiting-prepare, approved-awaiting-merge, approved-awaiting-advance) across the seven-entity corpus and passes; falsified by severing `gates.Read` in a throwaway copy — it fails on four entities. `TestArchivedProviderResultDecodesWithoutAnnotationProseFields` passes; its fixture is byte-identical to the real archive (`diff` clean against `_archive/.../provider/result.json`); falsified by making `decodeAuthorityJSON` strict in a throwaway copy — it fails on the first unknown field. Doc sentence: `--fields gate-readiness` on the f6f8c6ba1 binary prints a populated GATE-READINESS column; old `--fields gate-state,gate-decision,gate-resolution` prints three empty columns at exit 0. docs/site grep is clean of column tokens (sole substring hit is the `gate-briefing.json` room-artifact filename in command-reference.md, a live keep-boundary, not the column).
- DONE: Suite differential against clean 4d1912a69 with -timeout 40m; environmental failures reproduced before attribution; note the 10-file vs 8-file declared surface and rule on it; verdict PASSED or REJECTED with per-AC citations
  Three runs: worktree plain (`-timeout 40m`), clean-baseline plain on a `git archive` copy of 4d1912a69, worktree `-race -count=1` (fully fresh). Each fails exactly one test, `TestCodexResolveManifestAgainstInstalledHost`, with the byte-identical sandbox error (`Failed to read config file ~/.codex/config.toml: Operation not permitted`) on baseline and candidate — environmental, reproduced before attribution; differential delta is zero. The implementation-noted ensigncycle 250 ms contention flakes did not reproduce in any run (ensigncycle green: 167s/191s/203s). Surface ruling: 10 files vs 8 declared — the two extras are the AC-5 test (+31) and its fixture (+1), which the tolerance clause admits explicitly ("±2 files ... plus the ~25-line new test if it ships"); -97 vs -124 is 27 shallower, inside ±40. Within tolerance, AC unchanged.

### Summary

Verdict: PASSED — AC-1 through AC-6 all verified by independent re-exercise, with two mutation probes proving the AC-4 and AC-5 tests can actually fail. No material findings. Polish (non-blocking): AC-6's literal grep wording collides with the `gate-briefing.json` filename in command-reference.md; the AC's substance (no live doc names a removed column) holds under token-boundary grep. Deferred coordination note, already declared in the body: the reconciled doc paragraph also carries the `status --validate`/`gate validate` sentence that `remove-gate-validate-subcommand` edits, so merge sequencing of those two entities still applies.
