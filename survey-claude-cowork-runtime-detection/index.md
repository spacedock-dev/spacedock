...
1. After the blocking live binding passes, bind `project_root` from `system init.cwd` and set `COWORK_BIN="$project_root/.spacedock/bin/spacedock"`. Never substitute `pwd`, a fixture argument, or PATH. Execute this probe block as written; `COWORK_INSTALL_ABSENT` enters step 2, `COWORK_INSTALL_BROKEN` stops, and silent success reuses the mounted binary:
...
Verified by: run 1 verifies in session space, validates/creates real parent directories, streams bytes through one noclobber exclusive open, chmods, and directly verifies. Concurrent creators prove exactly one open succeeds. Parent symlink/non-directory mutants for both `.spacedock` and `bin` stop before following/writing. Snapshots at initial inspection, each mkdir, exclusive-write interruption/failure, chmod failure, and final-verify failure assert only the honestly possible directories/partial file. Run 2 uses fresh HOME plus the same mount and executes the mounted binary with 0 request/install events. A deletion-denied fixture proves the successful path without unlink/rename.
...
- **Capability/path routing (medium):** after the live binding passes, replay available/unavailable classes without storing roots. Execute exact-final probes for absent, working, non-executable, failing, dangling symlink, and later-PATH shadows. Add `.spacedock` symlink escape, `.spacedock` non-directory, `bin` symlink escape, and `bin` non-directory mutants; each must stop without following/writing outside the bound root.
- **Installer/first-create CLI tests (medium):** keep consent/request/architecture/checksum/temp-cleanup seams and session verification before project mutation. Exercise sequential validated mkdir -> final absent recheck -> `( set -C; cat "$VERIFIED_BIN" > "$COWORK_BIN" )` -> chmod -> direct verify. A concurrent-creator barrier releases two writers and asserts one exclusive open succeeds, one fails, and final bytes equal the verified source. Snapshot every boundary: pre-first-mkdir failure is byte-identical; first/second mkdir failures may leave prior directories; cat interruption/failure may leave partial final bytes; chmod/final-verify failures leave a broken final. No rollback is expected. Run on deletion-denied mount and assert no unlink/rename.
...
The captain confirms the selected lane's current project folder is mounted and persistent, superseding the no-cache/session-only design, but the exact `system init.cwd` binding remains a blocking live spike. After that passes, the riskiest mechanism is exclusive first creation on a deletion-denied mount. Implementation begins with parent/snapshot/concurrent-writer fixtures and then the live HOME-A/HOME-B same-mount proof. No atomicity is claimed: failures after the exclusive open begins may leave a partial broken final path, and recovery requires a separate explicit repair decision.
...
The mounted-project design is now conditional on a concrete live proof of `system init.cwd`, not a fixture-injected root. Once proven, installation validates both parents, streams verified bytes through a single exclusive open, and honestly records every deletion-denied partial-state boundary; fresh-HOME reuse, exact-path isolation, privacy, and short consent guidance remain intact.
