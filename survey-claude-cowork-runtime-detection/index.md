...
1. Probe a durable user install by invoking `PATH="$HOME/.local/bin:$PATH" spacedock --version`; do not use `command -v`, and do not download when this succeeds. If `$HOME/.local/bin/spacedock` exists but this invocation fails, stop and report the unusable existing install; do not overwrite it automatically.
2. If no file exists and the invocation is unavailable, display the exact prompt below and stop. Before affirmative consent there must be no `WebFetch`, `WebSearch`, `curl`, GitHub API call, installer fetch, release fetch, or network-capability test.
3. After affirmation, acquire the installer with `curl -fsSL -o "${TMPDIR:-/tmp}/spacedock-install.sh" https://raw.githubusercontent.com/spacedock-dev/spacedock/main/install.sh`, then run `SPACEDOCK_INSTALL_DIR="$HOME/.local/bin" sh "${TMPDIR:-/tmp}/spacedock-install.sh"`. The complete allowlist disclosed before consent is: `raw.githubusercontent.com` for installer acquisition, `api.github.com` for latest-release resolution, `github.com` for the tarball and `checksums.txt`, and the redirect host `release-assets.githubusercontent.com`. Do not send credentials and do not try alternate mirrors. The installer owns `uname -s`/`uname -m` normalization (`Linux` plus `x86_64|amd64` -> `linux_amd64`; `arm64|aarch64` -> `linux_arm64`), release selection, and checksum verification. Verify with `PATH="$HOME/.local/bin:$PATH" spacedock --version`.
...
**AC-1 - A successful, lazy-load-safe Cowork capability probe selects exactly one Cowork evidence path, while missing/error cases retain existing local behavior.**
...
Verified by: an event-ledger test with a fake `$HOME` and transport. A working explicit-PATH stub yields one version invocation and 0 requests; absence or denial yields 0 requests and no file; affirmation permits requests only to `raw.githubusercontent.com`, `api.github.com`, `github.com`, and `release-assets.githubusercontent.com`, in the installer flow. An existing-but-broken path stops without download or overwrite.
...
- **Installer/consent CLI tests (medium, existing installer seams):** wrap the exact durable-install probe and post-consent acquisition commands with a fake `$HOME`, stub `curl` request ledger, stub `uname`, and fixture assets/checksums. Execute the real `install.sh`; assert all four allowed domains, zero pre-consent requests, no redownload/overwrite for working or broken existing paths, architecture mapping, fresh-install success, and only the documented pre-final-write fail-closed cases.
