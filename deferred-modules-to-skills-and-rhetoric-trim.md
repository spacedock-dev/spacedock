...
- **AC-4 (regression gate).** `go build ./...` and `go test ./internal/contractlint/ ./internal/ensigncycle/` are green from the root — including the re-pointed greet-independence lane (AC-2), BOTH rewritten registry-coupled contractlint tests (AC-3: `boot_resident_closure_test.go` + `fn_consolidation_structure_test.go`), and the measured shallow-boot window (`assertShallowBootMeasured`, greet-turn context stays below the 60k ceiling). The always-on catalog cost is a WASH, not a "negligible add": each skill's `description` enters the boot-time skill catalog, but the registry pointer row it replaces LEAVES the always-on boot core — description-in ≈ registry-row-out, so boot-window occupancy is ≈neutral. The AC-1 file-byte win comes from the rhetoric cut, NOT from any occupancy trade. **Test:** the two `go test` invocations green; the measured-window oracle unchanged and passing.
- **AC-5 (authority-doc alignment; carries the REFINED discriminator from M3).** `docs/runtime-support.md` states the discriminator precisely — NOT the loose "adapter-less": **a deferred module that loads a per-host adapter section ALONGSIDE its core at the trigger (dispatch/merge → `claude-fo-dispatch.md`) stays a core file the boot core names by path; a deferred module whose host binding is already boot-resident or absent AT ITS LOAD POINT realizes as a non-user-invocable skill invoked via `Skill(skill="spacedock:…")`.** `fo-write-core` IS a skill under this rule even though its `spacedock new` binding is per-host, because that binding lives boot-resident in the adapter's `## Filing New Entities`, not in a section loaded at the write trigger. This keeps the `→ runtime-binding` / deferred-module convention text (`docs/runtime-support.md:21`) from drifting from the shipped two-skills-two-references shape. **Test:** cross-check the documented refined discriminator against the shipped skills/references (both skills' bindings are boot-resident-or-absent; both references load a deferred adapter section); a one-paragraph doc edit.
