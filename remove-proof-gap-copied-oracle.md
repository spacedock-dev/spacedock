        ...
        - id: gate:pph11xwsa2s3z73ts983wd0k:validation
          ...
          attempts:
            - id: gate-attempt:pph11xwsa2s3z73ts983wd0k-validation-1
              ...
              resolution:
                type: Resolution
                id: resolution:spacedock:pph11xwsa2s3z73ts983wd0k:validation:1
                briefing: briefing:pph11xwsa2s3z73ts983wd0k:validation:attempt-1:revision-1
                by: person:captain
                at: "2026-08-15T23:54:32.834342Z"
                decision: approve
...
- DONE: Execute the gated design exactly: delete wantProofGaps, its DeepEqual, and the freed reflect import; readRuntimeProofGaps stays as the bare validating call
  Commit 046076625 on `spacedock-ensign/remove-proof-gap-copied-oracle`; the diff is exactly the three gated edits, 1 file +3/-8, net -5 (631 → 626 lines), inside the ±5-line ±0-file tolerance. `gofmt -l ./internal` empty, `go vet ./internal/contractlint/` clean.
- DONE: Value AC: XFAIL change on a live-proof test is a one-file edit leaving contractlint green, probe both sides on your branch
  AC-1 probed both sides on the branch: transferring the `TestLiveBreakGlassShimRecovery` XFAIL owner in `internal/ensigncycle/dispatch_recovery_live_test.go` and changing no other file FAILS at base ab112b6d3 (`live_registry_reconciliation_test.go:56`, DeepEqual mismatch, derived value correct and frozen copy stale) and PASSES with the deletion applied. Both probes reverted; `git status` shows the deliverable file only.
- DONE: Base is the stack top ab112b6d3; land as the next available layer per the ratified Stacked mode
  Branched from ab112b6d3, whose blob for the target file (02f347acb) is byte-identical to ideation's tip c908ac657, so the measured surface transfers unchanged. Landing is NOT done here: `_mods/pr-merge.md` gates push and PR creation on explicit captain approval at the merge hook, so the layer base is set and the PR is left for the gated ceremony.

### Summary

The removal landed as the single-file deletion ideation designed: `wantProofGaps`, its `reflect.DeepEqual`, and the freed `"reflect"` import are gone, and `readRuntimeProofGaps(t, repo, targets)` survives as a bare validating call under a two-line comment. No design deviation and no second file; the measured surface (-5 lines, 1 file) matches ideation's prediction exactly because the new base carries an identical blob.

Both value ACs were proven by exercising, not by grep. AC-1 was probed in both directions on the branch — the identical one-file XFAIL owner transfer fails at `:56` before the change and passes after, so retiring or moving a live-proof XFAIL is now a one-file edit. AC-2 was proven by mutation: `liveXFail("claude-sonnet", "bad-owner")` fails with `malformed liveXFail binding`, and the accompanying falsifier (deleting the retained call and re-running the same mutation) PASSES, confirming the retained line is load-bearing rather than decoration — deleting it would have taken the `parseLiveGap` keep-boundary dead in every automated run. The keep-boundary symbols and the `TestRuntimeLiveTODOOwnersAreActive` owner join are all still present and reachable.

One correction to the ideation record for the validator: the malformed-binding failure surfaces at `live_registry_reconciliation_test.go:175`, not `:174` as the Test plan and Spike record state. The behavior and the message are what ideation described; only the cited line number was off by one. AC-4 is clean absolutely rather than against a degraded baseline: `go test -count=1 ./internal/contractlint/` ok, `-race` ok (19.2s), and `go test ./...` exit 0 with 20 packages ok and 0 FAIL. The registry doc's prose "Expected failure" third copy remains out of scope and still wants a follow-up entity.

## Stage Report: validation

- DONE: Independently re-exercise the ACs against worktree commit 046076625, never by reading the report: 1 file +3/-8 net -5; wantProofGaps, its DeepEqual, and the reflect import gone; readRuntimeProofGaps still called bare; the owner-join and parseLiveGap validation intact
  `git show --stat 046076625` = 1 file, +3/-8 (631 to 626, net -5, inside the ±5/±0 band); `git grep wantProofGaps 046076625` empty repo-wide; no `reflect` left in the file; bare call at `:52`, definition `:139`, `parseLiveGap` `:515`, owner-join loop `:121-135` with its `SPACEDOCK_LIVE_STATE_DIR` skip guard intact (AC-3).
- DONE: Value AC both sides: a one-file XFAIL edit on a live-proof test reds contractlint at base ab112b6d3 and stays green on the branch; the malformed-binding falsifier still fires on the branch
  All probes ran in a namespaced git-archive scratch; the implementation worktree was never mutated. AC-1: transferring the owner at `dispatch_recovery_live_test.go:108` FAILs the base extraction at `live_registry_reconciliation_test.go:56` (DeepEqual mismatch — derived value correct, frozen copy stale) and the identical edit is `ok` on the branch extraction; this would fail if any second-file oracle survived. AC-2: `liveXFail("claude-sonnet", "bad-owner")` FAILs the full package at `:175` with `malformed liveXFail binding` — independently confirming implementation's line correction of ideation's `:174` — and the falsifier (retained call deleted, same mutation, full unfiltered package) PASSES, proving the surviving bare call is the only default-run consumer of the shape validation and is load-bearing.
- DONE: contractlint green plain and -race; verdict PASSED or REJECTED with per-AC citations; note the branch bases on ab112b6d3 whose 42c content is under rework - your verdict covers pp's own diff, and the restack is FO-owned
  AC-4 on the pristine branch extraction: `gofmt -l ./cmd ./internal` empty, `go vet ./internal/contractlint/` clean, contractlint `ok` plain (2.1s) and `-race` (20.3s), `go test ./...` exit 0 with 20 packages ok and 0 FAIL — stated absolutely, no degraded-baseline carve-out. Base note: ab112b6d3 (the 42c stack-link guard) is under rework; this verdict covers pp's own one-file diff only, and the restack is FO-owned.

### Summary

PASSED. All four ACs re-verified by exercising, not by reading the reports: AC-1 both sides (base red at `:56`, branch green), AC-2 by mutation plus the both-directions falsifier, AC-3 by diff stat and symbol reachability (reachability proven by AC-2 firing, not grep), AC-4 absolutely clean. Semantic adversarial pass found no material finding: the one detection the deletion gives up — retiring or moving a live-proof XFAIL no longer trips any lint — is the entity's declared value AC, captain-approved, not a regression; `docs/runtime-live-ci-registry.md:351` ("does not use a copied gap oracle") is true again with no doc edit.

Deferred risk (pre-declared out of scope in ideation): the prose "Expected failure" line near `docs/runtime-live-ci-registry.md:290` remains a third, unparsed copy of the binding. Trigger is outside the current promise — nothing parses that prose (`git grep "Expected failure" -- '*.go'` empty) and no value AC fails. Promote to material if a consumer of that prose is added or the follow-up entity that ideation flagged picks it up and finds it diverged.
