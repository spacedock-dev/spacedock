...
- DONE: dispatch build --advance ships all 7 mode deltas from the approved design (advance header, continue-on-entity line, next-stage Done: block with commits-first clause, -advance filename suffix, spawn-field-free envelope with model retained, --advance + --bare-mode exit 2, per-host pointer message) with golden fixtures for plain / split-root / feedback-reflow / codex-host shapes, plus the AC-1 measurement test: emitted prompt ≤ 300 bytes and invariant to stage-section size, ≥5x smaller than the in-test old-template assembly.
  internal/dispatch/build.go (runBuildFields advance branches, dispatchAdvancePointerPrompt, buildAdvanceOutput); TestBuildAdvanceGoldens (4 shapes) + TestBuildAdvancePromptSizeInvariant (185 B prompt, byte-identical across a 1.0KB and 5.3KB stage section, old-template ≥5x); commit 4488f644.
