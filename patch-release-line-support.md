        ...
        - id: gate:d0g21c517b5nvga1ybwckapk:validation
          ...
          attempts:
            - id: gate-attempt:d0g21c517b5nvga1ybwckapk-validation-1
              ...
              resolution:
                type: Resolution
                id: resolution:spacedock:d0g21c517b5nvga1ybwckapk:validation:1
                briefing: briefing:d0g21c517b5nvga1ybwckapk:validation:attempt-1:revision-1
                by: person:captain
                at: "2026-08-26T04:36:48.959978Z"
                decision: approve
...
- DONE (amended; recorded FAILED at cycle 1): surface within the approved net +281 across 9 files (tolerance +197..+365, ±2 files)
  **Delivered surface: net +381 across 10 files** (+478/-97). Against the ORIGINAL band that is 136% of the +281 estimate and +16 past the +365 ceiling, with the file count inside tolerance (10 of 7..11). What changed the disposition is not the measurement but the baseline: **Cycle 2 ruling — the captain accepted net +381 across 10 files as the amended baseline (2026-08-26, 'approve'); validation proceeds against it.** Superseded cycle-1 measurement, kept for provenance: net +644 across 10 files (+741/-97), 229% and +279 past the ceiling, of which the deleted replay harness was +233. Residual overage against the original estimate, after the harness went: cmd wrapper +37, `if:`-guard +46, release.yml +25, gate library +19, docs +15 — ordinary per-file under-pricing, not scope. The 10th file is `runtime_live_evidence_workflow_test.go`, +2/-2 and net zero — pre-existing gofmt drift the mandated `gofmt -w ./cmd ./internal` corrected, not task content.
...
- DONE: Re-measure the surface with numstat and update the FAILED item per the Cycle 1 line.
  ...
  **Amended after the Cycle 2 ruling:** the captain accepted net +381 across 10 files as the amended baseline (2026-08-26, 'approve'), so the cycle-1 surface item above now reads DONE against that baseline. Its measured numbers and the original-band comparison (136%, +16 past +365) are kept in its evidence line — the disposition changed because the baseline moved, not because the surface shrank to fit.
- DONE (cycle-1 claim was unverified — corrected): go test ./... and -race green
  Correcting my own cycle-1 claim: it was **not verified**. My cycle-1 command piped `go test` into `grep`, so `$?` was the grep's status and the run's real exit code was never read. I reported green on a masked status. Re-run properly and serially: `go test ./... -count=1 -timeout 20m` exits **0** (493s) and `go test ./... -race -timeout 30m` exits **0**. No test of this task's fails in any run, and the suites are genuinely verified green — which is why this item now reads DONE while the correction history above stays on the record.
  **Standing caveat, deliberately kept so the DONE does not overstate.** At the DEFAULT `go test ./...` timeout the suite intermittently fails, and this task cannot make it reliably green. The cause is the pre-existing `internal/ensigncycle` timeout edge: measured at the BASE commit 4b5b4073a with the cache busted, that package took 570.9s of its 600s budget (95.2%, 29s of headroom) and passed; on this branch it measured 490.5s (pass) and 600.4s (timeout). The package is untouched by this diff, and this task strictly reduces suite time. That condition is FO-authorized as declined, classified Deferred risk, and filed separately as `ensigncycle-suite-timeout-headroom`. Anyone re-running these suites should raise the timeout, or expect that flake.
...
- DONE: Every AC (AC-1 through AC-4) re-verified first-hand on the two-commit candidate (284fa8566 + b8e186df5 atop 4b5b4073a) UNDER THE RELEASE-MACHINERY PROOF POSTURE; go test ./... -count=1 and -race with raised timeouts, reading the runs' own exit codes; gofmt; STE spot-check.
  AC-1: the 10-row unit table passes; flipping the predicate to `<= 0` reds exactly the equality row; renaming the gate step away on disk reds the presence guard; the step sits in `e2e-gate` with the designed shell. AC-2: the `if:`-equality guard passes and reds on an on-disk widen. AC-3: both edge-advance steps invoke `edge-pre0-version "$RELEASE_VERSION"` (2 occurrences — same command, same input); `Pre0EdgeVersion` is unit-covered; the stamp sits after the verify poll. AC-4: all six doc blocks applied; step 9 gone, 9 steps ending at "Clean up"; the changed prose has no >25-word sentence and no semicolon. Suites, from their own exit codes: `go test ./... -count=1 -timeout 20m` exit 0 (20 pkgs ok; ensigncycle 464.5s) and `go test ./... -race -timeout 30m` exit 0 (20 pkgs ok; ensigncycle 578.7s; no data race). `gofmt -l ./cmd ./internal` prints nothing. No replay harness anywhere: b8e186df5 is −330/+67 across two test files, zero production bytes.
- DONE: Detached adversarial audit on a THROWAWAY checkout: attack the SURVIVING proof set plus the AC-provenance trigger; findings enter the Review-finding disposition checkpoint.
  Fresh clone at b8e186df5 in the scratchpad, deleted after use; candidate bytes untouched. Live mutations, each restored: (1) predicate flip → equality row red; (2) on-disk gate-step rename → presence guard red; (3) on-disk stamp-`if:` widen → equality guard red; (4) BOTH `if:` widened symmetrically → two guards red independently, so the equality check cannot be gamed by symmetric widening; (5) stamp step reordered before the pre0 cut → `assertAlwaysCutPre0` reds, because the first-match finder latches onto the stamp. The guards red under real on-disk mutations, not only their in-test string copies (audit target 4: yes). Probe: the gate step's shell gutted to always-pass keeps every test green — TRUE, and it is the posture's recorded accepted consequence, not a new finding. Probe: the stamp's version source rewritten to `$RELEASE_VERSION` keeps every test green — matches AC-3's declared by-construction-plus-next-cut proof; the AC never claims a test reds there. AC-provenance: every cited test exists and was exercised; no AC is evidence-free or self-referential; the unit table covers every decision input class release.yml can deliver (the one unexercised class — a bare tag newer than a prerelease stable version — is unreachable through the shipped manifest-tag-gate, and its worst outcome is a loud false block).
- DONE: Stack and surface: branch exactly two commits atop 4b5b4073a with worktree clean; surface confirmed against the captain-amended baseline net +381 across 10 files; the default-timeout caveat cites the filed ensigncycle-suite-timeout-headroom task; recommendation with findings separated.
  `git rev-list --count 4b5b4073a..HEAD` = 2, `git status --porcelain` empty; numstat +478/−97 = net +381 across 10 files, matching the Cycle 2 ruling's baseline; the caveat's task exists at `ensigncycle-suite-timeout-headroom.md` (backlog, evidence seeded). Recommendation: PASSED.

### Findings

- Material: none.
- Deferred risks (all previously recorded and authorized; revisit conditions stand): (1) the gate's shell arms have no standing test — posture-accepted, observed at the next real cut; the audit confirmed the greenness claim is true. (2) AC-1's harm premise in `.goreleaser.yaml` is unguarded — briefing-1 cut. (3) the cmd usage-error exit-2 path is unexercised — briefing-1 cut. (4) the `internal/ensigncycle` timeout edge — FO-authorized decline, filed separately; the race run's 578.7s reinforces it. (5) a non-regressing patch cut still freezes the stable channel at the next latest-line cut — deferred whole to `patch-line-stable-delivery-merge-commit`; the do-not-cut-a-patch-line constraint stands.
- Polish (new, recommendation only; no candidate bytes changed): the stamp-reorder catch is incidental (a first-match artifact of `edgeAdvanceAutoPre0Step`) and the stamp step's `edge-pre0-version` call has no structural pin. One explicit order assertion and one run-block content check would make both catches intentional.
