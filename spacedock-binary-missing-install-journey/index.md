...
completed: 2026-06-08T18:01:47Z
verdict: PASSED
worktree:
issue:
...
Threaded the plugin manifest's display `version` (newly read in `readManifest`) and the binary `Version` (passed from the cli callers; `contract` cannot import `cli`) into the doctor/gate compatibility messages, dropping the internal `contract N` / `>=N,<M` jargon. The full suite is green (build, vet, all packages) and the change is verified end-to-end against a stamped 0.19.6 binary: mismatch paths exit 1 naming both versions with the brew-led remedy, the OK path exits 0 naming both versions, and `--version` keeps its load-bearing `(contract 1)` token for the FO/ensign gate parser. Existing tests that encoded the old copy (frontdoor/init mismatch + OK assertions, `Compare` message-shape and branch-substitution tests) were updated to track the redesigned contract — oracles retargeted to the version-bearing copy, not deleted. AC-3 is the prose payoff append with no fabricated prose-grep test, per the quarantine policy and the entity's own legitimate-text framing.

comm-officer polish arrived after the first commit (32ceb73e): adopted the active-voice direction sentences ("Upgrade the binary to continue." / "Update the plugin to continue.") and parallel label-then-command remedy lines. REJECTED comm-officer's suggested "Or downgrade the plugin: spacedock install" relabel (comm-officer itself flagged it for review) — `spacedock install` reinstalls the CURRENT marketplace plugin, so "downgrade" misdescribes it; kept the accurate "Or refresh the plugin instead: spacedock install". Re-verified end-to-end against the stamped binary and full suite green after the polish; the AC oracles (both versions, no `contract N`/`>=,<`, brew-led remedy, `spacedock install` distinction) still hold.


## Stage Report: validation

- DONE: Reproduce AC-1 independently (RunDoctor over too-old-binary AND too-old-plugin → exit 1, stderr names BOTH versions, no `contract \d` / `>=…,<` range); confirm gate parity
  Independent `RunDoctor` harness with my own binary-version oracle `9.9.9-repro` (a value the message cannot pre-bake) → both fixtures exit 1, name plugin `0.13.0`/`0.10.0` + `9.9.9-repro`, no `contract \d`, no `>=,<`. Live `spacedock doctor` (stamped `0.19.4-validate` binary) matched. Gate parity: `gateHost`(frontdoor.go:130) calls `ManifestVerdict(…, Version)` and prints `res.Message` on mismatch (line 138-140); `TestClaudeFrontDoorFailFastOnMismatch`/`TestCodexFrontDoorFailFastOnMismatch` + `assertGateMismatchMessage` assert the same version-bearing message — not doctor-only.
- DONE: Reproduce AC-2 (compatible exit 0 names both versions, no `contract \d`; too-old-binary remedy has `brew upgrade spacedock` AND `spacedock install`, branch-agnostic — no `@next`/`@latest`)
  Independent harness: compatible exits 0 naming `0.12.1` + `9.9.9-repro`, no `contract \d`; too-old-binary remedy (run with branch=`next`) carries `brew upgrade spacedock` + `spacedock install`, no `@next`/`@latest`. Live CLI confirmed identical. `TestTooOldBinaryRemedyLeadsWithBrew` + `TestTooOldBinaryRemedyIsBranchAgnostic` lock it.
- DONE: Test-strength + AC-3 (oracles NON-VACUOUS; FO binary-absent class names `spacedock claude` + omits `spacedock doctor`; NO tautological prose-grep test; `--version` keeps `(contract N)`)
  Detached adversarial audit on a throwaway checkout at HEAD `32ceb73e` (never the impl worktree): 3 breaking edits each turned the suite RED — (A) re-introduce `contract 1 … >=2,<3` jargon → FAIL `TestMismatchShowsVersionsNotContract`/`TestCompareMessageShape` + gate-parity; (B) hardcode wrong binary version (ignore threaded param) → FAIL same set incl. gate; (C) revert remedy to `go install …@next` → FAIL `TestTooOldBinaryRemedyLeadsWithBrew`/`…IsBranchAgnostic`. NO material findings. AC-3 verified directly: FO line 10 binary-absent class names "launch with `spacedock claude`" and keeps the "Do NOT run `spacedock doctor`" prohibition (no doctor remedy). Confirmed NO `.go` test asserts the payoff phrase and the commit touched zero contractlint/ensigncycle test files (quarantine respected). `--version` live: `spacedock 0.19.4-validate (contract 1)`; `TestVersionContractToken` (cli_test.go:100) untouched.

### Summary

PASSED. All three ACs reproduced with oracles OUTSIDE the message file: AC-1/AC-2 via an independent `RunDoctor` harness keyed on a `9.9.9-repro` binary-version oracle plus a live `spacedock doctor` run against a stamped binary; AC-3 via the admitted legitimate-text exception (PR #262 precedent) with no fabricated prose-grep test. Full module suite green at HEAD `32ceb73e` (real exit 0, 1155 tests; build + vet clean). Detached adversarial audit found NO material holes — three independent regressions (jargon re-intro, ignored binary-version param, stale `go install` remedy) each turn the suite RED, so the oracles are non-vacuous and the gate-parity assertion is real. One process note (does not affect the verdict): rtk-filtered `git log`/`diff` output was stale and initially hid the `32ceb73e` comm-officer-polish commit and gave a bogus "files identical" — caught by comparing raw blob SHAs; the validation is against the true HEAD `32ceb73e`, whose polish (active-voice direction sentences, "Upgrade via Homebrew:" remedy label) is a copy refinement that weakens no oracle.
