# Briefing: merge-guard armed-is-not-a-stopping-point — ideation gate

**Chosen direction:** append ONE clause to the merge module — an armed result is a valid return from one guard invocation, never a valid end to the FO's turn; the FO invokes the merge hook in the SAME turn or halts only at the presented captain PR gate. The existing done-when is deliberately untouched (correct at its granularity). Net contract bytes = the one clause.
**FO recommends approve.**
Checklist: 4 done, 0 skipped, 0 failed. AC-1 (value): a skeptic given the armed output must fail to justify parking against the new text and succeed against the old — baseline: two real parking incidents. AC-2 (mechanism, paired): the clause ships at the right site, nothing else changes. Enforcement: judgment-rung, no machinery.
Decision: approve = recorded pending advance to implementation (Commander applies). Reject = bounce with your asks.
