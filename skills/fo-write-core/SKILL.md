...
| class | source | rule |
| --- | --- | --- |
| allowed-state | resolved state/entity/archive paths | Entity frontmatter via `${SPACEDOCK_BIN:-spacedock} status --set`, `spacedock new`, archive moves, state-transition commits, and `### Feedback Cycles` under the existing state/worktree rules. |
