---
name: fo-write-core
description: "First-officer main-branch write-authority boundary — what the FO may write on main, externally captured gate-decision apply, the `spacedock new` atomic-create procedure, and new-entity id-style minting. Invoke at the first write to main (`status --set`, `status --apply-gate`, `spacedock new`, archive move, or `### Feedback Cycles` write)."
...
## Externally Captured Gate Decisions

Use `${SPACEDOCK_BIN:-spacedock} status --apply-gate --gate {gate_id} --entity {ref} --verdict approve|revise|reject` only when a gate UI, chat approval recovery path, handoff packet, or other harness already captured a human gate decision.

`gate_id` is generated by the surface that captures the gate decision, not by `status --apply-gate`. The apply command requires a caller-supplied stable gate id so Spacedock can record which external decision was applied. Good sources are the gate UI packet id, the chat/handoff decision id, or another durable harness id. Do not invent a fresh id while applying an older decision; recover and reuse the id from the captured decision packet.

`status --apply-gate` resolves the entity, verifies it is currently at a declared `gate: true` stage, advances `approve` to the next stage, routes `revise`/`reject` to the stage's `feedback-to` target when present, and records `gate-id` / `gate-verdict` separately from terminal `verdict`.
