...
For `pi-subagents`, completion is recognized via the `completion-signal` capability (DUAL): either (a) the subagent result returns to the parent (`subagent({action:"status"})` reports `status: completed`) or (b) an inbound done-message arrives via `contact_supervisor` or `intercom send`. After either signal arrives, read the entity file and verify the stage report exactly as the shared core requires. The file-verify is the gate — neither signal alone advances state. Do not advance state based only on a cheerful worker summary.
...
- `completion-signal` — DUAL. Completion may arrive as (a) the subagent return value (`subagent({action:"status"})` returns `status: completed`) or (b) an inbound done-message via `contact_supervisor` or `intercom send`. Both signals trigger the same verify path: read the entity file, verify the stage report. The file-verify is the gate — neither signal alone advances state.
