...
- `«addressable-worker»`: PRESENT — the `contact_supervisor`/`intercom` back-channel. Fresh redispatch remains the default first Pi slice; normal follow-up and retry dispatches are fresh assignment cycles, not context resumes. Reuse-advance over a kept-alive handle is deferred; a non-fresh resume is only an explicit manual/debug exception tied to durable metadata and the full `«worker-identity»` schema. On `pi-agent-teams`, follow-up steering maps to `message_dm`.
