...
1. **Binary version gate.** Before discovery or boot read, run `${SPACEDOCK_BIN:-spacedock} --version` and parse line 1: `spacedock <version>`. These skills require binary minor 0.27 (same major.minor; patch and prerelease skew are fine). Abort by class:
