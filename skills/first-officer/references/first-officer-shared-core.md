...
- **Captain intent routing.** When a bridge-inbox drain is registered, a drained record's `target` selects scope: `all` (or absent) applies fleet-wide; a `{slug}` applies only to that member. One fleet FO owns one cursor, so there is no cross-session addressing race.
