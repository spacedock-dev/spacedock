...
**Launcher command invariant:** Resolve ONE launcher at the version gate — `SPACEDOCK_BIN` when set/executable, else `spacedock` on `$PATH` — report its path/version, and use THAT launcher (written `${SPACEDOCK_BIN:-spacedock}`) for every later Spacedock helper call. Never switch to a bare `spacedock` mid-session: a different `$PATH` binary changes the command surface, so you reason from the wrong capabilities. Bare `spacedock` is correct only in the version-gate fallback probe and diagnostic/install hints, never as a post-gate helper call.
