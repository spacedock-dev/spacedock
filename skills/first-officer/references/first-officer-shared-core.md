...
**Supersede-shutdown.** When the FO decides fresh dispatch because of a cycle increment (the new dispatch name carries a `-cycleN` suffix that the prior agent's name lacks) OR a feedback-rework re-enters the prior stage, shut down the prior cohort BEFORE issuing the new dispatch — in a SEPARATE message. The prior cohort is every team-roster member whose worker handle decomposes to the same `(slug, stage)` pair as the new dispatch's name. Issue the runtime adapter's cooperative-shutdown call (`SendMessage {"type":"shutdown_request"}` on Claude teams; the runtime adapter's analogous teardown call elsewhere) to each prior-cohort member; drop them from session-memory tracking. This step is **mandatory at the supersede boundary itself** — the shared core does not promise a cross-runtime safety net here. Runtimes that ship one (e.g. the Claude state-keyed reconcile sweep) declare it in their own adapter; runtimes that do not (Codex) document the absence explicitly in their adapter, so the FO knows there is no auto-catch.
...
10. **Teardown agents at terminal.** Derive the entity's agent cohort from the live team roster: every worker whose handle decomposes to a slug equal to this entity's slug (the concrete roster source and name-decomposition rule are the runtime adapter's — Claude's adapter reads its team-roster config and decomposes via the `spacedock dispatch reconcile` helper's `decompose(name)`; Codex's adapter declares its analog or its absence). Issue the runtime adapter's cooperative-shutdown call (`SendMessage {"type":"shutdown_request"}` on Claude teams; the runtime adapter's analogous teardown call elsewhere) to each — best-effort, fire-and-forget — and drop them from session-memory tracking. This step is **mandatory at the terminal boundary itself** — the shared core does not promise a cross-runtime safety net. Runtimes that ship a periodic state-keyed sweep (e.g. the Claude `spacedock dispatch reconcile` Class A detection wired into the Claude event loop) declare that backstop in their own adapter; runtimes that do not (Codex) document the absence explicitly so the FO knows a missed teardown stays missed.
