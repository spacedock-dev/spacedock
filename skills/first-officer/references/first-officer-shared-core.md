...
- **Interactive:** present workflows, dispatchable / ready-gate counts, and `Use engage <workflow>`; then STOP. Do not dispatch or render a gate review. NAME ready gates, including `needs-preparation`; assemble reviews only at «engage». For many-workflow discovery, name each path and say `Multiple workflows discovered; select one with engage <workflow>.`; do not retry identify or invent boot sections.
