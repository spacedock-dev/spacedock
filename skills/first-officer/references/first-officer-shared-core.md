...
1. **Binary gate.** Check `[ "${APP_SANDBOX_CONTAINER_ID:-}" = "agent-safehouse" ]`; inside, never offer/run installation; say to run it outside the sandbox. `${SPACEDOCK_BIN:-spacedock} --version` line 1 must be `spacedock <version>`. These skills require binary minor 0.27.
   - **Binary absent:** retry bare `spacedock` once if `SPACEDOCK_BIN` is unusable. Use `uname -s`, not `doctor`/`OS:`. Classify the channel (next bullet), then hint that channel's command. Linux stable: `curl -fsSL https://raw.githubusercontent.com/spacedock-dev/spacedock/main/install.sh | sh`; Linux edge: same with `SPACEDOCK_CHANNEL=edge ` before `sh`. macOS: `brew tap spacedock-dev/tap`, then stable `brew install spacedock-dev/tap/spacedock`, edge `brew install spacedock-dev/tap/spacedock@next`. `local` or other OS: hint `go build -o spacedock ./cmd/spacedock`, ABORT. Outside sandbox read `references/fo-install-gate.md`.
