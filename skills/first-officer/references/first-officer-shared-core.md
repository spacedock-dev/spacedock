...
If the stage is gated, `«gate.lifecycle»(slug, stage)`. That procedure binds and validates the presented package, invokes `«gate.assemble-verdict»`, records and validates the decision, and authorizes any approval through one-use consume before the existing reuse-or-fresh dispatch path. It routes revise through `«feedback.route»`; hold and every ineligible condition stop at the gate.
