...
- `references/fo-bridge.md` — first permission block: the `spacedock bridge alert permission` helper (deny / approve-once / approve-rule).
...
- **effect — drain before presenting (honor a queued Bridge decision):** BEFORE assembling, fire THIS entity's `idle` hooks once (keyed by its `$SLUG`) so a Bridge-queued `decision` record for this gate is drained and applied now; re-read status. Bridge wake is best-effort, delivery confirmed only by this FO-owned drain and ack, so a decision could otherwise sit unprocessed while you redundantly present. If the drain advanced the entity past `{stage}`, do NOT present — report what you applied and return (the **block** below forbids inventing a verdict). Otherwise present normally.
