...
2. **Default local merge** when no merge hook is registered: merge `{branch}` onto the trunk (`${SPACEDOCK_BIN:-spacedock} dispatch trunk --workflow-dir {workflow_dir}`, default `main`) from the stage worktree branch. `«merge.guard»` never local-merges under any policy.
