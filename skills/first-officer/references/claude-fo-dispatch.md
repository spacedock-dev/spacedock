...
`output.prompt` is the reuse-advance pointer message — an `## Advancing to next stage: {stage}` header plus a `Read {dispatch_file_path}` instruction, not the stage section itself. Forward it verbatim; do not paraphrase. The target handle is the FO's own session-roster tracking of the live worker (fo-dispatch-core.md `## Context Budget and Dead Ensign Handling`), not a field the helper emits — `--advance` never emits `name`/`team_name`.
