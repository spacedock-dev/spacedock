...
**Reuse-advance handle (SendMessage):** When advancing a reused ensign (fo-dispatch-core.md `## Reuse and Fresh Dispatch`, "If reuse"), run `spacedock dispatch build --advance` (the same helper, advance mode: `--workflow-dir`, `--entity-path`, `--stage {next_stage}`, `--checklist-file`; `--feedback-context-file` + `--feedback-reflow` when routing rejection findings). On a zero-exit run, send:
...
**Break-glass reuse-advance (fallback ONLY when `spacedock dispatch build --advance` exits non-zero or is unavailable):** Do NOT use this template while the helper is working. Report the helper failure to the captain before proceeding. Use this verbatim-section template as a degraded fallback, now carrying the next-stage completion-signal line the built pointer would have pinned:
