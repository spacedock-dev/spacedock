...
- **Wake is in-session, not external.** A parked Claude transcript has no write locking, so there is NO `bridge ingress wake --host claude` — do not add one. The Stop hook returns `block` while intent is queued so the FO drains before stopping; a truly idle session shows a queued count for the captain to nudge.
