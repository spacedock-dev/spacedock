...
### Idle-monitoring interruption

The operator cue must state that an interruption returns control; the worker is not failed, closed, or redispatched, and async idle monitoring is reinstalled for the same unresolved worker only when that is again the next useful idle action in the captain-authorized active scope.
