...
Async idle monitoring applies only after `«dispatch.next-action»()` has performed its one idle/reconcile/retry and finds an active unresolved Codex worker with no dispatchable, gate, mod/PR, or other state work in the captain-authorized scope. In that condition, the FO MUST call `wait_agent(timeout_ms: 300000)` before ending the turn or reporting idle/status. A completed, errored, or absent worker set never qualifies for `wait_agent`; report the loop's explicit stop instead. Captain input resumes the FO's active loop while workers continue unchanged. Answer status/report/why questions in commentary, route ready authorized work, and resume unresolved-worker monitoring in the same turn once idle. Apply the shared final-response rule; answering the question alone is not a stop. Do not preface each monitoring epoch with an interruption disclaimer.
