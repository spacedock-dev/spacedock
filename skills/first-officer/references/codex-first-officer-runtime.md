...
- `«addressable-worker»` when `send_message(target,message)` and `followup_task(target,message)` are present. `send_message` is non-triggering context/preservation text. `followup_task` starts a worker turn on the addressed worker, including a completed-but-still-addressable worker; use that turn-starting binding for critical-path reuse and re-review.
