...
- `Prompt ownership` -> the First Officer keeps the helper-emitted pointer intact and may append ordinary scope instructions directly. Read the artifact first, then follow it together with those instructions; stage, entity and checklist remain in the artifact, while supplemental notes live in the worker conversation.
...
`«context-budget»` is unavailable unless a future probe binds it. The FO owns reuse decisions. Codex dispatch build prompts are file pointers carrying fetch commands, worktree rules, split-root commit guidance, checklist, and completion-signal wording. Use the generated file as the standard assignment together with any supplemental FO instructions in the worker conversation.
