...
**Boot projection.** Use `ready_gates` from `status --boot --identify --json`; retain `definition_dir`, `entity_dir`, slug, stage, and readiness. After this skill loads, engage `slug` first through `gate prepare-review <slug> --workflow-dir WORKFLOW_DIR --json`, never another read, listing, search, help, or Git probe. `needs-preparation` needs report review; `awaiting-captain` is open; `withdrawn-awaiting-prepare` needs a successor; approved routes are unblocked. Malformed/ambiguous fails closed.

**Prepare and bind.** Resolve `${SPACEDOCK_BIN:-spacedock}`. From the inspect envelope, judge and select the Markdown gate-review Artifact and References, then author the question, summary, and recommendation. Trust its committed bytes and identities; pass exact `input_path` values from `launch_cwd`. Supply judgment only; never author binary-owned JSON, ids, digests, Git-root locators, or room coordinates. Invoke its emitted grammar once:

```text
${SPACEDOCK_BIN:-spacedock} gate prepare-review ENTITY --workflow-dir WORKFLOW_DIR --publish --question QUESTION --artifact REVIEW --summary SUMMARY --recommendation RECOMMENDATION [--reference FILE ...] --json
```

**Cold report candidate.** Structurally review the bundled entity, current-stage prose, and committed candidates. An insufficient obligation, claim, Summary, or scope stops once with `report-incomplete: <concrete defect>` and no publish, mutation, presentation, idle, or repeat-next. Otherwise publish once; nonzero/mismatch stops with its exact phase and no retry or recorder call. Require `phase=complete`, emitted open preparation, sync result, checklist, acceptance evidence, and presentation inputs; never reconstruct the room or run standalone prepare, commit, checklist, AC, or presenter-evidence reads. Load `spacedock:present-gate` and present once. No conn: ask and stop open. Explicit conn: immediately record and consume; never final after presentation.
