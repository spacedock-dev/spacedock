...
6. **Record only direct binding authority.** After the provider returns, run `${SPACEDOCK_BIN:-spacedock} gate record <entity> --room <gate-room>`. The recorder verifies the room's frozen request digest, gate, attempt, Briefing digest, and equal actor/approver authority; derives the complete Artifact/Reference association; and accepts the wrapper-free binding Result through `Resolution.by`. An advisory Result remains evidence and cannot close the gate through an adoption note.
