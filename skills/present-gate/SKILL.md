...
5. **Associate only what the reviewer saw.** Build `spacedock-result-association` from every provider Artifact and Reference actually presented, with each mapped one-to-one to the complete canonical Artifact inventory. Hand the exact retained Result and association to `${SPACEDOCK_BIN:-spacedock} gate record ... --result FILE --association FILE --actor ID`. The recorder rejects an incomplete map; only after it verifies the frozen Briefing digest, actor, and exact Result does it normalize the provider Briefing id.
