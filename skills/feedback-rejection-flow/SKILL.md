...
8. After a PASSED re-review, append the workflow-owned Cycle line and publish validation/2 through the neutral round recorder. Re-enter the normal gate flow immediately: invoke `Skill(skill="spacedock:fo-gate-lifecycle")` and run `«gate.lifecycle»` for the updated stage. A `needs-preparation` row is work to perform, not a stopping condition: prepare and present exactly one fresh open gate, then stop without resolving or applying it, changing terminal state, or dispatching a successor.
