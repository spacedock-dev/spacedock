...
6. Re-run the reviewer after fixes. The reviewer is ALWAYS a separate worker from the fix agent you routed to in step 5: the worker that applied the fix must never review its own rework, so the re-review never goes back to the `feedback-to` target. When the existing REVIEWER (not the fix agent) remains addressable and reuse conditions pass, re-run the kept-alive reviewer through the same `«addressable-worker»` capability used for feedback routing; the message must ask that reviewer to re-review the updated entity state, not validate its own fix work. Fresh-dispatch the reviewer only when the existing reviewer is no longer addressable or reuse conditions fail — which includes every bare-mode flow, where no kept-alive reviewer exists, so the re-review is always a fresh validation worker, never the fix agent's handle.
