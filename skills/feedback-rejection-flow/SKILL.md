...
5. **Re-enter the gate.** Invoke `Skill(skill="spacedock:fo-gate-lifecycle")` and run `«gate.lifecycle»` for the updated stage. A `needs-preparation` row is work to perform, not a stopping condition. That row appears only once step 4's re-review has posted a non-rejecting verdict, so before it the gate is unreachable rather than merely unprepared.
