# Gate review: Gate recorder (3k) — ideation, attempt 7 (two stacked diagrams)

Same design, same package shape — the contract now carries TWO stacked frameless diagrams (the record chain; the cross-owner flow) — the subgraph frames that set the width floor are gone. Worker's honest floor read: each diagram still has one semantic ~2-column branch; if this still reads too wide, the next move is a linked appendix, not a fifth render round. Honest caveat: the worker could syntax-check but not pixel-render it; this float IS the render check. If it is still too wide, annotate and two levers are ready (drop the spine edge; restructure the sibling subgraphs). Three artifacts: this summary, the contract spec (open it for the ownership mermaid, right after its first section — the TUI shows the diagram source; it renders graphically on GitHub/the docs site), and the frozen entity snapshot.
