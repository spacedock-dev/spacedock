...
- FAILED: The AC-1 live captive-PATH edge install evidence is recorded in the stage report (observed --version Channel/minor lines pasted), and the surface lands within the approved +150 net / 4 files (tolerance +/-40, +/-1).
  Evidence recorded (below), but the SURFACE IS OUT OF TOLERANCE: **+328 net / 4 files** vs the approved +150 ±40, i.e. 73% past the +190 ceiling. Files match (4, ±1 ok); the entire overage is the test file at 313 lines vs the estimated 130. This needs a captain-visible decision — see Summary.
