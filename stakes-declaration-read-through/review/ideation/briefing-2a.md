# Briefing: Stakes declaration read-through — ideation gate

**Gate review: Stakes declaration read-through — a declared `## Stakes` reaches every dispatch packet and reviewer context — ideation**
**Chosen direction:** one source — a prose four-part `## Stakes` section in the workflow README — carried verbatim by two non-diverging code paths: a `stakes` key in `status --boot --json` and an inline block in every `dispatch build` packet. Shape is the opinionated four parts (who depends/what breaks; promised surfaces; costliest defect classes; explicitly outside the promise), NOT a bare level label and NOT a free-form hint. Undeclared workflows resolve to one fixed system default — prototype-grade — with a boot/gate nag until declared. The value is captain-only: edits are captain-authored (write-classifier carve-out recommended, FO-proposes-captain-approves the named alternative), re-affirmed at ceremonies (scope-lock, release cut) not on a timer, with a sanctioned stakes-drift flag for contradicting evidence. Injection is uniform across stages because stakes parameterizes stage rules (shifts spike SELECTION, never polish); stage definitions keep the per-stage mechanics.
**FO recommends approve.**

Checklist (Stage Report cycle 3): 4 done, 0 skipped, 0 failed.
AC cross-check: 5 ACs. AC-1 (VALUE) measures stakes-declaration bytes in the packet against a measured 0 baseline, with edit-follows and stage-invariance (AC-1d) clauses that a hardcoded or stage-varying copy fails. AC-5 measures the fixed prototype-grade default + the `STAKES: none declared` boot nag via a presence-toggle, plus default-constant invariance across two undeclared fixtures (proving system-fixed, not per-workflow inference). AC-2/3/4 pair boot exposure, one-source identity, and the live docs/dev carry to AC-1. Spike evidence stands: byte-identical `## Stakes` round-trip through the shipped parser; AGENTS.md-ingestion canary null across 3 runs with the CLAUDE.md positive control.

Resolved since 1a: probe 1 (governance/evolution/default) and probe 3 (stage-differential) were insufficient-evidence against briefing-1a; this briefing makes both answerable per the captain rulings (prototype-grade default + nag; captain-only edits + ceremony re-affirmation + drift flag; four-part shape; parameterization). One open item surfaced for the gate: whether the captain-only write-classifier carve-out ships with this entity or adjacent (probe 1).

Decision: approve closes this gate in 3k notation (application advance -> implementation, state pending, Commander applies). Reject bounces to ideation rework with your annotations.

---

## Full task body follows
