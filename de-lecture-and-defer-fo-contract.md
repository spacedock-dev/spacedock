          ...
          attempts:
            - id: gate-attempt:wn3dg7txnrte0jrcxf56b859-ideation-1
              ...
              application:
                ...
                state: consumed
        - id: gate:wn3dg7txnrte0jrcxf56b859:validation
          stage: validation
          attempts:
            - id: gate-attempt:wn3dg7txnrte0jrcxf56b859-validation-1
              briefing:
                id: briefing:wn3dg7txnrte0jrcxf56b859:validation:attempt-1:revision-1
...
- DONE: reproduce the binding evidence cheaply - the byte accounting (commit-1 reduction ~-3,355 B, commit-2 sideways ~+145 B, contract subtotal -3,210 B inside the binding -3,693 +/-800 band, boot-resident core 22,545 B with the cap ratchet), the three moved-section falsifications re-red and restored, and the diff-bound targeted suites (internal/contractlint plus go build/vet)
  All numbers reproduce exactly from git objects at `9d6b948e8` vs parent `ef43c7ca8`: directory sums 111,342 -> 107,987 (-3,355) -> 108,132 (+145), subtotal -3,210 in band; shared core 22,545 B; cap 23,500 present at `fo_function_reference_invariant_test.go:48` and enforced (package green would fail past it). The moved section is BYTE-IDENTICAL between post-strike source and move target (2,745 B, `diff` empty), and all three repaired references resolve to `fo-dispatch-core.md:38` (shared-core lines 21, 46, 171). Falsifications re-red with the exact expected messages, each restored to green after: duplicate heading -> `still appears 1 times in ...first-officer-shared-core.md`; section moved to unnamed `claude-fo-dispatch.md` -> `deferred load-points block does not name`; block entry renamed to `-nope.md` -> `TestBootResidentDeferredLoadPointsResolve` dangling stat (plus the reachability assertion, a second independent red). `go test ./internal/contractlint` ok, `go build ./...` ok, `go vet ./...` ok; full matrix intentionally not re-run per the lean order (implementation ran it unfiltered twice; PR CI runs at the stack tip). `internal/cli` not run here, so the known machine-local codex red was not touched or re-attributed.
- DONE: Rule on the two flagged deviations
  1. Net LOC +59 vs +32+/-25: reproduced via `git diff --numstat` (+107/-48, 7 files). The overage is entirely test-side (+63 test LOC: the moved-section table at 56 lines vs the 38 estimated, plus an 8-line provenance comment on the cap ratchet; contract files net -4, inside their -5 declaration). The ideation names the byte band as the binding metric and it holds. Recommend classification: Polish (estimate variance on the non-binding metric, no value AC touched) - recorded, not blocking, not silent.
  2. fo-write-core next-id strike: the prohibition does NOT genuinely survive at ID Styles. What survives (`fo-write-core.md:51`) is hazard framing conditioned on concurrency ("a peer's filing... can shift it, so a hand-assembled file can land a stale id") - a reader in a single-writer context can follow ID Styles, judge the window harmless, and land the forbidden preview-then-hand-write two-step without contradicting any surviving sentence. This strike removed a rule, not a rationale - the inverse of the task's own KEEP lens. Recommend restore-one-line: reinstate "Do NOT pair `--next-id` with a hand-written file - `new` is the path; `--next-id` is candidate-preview only." at ID Styles (~110 B on a deferred, non-boot file; subtotal stays in band). Classification proposal: outcome defect against AC-2's parity clause, material-if-unaddressed with a one-line FO-authorizable fix inside the approved surface; if the captain instead accepts the strike at GitHub review, record the parity exception and pass.
- DONE: AC-1 value measurement: attempt the stack-base-vs-tip A/B greet occupancy via a --plugin-dir launch of the worktree checkout if practical in this environment; if impractical, record the byte-derived expectation and name the unmeasured live delta EXPLICITLY
  Ruled impractical after probing (claude 2.1.226 and the plugin manifest exist): a fresh greet boots against the shared live `docs/dev/.spacedock-state`, which multiple sibling ensigns are committing to mid-run, so the base and tip greets would render different status tables and the noise swamps the ~1,280-tok signal; isolating a frozen fixture project for both runs is a harness build the lean order excludes. Byte-derived EXPECTATION (not a measurement): boot-resident core -3,326 B ~ -1,280 tok at s6q's measured 2.58 B/tok ratio. The live A/B delta is UNMEASURED - this is the lean posture's accepted residual, left explicitly for the captain's GitHub review and the stack-tip lanes.
...
Delta-confirm at correction-round candidate `360e4afb9`: the restore commit carries the exact prohibition sentence ("Do NOT pair `--next-id` with a hand-written file — `new` is the path; `--next-id` is candidate-preview only.") at ID Styles in `fo-write-core.md`, one line changed and nothing else; contract subtotal -3,099 B, inside the binding -3,693 ±800 band; `go test ./internal/contractlint` green at head. The sole material finding is resolved — recommendation flips to unconditional PASSED.
