          ...
          attempts:
            - id: gate-attempt:5xwwj9c2w50921t16s840p49-validation-1
              ...
              withdrawal:
                ...
                at: "2026-08-21T04:59:57.608883281Z"
                reason: Validation report re-filed after captain-directed test trim (commit c4b50fbb0 replaces bb6ebbadc); the prepared room's artifact digest is stale. Re-preparing against the current report.
            - id: gate-attempt:5xwwj9c2w50921t16s840p49-validation-2
              briefing:
                id: briefing:5xwwj9c2w50921t16s840p49:validation:attempt-2:revision-1
...
- DONE: Independently verified AC-1 (value) — with npm:pi-subagents registered in settings.json `packages`, the production gate `if !check.packageStatus.subagentsRegistered` in `runPi` (internal/cli/pi.go) drops the explicit `--extension`/`--skill` flags so pi's own package discovery is the sole load path (one specifier, no 'Tool "subagent" conflicts'); with it unregistered, the gate keeps the flags (the only load path). The deleted fake-argv behavior test is gone (`grep -c TestRunPi_RegisteredSubagentsDedupesExtensionLoad internal/cli/pi_frontdoor_test.go` = 0); the value is now proven by the production gate logic + the detection unit test, not a fake-argv test.
  The gate reads `check.packageStatus.subagentsRegistered`, set by `piSpacedockPackageStatus` when a package named `pi-subagents` is found in the settings.json `packages` scan. When true the explicit flags are omitted (pi discovery loads `<pkg>/index.ts` as the sole specifier); when false (zero value) the flags are appended (current behavior). Diff is +85/-17 across 2 files (internal/cli/pi.go +44/-17, internal/cli/pi_frontdoor_test.go +58); commit 9dda7ffcb on branch spacedock-ensign/pi-subagents-duplicate-extension-load, HEAD confirmed at 9dda7ffcb, worktree clean.
- DONE: Independently verified AC-2 (mechanism) — the detection unit test `TestPiSpacedockPackageStatus_SubagentsRegistered` (3/3 sub-tests PASS, table-driven, asserting only the `subagentsRegistered` bool) confirms `piSpacedockPackageStatus` sets `subagentsRegistered` from a settings.json listing `npm:pi-subagents` and unset otherwise. Falsifying change: changing `} else if name == "pi-subagents" {` to `} else if name == "pi-subagents-BROKEN" {` in pi.go fails 2/3 sub-tests (`subagentsRegistered = false, want true`); `git checkout -- internal/cli/pi.go` restored. The dev-override Spacedock extension block (cfg.repoRoot != "") is unconditional in the production code — appended regardless of `subagentsRegistered` — so it is unaffected by the gate.
  `go test ./internal/cli/ -run 'TestPiSpacedockPackageStatus_SubagentsRegistered' -count=1 -v`: 3/3 PASS (with_pi-subagents, without_pi-subagents, pi-subagents_only). Falsifying change: 2/3 FAIL with `subagentsRegistered = false, want true` — the test asserts the field is driven by the `name == "pi-subagents"` match. The dev-override block is a separate unconditional `if cfg.repoRoot != ""` append, not gated on `subagentsRegistered`.
- DONE: Ran `go test ./internal/cli/ -count=1` on the worktree at 9dda7ffcb; the only failures are the 4 known pre-existing/environmental ones (TestVersionAmbiguousMarkersExitZero, TestCodexChannelInstallLeavesCoHostedPluginInstalled, TestCodexModeSwitchRoundTripPreservesExclusivity, TestCodexPluginInstallIsHostNative) — identical to the prior validation run and pre-existing on the rebased-725 base (e68aa5b7a). `go build ./...` is clean; `gofmt -l internal/cli/pi.go internal/cli/pi_frontdoor_test.go` is clean; no staged files.
  `go test ./internal/cli/ -count=1`: 4 failures only (the 4 known pre-existing). `go build ./...`: no output (clean). `gofmt -l`: no output (clean). `git status --short`: no output (clean, no staged or unstaged files). HEAD = 9dda7ffcb on branch spacedock-ensign/pi-subagents-duplicate-extension-load.

### Summary

Validation PASSED. The production gate in `runPi` drops the explicit pi-subagents `--extension`/`--skill` when `subagentsRegistered` is true (pi discovery is the sole load path → one specifier, no `Tool "subagent" conflicts`) and keeps them when false (the only load path, current behavior preserved). The detection unit test `TestPiSpacedockPackageStatus_SubagentsRegistered` (3/3 sub-tests, table-driven on the `subagentsRegistered` bool) confirms the field is set from a settings.json listing `npm:pi-subagents` and unset otherwise; a falsifying change (`name == "pi-subagents-BROKEN"`) fails the test, proving the assertion is driven by the match. The dev-override Spacedock extension block (cfg.repoRoot != "") is unconditional and unaffected by the gate. The 4 pre-existing/environmental failures are identical on the rebased-725 base and unrelated to this diff. Diff is +85/-17 across 2 files; no staged files; gofmt and go build clean. Recommend PASSED.
