...
**AC-1 — The sonnet live cycle PASSES by grading the TERMINAL-STATUS MARKER + HOLD + launcher kill (runtime-observable oracle).**
Verified by: a live-e2e sonnet run driven by NET-NEW watcher machinery (own it in implementation — the current live test has no teardown-grade step; it watches only TeamCreate → dispatch-close → expectExit, and shutdown_request/TeamDelete appear in no live-tagged file) that grades the FIX's unique tail: `shutdown_request` → member approval → a BOUNDED count of `TeamDelete` attempts → the FO emits the **defined terminal-status marker** (see Proposed approach #2) and then HOLDS (no further teardown tool_use), then the launcher kills the subprocess via `defer poller.kill()` (live_test.go:133) and the cycle PASSES. NO clean self-exit required, NO `exitCode==0` gate (the `expectExit`-flat-cap-then-fail is gone). The grade keys on the MARKER+hold, NOT on the beats — `shutdown_request`/`TeamDelete` appear in BOTH bug shapes, so a beats-present grade would green the very run yy's fix failed; only the marker+hold is unique to the fix. By-construction-pending-live; MUST pass on a real sonnet run.
...
- The live `TestLiveEnsignCycle` now passing through `live_test.go:215` because the gate forces the live sonnet FO to react to the refusal and re-set with a verdict.
