...
- **(C, kept and central) Make `expectExit` a NO-PROGRESS (quiet) budget, not a flat cap.** `expect`/`expectDispatchClose` already reset their deadline to `now+budget` on every drained line (streamwatch.go:157-159, 202-205), so they cap SILENCE, not total wallclock — a stage running for minutes never trips as long as the stream keeps moving, all while the literal stays ≤60s (the captain's invariant). `expectExit` is the ONE step that does NOT do this: it is a flat `exitBudgetDefault = 60s` cap (streamwatch.go:239-257 — it drains but never resets `deadline`). During a slow member settle the FO's retry loop IS emitting stream lines the whole time (TeamDelete attempts, `sleep` Bash, re-sent `shutdown_request`), so making `expectExit` reset on activity like its siblings tolerates an arbitrarily long settle yet trips on a GENUINELY silent hang in ≤60s. **This needs NO literal >60s, so it does not touch the ≤60s invariant or `TestNoTimeoutLiteralExceeds60s` — the budget stays 60s; only its RESET-on-activity semantics change.** The streamwatcher already fires `expectExit` on ANY clean process exit and does NOT require TeamDelete success — that half of C is already satisfied (see `TestExpectExitWaitsThenKills/exits_cleanly`); the missing half is the no-progress reset.
...
**≤60s invariant interaction (folded in per team-lead constraint).** `TestNoTimeoutLiteralExceeds60s` (live_budget_test.go:30) AST-parses every `N * time.Unit` literal in `liveBudgetSources = {"streamwatch.go", "live_test.go"}` and reds if any evaluates to >60s. The C reframe deliberately routes AROUND this: the slow-settle tolerance comes from `expectExit`'s deadline RESETTING on stream activity (the no-progress pattern), NOT from a bigger literal — so `exitBudgetDefault` stays `60 * time.Second` and the guard stays GREEN. We do NOT revise the ≤60s invariant. The chatty-hang gap that the no-progress pattern leaves (a process that emits forever but never exits) is already a conscious, documented trade-off on the live path (live_test.go:142-150 "KNOWN GAP") backstopped by Go's default test timeout; extending the same pattern to `expectExit` inherits the same backstop and does not widen the gap beyond what `expect`/`expectDispatchClose` already accept.
...
**AC-3 — `expectExit` tolerates a slow member settle via a NO-PROGRESS reset, with no timeout literal >60s.** Verified by TWO gates: (a) a unit test on `streamWatcher.expectExit` asserting it does NOT trip while the proc stays alive AND the stream keeps emitting lines past the budget (deadline resets on activity — the same property `TestExpectResetsDeadlineOnActivity` proves for `expect`), and DOES trip on a GENUINELY silent hang within the budget — RED on the current flat-cap `expectExit` that trips on a chatty-but-alive proc; (b) `TestNoTimeoutLiteralExceeds60s` stays GREEN — the change is reset-on-activity semantics, not a bigger literal, so `exitBudgetDefault` remains `60 * time.Second`. This is a real CODE seam (`internal/ensigncycle/streamwatch.go:40,239-257`), and the ≤60s guard (live_budget_test.go:30) is a second code gate the change must not red.
...
- **≤60s invariant interaction (explicit, per team-lead).** `TestNoTimeoutLiteralExceeds60s` (live_budget_test.go) bounds every timeout literal in streamwatch.go/live_test.go to ≤60s. The C reframe achieves slow-settle tolerance via `expectExit`'s no-progress RESET-on-activity, NOT a larger literal, so `exitBudgetDefault` stays `60 * time.Second` and the guard stays GREEN — the ≤60s invariant is NOT revised. AC-3 gate (b) explicitly asserts this guard stays green, so a future implementer who tries to "fix" the settle by bumping the literal past 60s gets a RED instead of a silent invariant breach. This was flagged at ideation so it cannot surface as a validation surprise.
