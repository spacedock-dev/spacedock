...
- **M1 (Claude degraded-variant detection): STILL OPEN — the load-bearing remaining defect.** `claude-fo-dispatch.md:99` still hardcodes "a `ToolSearch(select:TeamCreate)` no-match … is the normal path where the named-background-`Agent` + `SendMessage` back-channel works." No boot-time probe; that file isn't in the diff. The spike proved this false on every current Claude session: a fresh `claude -p` enumerates `Agent`/`Task*`/`TaskOutput`/`TaskStop` and **`SENDMESSAGE: no`, `TEAMCREATE: no`**; spacedock source has no Claude-side SendMessage/TeamCreate (the back-channel substrate is Pi-only, `internal/cli/pi.go`); superpowers maps Claude to `Agent` + `TaskOutput`/`TaskStop`, no SendMessage. A live Claude FO IS the degraded variant.
  ...
  **Root cause (confirmed by forcible call + transcript forensics): the back-channel is gated behind the experimental env flag `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`, OFF by default.** A forcible `SendMessage` call returned *"SendMessage exists but is not enabled in this context."* `ToolSearch(select:SendMessage)` flipped from no-match to returning the schema the instant the flag was set. Transcript forensics: same `agentSetting: spacedock:first-officer` sessions split on the flag — flag-on sessions used `SendMessage` 20–55× alongside `TeamCreate`; flag-off sessions (this one) have zero. **"named ≠ addressable":** the `Agent` spawn (naming) is always available; *addressing* a running worker (`SendMessage`) needs the flag. So the binding is **`→ Claude: PRESENT when CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS is set (probe SendMessage at boot), else ABSENT`** — detect, never assume. **Upstream fix (separate, bigger than this PR): `spacedock claude` should export `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` at launch** so FO sessions reliably get the back-channel by construction rather than depending on ambient env.
