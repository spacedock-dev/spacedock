...
- DONE: Reconcile the clean nv candidate onto confirmed origin/main 9021cbf by replaying only the five nv commits after 8728da3a0; abort on any conflict and preserve no-manifest, no-internal/gates, no-kd, and no-assignment/pointer boundaries.
  Evidence: `git fetch origin` followed by `git rebase --onto origin/main 8728da3a0` completed without conflict. The exact new candidate HEAD is `972f7eabf42a026486c5156e8f831c57754aa484`; `origin/main` and the merge-base are both `9021cbf374bb1740d1b2e45155041e1f809372c4`, with exactly five commits in `origin/main..HEAD`. `git range-diff 8728da3a0..304e09a09864889b1375fe7d41eeabd4d41e5153 origin/main..HEAD` accounts for the five replays as `0295894d0 -> 2b540f237`, `77589b581 -> a1eff7178`, `f8d0db290 -> d753e746b`, `ca19b01dc -> 0f709edf9`, and `304e09a09 -> 972f7eabf`. The published diff is 16 files (`549 insertions, 40 deletions`) with no manifest, manifest-testdata, `internal/gates`, kd, assignment, pointer, or oracle paths. `113738b20` is now an ancestor only because the 0y changes are contained in the confirmed latest main; the 0y worktree was not touched or merged separately. Main's current Claude role mapping remains present, while the nv Codex behavior remains in the replayed diff. The candidate is clean and the branch was pushed normally at the exact HEAD.
- FAILED: Audit the new exact HEAD and publish the branch normally so CI runs against that SHA; run the documented repository and live checks only against the new SHA and retain concrete evidence.
  Evidence: the new-HEAD focused dispatch/process/recorded-gate/AC-reanchor checks, deterministic Runtime Live CI controls, full `go test ./...`, `go test ./... -race`, `gofmt -l ./cmd ./internal`, `git diff --check`, and `TestRuntimeLiveRegistryReconciliation` all passed. The detached audit at `/tmp/spacedock-validation-nvz-audit-new.dQcI2n/checkout` was based on `972f7eabf`; its baseline passed, and each independent falsifier went red: removing the Codex `$spacedock:ensign` prefix made `TestCodexFreshDispatchBoundaryFixture` exit 1; accepting `turn.completed` without a final message made `TestCodexProcessRequiresFinalMessageForTerminalTurn` exit 1; suppressing quiet-timeout failure made `TestCodexProcessQuietTimeoutPreservesFaultEvidence` exit 1; weakening the absolute successor-path wording made `TestRecordedGatePromptAnchorsSuccessorEntityPath` exit 1; and accepting duplicate dispatches made `TestRecordedGateLifecycleRealCLIReplay` exit 1. The clone was restored clean at `972f7eabf` with no forbidden diff names.
  Host evidence blocker (not SKIPPED): the exact new-HEAD Claude command was `SPACEDOCK_LIVE_RUNTIME=claude SPACEDOCK_LIVE_MODEL=claude-sonnet-5 SPACEDOCK_BIN=/Users/clkao/git/spacedock-research/spacedock-v1/.worktrees/spacedock-ensign-restore-codex-ensign-contract-bootstrap/spacedock SPACEDOCK_REPO_ROOT=/Users/clkao/git/spacedock-research/spacedock-v1/.worktrees/spacedock-ensign-restore-codex-ensign-contract-bootstrap SPACEDOCK_LIVE_ARTIFACT_DIR=/tmp/spacedock-validation-nvz-claude-new go test -tags live -count=1 -timeout 40m -run '^TestLiveCommon' -failfast ./internal/ensigncycle -v`; it exited 1 before FO work with `Failed to authenticate: OAuth session expired and could not be refreshed`, artifact `/tmp/spacedock-validation-nvz-claude-new/claude-shared-scenarios/full-ensign-cycle/claude-stream.jsonl`. The exact new-HEAD default-Pi command was `SPACEDOCK_LIVE_RUNTIME=pi SPACEDOCK_PI_LIVE_CHILD_MODEL=openrouter/openai/gpt-5.4 SPACEDOCK_BIN=/Users/clkao/git/spacedock-research/spacedock-v1/.worktrees/spacedock-ensign-restore-codex-ensign-contract-bootstrap/spacedock SPACEDOCK_REPO_ROOT=/Users/clkao/git/spacedock-research/spacedock-v1/.worktrees/spacedock-ensign-restore-codex-ensign-contract-bootstrap SPACEDOCK_LIVE_ARTIFACT_DIR=/tmp/spacedock-validation-nvz-pi-new go test -tags live -count=1 -timeout 40m -run '^TestLiveCommon' -failfast ./internal/ensigncycle -v`; it exited 1 at `full-ensign-cycle` with OpenRouter HTTP 402 (`requested up to 128000 tokens, can only afford 114285`), artifact `/tmp/spacedock-validation-nvz-pi-new/pi-common/full-ensign-cycle`. `gh run list --repo spacedock-dev/spacedock --branch spacedock-ensign/restore-codex-ensign-contract-bootstrap ...` and the matching `gh pr list` both returned `[]`; `.github/workflows/runtime-live-e2e.yml` triggers this lane only on `workflow_dispatch` or `pull_request`, so no CI result or new-SHA host artifact is visible. These are external authentication/credit/CI evidence blockers, not candidate defects, and required host evidence is recorded FAILED rather than SKIPPED.
- DONE: Append the validation reconciliation report with explicit DONE/SKIPPED/FAILED counts, naming the new SHA and any CI or host evidence blocker; do not prepare or decide a new gate in this assignment.
  Evidence: this report is appended after `status --read --json` reported `total_lines=364`; no frontmatter or gate record was changed. The state commit is path-scoped to this entity, and the pre-existing dirty state file `keep-journey-delta-green-on-rerun.md` is deliberately excluded. No new validation gate was prepared or decided.

### Summary

The nv candidate was rebased cleanly onto `origin/main` and published at exact HEAD `972f7eabf42a026486c5156e8f831c57754aa484`, with the five replayed commits and scope boundaries accounted for. New-HEAD repository checks, CI-equivalent controls, and the detached adversarial audit passed. The exact Claude and default Pi common selectors failed before useful host work because Claude OAuth could not refresh and Pi lacked OpenRouter credits; GitHub exposed no PR or live workflow run because the workflow is PR/manual-dispatch based. The unresolved host/CI evidence is recorded as FAILED, with no gate decision made. Counts: DONE 2, SKIPPED 0, FAILED 1.
