...
Verified by: after the strict build, `site/llms.txt` exists (AC-8) and `AGENTS.md` is reachable from the built site (a `site/agents/…` page that includes/links it), and the for-agents surface is discoverable from the document head — the two `<link rel="alternate" type="text/markdown">` tags (one for llms.txt, one for the agents page) render in the built pages' `<head>` (machine-discoverable, not human chrome) with subpath-safe base-aware hrefs (root page `href="llms.txt"`/`"agents/"`, nested pages `href="../../llms.txt"`/`"../../agents/"`), `llms.txt` present at the build-output root. Checked by build-output file existence + the rendered head-link tags resolving to the real site-root files from a nested page + the strict build, not by grepping prose.
