...
Verified by: the "Concepts → A worked example" `nav:` entry (relocated from Running workflows at the captain's validation-gate refinement) resolves to a committed file (`docs/site/concepts/worked-example.md`), so `mkdocs build --strict` passes with the slot present; e6 later replaces the stub's content and its link resolves under the same strict build.
...
**AC-9 — The agent surfaces are reachable in the build output, split by audience (captain correction: two different agents).**
Verified by: after the strict build, (a) the PRODUCT-USING-agent surface — `site/llms.txt` — exists at the build-output root (AC-8), the canonical `/llms.txt` convention an agent reading the docs (e.g. a user's first officer) and a14y both score; and (b) the REPO-DEVELOPMENT-agent surface — `AGENTS.md` — is reachable as a real Contributing nav page at `site/contributing/agent-development/index.html` (the `docs/site/contributing/agent-development.md` symlink to root `AGENTS.md`), framed as contributor docs, not a standalone "for agents" surface. There is NO head-link / banner surface (the earlier `<link rel>` and banner conflated the two agents; both removed — `docs/overrides/main.html` deleted, `theme.custom_dir` dropped). Checked by build-output file existence (`site/llms.txt` at root + `site/contributing/agent-development/index.html` rendering the AGENTS content) + zero `<link rel="alternate">` tags in the output + the strict build, not by grepping prose.
...
- DONE: Wire the agent-readable surface + publish CI — mkdocs-llmstxt generates `site/llms.txt`; the "For agents" link + AGENTS.md reachable in the build (AC-8/AC-9); a GitHub Pages workflow builds with `--strict` as a PR check and deploys on push to the stable branch.
  `site/llms.txt` generated, 3178 bytes / 54 lines, sectioned index of all pages (AC-8). AGENTS.md reachable via symlinked `agents/index.html` (AC-9). For-agents surface is machine-discoverable from the document head: `docs/overrides/main.html` adds two `<link rel="alternate" type="text/markdown">` tags (`/llms.txt`, `/agents/`) to the `extrahead` block — verified rendered inside the `<head>` of `site/index.html` and nested pages (the visible banner was dropped at the captain's validation-gate refinement; a banner is human chrome for a machine resource). `.github/workflows/docs.yml`: build job runs `mkdocs build --strict` on PR + push to main; deploy job (`actions/deploy-pages@v4`) gated to `refs/heads/main` (AC-6).
...
## Captain validation-gate refinements (post-validation)

After the PASSED validation above (at worktree commit 14cab445), the captain requested four refinements at the validation gate. Folded by implementation; re-verify against worktree commit 8e921725. The single load-bearing proof re-run from clean: `rm -rf site && mkdocs build --strict` → exit 0, zero warnings.

- DONE: Swap the visible "For agents" banner for machine-discoverable `<link rel>` head tags (AC-9 mechanism change; AC-9 "Verified by" updated above).
  `docs/overrides/main.html` now adds two `<link rel="alternate" type="text/markdown">` tags to `extrahead` and drops the `announce` banner. Verified rendered inside `<head>` of `site/index.html` and nested pages; `md-banner`/"For agents:" count = 0 in output. `/llms.txt` kept at the build-output root, AGENTS.md page still reachable. Removed the now-unused `extra.agents_url`/`agents_md_url` keys. Worktree commit 178a44a0.
- DONE: Make the for-agents head links subpath-safe (the topology catch — superseding the root-absolute form above).
  Changed the `extrahead` hrefs from root-absolute `{{ '/llms.txt' | url }}` / `{{ '/agents/' | url }}` to base-aware `{{ 'llms.txt' | url }}` / `{{ 'agents/' | url }}` (no leading slash). Rendered output is now depth-correct per page: root page `href="llms.txt"` / `"agents/"`; nested pages (e.g. `concepts/worked-example/`, `get-started/install/`) `href="../../llms.txt"` / `"../../agents/"` — each resolving to the real site-root file (`../../llms.txt` from the nested page → the 3168-byte `site/llms.txt`; `../../agents/` → `site/agents/index.html`). This resolves correctly whether the site deploys at the domain root OR under a `/spacedock/` project-Pages subpath, closing the topology gap flagged above. Worktree commit b411406e.
- DONE: Declutter the nav — collapse the secondary headers (AC-2 presentation change, noted above).
  `mkdocs.yml` `theme.features`: dropped `navigation.sections`, added `navigation.tabs` + `navigation.tabs.sticky` (kept `navigation.instant`). Built output: `md-tabs` present (top tab bar), features JSON carries `navigation.tabs`/`navigation.tabs.sticky`, `navigation.sections` gone. Top-level sections are top tabs; the sidebar shows only the active section's pages.
- DONE: Move the worked example into Concepts (AC-5 location change; AC-5 "Verified by" updated above).
  `git mv` (then stubbed) `running-workflows/example.md` → `concepts/worked-example.md`; nav entry relocated to "Concepts → A worked example"; old `running-workflows/example/` path gone from output; `concepts/worked-example/index.html` renders (title "A worked example"); `llms.txt` indexes it under Concepts. Stays the e6 stub.
- DONE: [TODO]-stub the synthesized content so the site is RELEASABLE now (structure ships; prose fills incrementally).
  Replaced every synthesized/thin page body with an honest one-line `[TODO] — <what this page covers>`: Home synthesis (kept the README pitch line + the For-agents section), Get-started first-launch/first-workflow, all 4 Concepts intros, the worked-example, all 4 Running-workflows pages, the 3 synthesized Advanced pages, the 2 synthesized Reference pages, and the 3 synthesized Contributing pages (proof-policy, adding-a-runtime, architecture-notes). KEPT REAL and unchanged: the moved `install` page, the 5 symlinked docs (dev/README, releasing, AGENTS, runtime-support, specs/state-behavior-extension), the drafted **Voice & tone** page, and `/llms.txt`. Verified: stubbed pages carry `[TODO]`; Voice & tone retains its canonical-terminology table (no TODO); install + dev-README symlink carry no TODO. Nav complete, every page resolves, `--strict` green — a releasable shell, not fake depth.

### Refinements summary

Four captain validation-gate refinements folded into one revision and re-proven green. The for-agents surface moved from a human banner to head `<link rel>` tags; the nav switched to top tabs (active-section-only sidebar); the worked example moved Running workflows → Concepts; and every synthesized page is now an honest `[TODO]` stub while the real-content pages (install, the 5 symlinks, Voice & tone, `/llms.txt`) are untouched. `mkdocs build --strict` exits 0 with zero warnings; `go test ./...` unaffected (no Go/move changes this revision). Deliverable on branch `spacedock-ensign/mkdocs-material-docs-site`, commits 178a44a0 + 8e921725.

## Captain gate correction: split the two agents (supersedes the head-link surface)

The captain flagged that the "For agents" surface conflated two different audiences: **AGENTS.md is for an agent DEVELOPING the repo** (contributor/repo-dev guidance), NOT for an agent USING the product — a product-using agent reads the docs, which `/llms.txt` already covers. This correction supersedes the prior banner → head-link → subpath-safe-head-link work entirely. Re-verify at worktree commit 901ecc6c; `rm -rf site && mkdocs build --strict` → exit 0.

- DONE: Cut the "For agents" head-link surface entirely.
  Removed the `<link rel="alternate" type="text/markdown">` block; the override (`docs/overrides/main.html`) had no other purpose, so the file was deleted and `theme.custom_dir: docs/overrides` dropped from `mkdocs.yml`. Build output: zero `<link rel="alternate">` tags across all pages, zero `md-banner`. The top-level "For agents" nav entry and the "For agents" `llmstxt` section were also removed.
- DONE: Keep `/llms.txt` at the root as the product-using-agent surface.
  `mkdocs-llmstxt` unchanged; `site/llms.txt` present at the build-output root (3183 bytes), now indexing the 6 human sections (the "For agents" section gone, the agent-development page folds under Contributing). Discoverable via the canonical `/llms.txt` path; no head link needed.
- DONE: Re-home AGENTS.md as CONTRIBUTOR content.
  `git mv docs/site/agents/index.md → docs/site/contributing/agent-development.md` (the symlink to root `AGENTS.md`; target `../../../AGENTS.md` still resolves at the new depth). Added a Contributing nav entry "Agent development" (replacing the top-level "For agents" entry). Built: `site/contributing/agent-development/index.html` renders the AGENTS content (Agent Instructions / State-Branch Bootstrap); old `site/agents/` gone (no orphan; `--strict` green). Home agent note reframed: `/llms.txt` for product-using agents, repo-dev guidance → Contributing → Agent development.
- DONE: The subpath/topology note about the head links is now moot.
  The head links are gone, so the root-absolute-vs-base-aware href question no longer applies. `/llms.txt` is referenced by its canonical root path only.

### Correction summary

The for-agents head-link surface is removed (override file + `custom_dir` deleted; 0 head-link tags). The two agents are now cleanly split: `/llms.txt` stays at the build root for agents USING the product (a14y-scored via the canonical path), and AGENTS.md is surfaced as a real Contributing nav page (`Contributing → Agent development`) for agents DEVELOPING the repo. `mkdocs build --strict` exits 0; `go test ./...` unaffected (no Go/move changes). Deliverable on branch `spacedock-ensign/mkdocs-material-docs-site`, commit 901ecc6c.
