...
- DONE: Swap the visible "For agents" banner for machine-discoverable `<link rel>` head tags (AC-9 mechanism change; AC-9 "Verified by" updated above).
  `docs/overrides/main.html` now adds two `<link rel="alternate" type="text/markdown">` tags (`/llms.txt`, `/agents/`) to `extrahead` and drops the `announce` banner. Verified rendered inside `<head>` of `site/index.html` and nested pages; `md-banner`/"For agents:" count = 0 in output. `/llms.txt` kept at the build-output root, AGENTS.md page still reachable. Removed the now-unused `extra.agents_url`/`agents_md_url` keys. Worktree commit 178a44a0. (Topology note for the captain: the head links render root-absolute `/llms.txt` and `/agents/` as literally specified — correct for a custom/apex/org-Pages root; under a `/spacedock/` project-Pages subpath the canonical would be `/spacedock/llms.txt`. Flagged, left as specified.)
