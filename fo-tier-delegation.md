...
- DONE: Detached adversarial audit on a THROWAWAY checkout (front-door launcher + shipped FO contract are 2 of the 4 high-stakes surfaces): construct claim-breaking edits and confirm a test catches EACH; refute that the fail-safe default and the launcher env-pass are real; record edits + outcomes.
  Throwaway detached worktree at `/tmp/fo-tier-audit-throwaway` (branch tip 883ea091, never the impl worktree), baseline green, restored to a clean `git diff` after each edit. Edit#1 (Resolve default `Level2Only`→`Level3Capable`, break fail-safe): AC-2 `unset`+`garbage` rows turned RED (`Tier=level-3-capable, want level-2-only`); opus/sonnet rows stayed green. Edit#2 (`foModelEnvPassFlags`→always nil, drop safehouse forward): `TestClaudeSafehouseForwardsFOModel` RED (`wrapped argv missing --env-pass SPACEDOCK_FO_MODEL before --`); env-export + omit tests stayed green (independent surfaces). Edit#3 (add `haiku` to `capableModels`): `TestResolveTierArmingFiveCases/haiku…` + `TestResolveNormalizesBeforeMapping` RED. Each surface refuted: the fail-safe default and the launcher env-pass are both real and independently tested. Edits orthogonal (no false collateral RED).
- DONE: AC-2 fail-safe five-case fixture pins an un-provable session ROUTES (never self-approves); flipping the default to capable must turn unset/garbage rows RED. AC-4 covers only explicitly-capable opus/sonnet.
  AC-2 `TestResolveTierArmingFiveCases` reproduced: haiku+gated→armed, haiku+no-gate→inert, opus/sonnet+gated→inert, UNSET+gated→armed, GARBAGE+gated→armed — all green; Edit#1 proved the unset/garbage rows go RED when the default flips to capable. AC-4 `TestResolveCapableFOIsInert` asserts `route_gate_verdicts=false` for explicit opus/sonnet only; entity AC-4 prose (line 96) explicitly excludes unset — the AC-2/AC-4 contradiction is resolved, no residual. Assertions compare against the AC spec (independent source), not the file under test — not tautological.
- DONE: AC-3 level-3-judge mod (model: opus) spawns via EXISTING spawn-standing-all (real docs/dev tree); the three contract doc-diffs applied; the launcher var reaches a launched FO and is unset on garbage; full go test ./... green. AC-1 OUT — rides kt.
  AC-3 verified by the real binary: `spacedock dispatch spawn-standing-all --workflow-dir ./docs/dev` emits `level-3-judge` (`model: opus`, `subagent_type: general-purpose`, full Agent Prompt) alongside comm-officer — same path, no new machinery; `TestLevel3JudgeModSpawnsWithOpus` green. Three doc-diffs present in correct sections of `first-officer-shared-core.md`: Startup step 1.5 (L15), gated-stage verdict-route bullet (L121), `### Gate Verdicts` Write-Scope bullet (L173) + off-limits clause amended (L178); internally consistent (boot-line text, tier map, `level-3-judge` target all match code). Launcher var: `TestClaudeFrontDoorExportsFOModel` (7 cases incl. absent/garbage→unset) exercises production `runClaude` through the real host-ops seam (records the assembled launch env, not a fabricated value). Full `go test -count=1 ./...` green: 17 packages, 0 failures.
- SKIPPED: AC-1 (live Haiku FO drive of the standing route writing the `### Gate Verdicts` durable line).
  Out of this stage's scope per the checklist — rides the haiku-drive-validation member (kt), sequenced last after 2y lands. This stage validates the mechanism AC-1 exercises (tier function, launcher export, standing mod, contract route), not the live drive.

### Summary

PASSED. The detached adversarial audit on a throwaway checkout caught all three claim-breaking edits with a RED test each, refuting that the fail-safe default and the launcher `--env-pass` forward are real: flipping `Resolve`'s default to capable turns the AC-2 unset/garbage rows RED, dropping `foModelEnvPassFlags` turns the safehouse-forward test RED, and adding haiku to the capable set turns two tier tests RED — each edit orthogonal, no false collateral failures. AC-2/AC-3/AC-4 reproduced against the real tree and the real binary (spawn-standing-all emits level-3-judge with model opus); the three FO contract doc-diffs are present in their correct sections and internally consistent; the launcher-export tests exercise production `runClaude` through the legitimate host-ops seam (not mocked logic); full `go test -count=1 ./...` is green (17 packages). Notable: `--model default` resolves to opus→capable by deliberate alias (an explicit operator choice for the host default model), distinct from the unset/garbage path the fail-safe protects — a design observation, not a finding. AC-1's live Haiku drive is correctly out of scope (rides kt).

## Stage Report: implementation (cycle 2 — rebase)

- DONE: Rebase spacedock-ensign/fo-tier-delegation (deferred tip 883ea091) onto current origin/main, resolving conflicts; confirm go build/go vet/go test ./... clean afterward.
  132 commits behind at merge-base 2c9609f8. `5f87ea04` (fotier) and `57bf3aef` (level-3-judge mod) applied clean. `643fc3e8` (launcher patch) conflicted in `internal/cli/frontdoor.go` against main's unrelated `withAgentTeams` addition and its `Launch` exit-code-propagation change (`return code` vs `return 0`) — resolved by combining both env-wrapper calls (`withFOModelEnv(withAgentTeams(...))` for claude, `withFOModelEnv(...)` for codex, Claude-only per existing comment) and keeping `code, err :=` / `return code`. `883ea091` (the doc-diff-only commit, touches only `first-officer-shared-core.md`) conflicted against the file's full Startup/Completion-and-Gates/FO-Write-Scope restructure (230 lines changed on main) — skipped via `git rebase --skip` per the checklist's explicit deferral, not re-authored. Rebased branch pushed (`--force-with-lease`) to `origin/spacedock-ensign/fo-tier-delegation`, new tip `7b55f570`. `go build ./...` clean, `go vet ./...` clean, `go test ./...` green: 16 packages, 0 failures (`internal/cli` 85s, `internal/dispatch` 56s, `internal/status` 63s, `internal/fotier` 1.3s, all ok).
- DONE: Report exactly what conflicted and the post-rebase state of the three shipped pieces (internal/fotier, frontdoor.go SPACEDOCK_FO_MODEL launcher patch, docs/dev/_mods/level-3-judge.md).
  Conflicted: only `internal/cli/frontdoor.go` (real, resolved) and `skills/first-officer/references/first-officer-shared-core.md` (the doc-diff commit, skipped — see below). Post-rebase, `git diff --stat origin/main..HEAD` shows exactly 6 files: `internal/fotier/{fotier.go,fotier_test.go}` (new package, untouched by the conflict, present verbatim), `internal/cli/frontdoor.go` + `internal/cli/frontdoor_fo_model_test.go` (the launcher patch, merged atop main's `withAgentTeams`/exit-code changes), `docs/dev/_mods/level-3-judge.md` (new mod file, present verbatim, untouched by the conflict), and `internal/dispatch/level_3_judge_mod_test.go` (AC-3 spawn-path test, present verbatim). All three shipped pieces are intact on the rebased branch; `skills/first-officer/references/first-officer-shared-core.md` now shows ZERO diff against origin/main (the doc-diff commit is fully dropped).
- SKIPPED: Re-authoring the three contract doc-diffs (Startup tier-check, gate-verdict routing, FO Write Scope) against the restructured contract.
  Explicitly deferred by the checklist to a follow-up dispatch once the rebase itself is confirmed clean; this dispatch confirms it (build/vet/test green, branch pushed). The old insertion points no longer exist in a compatible form: Startup was restructured from a linear numbered list into `## Startup` (binary version gate) + `## «engage»(workflow)` with `--boot --identify --json` replacing the old `--boot` shape and the old contract-<N> gate replaced by a `spacedock <version>` minor-pin gate; `## Completion and Gates` and `## FO Write Scope` also moved/reworded. The follow-up dispatch needs a fresh design pass over the new structure, not a mechanical reapply.

### Summary

Rebased the 4-commit deferred branch (tip 883ea091) onto current origin/main (132 commits ahead at the old merge-base). The three code/mod-shipping commits (fotier, launcher, level-3-judge mod) replayed with one real conflict, in `frontdoor.go`, from main's unrelated `withAgentTeams` env-wrapper and `Launch` exit-code-propagation work — resolved by composing both sides' env wrappers and preserving both features. The fourth commit (883ea091, pure doc-diff against `first-officer-shared-core.md`) conflicted against that file's full restructuring on main and was skipped per the checklist's explicit instruction, deferring doc-diff re-authorship to a follow-up dispatch. Post-rebase: `go build ./...`, `go vet ./...`, and `go test ./...` are all clean (16 packages, 0 failures); the rebased branch (new tip `7b55f570`) is pushed to origin. All three shipped pieces (internal/fotier, the frontdoor.go launcher patch, docs/dev/_mods/level-3-judge.md) are present and intact; `first-officer-shared-core.md` carries no diff against origin/main.
