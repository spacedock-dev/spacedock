...
## Stage Report: implementation (cycle 1 — portable load-bearing checksum tamper)

- DONE: Make the AC-1 checksum tamper portable so TestChecksumGateGuardIsLoadBearing passes on Linux CI
  Commit 97dad74: the byte-APPEND tamper was caught by Linux `tar` itself (not the checksum gate), so the gateless install exited 1 on Linux instead of the expected 0 — the test didn't isolate the gate. Swapped the tamper for a structurally-VALID tar.gz whose `spacedock` payload differs, leaving checksums.txt unchanged (added `tamperFixtureTarball` helper + a `checksum` field on installFixture; removed the now-unused `appendBytes`). The archive extracts cleanly on every platform, so the sole post-strip rejection is sha256 != checksums.txt — the gate, isolated. Applied to both the main tamper case and the load-bearing sub-test.
- DONE: Leave install.sh and the checksum gate untouched (test-only fix)
  `git status install.sh` clean; gate lines 164-169 intact.

### Summary

Test-portability fix only, on spacedock-ensign/pre-cut-audit-cleanups-0199 (commit 97dad74), no install.sh / gate change. Root cause: a byte-appended tarball is rejected by Linux `tar` before the gate runs, masking the gate on Linux while passing on macOS; the new tamper is a fully-valid archive with a mismatched hash, portable by construction. Verified: `tar -xzf … spacedock` extracts the tampered archive rc=0; deleting install.sh's gate still reds the live tamper assertion; internal/release green (`go test -count=1`).
