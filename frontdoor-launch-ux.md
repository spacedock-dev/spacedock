...
- FAILED: AC-B banner workflow line naming the captain's example `Workflow: docs/dev`.
  The spec's MECHANISM (`filepath.Rel(dir, workflowDir)`, fall back to absolute) is implemented and AC-validated, but DiscoverWorkflowDir WALKS UP — it returns `dir` or an ancestor, never a descendant. Live drive from inside docs/dev renders `Workflow: .`; from a subdir `Workflow: ..`. The spec's example `docs/dev` is unachievable with walk-up semantics. Honest but cryptic. Surfaced to team-lead with a 5-line enhancement option (base-name on rel==".", or render relative to the git repo root → "docs/dev"); awaiting steer. Committed behavior = literal spec mechanism. All other ACs unaffected.
