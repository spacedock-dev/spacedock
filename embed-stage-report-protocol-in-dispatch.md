...
- DONE: AC-1 evidence — the non-self-describing Pi live lane writes a complete `## Stage Report: implementation` + clean state commit
  The live lane `TestLivePiNonSelfDescribingDispatch` (`internal/ensigncycle/pi_nonself_describing_live_test.go`, `//go:build live`) dispatches a Pi worker with a checklist naming no skill path, heading, or DONE/Summary structure, and `assertPiNonSelfDescribingSmokeResult` asserts the entity carries `## Stage Report: implementation` + `- DONE:` + `### Summary` and a clean path-scoped state commit. Build guard step passed (body carried tokens); the live dispatch step was auth-blocked ("No API key found for openai-codex") — compiles under `-tags live`, did not run end-to-end.
- DONE: AC-2 evidence — the fixture asserts the body carries the protocol tokens for host=pi and NOT for claude/codex
  `TestBuildPiArtifactCarriesStageReportProtocol` (`internal/dispatch/build_stage_report_protocol_test.go`) asserts pi body contains `## Stage Report:`, `- DONE:`, `- SKIPPED:`, `- FAILED:`, `### Summary`, `### Stage Report format`; asserts claude/codex bodies do NOT contain `### Stage Report format` or `## Stage Report: {stage}`. Passed. `TestPiNonSelfDescribingDispatchBuildBodyCarriesProtocol` (offline ensigncycle guard) also asserts the body carries the tokens while the checklist stdin carries no format hint. Passed.
...
- DONE: Run `go test ./internal/dispatch/... ./internal/ensigncycle/...` (non-live) and report pass/fail per package; `gofmt -l` and `go vet` on changed files
  `go test ./internal/dispatch/... ./internal/ensigncycle/...` → both `ok` (dispatch 26.7s, ensigncycle 244.1s). `gofmt -l` on all 4 changed Go files → no output (clean). `go vet ./internal/dispatch/... ./internal/ensigncycle/...` → no output (clean). Race test on the new test functions → `ok`.
- DONE: Live lane compiles under `-tags live`; report whether it ran or was auth-blocked
  `go build -tags live ./internal/ensigncycle/...` → exit 0. `go test -tags live -run TestLivePiNonSelfDescribingDispatch` with `SPACEDOCK_BIN` unset: build guard passed (body carried tokens), live dispatch failed with "No API key found for openai-codex" — auth-blocked, not silently skipped.
...
- DONE: Surface deviation assessment — ideation estimated ~+130 insertions across ~3 files; actual is 482 insertions across 5 files
  Production code: build.go +31/-2 (within estimate). Test code: dispatch fixture +136, ensigncycle offline build guard +131, ensigncycle live lane +170, registry entry +16. The two files not named in the ideation surface estimate are `pi_nonself_describing_build_test.go` (offline build guard serving AC-3 tautology-closure on every run, not just live) and `docs/runtime-live-ci-registry.md` (standard live-lane registry entry per project docs). Both are test infrastructure serving the stated ACs, not production scope creep. Total insertions 482 vs ~130 estimated exceeds the ±50% tolerance, but the deviation is entirely in test scaffolding; the production change is within estimate. Recommend FO note the surface breach for the record.

### Summary

Validation complete. All three ACs are evidenced: AC-2 by the dispatch fixture test (pi body carries tokens, claude/codex do not), AC-3 by the adversarial revert (pi goes RED, claude/codex stay green, self-describing lane unaffected), and AC-1 by the live lane which compiles and passes its build guard but was auth-blocked on the live dispatch step (no OpenAI/Codex API key in this environment). The Pi `firstActionBlock` overclaim is removed. Non-live tests, gofmt, go vet, and race tests are all clean. Surface deviation: 482 insertions across 5 files vs ~130 across 3 estimated — the two extra files are test infrastructure (offline build guard + live registry entry) serving the ACs, not production scope creep; production code (build.go +31/-2) is within estimate. No staged files; working tree clean.
