...
- DONE: Reproduce `go test ./internal/cli/` GREEN over the WHOLE package and confirm the new permission-mode tests exist and pass AND that no unwrapped-launch exact-argv oracle still asserts the stale (no-flag) argv.
  Reproduced from worktree at 2289e998: `go test ./internal/cli/` = 292 passed; full repo `go test ./...` = 1263 passed (16 pkgs); `go vet`/`go build ./...` clean. New `frontdoor_permission_mode_test.go` (4 tests, AC-1..AC-4) passes; inspected the full test diff — every `equalArgv` unwrapped-launch oracle was folded to the new argv, none left asserting the stale no-flag argv.
- DONE: Verify each AC-1..AC-4 with cited evidence reproduced from the worktree; flag any AC without reproducible evidence.
  AC-1 `TestClaudeUnsandboxedInjectsAutoPermissionMode` (unsandboxed = `--permission-mode auto`, no skip-perms; sandboxed = skip-perms, no injected mode), prod gate `frontdoor.go:310`. AC-2 `TestCodexUnsandboxedInjectsOnRequestApproval` (option A `--ask-for-approval on-request`; sandboxed bypass unchanged), prod `frontdoor.go:492` + `--ask-for-approval`/`-a` in `valueTakingHostFlags`. AC-3 `TestOperatorPermissionFlagSuppressesInjection` (operator `plan`/`untrusted` single occurrence, operator wins; space+equals+short-form). AC-4 `TestResumeUnsandboxedSuppressesInjection` (`--resume`/leading `resume` → no injection). All four reproduced; ran together with pre-existing `TestClaudeResumeFamilySuppressesBootstrapPrompt` = 20 passed. Each AC's proof drives real `runClaude`/`runCodex` over the `fakeHost` seam against test-supplied independent values — no prose/substring proof.
- DONE: Confirm zero `.md` edits in the commit; give a PASSED/REJECTED recommendation.
  `git show --stat HEAD` = 7 files, all `.go`, zero `.md` (anti-collision held; absent README edit is yw's, intentional). Verified spec-unlisted folds are correct not over-broad: each (`frontdoor_test.go:488` skip-check, the `launch_parity_test.go` + `TestStrayPromptGuardNegatives` sites) is genuinely `!wrap && !resume` (`t.TempDir()`, gate-relax flags don't sandbox). Recommendation: PASSED.

### Summary
Validated commit 2289e998 from the implementation worktree. Whole-package green reproduced (`internal/cli` 292, full repo 1263) — the stated success condition, not new-tests-in-isolation. All four ACs proven by reproduced Go tests driving the real launchers over the `fakeHost` seam against independent expected values. The folded exact-argv oracles — including the spec-unlisted `launch_parity_test.go`, `frontdoor_test.go:488`, and three `TestStrayPromptGuardNegatives` cases — are each confirmed to be genuine `!wrap && !resume` launches, so injecting the flag there is correct, not over-broad. The seemingly-contradictory `codex --plugin-dir … -- resume abc123` case is correct: `codexResume` keys on `passthrough[0] == "resume"`, but the parser prepends the `--plugin-dir` prefix, so it is not a resume — the prompt and injected flag ride the same `!resume` gate together. Wrapped/resume oracles correctly left untouched. Zero `.md` in the commit (README diff is yw's). PASSED.
