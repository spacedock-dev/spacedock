...
- DONE: AC-1 (value-measuring) — compaction path injects boot record, not contract re-injection
  Test: fire `session_compact`; `await` `context`; assert injected message contains `"command":"boot"` (boot record present) and does NOT contain `FO_BOOTSTRAP_MARKER` ("SPACEDOCK-FO-BOOTSTRAP-v1") or "Load the $spacedock:first-officer skill". Result: `countBootstraps(afterCompact.messages) === 0` and `countBootRecords(afterCompact.messages) === 1` — PASS.
- DONE: AC-2 (mechanism) — `session_compact` → `context` fires `pi.exec` with the boot-read args
  Test: mock `pi.exec` to return a fixed boot JSON; fire `session_compact`; `await` `context`; assert `pi.exec` called with `("spacedock", ["status","--boot","--identify","--json"])` exactly once. Result: `execCalls.length === 1`, `execCalls[0].command === "spacedock"`, `JSON.stringify(execCalls[0].args) === JSON.stringify(['status','--boot','--identify','--json'])` — PASS.
- DONE: AC-3 (scope boundary) — `session_start` path unchanged (still injects `FO_BOOTSTRAP_TEXT`)
  Test: fire `session_start`; `await` `context`; assert injected message contains `FO_BOOTSTRAP_MARKER` and "Load the $spacedock:first-officer skill". Result: `countBootstraps(first.messages) === 1` and `textOf(first.messages[0]).includes('Load the $spacedock:first-officer skill')` — PASS.
- DONE: AC-4 (child exemption) — `PI_SUBAGENT_CHILD=1` injects zero bootstrap on both `session_start` and `session_compact`
  Test: existing `TestSpacedockPiExtensionChildExemption` updated with `await` on async context handler + `pi.exec` mock; asserts `afterStart === undefined` and `afterCompact === undefined`. Result: PASS (unchanged behavior; child returns early before either flag path).
- DONE: AC-5 (deduplication) — existing boot record suppresses second injection
  Test: fire `session_compact`; `await` `context` (injects boot record); fire `session_compact` again; `await` `context` with the same messages; assert `deduped === undefined`. Result: PASS.
- DONE: Expected surface and tolerance (net LOC change and files, with observable-semantics declaration)
  Actual: +124 insertions, -33 deletions (net +91) across 2 files — `.pi/extensions/spacedock.ts` (+83/-19) and `internal/piruntime/spacedock_extension_test.go` (+41/-14). The ideation estimate was +15–20 net; actual is +91 net because the two-path context handler (boot-record + bootstrap) is longer than the original single-path handler, and the test harness grew with `pi.exec` mock, `execCalls` tracking, `await`, and new assertions. The observable-semantics declaration holds: no change to FO bootstrap content, gate/dispatch/state mechanics, command grammar, stored formats, or authority; only the compaction-boundary context content changes (boot record instead of `FO_BOOTSTRAP_TEXT`).
- DONE: `pi.exec` failure handling — minimal-framing fallback (never blocks the session)
  The `context` handler wraps `pi.exec` in `try/catch`; on failure (exception or nonzero code), `bootRecordJson` stays empty and the injected message is the directive alone (`FO_BOOT_RECORD_DIRECTIVE` without the JSON body). The compaction boundary never blocks the session even if `spacedock status` is unavailable.
