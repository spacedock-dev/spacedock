...
source: recorded Pi First Officer boot session at this repository root, cross-checked against PR #480 / commit `0ba08c54` and the current `skills/first-officer/references/first-officer-shared-core.md` boot contract. This task is the ab sibling of `shallow-boot-before-greet-advance-contract-drift`: that sibling repairs an artificial live-scenario prompt that contradicted #480, while this task handles the genuine residual #480 left for real multi-workflow roots — the read-only many-workflow boot branch is correct, but its sparse machine record lets LLMs mistake a complete terminal discovery response for a failed or truncated boot.
